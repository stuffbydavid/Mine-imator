/// tl_value_set_start_filtered(script, combine, target_index_map, filter_targets)
/// @arg script
/// @arg combine
/// @arg target_index_map
/// @arg filter_targets
/// @desc Prepares editable keyframes and history, optionally restricted to captured timelines.
...
function tl_value_set_start_filtered(script, combine, target_index_map, filter_targets)
{
	...
	else
	{
		...
		with (obj_timeline)
		{
			if (!selected || (filter_targets && !ds_map_exists(target_index_map, save_id_get(id))))
				continue

			history_data.tl_set_save_id[history_data.tl_set_amount] = save_id_get(id)
