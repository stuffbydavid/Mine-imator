...
function view_control_scale(view)
{
	...
	mat = view_transform_gizmo_matrix(e_value_type.TRANSFORM_SCA, mat)
	...
	else if (window_busy = "rendercontrol" && view_control_edit_view = view && view_control_edit >= e_view_control.SCA_X && view_control_edit <= e_view_control.SCA_Z) // Dragging single axis
	{
		...
		if (veclen > 0 && !mouse_still)
		{
			...
			if (view_control_transform_mode != e_rotation_space.GIMBAL)
			{
				...
				var axes = vec3(0);
				scale_move[axis_edit] = view_control_move_distance
				axes[axis_edit] = 1
				view_transform_scale_apply(view_transform_scale_factors(scale_move, axes))
			}
			else
			{
				snapval = (dragger_snap ? setting_snap_size_scale : snap_min)
