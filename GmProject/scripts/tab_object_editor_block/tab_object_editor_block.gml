function tab_object_editor_block()
{
	...
	sortlist_draw(tab.block_list, dx, dy, dw, dh - statesh, obj_edit.block_name, false)
	menu_filter = tab.block_list.search_tbx.text
	menu_filter_normal = sortlist_column_get(tab.block_list, obj_edit.block_name, 0)
	...
	for (var i = 0; i < statelen; i += 2)
	{
		...
		menu_block_current = block
		menu_block_state_current = block ? block.states_map[?state] : null
		draw_button_menu(state, e_menu.LIST, dx, dyy, dw, 24, obj_edit.block_state[i + 1], minecraft_asset_get_name("blockstatevalue", obj_edit.block_state[i + 1]), action_lib_block_state, false, null, null, "", c_white, 1, capwid)
		dyy += 32
	}
