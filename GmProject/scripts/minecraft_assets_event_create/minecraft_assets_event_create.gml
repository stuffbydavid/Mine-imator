...
function minecraft_assets_event_create()
{
	...
	block_texture_list = array_create(e_block_sheet.static_amount)
	for (var size = 0; size < e_block_sheet.static_amount; size++)
		block_texture_list[size] = ds_list_create()
	block_texture_ani_list = ds_list_create()
	block_texture_slot_map = ds_string_map_create()
	block_texture_opaque_slot_map = ds_string_map_create()
	block_texture_color_map = ds_map_create()
	item_texture_list = array_create(e_item_sheet.amount)
	for (var size = 0; size < e_item_sheet.amount; size++)
		item_texture_list[size] = ds_list_create()
