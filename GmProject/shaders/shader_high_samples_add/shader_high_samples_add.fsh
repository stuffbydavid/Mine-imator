...
void main()
{
	...
	vec4 sample_ = texture2D(uSample, vTexCoord);
	vec2 channel = vec2(0.0);
	
	// Red
	channel = packSample(unpackSample(sampleExp.r, sampleDec.r) + sample_.r * 255.0);
	
	gl_FragData[0].r = channel.x;
	gl_FragData[1].r = channel.y;
	
	// Green
	channel = packSample(unpackSample(sampleExp.g, sampleDec.g) + sample_.g * 255.0);
	
	gl_FragData[0].g = channel.x;
	gl_FragData[1].g = channel.y;
	
	// Blue
	channel = packSample(unpackSample(sampleExp.b, sampleDec.b) + sample_.b * 255.0);
	...
	// Alpha
	channel = packSample(unpackSample(sampleAlpha.r, sampleAlpha.g) + sample_.a * 255.0);
