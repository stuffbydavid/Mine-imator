...
namespace CppProject
{
	...
	void GraphicsApiHandler::Init()
	{
		...
		#endif
			D3DCheckError(D3D11CreateDevice(0, D3D_DRIVER_TYPE_HARDWARE, 0, creationFlags,
				featureLevels, ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &d3dDevice, 0, &d3dContext));
		...
		if (IS_OPENGL)
		{
			...
			if (logger->initialize())
			{
				logger->connect(logger, &QOpenGLDebugLogger::messageLogged, [&](const QOpenGLDebugMessage& message)
					{
						if (!glEnableLogger) return;
						QString msg = message.message();
						if (msg.contains("error")) DEBUG("[OpenGL error] " + msg);
	...
	void GraphicsApiHandler::SetBlendingFuncs(IntType src, IntType dest, IntType alphasrc, IntType alphadest)
	{
		...
		if (IS_D3D11)
		{
			...
			{
				if (prevState.src == src && prevState.dst == dest &&
					prevState.srcAlpha == alphasrc && prevState.dstAlpha == alphadest)
				{
	...
	#if OS_WINDOWS
		if (IS_D3D11)
			return d3dDevice->GetFeatureLevel() < D3D_FEATURE_LEVEL_11_0 ?
