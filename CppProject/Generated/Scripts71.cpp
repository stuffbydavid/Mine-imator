...
namespace CppProject
{
	...
	void view_shape_particles(IntType tl)
	{
		...
		if (temp == global::temp_edit && ObjType(obj_tab, global::_app->template_editor)->show > 0)
		{
			...
			{
				if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1322))
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sVar(window_focus) == string(view))
		{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3833))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2850);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2851);
		...
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				...
				if (sStr(window_state) == /*"startup"*/ STR(25))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2661))
					{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3108) }) + /*" - Mine-imator"*/ STR(674));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3836) : /*"exporttimelefthours"*/ STR(3837)), string(timelefthours) }) + /*", "*/ STR(1014);
		...
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3846), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1101) + samplecount + /*")"*/ STR(1077)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		draw_label(self, { /*"Mine-imator v"*/ STR(3851) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading) });
