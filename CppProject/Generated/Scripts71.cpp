...
namespace CppProject
{
	...
	VarType vbuffer_create_path(Scope<obj_timeline> self, IntType path, BoolType small_)
	{
		...
		length = IntType(0);
		plength = IntType(0);
		for (RealType i = IntType(0); i < array_length(VarType::CreateRef(frames)) - IntType(1); i++)
		{
			global::vertex_rgb = make_color_hsv((IntType)((i / array_length(VarType::CreateRef(frames))) * IntType(128)), IntType(255), IntType(255));
			if (!rail)
			{
				jp = IntType(0);
				j = 1.0 / detail;
	...
	{
		log({ /*"Create vertex format"*/ STR(3875) });
		vertex_format_begin();
	...
	{
		sReal(view_area_x) = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(195)))->size_real_ani + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(191)))->size_real_ani;
		sReal(view_area_y) = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(194)))->size_real_ani;
		sReal(view_area_width) = sReal(panel_area_width) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(195)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(191)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(196)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(192)))->size_real_ani;
		sReal(view_area_height) = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(194)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(193)))->size_real_ani;
		view_draw(self, sInt(view_main));
		view_draw(self, sInt(view_second));
		if (sVar(window_busy) == /*"viewresizehor"*/ STR(3876) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3877))
		{
			sInt(mouse_cursor) = cr_size_we;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"left"*/ STR(195)))
		...
		if (sVar(window_busy) == /*"viewresizever"*/ STR(3878) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3877))
		{
			sInt(mouse_cursor) = cr_size_ns;
			if (string_contains(ObjType(obj_view, sInt(view_second))->location, /*"top"*/ STR(194)))
		...
		if (sVar(window_busy) == /*"viewresizeboth"*/ STR(3877))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(196) || ObjType(obj_view, sInt(view_second))->location == /*"left_secondary"*/ STR(191))
				sInt(mouse_cursor) = cr_size_nesw;
			else
				sInt(mouse_cursor) = cr_size_nwse;
			
		}
		if (sVar(window_busy) == /*"viewresizesplithor"*/ STR(3879))
		{
			sInt(mouse_cursor) = cr_size_we;
			sReal(view_split) = clamp((gmlGlobal::mouse_x - sReal(view_area_x)) / sReal(view_area_width), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"right"*/ STR(196))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
		if (sVar(window_busy) == /*"viewresizesplitver"*/ STR(3880))
		{
			sInt(mouse_cursor) = cr_size_ns;
			sReal(view_split) = clamp((gmlGlobal::mouse_y - sReal(view_area_y)) / sReal(view_area_height), 0.1, 0.9);
			if (ObjType(obj_view, sInt(view_second))->location == /*"bottom"*/ STR(193))
				sReal(view_split) = IntType(1) - sReal(view_split);
			if (!sBool(mouse_left))
				sVar(window_busy) = /*""*/ STR(0);
		}
