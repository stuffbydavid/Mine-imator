...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"viewmove"*/ STR(3908) && view == sInt(view_second))
		{
		...
		if (view == sInt(view_main) && ObjType(obj_view, sInt(view_second))->show > 0 && !window_exists(e_window_VIEW_SECOND) && sVar(window_busy) != /*"viewmove"*/ STR(3908))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3907))
				captionx += ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3905))
				dx -= ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3907) || ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3905))
		...
		if (view == sInt(view_second))
		{
			if (window_get_current() != e_window_VIEW_SECOND)
			{
				if (draw_button_icon(self, { /*"viewclose"*/ STR(3909), dx, dy, sVar(dw), sVar(dh), false, icons_CLOSE, null_, false, /*"viewclose"*/ STR(3909) }))
				{
					ObjType(obj_view, view)->show = false;
					sBool(view_render) = false;
				}
				dx -= sVar(dw) + padding;
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3910), dx, dy, sVar(dw), sVar(dh), false, icons_EXTERNAL, null_, false, /*"viewpopout"*/ STR(3910) }))
					window_create(e_window_VIEW_SECOND, boxx, boxy, boxw, boxh);
			}
			else
			{
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3910), dx, dy, sVar(dw), sVar(dh), false, icons_INTERNAL, null_, false, /*"viewpopin"*/ STR(3911) }))
				{
		...
		else
		{
			...
			if (draw_button_icon(self, { /*"viewsecond"*/ STR(3912), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, sInt(view_second))->show, icons_VIEWPORT_SECONDARY, null_, false, (ObjType(obj_view, sInt(view_second))->show > 0) ? /*"viewseconddisable"*/ STR(3913) : /*"viewsecondenable"*/ STR(3914) }))
		...
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewqualitysettings"*/ STR(3915), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3915)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewqualitysettings"*/ STR(3915)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_quality_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3915)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
		...
		if (draw_button_icon(self, { /*"viewmoderender"*/ STR(3916), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_RENDER, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_MATERIAL__DARK : icons_SPHERE_MATERIAL, null_, false, /*"viewmoderender"*/ STR(3916) }))
		{
		...
		dx -= sVar(dw) + padding;
		tip_set_keybind(self, e_keybind_RENDER_MODE);
		if (draw_button_icon(self, { /*"viewmodeshaded"*/ STR(3917), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_SHADED, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_SHADING__DARK : icons_SPHERE_SHADING, null_, false, /*"viewmodeshaded"*/ STR(3917) }))
			ObjType(obj_view, view)->quality = e_view_mode_SHADED;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewmodeflat"*/ STR(3918), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_FLAT, icons_SPHERE_FLAT, null_, false, /*"viewmodeflat"*/ STR(3918) }))
			ObjType(obj_view, view)->quality = e_view_mode_FLAT;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewparticles"*/ STR(3919), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->particles, icons_FIREWORKS, null_, false, (ObjType(obj_view, view)->particles > 0) ? /*"viewparticlesdisable"*/ STR(3920) : /*"viewparticlesenable"*/ STR(3921) }))
			ObjType(obj_view, view)->particles = !(ObjType(obj_view, view)->particles > 0);
		if (cam != null_)
		{
			dx -= sVar(dw) + padding;
			if (draw_button_icon(self, { /*"vieweffects"*/ STR(3922), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->effects, icons_WAND, null_, false, (ObjType(obj_view, view)->effects > 0) ? /*"vieweffectsdisable"*/ STR(3923) : /*"vieweffectsenable"*/ STR(3924) }))
			{
		...
		draw_divide_vertical(dx, dy, sVar(dh));
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewgridsettings"*/ STR(3925), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3925)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewgridsettings"*/ STR(3925)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_grid_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3925)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		if (draw_button_icon(self, { /*"viewgrid"*/ STR(3926), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->grid, icons_GRID, null_, false, (ObjType(obj_view, view)->grid > 0) ? /*"viewgriddisable"*/ STR(3927) : /*"viewgridenable"*/ STR(3928) }))
			ObjType(obj_view, view)->grid = !(ObjType(obj_view, view)->grid > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewaspectratio"*/ STR(3929), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->aspect_ratio, icons_LETTERBOX, null_, false, (ObjType(obj_view, view)->aspect_ratio > 0) ? /*"viewaspectratiodisable"*/ STR(3930) : /*"viewaspectratioenable"*/ STR(3931) }))
			ObjType(obj_view, view)->aspect_ratio = !(ObjType(obj_view, view)->aspect_ratio > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewoverlays"*/ STR(3932), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->gizmos, icons_OVERLAYS, null_, false, (ObjType(obj_view, view)->gizmos > 0) ? /*"viewoverlaysdisable"*/ STR(3933) : /*"viewoverlaysenable"*/ STR(3934) }))
		...
		if (view == sInt(view_main))
		{
			...
			draw_divide_vertical(dx, dy, sVar(dh));
			dx -= 16.0 + padding;
			if (draw_button_icon(self, { /*"viewsnapsettings"*/ STR(3935), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3935)), icons_CHEVRON_DOWN_TINY }))
			{
				menu_settings_set(self, dx, dy, (string(view) + /*"viewsnapsettings"*/ STR(3935)), IntType(24));
				sInt(settings_menu_view) = view;
				sInt(settings_menu_script) = ID_menu_snap_settings;
			}
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3935)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
			tip_set_keybind(self, e_keybind_SNAP);
			if (draw_button_icon(self, { /*"viewsnap"*/ STR(3936), dx, dy, sVar(dw), sVar(dh), sReal(setting_snap), icons_MAGNET, null_, false, (sReal(setting_snap) > 0) ? /*"viewsnapdisable"*/ STR(3937) : /*"viewsnapenable"*/ STR(3938) }))
				sReal(setting_snap) = !(sReal(setting_snap) > 0);
		}
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1507) : /*"viewcamerasecond"*/ STR(1508));
		menuactive = false;
		...
		if (ObjType(obj_view, view)->camera == -IntType(4) && cam == -IntType(4))
			camname = text_get({ /*"viewworkcamera"*/ STR(3939) });
		else
			if (ObjType(obj_view, view)->camera == -IntType(5))
				camname = text_get({ /*"viewactivecamera"*/ STR(3940), (cam == -IntType(4) ? text_get({ /*"viewworkcamera"*/ STR(3939) }) : string_remove_newline(idVar(cam, display_name))) });
			else
		...
		if (location != /*"full"*/ STR(2781) && location != /*"top"*/ STR(190) && location != /*"bottom"*/ STR(189))
		{
			...
			if (!string_contains(location, /*"right"*/ STR(192)))
				sVar(content_width) -= 3.0;
			if (location == /*"right_top"*/ STR(3905) || location == /*"left_top"*/ STR(3907))
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(118) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3941));
		...
		if (view == sInt(view_main))
		{
			...
			if (sBool(bench_open))
			{
				sStr(bench_show_ani_type) = /*"show"*/ STR(761);
			...
			if (ObjType(obj_view, sInt(view_main))->mouseon && app_mouse_box(self, benchx, benchy, IntType(86), IntType(86)) && !sBool(popup_mouseon) && !sBool(toast_mouseon) && !sBool(context_menu_mouseon) && !(ObjType(obj_view, sInt(view_second))->show > 0 && ObjType(obj_view, sInt(view_second))->mouseon))
			{
				...
				tip_set(self, text_get({ /*"viewworkbenchtip"*/ STR(3942) }), benchx, benchy, IntType(86), IntType(86), false);
			...
			if (sVar(window_busy) == /*"bench"*/ STR(762) || sVar(bench_show_ani) == IntType(1))
			{
				sReal(bench_hover_ani_goal) = .5;
				sReal(bench_click_ani_goal) = .9;
			}
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			if (ObjType(obj_view, view)->mouseon && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
			{
				...
				{
					if (ObjType(obj_view, view)->location == /*"right_top"*/ STR(3905) || ObjType(obj_view, view)->location == /*"left_bottom"*/ STR(3906))
					...
					if (sBool(mouse_left_pressed))
					{
						sVar(window_busy) = /*"viewresizeboth"*/ STR(3901);
					...
					if (mouseonresizehor)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							sVar(window_busy) = /*"viewresizehor"*/ STR(3900);
							sVar(view_resize_width) = ObjType(obj_view, view)->width;
						}
					}
					else
						if (mouseonresizever)
						{
							...
							if (sBool(mouse_left_pressed))
							{
								sVar(window_busy) = /*"viewresizever"*/ STR(3902);
								sVar(view_resize_height) = ObjType(obj_view, view)->height;
							}
						}
						else
							if (app_mouse_box(self, boxx, boxy, boxw, captionh) && !sBool(popup_mouseon) && sBool(mouse_left_pressed))
								sVar(window_busy) = /*"viewclickcaption"*/ STR(3943);
					
				
				
			}
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3943))
			{
				if (sReal(mouse_move) > IntType(10))
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2781);
					sVar(window_busy) = /*"viewmove"*/ STR(3908);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3908))
			{
				...
				if (gmlGlobal::mouse_y < sReal(view_area_y) + sReal(view_area_height) * 0.3)
					mouselocation += /*"_top"*/ STR(1968);
				else
					if (gmlGlobal::mouse_y >= sReal(view_area_y) + sReal(view_area_height) * 0.7)
						mouselocation += /*"_bottom"*/ STR(1969);
				
				if (mouselocation == /*"_top"*/ STR(1968))
					mouselocation = /*"top"*/ STR(190);
				if (mouselocation == /*"_bottom"*/ STR(1969))
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3908))
			{
				...
				RealType ani = ceil((sVar(view_glow_ani) - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(567), sVar(view_glow_ani))));
		...
		if (mouseonresizesplit && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
		{
			...
			{
				sInt(mouse_cursor) = cr_size_we;
				mouselocation = /*"hor"*/ STR(3944);
			}
			else
			{
				sInt(mouse_cursor) = cr_size_ns;
				mouselocation = /*"ver"*/ STR(3945);
			}
			
			if (sBool(mouse_left_pressed))
				sVar(window_busy) = /*"viewresizesplit"*/ STR(3946) + mouselocation;
		}
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(190) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3904)))
				linetop = true;
			if (location == /*"bottom"*/ STR(189) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3904)))
				linebottom = true;
			if (location == /*"left"*/ STR(191) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3903)))
				lineright = true;
			if (location == /*"right"*/ STR(192) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3903)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3901) || sVar(window_busy) == /*"viewresizever"*/ STR(3902) || sVar(window_busy) == /*"viewresizehor"*/ STR(3900)))
			{
				if (location == /*"left_top"*/ STR(3907))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3900) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3902) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
				}
				if (location == /*"right_top"*/ STR(3905))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3900) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3902) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
				}
				if (location == /*"left_bottom"*/ STR(3906))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3900) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3902) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
				}
				if (location == /*"right_bottom"*/ STR(2782))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3900) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3902) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3901));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			if (sBool(view_render_real_time))
				infotext = text_get({ /*"viewrenderfps"*/ STR(3947), string(gmlGlobal::fps), max({ IntType(1), global::render_samples }), sVar(project_render_samples) });
			else
				infotext = /*""*/ STR(0);
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3948), text_get({ /*"viewmodepass"*/ STR(1506) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2781);
		self->location_last = /*"full"*/ STR(2781);
		self->surface = null_;
