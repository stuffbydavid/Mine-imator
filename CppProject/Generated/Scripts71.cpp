...
namespace CppProject
{
	...
	void view_shape_particles(IntType tl)
	{
		...
		if (temp == global::temp_edit && ObjType(obj_tab, global::_app->template_editor)->show > 0)
		{
			...
			if (idVar(temp, pc_bounding_box_type) == /*"custom"*/ STR(592))
			{
				draw_set_color(c_red);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(113)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3938) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		if (sVar(window_focus) == string(view))
		{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3937))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2946);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2947);
		...
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				...
				if (sStr(window_state) == /*"startup"*/ STR(25))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2752))
					{
			...
			case e_window_VIEW_SECOND:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(ObjType(obj_view, sInt(view_second))->title + /*" - Mine-imator"*/ STR(713));
				break;
			}
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3207) }) + /*" - Mine-imator"*/ STR(713));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3940) : /*"exporttimelefthours"*/ STR(3941)), string(timelefthours) }) + /*", "*/ STR(1062);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3942) : /*"exporttimeleftminutes"*/ STR(3943)), string(timeleftmins) }) + /*" "*/ STR(17) + text_get({ /*"exporttimeleftand"*/ STR(3944) }) + /*" "*/ STR(17);
		...
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3950), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1149) + samplecount + /*")"*/ STR(1125)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		window_set_caption(loadtext + /*" - Mine-imator"*/ STR(713));
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(49) ? text_get({ /*"exportmovietitle"*/ STR(3951) }) : text_get({ /*"exportimagetitle"*/ STR(3952) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
