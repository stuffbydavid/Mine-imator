...
namespace CppProject
{
	...
	RealType window_draw(ScopeAny self)
	{
		switch ((IntType)window_get_current())
		{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3105) }) + /*" - Mine-imator"*/ STR(673));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3833) : /*"exporttimelefthours"*/ STR(3834)), string(timelefthours) }) + /*", "*/ STR(1013);
		...
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3843), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1100) + samplecount + /*")"*/ STR(1076)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
