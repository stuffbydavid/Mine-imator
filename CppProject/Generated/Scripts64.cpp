...
namespace CppProject
{
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3798), string(ds_list_size(sInt(pc_type_list))) });
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3298), minecraft_asset_get_name(/*"modelpart"*/ STR(748), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(8), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3299) });
