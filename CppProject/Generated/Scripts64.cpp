			...
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3298), minecraft_asset_get_name(/*"modelpart"*/ STR(750), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(8), idVar(sVar(model_file), name)) });
					else
