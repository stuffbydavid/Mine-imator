...
namespace CppProject
{
	...
	void tl_filter_draw(ScopeAny self)
	{
		...
		RealType colorwid;
		VarType px;
		switchwid = text_max_width({ /*"timelinehideghosts"*/ STR(3735) }) + IntType(16) + IntType(24);
		colorwid = max({ text_max_width({ /*"timelinefiltertags"*/ STR(3736) }), (IntType(24) * IntType(9)) - IntType(4) });
		draw_label(self, { text_get({ /*"timelinefiltertags"*/ STR(3736) }), sVar(dx), sVar(dy) + IntType(9), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(20);
		...
		for (IntType i = IntType(0); i <= IntType(8); i++)
		{
			...
			microani_set(/*"hidecolortag"*/ STR(3737) + string(i), null_, false, false, sVar(timeline_hide_color_tag).Value(i));
		...
		draw_switch(self, { /*"timelinehideghosts"*/ STR(3735), sVar(dx), sVar(dy), sVar(setting_timeline_hide_ghosts), ID_action_setting_timeline_hide_ghosts, /*"timelinehideghoststip"*/ STR(3738) });
		tab_next(self);
	...
	BoolType tl_get_visible(ScopeAny self)
	{
		...
		if (global::render_active == /*"image"*/ STR(90))
		{
			...
			if (global::render_active == /*"movie"*/ STR(1098))
			{
	...
	void tl_interval_settings_draw(ScopeAny self)
	{
		...
		draw_dragger(self, { /*"timelineintervalssize"*/ STR(3739), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_size), .1, IntType(1), no_limit, sVar(project_tempo), IntType(1), idInt(sVar(timeline), tbx_interval_size), ID_action_tl_interval_size });
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, { /*"timelineintervalsoffset"*/ STR(3740), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_offset), .1, -no_limit, no_limit, IntType(0), IntType(1), idInt(sVar(timeline), tbx_interval_offset), ID_action_tl_interval_offset });
		tab_next(self);
		sVar(settings_menu_w) = (text_max_width({ /*"timelineintervalssize"*/ STR(3739), /*"timelineintervalsoffset"*/ STR(3740) }) + IntType(16) + dragger_width) + IntType(24);
	}
	...
	void tl_keyframes_paste(ScopeAny self, VarType pos)
	{
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			{
				pastemode = /*"fixed"*/ STR(3742);
				break;
			}
			tllast = sArr(copy_kf_tl_save_id).Value(k);
		}
		if (pastemode == /*"fixed"*/ STR(3742))
		{
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			if (tladd == null_)
				continue;
			if (pastemode != /*"free"*/ STR(3741) && idInt(tladd, part_list) != null_ && sArr(copy_kf_tl_model_part_name).Value(k) != /*""*/ STR(0))
			{
				VarType part;
			...
			if (sVar(part_of) != null_)
			{
				if (sVar(type) == e_tl_type_BODYPART)
				{
					if (sVar(model_part) != null_)
						...
						sVar(display_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3743) });
