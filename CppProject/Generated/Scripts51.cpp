...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2476) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2477) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2478);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1267: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2485), /*"unzip"*/ STR(1267) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2486) });
						error(/*"errorunzippack"*/ STR(2487));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1349);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2488) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2485), /*"modeltextures"*/ STR(1349) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2489);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2490) });
			...
			case 2489: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2485), /*"blocktextures"*/ STR(2489) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(982), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(508));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2491);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2492) });
			...
			case 2491: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2485), /*"itemtextures"*/ STR(2491) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2493), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(507));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(607), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(612), /*"_s"*/ STR(2494));
				res_load_pack_item_textures(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2495));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2496) });
