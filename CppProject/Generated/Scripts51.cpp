...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2565) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2566) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2567);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1314: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2574), /*"unzip"*/ STR(1314) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2575) });
						error(/*"errorunzippack"*/ STR(2576));
						withOne (app, global::_app->id, self->id)
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2577) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2574), /*"modeltextures"*/ STR(1396) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2578);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2579) });
			...
			case 2578: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2574), /*"blocktextures"*/ STR(2578) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(1029), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(516));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2580);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2581) });
			...
			case 2580: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2574), /*"itemtextures"*/ STR(2580) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2582), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(515));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(647), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(652), /*"_s"*/ STR(2583));
				res_load_pack_item_textures(self, /*"normal"*/ STR(641), /*"_n"*/ STR(2584));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2585) });
