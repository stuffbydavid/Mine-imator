...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2470) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2471) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2472);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1265: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2479), /*"unzip"*/ STR(1265) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip({ fname }))
					{
						log({ /*"Error unzipping pack"*/ STR(2480) });
						error(/*"errorunzippack"*/ STR(2481));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1347);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2482) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2479), /*"modeltextures"*/ STR(1347) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2483);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2484) });
			...
			case 2483: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2479), /*"blocktextures"*/ STR(2483) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(980), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(507));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2485);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2486) });
			...
			case 2485: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2479), /*"itemtextures"*/ STR(2485) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2487), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(506));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(606), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(611), /*"_s"*/ STR(2488));
				res_load_pack_item_textures(self, /*"normal"*/ STR(600), /*"_n"*/ STR(2489));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2490) });
