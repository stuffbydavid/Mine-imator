...
namespace CppProject
{
	...
	RealType render_world_ground(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2538), c_white, IntType(1));
		VarType blend = block_texture_get_blend(sVar(background_ground_name), sVar(background_ground_tex));
		BoolType iswater = (sVar(background_ground_name) == /*"block/water_flow"*/ STR(1734) || sVar(background_ground_name) == /*"block/water_still"*/ STR(1732));
		render_set_uniform_int(/*"uIsGround"*/ STR(2539), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1916), blend, IntType(1));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2540), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2541), IntType(0));
		render_set_uniform_int(/*"uFogShow"*/ STR(1925), global::_app->background_fog_show);
		render_set_uniform_int(/*"uIsWater"*/ STR(2537), iswater);
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), idVar(sVar(background_ground_tex_material), material_format));
		if (sVar(background_ground_tex_material) == global::mc_res)
		{
			render_set_uniform(/*"uMetallic"*/ STR(2534), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2535), (iswater && global::_app->project_render_water_reflections > 0 ? .07 : 1.0));
			render_set_uniform(/*"uEmissive"*/ STR(2536), IntType(0));
		}
		else
		{
			render_set_uniform(/*"uMetallic"*/ STR(2534), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2535), IntType(0));
			render_set_uniform(/*"uEmissive"*/ STR(2536), IntType(0));
		}
		...
		if (sBool(background_ground_material_ani))
			render_set_texture(sArr(background_ground_ani_texture_material).Value(block_texture_get_frame()), /*"Material"*/ STR(2532));
		else
			render_set_texture(sInt(background_ground_texture_material), /*"Material"*/ STR(2532));
		
		if (sBool(background_ground_normal_ani))
			render_set_texture(sArr(background_ground_ani_texture_normal).Value(block_texture_get_frame()), /*"Normal"*/ STR(2533));
		else
			render_set_texture(sInt(background_ground_texture_normal), /*"Normal"*/ STR(2533));
	...
	void render_world_item(VarType vbuffer, VarType is3d, VarType facecamera, VarType bounce, VarType rotate, ArrType res)
	{
		...
		if (res.Value(IntType(1)) != null_ && res.Value(IntType(1)) != global::mc_res)
		{
			...
			if (idInt(res.Value(IntType(1)), item_sheet_texture_material) != null_)
				render_set_texture(idInt(res.Value(IntType(1)), item_sheet_texture_material), /*"Material"*/ STR(2532));
			else
				render_set_texture(idVar(res.Value(IntType(1)), texture), /*"Material"*/ STR(2532));
		...
		else
		{
			...
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), e_material_FORMAT_NONE);
		}
		
		if (res.Value(IntType(2)) != null_ && res.Value(IntType(2)) != global::mc_res)
		{
			if (idInt(res.Value(IntType(2)), item_sheet_tex_normal) != null_)
				render_set_texture(idInt(res.Value(IntType(2)), item_sheet_tex_normal), /*"Normal"*/ STR(2533));
			else
				render_set_texture(idVar(res.Value(IntType(2)), texture), /*"Normal"*/ STR(2533));
	...
	void render_world_model_file_parts(ScopeAny self, VarType modelfile, VarType res, IntType texnamemap, IntType hidelist, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, IntType matrixmap)
	{
		...
		for (IntType p = IntType(0); p < ds_list_size(partlist); p++)
		{
			...
			if (idBool(part, part_mixing_shapes))
				render_set_uniform_int(/*"uColorsExt"*/ STR(2083), idBool(part, part_mixing_shapes));
	...
	RealType render_world_model_part(ScopeAny self, VarType part, VarType res, IntType texnamemap, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, VarType tlobject)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			if (tlobject > 0)
			{
				...
				render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), sArr(model_part_shape_material_res).Value(s));
				...
				{
					render_set_texture(sArr(model_part_shape_tex_material).Value(s), /*"Material"*/ STR(2532));
				...
				if (sArr(model_part_shape_tex_normal).Value(s) == null_)
					...
					render_set_texture(sArr(model_part_shape_tex_normal).Value(s), /*"Normal"*/ STR(2533));
			...
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(1997))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"chestplate"*/ STR(1998))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"leggings"*/ STR(1999))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"boots"*/ STR(2000))
				{
			...
			{
				if (tlobject != null_)
					render_set_uniform_color(/*"uMixColor"*/ STR(2089), merge_color((IntType)(ObjType(obj_model_shape, shape)->color_mix), (IntType)(sArr(value_inherit).Value(e_value_MIX_COLOR)), sArr(value_inherit).Value(e_value_MIX_PERCENT)), lerp(ObjType(obj_model_shape, shape)->color_mix_percent, sArr(value_inherit).Value(e_value_MIX_PERCENT), sArr(value_inherit).Value(e_value_MIX_PERCENT)));
				else
					render_set_uniform_color(/*"uMixColor"*/ STR(2089), ObjType(obj_model_shape, shape)->color_mix, ObjType(obj_model_shape, shape)->color_mix_percent);
	...
	void render_world_shape(VarType type, VarType vbuf, VarType facecamera, ArrType tex)
	{
		...
		render_set_texture(tex.Value(IntType(1)), /*"Material"*/ STR(2532));
		render_set_texture(tex.Value(IntType(2)), /*"Normal"*/ STR(2533));
		vbuffer_render({ vbuf });
	}
	...
	RealType render_world_sky_clouds(ScopeAny self)
	{
		if (!(sVar(background_sky_clouds_show) > 0) || !(global::render_background > 0) || global::render_mode == e_render_mode_DEPTH_NO_SKY)
			return IntType(0);
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2538), c_black, IntType(1));
		VarType res = sVar(background_sky_clouds_tex);
		if (!res_is_ready(res))
			res = global::mc_res;
		render_set_uniform_int(/*"uIsSky"*/ STR(2542), IntType(1));
		...
		render_set_uniform(/*"uRoughness"*/ STR(2535), IntType(1));
		render_set_uniform(/*"uEmissive"*/ STR(2536), IntType(0));
		render_set_uniform(/*"uLightSpecular"*/ STR(2543), IntType(0));
	...
	RealType render_world_tl(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_CLICK)
		{
			if (sReal(selected) > 0 || sVar(lock) > 0 || !tl_update_list_filter(self->id))
				return IntType(0);
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2538), self->id, IntType(1));
		}
		...
		if (global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2538), (sVar(ssao) > 0) ? merge_color(c_black, c_white, global::shader_blend_alpha) : c_black, IntType(1));
		if (sBool(colors_ext) != global::shader_uniform_color_ext || sArr(value_inherit).Value(e_value_RGB_ADD) != global::shader_uniform_rgb_add || sArr(value_inherit).Value(e_value_HSB_ADD) != global::shader_uniform_hsb_add || sArr(value_inherit).Value(e_value_RGB_SUB) != global::shader_uniform_rgb_sub || sArr(value_inherit).Value(e_value_HSB_SUB) != global::shader_uniform_hsb_sub || sArr(value_inherit).Value(e_value_HSB_MUL) != global::shader_uniform_hsb_mul || sArr(value_inherit).Value(e_value_MIX_COLOR) != global::shader_uniform_mix_color || sArr(value_inherit).Value(e_value_MIX_PERCENT) != global::shader_uniform_mix_percent)
		{
			...
			render_set_uniform_int(/*"uColorsExt"*/ STR(2083), global::shader_uniform_color_ext);
			render_set_uniform_color(/*"uRGBAdd"*/ STR(2084), global::shader_uniform_rgb_add, IntType(1));
			render_set_uniform_color(/*"uHSBAdd"*/ STR(2086), global::shader_uniform_hsb_add, IntType(1));
			render_set_uniform_color(/*"uRGBSub"*/ STR(2085), global::shader_uniform_rgb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBSub"*/ STR(2087), global::shader_uniform_hsb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBMul"*/ STR(2088), global::shader_uniform_hsb_mul, IntType(1));
			render_set_uniform_color(/*"uMixColor"*/ STR(2089), global::shader_uniform_mix_color, global::shader_uniform_mix_percent);
		}
		...
		if (sArr(value_inherit).Value(e_value_SUBSURFACE) != global::shader_uniform_sss || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_RED) != global::shader_uniform_sss_red || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_GREEN) != global::shader_uniform_sss_green || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_BLUE) != global::shader_uniform_sss_blue || sArr(value_inherit).Value(e_value_SUBSURFACE_COLOR) != global::shader_uniform_sss_color)
		{
			...
			render_set_uniform(/*"uSSS"*/ STR(2544), global::shader_uniform_sss);
			render_set_uniform_vec3(/*"uSSSRadius"*/ STR(2545), global::shader_uniform_sss_red, global::shader_uniform_sss_green, global::shader_uniform_sss_blue);
			render_set_uniform_color(/*"uSSSColor"*/ STR(2546), global::shader_uniform_sss_color, 1.0);
		}
		...
		if (sVar(glow) != global::shader_uniform_glow || sVar(glow_texture) != global::shader_uniform_glow_texture || sArr(value_inherit).Value(e_value_GLOW_COLOR) != global::shader_uniform_glow_color)
		{
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2547), IntType(1));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2541), sVar(glow_texture));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2540), global::shader_uniform_glow_color, IntType(1));
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2547), IntType(0));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2541), IntType(0));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2540), c_black, IntType(0));
			}
		...
		if (DsMap(idInt(global::render_shader_obj, uniform_map)).Value(/*"uGlintEnabled"*/ STR(2548)) > -IntType(1))
			texture_set_stage(DsMap(idInt(global::render_shader_obj, sampler_map)).Value(/*"uGlintTexture"*/ STR(2549)), sprite_get_texture((IntType)(tex), IntType(0)));
		spd = global::_app->background_time * sVar(glint_speed) * global::_app->project_render_glint_speed;
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2548), sVar(glint_mode) == e_glint_NONE ? IntType(0) : IntType(1));
		render_set_uniform_vec2(/*"uGlintOffset"*/ STR(2550), spd * (0.000625), spd * (0.00125));
		render_set_uniform(/*"uGlintStrength"*/ STR(2551), global::_app->project_render_glint_strength * sVar(glint_strength));
		render_set_uniform_vec2(/*"uGlintSize"*/ STR(2552), sprite_get_width((IntType)(tex)) * IntType(2) * sVar(glint_scale), sprite_get_height((IntType)(tex)) * IntType(2) * sVar(glint_scale));
		if (sVar(type) != e_tl_type_PARTICLE_SPAWNER)
		{
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_tl_type_PATH:
				{
					if (sVar(path_vbuffer) != null_)
					{
						...
						else
						{
							...
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), idVar(sArr(value_inherit).Value(e_value_TEXTURE_MATERIAL_OBJ), material_format));
						}
						...
						render_set_texture(texnorm, /*"Normal"*/ STR(2533));
				...
				default:
				{
					...
					withOne (Object, sVar(temp), self->id)
					{
						...
						if (matres != null_)
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), idVar(matres, material_format));
						else
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2531), e_material_FORMAT_NONE);
