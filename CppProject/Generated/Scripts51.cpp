...
namespace CppProject
{
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2467) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2468) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2469);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1264: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2476), /*"unzip"*/ STR(1264) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip({ fname }))
					{
						log({ /*"Error unzipping pack"*/ STR(2477) });
						error(/*"errorunzippack"*/ STR(2478));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1346);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2479) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2476), /*"modeltextures"*/ STR(1346) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2480);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2481) });
			...
			case 2480: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2476), /*"blocktextures"*/ STR(2480) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(979), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(507));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2482);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2483) });
			...
			case 2482: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2476), /*"itemtextures"*/ STR(2482) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2484), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(506));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(606), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(611), /*"_s"*/ STR(2485));
				res_load_pack_item_textures(self, /*"normal"*/ STR(600), /*"_n"*/ STR(2486));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2487) });
