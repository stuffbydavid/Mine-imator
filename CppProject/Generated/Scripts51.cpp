...
namespace CppProject
{
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2576), IntType(0));
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2579) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2580) + filename_ext(fn));
					texture_export(sInt(block_sheet_texture), global::save_folder + /*"/"*/ STR(16) + sVar(filename));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2581);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1316: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2588), /*"unzip"*/ STR(1316) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2589) });
						error(/*"errorunzippack"*/ STR(2590));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1398);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2591) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2588), /*"modeltextures"*/ STR(1398) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2592);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2593) });
			...
			case 2592: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2588), /*"blocktextures"*/ STR(2592) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(1031), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(518));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2594);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2595) });
			...
			case 2594: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2588), /*"itemtextures"*/ STR(2594) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2596), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(517));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(649), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(654), /*"_s"*/ STR(2597));
				res_load_pack_item_textures(self, /*"normal"*/ STR(643), /*"_n"*/ STR(2598));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2599) });
