...
namespace CppProject
{
	...
	RealType render_world_ground(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2536), c_white, IntType(1));
		...
		render_set_uniform_int(/*"uIsGround"*/ STR(2537), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1914), blend, IntType(1));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2538), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2539), IntType(0));
		render_set_uniform_int(/*"uFogShow"*/ STR(1923), global::_app->background_fog_show);
		render_set_uniform_int(/*"uIsWater"*/ STR(2535), iswater);
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), idVar(sVar(background_ground_tex_material), material_format));
		if (sVar(background_ground_tex_material) == global::mc_res)
		{
			render_set_uniform(/*"uMetallic"*/ STR(2532), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2533), (iswater && global::_app->project_render_water_reflections > 0 ? .07 : 1.0));
			render_set_uniform(/*"uEmissive"*/ STR(2534), IntType(0));
		}
		else
		{
			render_set_uniform(/*"uMetallic"*/ STR(2532), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2533), IntType(0));
			render_set_uniform(/*"uEmissive"*/ STR(2534), IntType(0));
		}
		...
		if (sBool(background_ground_material_ani))
			render_set_texture(sArr(background_ground_ani_texture_material).Value(block_texture_get_frame()), /*"Material"*/ STR(2530));
		else
			render_set_texture(sInt(background_ground_texture_material), /*"Material"*/ STR(2530));
		
		if (sBool(background_ground_normal_ani))
			render_set_texture(sArr(background_ground_ani_texture_normal).Value(block_texture_get_frame()), /*"Normal"*/ STR(2531));
		else
			render_set_texture(sInt(background_ground_texture_normal), /*"Normal"*/ STR(2531));
	...
	void render_world_item(VarType vbuffer, VarType is3d, VarType facecamera, VarType bounce, VarType rotate, ArrType res)
	{
		...
		if (res.Value(IntType(1)) != null_ && res.Value(IntType(1)) != global::mc_res)
		{
			if (global::shader_uniform_metallic != IntType(0))
			{
				...
				render_set_uniform(/*"uMetallic"*/ STR(2532), global::shader_uniform_metallic);
			}
			if (global::shader_uniform_roughness != IntType(0))
			{
				global::shader_uniform_roughness = IntType(0);
				render_set_uniform(/*"uRoughness"*/ STR(2533), global::shader_uniform_roughness);
			}
			if (global::shader_uniform_emissive != IntType(0))
			{
				global::shader_uniform_emissive = IntType(0);
				render_set_uniform(/*"uEmissive"*/ STR(2534), global::shader_uniform_emissive);
			}
			if (idInt(res.Value(IntType(1)), item_sheet_texture_material) != null_)
				render_set_texture(idInt(res.Value(IntType(1)), item_sheet_texture_material), /*"Material"*/ STR(2530));
			else
				render_set_texture(idVar(res.Value(IntType(1)), texture), /*"Material"*/ STR(2530));
			
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), idVar(res.Value(IntType(1)), material_format));
		}
		else
		{
			render_set_texture(ID_spr_default_material, /*"Material"*/ STR(2530));
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), e_material_FORMAT_NONE);
		}
		
		if (res.Value(IntType(2)) != null_ && res.Value(IntType(2)) != global::mc_res)
		{
			if (idInt(res.Value(IntType(2)), item_sheet_tex_normal) != null_)
				render_set_texture(idInt(res.Value(IntType(2)), item_sheet_tex_normal), /*"Normal"*/ STR(2531));
			else
				render_set_texture(idVar(res.Value(IntType(2)), texture), /*"Normal"*/ STR(2531));
			
		}
		else
			render_set_texture(ID_spr_default_normal, /*"Normal"*/ STR(2531));
	...
	void render_world_model_file_parts(ScopeAny self, VarType modelfile, VarType res, IntType texnamemap, IntType hidelist, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, IntType matrixmap)
	{
		...
		for (IntType p = IntType(0); p < ds_list_size(partlist); p++)
		{
			...
			if (idBool(part, part_mixing_shapes))
				render_set_uniform_int(/*"uColorsExt"*/ STR(2081), idBool(part, part_mixing_shapes));
	...
	RealType render_world_model_part(ScopeAny self, VarType part, VarType res, IntType texnamemap, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, VarType tlobject)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			if (tlobject > 0)
			{
				...
				render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), sArr(model_part_shape_material_res).Value(s));
				...
				{
					render_set_texture(sArr(model_part_shape_tex_material).Value(s), /*"Material"*/ STR(2530));
				...
				if (sArr(model_part_shape_tex_normal).Value(s) == null_)
					...
					render_set_texture(sArr(model_part_shape_tex_normal).Value(s), /*"Normal"*/ STR(2531));
			...
			if (self->subAssetId == ID_obj_preview && Obj(sVar(select))->subAssetId != ID_obj_resource && idVar(sVar(select), model_name) == /*"armor"*/ STR(751))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(1995))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"chestplate"*/ STR(1996))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"leggings"*/ STR(1997))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"boots"*/ STR(1998))
				{
			...
			{
				render_set_uniform_color(/*"uBlendColor"*/ STR(1914), blendcolor, alpha);
			...
			{
				if (tlobject != null_)
					render_set_uniform_color(/*"uMixColor"*/ STR(2087), merge_color((IntType)(ObjType(obj_model_shape, shape)->color_mix), (IntType)(sArr(value_inherit).Value(e_value_MIX_COLOR)), sArr(value_inherit).Value(e_value_MIX_PERCENT)), lerp(ObjType(obj_model_shape, shape)->color_mix_percent, sArr(value_inherit).Value(e_value_MIX_PERCENT), sArr(value_inherit).Value(e_value_MIX_PERCENT)));
				else
					render_set_uniform_color(/*"uMixColor"*/ STR(2087), ObjType(obj_model_shape, shape)->color_mix, ObjType(obj_model_shape, shape)->color_mix_percent);
	...
	RealType render_world_particle(ScopeAny self)
	{
		...
		render_set_uniform_color(/*"uBlendColor"*/ STR(1914), global::shader_blend_color, global::shader_blend_alpha);
	...
	void render_world_shape(VarType type, VarType vbuf, VarType facecamera, ArrType tex)
	{
		...
		render_set_texture(tex.Value(IntType(1)), /*"Material"*/ STR(2530));
		render_set_texture(tex.Value(IntType(2)), /*"Normal"*/ STR(2531));
		vbuffer_render({ vbuf });
	}
	...
	RealType render_world_sky()
	{
		...
		if (global::_app->background_fog_show > 0 && global::_app->background_fog_sky > 0)
		{
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1914), global::_app->background_fog_color_final, IntType(1));
		...
		if (!(global::_app->background_image_show > 0))
		{
			...
			if (global::_app->background_night_alpha > IntType(0))
			{
				...
				render_set_uniform_color(/*"uBlendColor"*/ STR(1914), c_white, 0.4 * global::_app->background_night_alpha);
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1914), c_white, vis);
	...
	RealType render_world_sky_clouds(ScopeAny self)
	{
		if (!(sVar(background_sky_clouds_show) > 0) || !(global::render_background > 0) || global::render_mode == e_render_mode_DEPTH_NO_SKY)
			return IntType(0);
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2536), c_black, IntType(1));
		VarType res = sVar(background_sky_clouds_tex);
		if (!res_is_ready(res))
			res = global::mc_res;
		render_set_uniform_int(/*"uIsSky"*/ STR(2540), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1914), sInt(background_sky_clouds_final), sReal(background_clouds_alpha));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2538), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2539), IntType(0));
		render_set_uniform(/*"uMetallic"*/ STR(2532), IntType(0));
		render_set_uniform(/*"uRoughness"*/ STR(2533), IntType(1));
		render_set_uniform(/*"uEmissive"*/ STR(2534), IntType(0));
		render_set_uniform(/*"uLightSpecular"*/ STR(2541), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1923), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1923), global::_app->background_fog_show);
		return 0.0;
	}
	...
	void render_world_text(VarType vbuffer, VarType tex, VarType facecamera, VarType res, VarType outline)
	{
		...
		if (outline != null_)
		{
			render_set_uniform_color(/*"uBlendColor"*/ STR(1914), outline, global::shader_blend_alpha);
	...
	RealType render_world_tl(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_CLICK)
		{
			if (sReal(selected) > 0 || sVar(lock) > 0 || !tl_update_list_filter(self->id))
				return IntType(0);
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2536), self->id, IntType(1));
		}
		...
		if (global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2536), (sVar(ssao) > 0) ? c_white : c_black, IntType(1));
		if (sBool(colors_ext) != global::shader_uniform_color_ext || sArr(value_inherit).Value(e_value_RGB_ADD) != global::shader_uniform_rgb_add || sArr(value_inherit).Value(e_value_HSB_ADD) != global::shader_uniform_hsb_add || sArr(value_inherit).Value(e_value_RGB_SUB) != global::shader_uniform_rgb_sub || sArr(value_inherit).Value(e_value_HSB_SUB) != global::shader_uniform_hsb_sub || sArr(value_inherit).Value(e_value_HSB_MUL) != global::shader_uniform_hsb_mul || sArr(value_inherit).Value(e_value_MIX_COLOR) != global::shader_uniform_mix_color || sArr(value_inherit).Value(e_value_MIX_PERCENT) != global::shader_uniform_mix_percent)
		{
			...
			render_set_uniform_int(/*"uColorsExt"*/ STR(2081), global::shader_uniform_color_ext);
			render_set_uniform_color(/*"uRGBAdd"*/ STR(2082), global::shader_uniform_rgb_add, IntType(1));
			render_set_uniform_color(/*"uHSBAdd"*/ STR(2084), global::shader_uniform_hsb_add, IntType(1));
			render_set_uniform_color(/*"uRGBSub"*/ STR(2083), global::shader_uniform_rgb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBSub"*/ STR(2085), global::shader_uniform_hsb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBMul"*/ STR(2086), global::shader_uniform_hsb_mul, IntType(1));
			render_set_uniform_color(/*"uMixColor"*/ STR(2087), global::shader_uniform_mix_color, global::shader_uniform_mix_percent);
		}
		...
		if (sVar(wind) != global::shader_uniform_wind)
		{
			...
			render_set_uniform(/*"uWindEnable"*/ STR(1916), global::shader_uniform_wind);
		}
		if (sVar(wind_terrain) != global::shader_uniform_wind_terrain)
		{
			global::shader_uniform_wind_terrain = sVar(wind_terrain);
			render_set_uniform(/*"uWindTerrain"*/ STR(1917), global::shader_uniform_wind_terrain);
		}
		if ((global::_app->background_fog_show > 0 && sVar(fog) > 0) != global::shader_uniform_fog)
		{
			...
			render_set_uniform_int(/*"uFogShow"*/ STR(1923), global::shader_uniform_fog);
		}
		if (sArr(value_inherit).Value(e_value_SUBSURFACE) != global::shader_uniform_sss || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_RED) != global::shader_uniform_sss_red || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_GREEN) != global::shader_uniform_sss_green || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_BLUE) != global::shader_uniform_sss_blue || sArr(value_inherit).Value(e_value_SUBSURFACE_COLOR) != global::shader_uniform_sss_color)
		{
			...
			render_set_uniform(/*"uSSS"*/ STR(2542), global::shader_uniform_sss);
			render_set_uniform_vec3(/*"uSSSRadius"*/ STR(2543), global::shader_uniform_sss_red, global::shader_uniform_sss_green, global::shader_uniform_sss_blue);
			render_set_uniform_color(/*"uSSSColor"*/ STR(2544), global::shader_uniform_sss_color, 1.0);
		}
		if (sArr(value_inherit).Value(e_value_WIND_INFLUENCE) != global::shader_uniform_wind_strength)
		{
			...
			render_set_uniform(/*"uWindStrength"*/ STR(1919), global::shader_uniform_wind_strength);
			render_set_uniform(/*"uWindDirectionalStrength"*/ STR(1922), global::shader_uniform_wind_strength * global::_app->background_wind_directional_strength);
		}
		...
		if (sVar(glow) != global::shader_uniform_glow || sVar(glow_texture) != global::shader_uniform_glow_texture || sArr(value_inherit).Value(e_value_GLOW_COLOR) != global::shader_uniform_glow_color)
		{
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2545), IntType(1));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2539), sVar(glow_texture));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2538), global::shader_uniform_glow_color, IntType(1));
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2545), IntType(0));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2539), IntType(0));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2538), c_black, IntType(0));
			}
		...
		if (DsMap(idInt(global::render_shader_obj, uniform_map)).Value(/*"uGlintEnabled"*/ STR(2546)) > -IntType(1))
			texture_set_stage(DsMap(idInt(global::render_shader_obj, sampler_map)).Value(/*"uGlintTexture"*/ STR(2547)), sprite_get_texture((IntType)(tex), IntType(0)));
		spd = global::_app->background_time * sVar(glint_speed) * global::_app->project_render_glint_speed;
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2546), sVar(glint_mode) == e_glint_NONE ? IntType(0) : IntType(1));
		render_set_uniform_vec2(/*"uGlintOffset"*/ STR(2548), spd * (0.000625), spd * (0.00125));
		render_set_uniform(/*"uGlintStrength"*/ STR(2549), global::_app->project_render_glint_strength * sVar(glint_strength));
		render_set_uniform_vec2(/*"uGlintSize"*/ STR(2550), sprite_get_width((IntType)(tex)) * IntType(2) * sVar(glint_scale), sprite_get_height((IntType)(tex)) * IntType(2) * sVar(glint_scale));
		if (sVar(type) != e_tl_type_PARTICLE_SPAWNER)
		{
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_tl_type_PATH:
				{
					if (sVar(path_vbuffer) != null_)
					{
						...
						else
						{
							...
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), idVar(sArr(value_inherit).Value(e_value_TEXTURE_MATERIAL_OBJ), material_format));
						}
						...
						render_set_texture(tex);
						render_set_texture(texmat, /*"Material"*/ STR(2530));
						render_set_texture(texnorm, /*"Normal"*/ STR(2531));
				...
				default:
				{
					...
					withOne (Object, sVar(temp), self->id)
					{
						...
						if (matres != null_)
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), idVar(matres, material_format));
						else
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2529), e_material_FORMAT_NONE);
