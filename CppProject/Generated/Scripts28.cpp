...
namespace CppProject
{
	VarType draw_textfield_group(ScopeAny self, VarArgs argument)
	{
		...
		for (IntType i = IntType(0); i < sReal(textfield_amount); i++)
		{
			...
			context_menu_area(self, { fieldx, boxy, boxwid, boxhei, /*"contextmenuvalue"*/ STR(1042), sVar(textfield_value).Value(i), e_context_type_NUMBER, sVar(textfield_script).Value(i), sVar(textfield_default).Value(i) });
			...
			microani_update({ mouseon, mouseon && sBool(mouse_left), sVar(window_focus) == string(sVar(textfield_textbox).Value(i)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"drag"*/ STR(30)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1050)), false, active });
			...
			if (app_mouse_box(self, fieldx, boxy, boxwid, boxhei) && sBool(content_mouseon) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && drag > 0)
			{
				...
				if (sBool(mouse_left_pressed))
					sVar(window_busy) = sVar(textfield_name).Value(i) + /*"press"*/ STR(1050);
			}
			if (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1050))
			{
				sInt(mouse_cursor) = cr_size_we;
				if (!sBool(mouse_left))
				{
			...
			if (sVar(window_busy) != sVar(textfield_name).Value(i) + /*"press"*/ STR(1050) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && fieldupdate == VarType())
	...
	RealType draw_wheel(ScopeAny self, VarArgs argument)
	{
		...
		if (xx + rad < sVar(content_x) || xx - rad > sVar(content_x) + sVar(content_width) || yy + rad < sVar(content_y) || yy - rad > sVar(content_y) + sVar(content_height))
			return IntType(0);
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1042), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1042), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
