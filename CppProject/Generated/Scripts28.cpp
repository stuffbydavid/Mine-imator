...
namespace CppProject
{
	...
	RealType draw_recent(ScopeAny self, VarArgs argument)
	{
		...
		if (mode == /*"simple"*/ STR(1111))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				...
				if (hover)
				{
					if (draw_button_icon(self, { /*"recentdelete"*/ STR(1112) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1113) }))
				...
				microani_set(/*"recent"*/ STR(1114) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
		...
		if (mode == /*"list"*/ STR(1115))
		{
			...
			draw_label(self, { text_get({ /*"recentfilename"*/ STR(1116) }), namex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			draw_label(self, { text_get({ /*"recentlastopened"*/ STR(1117) }), timex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			recenty += IntType(28);
			draw_set_font(sInt(font_value));
			for (VarType i = liststart; i < sReal(recent_list_amount); i++)
			{
				...
				{
					if (draw_button_icon(self, { /*"recentpin"*/ STR(1118) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1119) }))
		...
		if (mode == /*"grid"*/ STR(653))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				if (cardy < yy + hei || cardy + IntType(240) + IntType(16) > yy)
				{
					...
					if (hover || idVar(item, pinned) > 0)
					{
						if (draw_button_icon(self, { /*"recentpin"*/ STR(1118) + string(item), iconx, cardy + IntType(12), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1119), null_ }))
							action_recent_pin(item);
					}
					iconx -= 28.0;
					if (hover)
					{
						if (draw_button_icon(self, { /*"recentdelete"*/ STR(1112), iconx, cardy + IntType(12), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1113), null_ }))
							action_recent_remove(item);
					}
					microani_set(/*"recent"*/ STR(1114) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
	...
	void draw_subheader(ScopeAny self, VarType cat, VarType xx, VarType yy, RealType w, RealType h)
	{
		...
		microani_set(ObjType(obj_category, cat)->name + /*"close"*/ STR(1120), null_, false, false, ObjType(obj_category, cat)->show, false);
		IntType color;
	...
	RealType draw_switch(ScopeAny self, StringType name, VarType xx, VarType yy, VarType active, IntType script, StringType tip, BoolType disabled)
	{
		...
		draw_box_bevel({ buttonx, buttony, IntType(8), IntType(10), IntType(1), ObjType(obj_theme, sInt(setting_theme))->name == /*"light"*/ STR(654) });
	...
	VarType draw_textfield_group(ScopeAny self, StringType name, VarType xx, VarType yy, VarType wid, VarType mul, VarType minval, VarType maxval, RealType snapval, RealType showcaption, RealType stack, RealType colortype, BoolType drag, BoolType textfield_update)
	{
		...
		for (IntType i = IntType(0); i < sReal(textfield_amount); i++)
		{
			...
			context_menu_area(self, { fieldx, boxy, boxwid, boxhei, /*"contextmenuvalue"*/ STR(1090), sVar(textfield_value).Value(i), e_context_type_NUMBER, sVar(textfield_script).Value(i), sVar(textfield_default).Value(i) });
			...
			microani_update({ mouseon, mouseon && sBool(mouse_left), sVar(window_focus) == string(sVar(textfield_textbox).Value(i)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"drag"*/ STR(30)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1098)), false, active });
			...
			if (app_mouse_box(self, fieldx, boxy, boxwid, boxhei) && sBool(content_mouseon) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && drag)
			{
				...
				if (sBool(mouse_left_pressed))
					sVar(window_busy) = sVar(textfield_name).Value(i) + /*"press"*/ STR(1098);
			}
			if (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1098))
			{
				sInt(mouse_cursor) = cr_size_we;
				if (!sBool(mouse_left))
				{
			...
			if (sVar(window_busy) != sVar(textfield_name).Value(i) + /*"press"*/ STR(1098) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && fieldupdate == VarType())
