...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		{
			json_save_array_start({ /*"parts"*/ STR(1806) });
		...
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1827), self->lock_bend);
			...
			json_save_var_bool(/*"bend"*/ STR(1829), self->inherit_bend);
		...
		{
			json_save_var_bool(/*"backfaces"*/ STR(1828), self->backfaces);
	...
	RealType recent_load(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->description = value_get_string(DsMap(projectmap).Value(/*"description"*/ STR(1809)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->filename = value_get_string(DsMap(projectmap).Value(/*"filename"*/ STR(1234)), /*""*/ STR(0));
	...
	void recent_save()
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"description"*/ STR(1809), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2518), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1234), json_string_encode(idVar(recent, filename)));
