...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var_nullable(/*"color_tag"*/ STR(2458), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(720), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2459), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2460), self->ghost);
		json_save_var(/*"depth"*/ STR(611), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2461), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			...
			json_save_var_save_id(/*"part_of"*/ STR(2462), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2463), self->part_root);
		...
		project_save_values(/*"default_values"*/ STR(2465), self->value_default, global::_app->value_default);
		...
		json_save_var(/*"parent_tree_index"*/ STR(2466), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1825), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2467), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2468) });
			...
			json_save_var_bool(/*"alpha"*/ STR(2300), self->inherit_alpha);
			...
			json_save_var_bool(/*"visibility"*/ STR(2470), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1827), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2469), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2471), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2472), self->inherit_select);
			json_save_var_bool(/*"pose"*/ STR(2473), self->inherit_pose);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2474), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2475), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2469), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(1826), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2476), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2389), self->texture_filtering);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2477), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2478), self->only_render_glow);
			json_save_var(/*"glint_mode"*/ STR(2479), self->glint_mode);
			json_save_var(/*"glint_scale"*/ STR(2480), self->glint_scale);
			json_save_var(/*"glint_speed"*/ STR(2387), self->glint_speed);
			json_save_var(/*"glint_strength"*/ STR(2388), self->glint_strength);
			json_save_var_save_id(/*"glint_tex"*/ STR(2481), self->glint_tex);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2482), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2483), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2484), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2485), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2486), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(532) });
			json_save_var_bool(/*"smooth"*/ STR(2487), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2456), self->path_closed);
			json_save_var(/*"detail"*/ STR(2457), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2488), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2489), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2490), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2491), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2492), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2493), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2494), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2495), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2511) });
		...
		self->sprite_template = /*"generic"*/ STR(2512);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2513);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->author = value_get_string(DsMap(projectmap).Value(/*"author"*/ STR(2324)), /*""*/ STR(0));
				...
				ObjType(obj_recent, recentobj)->last_opened = value_get_real(DsMap(projectmap).Value(/*"last_opened"*/ STR(2514)), -IntType(1));
				ObjType(obj_recent, recentobj)->pinned = value_get_real(DsMap(projectmap).Value(/*"pinned"*/ STR(2515)), false);
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2516);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2517) });
		...
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"author"*/ STR(2324), json_string_encode(idVar(recent, author)));
			json_save_var(/*"description"*/ STR(1807), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2518), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1232), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2514), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2515), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2519) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2520) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2521) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2522) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2523) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2524) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2525), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2526), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2527), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2528), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
