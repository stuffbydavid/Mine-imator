...
namespace CppProject
{
	...
	VarType history_save_var_start(ScopeAny self, IntType script, RealType combine)
	{
		...
		{
			history_push(self);
			debug({ /*"history_save_var_start("*/ STR(1184) + script_get_name(script) + /*", "*/ STR(1014) + string(combine) + /*")"*/ STR(1077) });
	...
	IntType history_set_res(ScopeAny self, IntType script, VarType fn, VarType oldres, IntType newres)
	{
		history_pop(self);
		history_push(self);
		log({ /*"Action Load resource"*/ STR(1186), script_get_name(script), fn });
	...
	VarType history_set_var(ScopeAny self, IntType script, VarType oldval, VarType newval, RealType combine)
	{
		...
		{
			history_push(self);
			log({ /*"Action Set variable"*/ STR(1187), script_get_name(script), oldval, newval, combine });
			hobj = new_history(script);
	...
	BoolType json_load_char()
	{
		if (buffer_is_eof())
		{
			global::json_error = /*"Unexpected end of file"*/ STR(1193);
			return false;
		}
		do
		{
	...
	BoolType json_load_number()
	{
		...
		do
		{
			...
			if (global::json_char == e_json_char_E || global::json_char == e_json_char_CAPITAL_E)
			{
				if (expart)
				{
					global::json_error = /*"Unexpected symbol"*/ STR(1194);
					return false;
				}
				global::json_char = buffer_read_byte();
				global::json_column++;
		...
		{
			global::json_error = /*"Invalid number \""*/ STR(1195) + str + /*"\""*/ STR(1104);
			return false;
		}
		return true;
	}
	...
	BoolType json_load_object(BoolType root)
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \":\""*/ STR(1196);
				break;
			}
			if (!json_load_char())
				break;
	...
	BoolType json_load_string()
	{
		...
		while (true)
		{
			...
			{
				global::json_error = /*"Unexpected linebreak in string"*/ STR(1197);
				return false;
			}
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1198);
					return false;
				}
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1214), fn });
