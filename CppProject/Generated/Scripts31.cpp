...
namespace CppProject
{
	...
	VarType history_save_var_start(ScopeAny self, IntType script, RealType combine)
	{
		...
		{
			history_push(self);
			debug({ /*"history_save_var_start("*/ STR(1182) + script_get_name(script) + /*", "*/ STR(1012) + string(combine) + /*")"*/ STR(1075) });
	...
	IntType history_set_res(ScopeAny self, IntType script, VarType fn, VarType oldres, IntType newres)
	{
		history_pop(self);
		history_push(self);
		log({ /*"Action Load resource"*/ STR(1184), script_get_name(script), fn });
	...
	VarType history_set_var(ScopeAny self, IntType script, VarType oldval, VarType newval, RealType combine)
	{
		...
		{
			history_push(self);
			log({ /*"Action Set variable"*/ STR(1185), script_get_name(script), oldval, newval, combine });
			hobj = new_history(script);
	...
	BoolType json_load_array()
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \",\""*/ STR(1190);
				break;
			}
		}
	...
	BoolType json_load_char()
	{
		if (buffer_is_eof())
		{
			global::json_error = /*"Unexpected end of file"*/ STR(1191);
			return false;
		}
		do
		{
	...
	BoolType json_load_number()
	{
		...
		do
		{
			...
			if (global::json_char == e_json_char_E || global::json_char == e_json_char_CAPITAL_E)
			{
				if (expart)
				{
					global::json_error = /*"Unexpected symbol"*/ STR(1192);
					return false;
				}
				global::json_char = buffer_read_byte();
				global::json_column++;
		...
		{
			global::json_error = /*"Invalid number \""*/ STR(1193) + str + /*"\""*/ STR(1102);
			return false;
		}
		return true;
	}
	...
	BoolType json_load_object(BoolType root)
	{
		...
		while (json_load_char())
		{
			...
			{
				global::json_error = /*"Expected \":\""*/ STR(1194);
				break;
			}
			if (!json_load_char())
				break;
	...
	BoolType json_load_string()
	{
		...
		while (true)
		{
			...
			{
				global::json_error = /*"Unexpected linebreak in string"*/ STR(1195);
				return false;
			}
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1196);
					return false;
				}
	...
	void json_save_var_bool(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		buffer_write_string((val > 0) ? /*"true"*/ STR(744) : /*"false"*/ STR(745));
		global::json_add_comma = true;
	}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(826) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	BoolType keybind_check(VarType keybindobj, StringType checkscript)
	{
		...
		if (checkscript == /*"keyboard_check"*/ STR(674))
		{
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1212), fn });
