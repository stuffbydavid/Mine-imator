...
namespace CppProject
{
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(743), name);
			name = /*"modelstate"*/ STR(743) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(762), name);
				name = /*"blockstate"*/ STR(762) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(718);
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(719);
			app_mouse_clear(self);
		}
		if (mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(719);
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1042));
	...
	RealType draw_button_transition(ScopeAny self, VarType xx, VarType yy, VarType transition)
	{
		...
		tipname = transition;
		if (tipname != /*"linear"*/ STR(534) && tipname != /*"instant"*/ STR(535) && tipname != /*"bezier"*/ STR(536))
		{
			tipname = string_replace(tipname, /*"easeinout"*/ STR(1043), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easein"*/ STR(1044), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easeout"*/ STR(1045), /*""*/ STR(0));
			tipname = /*"ease"*/ STR(1046) + tipname;
		}
		if (transition == /*"bezier"*/ STR(536) && !(sReal(setting_advanced_mode) > 0))
			return IntType(0);
		return draw_button_icon(self, { /*"menu"*/ STR(1041) + transition, xx, yy, IntType(36), IntType(36), active, null_, null_, false, /*"transition"*/ STR(1047) + tipname, DsMap(sVar(transition_texture_map)).Value(transition) });
	}
	...
	RealType draw_dragger(ScopeAny self, VarArgs argument)
	{
		...
		if (!(disabled > 0))
			context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1040), value, e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
		...
		if (dragmouseon && sBool(mouse_left_pressed))
			sVar(window_focus) = name + /*"press"*/ STR(1048);
		if (sVar(window_focus) == name + /*"press"*/ STR(1048))
		{
		...
		if (sVar(window_busy) != name + /*"drag"*/ STR(30) && sVar(window_busy) != name + /*"press"*/ STR(1048) && sVar(window_focus) != string(tbx))
			idVar(tbx, text) = string_decimals(value);
		return 0.0;
	}
	...
	RealType draw_dragger_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1040), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
	...
	RealType draw_inputbox(ScopeAny self, VarArgs argument)
	{
		...
		if (xx + w < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || yy + h < sVar(content_y) || yy > sVar(content_y) + sVar(content_height))
			return IntType(0);
		BoolType mouseon;
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == string(tbx) + /*"click"*/ STR(1049)) && !(disabled > 0);
		...
		if (string_contains(inputname, /*"search"*/ STR(1050)))
		{
