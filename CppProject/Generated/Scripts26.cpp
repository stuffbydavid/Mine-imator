...
namespace CppProject
{
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1084))) + /*"/"*/ STR(16);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(910);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(911);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(927);
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, RealType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1085);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1086);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1085))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1086))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1087) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, text_get({ /*"tooltipcustomaccentcolor"*/ STR(1088) }), xx, yy, width, height);
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1089), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	RealType draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, VarType def, BoolType hsvmode, IntType script)
	{
		...
		active = (sStr(settings_menu_name) == /*"colorpicker"*/ STR(717) && ObjType(obj_colorpicker, sInt(colorpicker))->value_name == name);
		mouseon = app_mouse_box(self, buttonx, yy, buttonw, h) && sBool(content_mouseon);
		mouseclick = mouseon && sBool(mouse_left);
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		click = context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1090), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(1046);
		}
		return click;
	}
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(783), name);
			name = /*"modelstate"*/ STR(783) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(804), name);
				name = /*"blockstate"*/ STR(804) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(758);
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(759);
			app_mouse_clear(self);
		}
		if (mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(759);
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1092));
