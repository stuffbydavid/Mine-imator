...
namespace CppProject
{
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		if (mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1045));
	...
	RealType draw_button_transition(ScopeAny self, VarType xx, VarType yy, VarType transition)
	{
		...
		{
			tipname = string_replace(tipname, /*"easeinout"*/ STR(1046), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easein"*/ STR(1047), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easeout"*/ STR(1048), /*""*/ STR(0));
			tipname = /*"ease"*/ STR(1049) + tipname;
		}
		if (transition == /*"bezier"*/ STR(537) && !(sReal(setting_advanced_mode) > 0))
			return IntType(0);
		return draw_button_icon(self, { /*"menu"*/ STR(1044) + transition, xx, yy, IntType(36), IntType(36), active, null_, null_, false, /*"transition"*/ STR(1050) + tipname, DsMap(sVar(transition_texture_map)).Value(transition) });
	}
	...
	RealType draw_dragger(ScopeAny self, StringType name, VarType xx, VarType yy, RealType wid, VarType value, RealType mul, RealType minval, VarType maxval, VarType def, RealType snapval, VarType tbx, IntType script, VarType capwidth, BoolType showcaption, BoolType disabled, StringType tip)
	{
		...
		if (!disabled)
			context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1043), value, e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
		...
		if (dragmouseon && sBool(mouse_left_pressed))
			sVar(window_focus) = name + /*"press"*/ STR(1051);
		if (sVar(window_focus) == name + /*"press"*/ STR(1051))
		{
		...
		if (sVar(window_busy) != name + /*"drag"*/ STR(30) && sVar(window_busy) != name + /*"press"*/ STR(1051) && sVar(window_focus) != string(tbx))
			idVar(tbx, text) = string_decimals(value);
		return 0.0;
	}
	...
	RealType draw_dragger_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1043), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
	...
	RealType draw_inputbox(ScopeAny self, VarArgs argument)
	{
		...
		if (xx + w < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || yy + h < sVar(content_y) || yy > sVar(content_y) + sVar(content_height))
			return IntType(0);
		BoolType mouseon;
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == string(tbx) + /*"click"*/ STR(1052)) && !(disabled > 0);
		...
		if (string_contains(inputname, /*"search"*/ STR(1053)))
		{
	...
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		name = /*"settingskey"*/ STR(1055) + idVar(keyobj, name);
		w = sVar(dw);
		...
		context_menu_area(self, { xx, yy, w, sReal(tab_control_h), /*"keybind"*/ STR(1056), keybindID, null_, null_, null_ });
		...
		if (draw_button_icon(self, { name + /*"edit"*/ STR(116), sVar(dx) + w - IntType(24), sVar(dy) + IntType(2), IntType(24), IntType(24), sVar(window_busy) == name, icons_PENCIL, null_, false, /*"tooltipeditkeybind"*/ STR(1057) }))
		{
