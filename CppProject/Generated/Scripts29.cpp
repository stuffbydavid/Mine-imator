...
namespace CppProject
{
			...
			if (char_ == /*" "*/ STR(17))
				...
				if (char_ == /*"("*/ STR(1072))
				{
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1075))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1076))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1072) || ds_stack_top(ops) == /*"-("*/ STR(1074)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1077))
							{
								lastoperator = /*"--"*/ STR(1073);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1077))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1034), /*"1"*/ STR(1081), /*"2"*/ STR(1082), /*"3"*/ STR(1083), /*"4"*/ STR(1084), /*"5"*/ STR(1085), /*"6"*/ STR(1086), /*"7"*/ STR(1087), /*"8"*/ STR(1088), /*"9"*/ STR(1089), /*"."*/ STR(1075) });
	...
	{
		log({ /*"execute"*/ STR(1090), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1091) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1092), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1093);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1094) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1095), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1096) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			global::render_active = /*"movie"*/ STR(1097);
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1034), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1098) + numstr + /*".png"*/ STR(6));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1099), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1100) + string(num++) + /*")"*/ STR(1076) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(16), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"\\"*/ STR(1101), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*":"*/ STR(754), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"*"*/ STR(1079), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"?"*/ STR(1102), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"\""*/ STR(1103), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"<"*/ STR(1104), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*">"*/ STR(1105), /*"_"*/ STR(1098));
		fn = string_replace_all(fn, /*"|"*/ STR(850), /*"_"*/ STR(1098));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1101) || c == /*"/"*/ STR(16))
				return fn + newext;
			if (c == /*"."*/ STR(1075))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1106) }) + /*"|"*/ STR(850) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1107) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1108) }) + /*" (*.ttf)|*.ttf"*/ STR(1109), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1110) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1111) }) + /*" (*.png; *.jpg)|*.png;*.jpg;*.jpeg;"*/ STR(1112), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1113) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1114) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1115), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1116) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1117) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1118), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1119) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1120) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1121), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1122) }));
	}
	
	StringType file_dialog_open_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1123) }) + /*" (*.zip)|*.zip"*/ STR(1124), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1125) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1126) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1127), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1128) }));
	}
	
	StringType file_dialog_open_project(ScopeAny self)
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1129) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1130) + text_get({ /*"filedialogopenbackup"*/ STR(1131) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1132), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1133) }));
	}
	
	StringType file_dialog_open_render()
	{
		return file_dialog_open(text_get({ /*"filedialogopenrender"*/ STR(1134) }) + /*" (*.mirender)|*mirender"*/ STR(1135), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenrendercaption"*/ STR(1136) }));
	}
	
	StringType file_dialog_open_scenery()
	{
		return file_dialog_open(text_get({ /*"filedialogopenscenery"*/ STR(1137) }) + /*" (*.schematic; *.nbt; *.blocks)|*.schematic;*.nbt;*.blocks"*/ STR(1138), /*""*/ STR(0), schematics_directory, text_get({ /*"filedialogopenscenerycaption"*/ STR(1139) }));
	}
	
	StringType file_dialog_open_sound()
	{
		return file_dialog_open(text_get({ /*"filedialogopensound"*/ STR(1140) }) + /*" (*.mp3; *.wav; *.ogg; *.flac; *.wma; *.m4a)|*.mp3;*.wav;*.ogg;*.flac;*.wma;*.m4a;"*/ STR(1141), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopensoundcaption"*/ STR(1142) }));
	}
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveimage"*/ STR(1143) }) + /*" (*.png)|*.png"*/ STR(1144), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveimagecaption"*/ STR(1145) }));
	}
	
	StringType file_dialog_save_keyframes(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavekeyframes"*/ STR(1146) }) + /*" (*.miframes)|*.miframes"*/ STR(1147), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsavekeyframescaption"*/ STR(1148) }));
	}
	
	StringType file_dialog_save_movie_mov(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemov"*/ STR(1149) }) + /*" (*.mov)|*.mov"*/ STR(1150), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1151) }));
	}
	
	StringType file_dialog_save_movie_mp4(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemp4"*/ STR(1152) }) + /*" (*.mp4)|*.mp4"*/ STR(1153), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1151) }));
	}
	
	StringType file_dialog_save_movie_png(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviepng"*/ STR(1154) }) + /*" (*.png)|*.png"*/ STR(1144), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1151) }));
	}
	
	StringType file_dialog_save_movie_wmv(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviewmv"*/ STR(1155) }) + /*" (*.wmv)|*.wmv"*/ STR(1156), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1151) }));
	}
	
	StringType file_dialog_save_object(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1157) }) + /*" (*.miobject)|*.miobject"*/ STR(1158), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1159) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1160) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1161), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1162) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1163) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1164) }) + /*" (*.mirender)|*.mirender"*/ STR(1165), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1166) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(672) + exts + /*"|*"*/ STR(1167) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1168) }));
	}
	...
	ArrType file_find(StringType dir, StringType exts)
	{
		...
		StringType f;
		ret = ArrType();
		f = file_find_first(dir + /*"*"*/ STR(1079), IntType(0));
		while (f != /*""*/ STR(0))
		{
	...
	{
		log({ /*"working_directory"*/ STR(1169), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1170) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1171);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1171);
		log({ /*"Trying to save files"*/ STR(1172) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1173) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1174) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1175) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1176) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1177) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1178) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1179) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1180) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1181) });
		return true;
	}
	...
	IntType font_add_lib(VarArgs argument)
	{
		...
		StringType tmpfile;
		VarType aa;
		tmpfile = file_directory + /*"tmp.ttf"*/ STR(1182);
		aa = true;
	...
	IntType hex_to_color(VarType oldstr)
	{
		...
		str = string_upper(str + string_repeat(/*"0"*/ STR(1034), (IntType)(IntType(6) - string_length(oldstr))));
		hex = /*"0123456789ABCDEF"*/ STR(1009);
