...
namespace CppProject
{
	...
	RealType draw_wheel(ScopeAny self, StringType name, VarType xx, VarType yy, VarType color, VarType value, RealType minval, RealType maxval, VarType def, RealType snapval, VarType tbx, IntType script, RealType rad, VarType sprite)
	{
		...
		StringType text;
		VarType labely;
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1090), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
		modval = mod_fix(value, IntType(360));
		capwid = string_width(text_get({ name }) + /*":"*/ STR(796)) + IntType(5);
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1090), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
	...
	VarType ease(VarType func, VarType xx)
	{
		...
		switch (StringType(func).id)
		{
			...
			case 555: // easeinquint
			{
				return xx * xx * xx * xx * xx;
			}
			case 556: // easeoutquint
			{
				return ((xxm1) * xxm1 * xxm1 * xxm1 * xxm1 + IntType(1));
			}
			case 557: // easeinoutquint
			{
				if (xx2 < IntType(1))
			...
			case 559: // easeoutsine
			{
				return sin(xx * (pi_ / 2.0));
			}
			case 560: // easeinoutsine
			{
			...
			case 562: // easeoutexpo
			{
				return -power(2.0, -IntType(10) * xx) + IntType(1);
			}
			case 563: // easeinoutexpo
			{
				if (xx2 < IntType(1))
			...
			case 565: // easeoutcirc
			{
				return sqrt(IntType(1) - (xxm1) * xxm1);
			}
			case 566: // easeinoutcirc
			{
				if (xx2 < IntType(1))
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(17))
				...
				if (char_ == /*"("*/ STR(1121))
				{
					if (lastoperator == /*"--"*/ STR(1122))
						ds_stack_push({ ops, /*"-("*/ STR(1123) });
					else
						ds_stack_push({ ops, /*"("*/ STR(1121) });
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1124))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1125))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1121) || ds_stack_top(ops) == /*"-("*/ STR(1123)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1126))
							{
								lastoperator = /*"--"*/ STR(1122);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1126))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1083), /*"1"*/ STR(1130), /*"2"*/ STR(1131), /*"3"*/ STR(1132), /*"4"*/ STR(1133), /*"5"*/ STR(1134), /*"6"*/ STR(1135), /*"7"*/ STR(1136), /*"8"*/ STR(1137), /*"9"*/ STR(1138), /*"."*/ STR(1124) });
	...
	{
		log({ /*"execute"*/ STR(1139), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1140) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1141), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1142);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1143) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1144), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1145) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			global::render_active = /*"movie"*/ STR(1146);
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1083), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1147) + numstr + /*".png"*/ STR(6));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1148), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1149) + string(num++) + /*")"*/ STR(1125) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(16), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"\\"*/ STR(1150), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*":"*/ STR(796), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"*"*/ STR(1128), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"?"*/ STR(1151), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"\""*/ STR(1152), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"<"*/ STR(1153), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*">"*/ STR(1154), /*"_"*/ STR(1147));
		fn = string_replace_all(fn, /*"|"*/ STR(893), /*"_"*/ STR(1147));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1150) || c == /*"/"*/ STR(16))
				return fn + newext;
			if (c == /*"."*/ STR(1124))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1155) }) + /*"|"*/ STR(893) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1156) }));
	}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1160) }) + /*" (*.png; *.jpg)|*.png;*.jpg;*.jpeg;"*/ STR(1161), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1162) }));
	}
