...
namespace CppProject
{
	...
	RealType draw_wheel(ScopeAny self, StringType name, VarType xx, VarType yy, VarType color, VarType value, RealType minval, RealType maxval, VarType def, RealType snapval, VarType tbx, IntType script, RealType rad, VarType sprite)
	{
		...
		StringType text;
		VarType labely;
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1092), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
		modval = mod_fix(value, IntType(360));
		capwid = string_width(text_get({ name }) + /*":"*/ STR(798)) + IntType(5);
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1092), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
	...
	VarType ease(VarType func, VarType xx)
	{
		...
		switch (StringType(func).id)
		{
			...
			case 557: // easeinquint
			{
				return xx * xx * xx * xx * xx;
			}
			case 558: // easeoutquint
			{
				return ((xxm1) * xxm1 * xxm1 * xxm1 * xxm1 + IntType(1));
			}
			case 559: // easeinoutquint
			{
				if (xx2 < IntType(1))
			...
			case 561: // easeoutsine
			{
				return sin(xx * (pi_ / 2.0));
			}
			case 562: // easeinoutsine
			{
			...
			case 564: // easeoutexpo
			{
				return -power(2.0, -IntType(10) * xx) + IntType(1);
			}
			case 565: // easeinoutexpo
			{
				if (xx2 < IntType(1))
			...
			case 567: // easeoutcirc
			{
				return sqrt(IntType(1) - (xxm1) * xxm1);
			}
			case 568: // easeinoutcirc
			{
				if (xx2 < IntType(1))
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(17))
				...
				if (char_ == /*"("*/ STR(1123))
				{
					if (lastoperator == /*"--"*/ STR(1124))
						ds_stack_push({ ops, /*"-("*/ STR(1125) });
					else
						ds_stack_push({ ops, /*"("*/ STR(1123) });
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1126))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1127))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1123) || ds_stack_top(ops) == /*"-("*/ STR(1125)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1128))
							{
								lastoperator = /*"--"*/ STR(1124);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1128))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1085), /*"1"*/ STR(1132), /*"2"*/ STR(1133), /*"3"*/ STR(1134), /*"4"*/ STR(1135), /*"5"*/ STR(1136), /*"6"*/ STR(1137), /*"7"*/ STR(1138), /*"8"*/ STR(1139), /*"9"*/ STR(1140), /*"."*/ STR(1126) });
	...
	{
		log({ /*"execute"*/ STR(1141), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1142) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1143), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1144);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1145) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1146), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1147) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			global::render_active = /*"movie"*/ STR(1148);
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1085), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1149) + numstr + /*".png"*/ STR(6));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1150), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1151) + string(num++) + /*")"*/ STR(1127) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(16), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"\\"*/ STR(1152), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*":"*/ STR(798), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"*"*/ STR(1130), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"?"*/ STR(1153), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"\""*/ STR(1154), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"<"*/ STR(1155), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*">"*/ STR(1156), /*"_"*/ STR(1149));
		fn = string_replace_all(fn, /*"|"*/ STR(895), /*"_"*/ STR(1149));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1152) || c == /*"/"*/ STR(16))
				return fn + newext;
			if (c == /*"."*/ STR(1126))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1157) }) + /*"|"*/ STR(895) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1158) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1159) }) + /*" (*.ttf)|*.ttf"*/ STR(1160), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1161) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1162) }) + /*" (*.png; *.jpg)|*.png;*.jpg;*.jpeg;"*/ STR(1163), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1164) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1165) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1166), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1167) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1168) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1169), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1170) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1171) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1172), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1173) }));
	}
	
	StringType file_dialog_open_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1174) }) + /*" (*.zip)|*.zip"*/ STR(1175), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1176) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1177) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1178), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1179) }));
	}
	
	VarType file_dialog_open_project(ScopeAny self)
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1180) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1181) + text_get({ /*"filedialogopenbackup"*/ STR(1182) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1183), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1184) }));
	}
