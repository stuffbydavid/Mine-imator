...
namespace CppProject
{
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(17))
				...
				if (char_ == /*"("*/ STR(1073))
				{
					if (lastoperator == /*"--"*/ STR(1074))
						ds_stack_push({ ops, /*"-("*/ STR(1075) });
					else
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1076))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1077))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1073) || ds_stack_top(ops) == /*"-("*/ STR(1075)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1078))
							{
								lastoperator = /*"--"*/ STR(1074);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1078))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1035), /*"1"*/ STR(1082), /*"2"*/ STR(1083), /*"3"*/ STR(1084), /*"4"*/ STR(1085), /*"5"*/ STR(1086), /*"6"*/ STR(1087), /*"7"*/ STR(1088), /*"8"*/ STR(1089), /*"9"*/ STR(1090), /*"."*/ STR(1076) });
	...
	{
		log({ /*"execute"*/ STR(1091), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1092) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1093), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1094);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1095) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1096), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1097) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			global::render_active = /*"movie"*/ STR(1098);
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(60))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1035), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1099) + numstr + /*".png"*/ STR(6));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1100), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1101) + string(num++) + /*")"*/ STR(1077) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(16), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"\\"*/ STR(1102), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*":"*/ STR(755), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"*"*/ STR(1080), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"?"*/ STR(1103), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"\""*/ STR(1104), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"<"*/ STR(1105), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*">"*/ STR(1106), /*"_"*/ STR(1099));
		fn = string_replace_all(fn, /*"|"*/ STR(851), /*"_"*/ STR(1099));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1102) || c == /*"/"*/ STR(16))
				return fn + newext;
			if (c == /*"."*/ STR(1076))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1107) }) + /*"|"*/ STR(851) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1108) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1109) }) + /*" (*.ttf)|*.ttf"*/ STR(1110), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1111) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1112) }) + /*" (*.png; *.jpg)|*.png;*.jpg;*.jpeg;"*/ STR(1113), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1114) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1115) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1116), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1117) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1118) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1119), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1120) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1121) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1122), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1123) }));
	}
	
	StringType file_dialog_open_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1124) }) + /*" (*.zip)|*.zip"*/ STR(1125), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1126) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1127) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1128), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1129) }));
	}
	
	StringType file_dialog_open_project(ScopeAny self)
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1130) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1131) + text_get({ /*"filedialogopenbackup"*/ STR(1132) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1133), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1134) }));
	}
	
	StringType file_dialog_open_render()
	{
		return file_dialog_open(text_get({ /*"filedialogopenrender"*/ STR(1135) }) + /*" (*.mirender)|*mirender"*/ STR(1136), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenrendercaption"*/ STR(1137) }));
	}
	
	StringType file_dialog_open_scenery()
	{
		return file_dialog_open(text_get({ /*"filedialogopenscenery"*/ STR(1138) }) + /*" (*.schematic; *.nbt; *.blocks)|*.schematic;*.nbt;*.blocks"*/ STR(1139), /*""*/ STR(0), schematics_directory, text_get({ /*"filedialogopenscenerycaption"*/ STR(1140) }));
	}
	
	StringType file_dialog_open_sound()
	{
		return file_dialog_open(text_get({ /*"filedialogopensound"*/ STR(1141) }) + /*" (*.mp3; *.wav; *.ogg; *.flac; *.wma; *.m4a)|*.mp3;*.wav;*.ogg;*.flac;*.wma;*.m4a;"*/ STR(1142), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopensoundcaption"*/ STR(1143) }));
	}
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveimage"*/ STR(1144) }) + /*" (*.png)|*.png"*/ STR(1145), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveimagecaption"*/ STR(1146) }));
	}
	
	StringType file_dialog_save_keyframes(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavekeyframes"*/ STR(1147) }) + /*" (*.miframes)|*.miframes"*/ STR(1148), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsavekeyframescaption"*/ STR(1149) }));
	}
	
	StringType file_dialog_save_movie_mov(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemov"*/ STR(1150) }) + /*" (*.mov)|*.mov"*/ STR(1151), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1152) }));
	}
	
	StringType file_dialog_save_movie_mp4(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemp4"*/ STR(1153) }) + /*" (*.mp4)|*.mp4"*/ STR(1154), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1152) }));
	}
	
	StringType file_dialog_save_movie_png(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviepng"*/ STR(1155) }) + /*" (*.png)|*.png"*/ STR(1145), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1152) }));
	}
	
	StringType file_dialog_save_movie_wmv(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviewmv"*/ STR(1156) }) + /*" (*.wmv)|*.wmv"*/ STR(1157), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1152) }));
	}
	
	StringType file_dialog_save_object(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1158) }) + /*" (*.miobject)|*.miobject"*/ STR(1159), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1160) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1161) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1162), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1163) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1164) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1165) }) + /*" (*.mirender)|*.mirender"*/ STR(1166), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1167) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(673) + exts + /*"|*"*/ STR(1168) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1169) }));
	}
	...
	ArrType file_find(StringType dir, StringType exts)
	{
		...
		StringType f;
		ret = ArrType();
		f = file_find_first(dir + /*"*"*/ STR(1080), IntType(0));
		while (f != /*""*/ STR(0))
		{
	...
	{
		log({ /*"working_directory"*/ STR(1170), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1171) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1172);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1172);
		log({ /*"Trying to save files"*/ STR(1173) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1174) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1175) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1176) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1177) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1178) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1179) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1180) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1181) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1182) });
		return true;
	}
	...
	IntType font_add_lib(VarArgs argument)
	{
		...
		StringType tmpfile;
		VarType aa;
		tmpfile = file_directory + /*"tmp.ttf"*/ STR(1183);
		aa = true;
	...
	IntType hex_to_color(VarType oldstr)
	{
		...
		str = string_upper(str + string_repeat(/*"0"*/ STR(1035), (IntType)(IntType(6) - string_length(oldstr))));
		hex = /*"0123456789ABCDEF"*/ STR(1010);
