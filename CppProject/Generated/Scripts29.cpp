...
namespace CppProject
{
	...
	RealType draw_recent(ScopeAny self, VarArgs argument)
	{
		...
		if (mode == /*"simple"*/ STR(1072))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				...
				if (hover)
				{
					if (draw_button_icon(self, { /*"recentdelete"*/ STR(1073) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1074) }))
				...
				microani_set(/*"recent"*/ STR(1075) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
		...
		if (mode == /*"list"*/ STR(1076))
		{
			...
			draw_label(self, { text_get({ /*"recentfilename"*/ STR(1077) }), namex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			draw_label(self, { text_get({ /*"recentlastopened"*/ STR(1078) }), timex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			recenty += IntType(28);
			draw_set_font(sInt(font_value));
			for (VarType i = liststart; i < sReal(recent_list_amount); i++)
			{
				...
				{
					if (draw_button_icon(self, { /*"recentpin"*/ STR(1079) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1080) }))
		...
		if (mode == /*"grid"*/ STR(613))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				if (cardy < yy + hei || cardy + IntType(240) + IntType(16) > yy)
				{
					...
					if (hover || idVar(item, pinned) > 0)
					{
						if (draw_button_icon(self, { /*"recentpin"*/ STR(1079) + string(item), iconx, cardy + IntType(12), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1080), null_ }))
							action_recent_pin(item);
					}
					iconx -= 28.0;
					if (hover)
					{
						if (draw_button_icon(self, { /*"recentdelete"*/ STR(1073), iconx, cardy + IntType(12), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1074), null_ }))
							action_recent_remove(item);
					}
					microani_set(/*"recent"*/ STR(1075) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
	...
	void draw_subheader(ScopeAny self, VarType cat, VarType xx, VarType yy, RealType w, RealType h)
	{
		...
		microani_set(ObjType(obj_category, cat)->name + /*"close"*/ STR(1081), null_, false, false, ObjType(obj_category, cat)->show, false);
		IntType color;
	...
	RealType draw_textfield(ScopeAny self, StringType name, VarType xx, VarType yy, VarType w, RealType h, IntType textbox, IntType script, VarType placeholder, StringType labelpos, BoolType err)
	{
		...
		fieldy = yy;
		draw_set_font(sInt(font_label));
		if (labelpos == /*"top"*/ STR(194))
			fieldy += (label_height + IntType(8));
		else
			if (labelpos == /*"none"*/ STR(878))
				capwidth = IntType(0);
			else
	...
	VarType draw_textfield_group(ScopeAny self, StringType name, VarType xx, VarType yy, VarType wid, VarType mul, VarType minval, VarType maxval, RealType snapval, RealType showcaption, RealType stack, RealType colortype, BoolType drag, BoolType textfield_update)
	{
		...
		for (IntType i = IntType(0); i < sReal(textfield_amount); i++)
		{
			...
			microani_update({ mouseon, mouseon && sBool(mouse_left), sVar(window_focus) == string(sVar(textfield_textbox).Value(i)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"drag"*/ STR(34)) || (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1059)), false, active });
			...
			if (app_mouse_box(self, fieldx, boxy, boxwid, boxhei) && sBool(content_mouseon) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && drag)
			{
				...
				if (sBool(mouse_left_pressed))
					sVar(window_busy) = sVar(textfield_name).Value(i) + /*"press"*/ STR(1059);
			}
			if (sVar(window_busy) == sVar(textfield_name).Value(i) + /*"press"*/ STR(1059))
			{
				sInt(mouse_cursor) = cr_size_we;
				if (!sBool(mouse_left))
				{
			...
			if (sVar(window_busy) != sVar(textfield_name).Value(i) + /*"press"*/ STR(1059) && sVar(window_focus) != string(sVar(textfield_textbox).Value(i)) && fieldupdate == VarType())
