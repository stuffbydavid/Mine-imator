...
namespace CppProject
{
	void tab_timeline_editor_appearance(ScopeAny self)
	{
		if (idVar(global::tl_edit, type) != e_tl_type_POINT_LIGHT && idVar(global::tl_edit, type) != e_tl_type_SPOT_LIGHT)
		{
			...
			draw_button_menu(self, { /*"timelineeditorblendmode"*/ STR(1493), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idVar(global::tl_edit, blend_mode), text_get({ /*"timelineeditorblendmode"*/ STR(1493) + idVar(global::tl_edit, blend_mode) }), ID_action_tl_blend_mode });
			tab_next(self);
			VarType text;
			if (idVar(global::tl_edit, alpha_mode) == e_alpha_mode_BLEND)
				text = text_get({ /*"renderalphamodeblend"*/ STR(1516) });
			else
				if (idVar(global::tl_edit, alpha_mode) == e_alpha_mode_HASHED)
					text = text_get({ /*"renderalphamodehashed"*/ STR(1515) });
				else
					text = text_get({ /*"renderalphamodedefault"*/ STR(1514) });
			
			
			tab_control_menu(self);
			draw_button_menu(self, { /*"timelineeditoralphamode"*/ STR(1513), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idVar(global::tl_edit, alpha_mode), text, ID_action_tl_alpha_mode });
			tab_next(self);
	...
	RealType tab_timeline_editor_hierarchy(ScopeAny self)
	{
		...
		if (idVar(global::tl_edit, part_of) == null_)
		{
			...
			if (par == global::_app->id)
				text = text_get({ /*"timelinenone"*/ STR(1700) });
			else
				text = string_remove_newline(idVar(par, display_name));
			
			tab_control_menu(self);
			draw_button_menu(self, { /*"timelineeditorparent"*/ STR(1089), e_menu_TIMELINE, sVar(dx), sVar(dy), sVar(dw), IntType(24), par, text, ID_action_tl_parent });
			tab_next(self);
			...
			{
				ArrType partname = ArrType::From({ /*"right"*/ STR(190), /*"left"*/ STR(189), /*"front"*/ STR(894), /*"back"*/ STR(896), /*"upper"*/ STR(928), /*"lower"*/ STR(1844) });
		...
		if (par != global::_app->id)
		{
			...
			draw_label(self, { text_get({ /*"timelineeditorinherit"*/ STR(3707) }), sVar(dx), sVar(dy) + IntType(10), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			tab_next(self);
			tab_set_collumns(self, { true, floor((RealType)sVar(content_width) / 150.0) });
			tab_control_checkbox(self);
	...
	void tab_timeline_editor_info(ScopeAny self)
	{
		...
		draw_textfield(self, /*"timelineeditorname"*/ STR(3726), sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_category, idInt(sVar(tab), info))->tbx_name, ID_action_tl_name, string_remove_newline(idVar(global::tl_edit, display_name)), /*"top"*/ STR(188));
		tab_next(self);
		if (idVar(global::tl_edit, type) == e_temp_type_TEXT)
		{
			...
			draw_textfield(self, /*"timelineeditortext"*/ STR(3150), sVar(dx), sVar(dy), sVar(dw), IntType(76), ObjType(obj_category, idInt(sVar(tab), info))->tbx_text, ID_action_tl_text, /*""*/ STR(0), /*"top"*/ STR(188));
			tab_next(self);
		}
	...
	void tab_timeline_editor_path(ScopeAny self)
	{
		...
		tab_next(self);
		tab_control_switch(self);
		draw_button_collapse(self, /*"path_shape"*/ STR(141), DsMap(global::collapse_map).Value(/*"path_shape"*/ STR(141)), ID_action_tl_path_shape_generate, idVar(global::tl_edit, path_shape_generate), /*"timelineeditorpathshape"*/ STR(3738));
		tab_next(self);
		if (DsMap(global::collapse_map).Value(/*"path_shape"*/ STR(141)) > 0 && idVar(global::tl_edit, path_shape_generate) > 0)
		{
			...
			tab_next(self);
			tab_control_switch(self);
			draw_button_collapse(self, /*"path_shape_tube"*/ STR(142), DsMap(global::collapse_map).Value(/*"path_shape_tube"*/ STR(142)), ID_action_tl_path_shape_tube, idVar(global::tl_edit, path_shape_tube), /*"timelineeditorpathshapetube"*/ STR(3745));
			tab_next(self);
			...
			{
				StringType fn = file_dialog_save_image(/*"path"*/ STR(527));
				if (fn != /*""*/ STR(0))
