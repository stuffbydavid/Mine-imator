...
namespace CppProject
{
	RealType tab_template_editor_particles_framebox(ScopeAny self)
	{
		...
		if (app_mouse_box(self, xx, yy, size, size) && sBool(content_mouseon))
		{
			...
			{
				sVar(window_busy) = /*"particleeditortypeanimationstartend"*/ STR(3688);
				action_lib_pc_type_sprite_frame_start(self, mouseframe, false);
				action_lib_pc_type_sprite_frame_end(self, mouseframe, false);
			}
		}
		if (sVar(window_busy) == /*"particleeditortypeanimationstartend"*/ STR(3688))
		{
			sInt(mouse_cursor) = cr_handpoint;
			action_lib_pc_type_sprite_frame_end(self, mouseframe, false);
			if (!sBool(mouse_left))
		...
		for (VarType f = min({ idVar(global::ptype_edit, sprite_frame_start), idVar(global::ptype_edit, sprite_frame_end) }); f <= max({ idVar(global::ptype_edit, sprite_frame_start), idVar(global::ptype_edit, sprite_frame_end) }); f++)
		{
			...
			if (f == idVar(global::ptype_edit, sprite_frame_start))
			{
				col = global::c_accent;
				alpha = IntType(1);
				draw_label(self, { text_get({ /*"particleeditortypespriteframeboxstart"*/ STR(3689) }), bx + floor(fwid / 2.0), min({ by + ((reverse > 0) ? 0.0 : fhei), yy + size }), fa_center, (reverse > 0) ? fa_bottom : fa_top, col, IntType(1), sInt(font_label) });
			}
			else
				if (f == idVar(global::ptype_edit, sprite_frame_end))
				{
					col = global::c_error;
					alpha = IntType(1);
					draw_label(self, { text_get({ /*"particleeditortypespriteframeboxend"*/ STR(3690) }), bx + floor(fwid / 2.0), min({ by + (!(reverse > 0) ? 0.0 : fhei), yy + size }), fa_center, !(reverse > 0) ? fa_bottom : fa_top, col, IntType(1), sInt(font_label) });
				}
	...
	RealType tab_template_editor_particles_preview(ScopeAny self)
	{
		...
		{
			if (draw_button_icon(self, { /*"particlesresetpreview"*/ STR(3691), xx + size + IntType(4), yy + size - IntType(24), IntType(24), IntType(24), false, icons_RESET, null_, false, /*"tooltipparticlesresetpreview"*/ STR(3692) }))
	...
	void tab_template_editor_particles_value(ScopeAny self, StringType name, VarType val, VarType israndom, VarType randommin, VarType randommax, RealType mul, RealType minval, RealType maxval, ArrType def, RealType snapval, ArrType tbx, ArrType scripts, VarType capwid, BoolType showcaption, VarType suffix)
	{
		...
		draw_button_icon(self, { /*"particleeditorrandom"*/ STR(3693) + name, sVar(dx) + sVar(dw) - sReal(ui_small_height), sVar(dy), sReal(ui_small_height), sReal(ui_small_height), israndom, icons_RANDOMIZE, scripts.Value(IntType(1)), false, /*"tooltipparticlesrandom"*/ STR(3664) });
		VarType suf1, suf2;
		...
		if (israndom > 0)
		{
			...
			textfield_group_add(self, { name + /*"min"*/ STR(1111), randommin, def.Value(IntType(1)), scripts.Value(IntType(2)), global::axis_edit, tbx.Value(IntType(0)), null_, mul, minval, randommax, /*"particleeditormin"*/ STR(3694) });
			textfield_group_add(self, { name + /*"max"*/ STR(1112), randommax, def.Value(IntType(2)), scripts.Value(IntType(3)), global::axis_edit, tbx.Value(IntType(1)), null_, mul, randommin, maxval, /*"particleeditormax"*/ STR(3695) });
	...
	void tab_timeline(ScopeAny self)
	{
		...
		timelabel = (sReal(timeline_show_frames) > 0) ? text_get({ /*"timelineframe"*/ STR(3696), floor(sVar(timeline_marker)) }) : string_time_seconds((RealType)sVar(timeline_marker) / sVar(project_tempo));
		...
		timelabel = (sReal(timeline_show_frames) > 0) ? /*" / "*/ STR(3697) + string(sVar(timeline_length)) : /*" / "*/ STR(3697) + string_time_seconds((RealType)sVar(timeline_length) / sVar(project_tempo));
		...
		{
			timelabel = /*" ("*/ STR(1151) + ((sReal(timeline_show_frames) > 0) ? VarType(string(sVar(timeline_region_end) - sVar(timeline_region_start))) : string_time_seconds((sVar(timeline_region_end) - sVar(timeline_region_start)) / sVar(project_tempo))) + /*")"*/ STR(1127);
		...
		draw_button_icon(self, { /*"timelinepreviouskeyframe"*/ STR(3698), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_KEYFRAME_PREVIOUS, ID_action_tl_keyframe_previous, sReal(timeline_playing), /*"tooltiptlpreviouskeyframe"*/ STR(3699) });
		buttonsx += IntType(24) + IntType(6);
		if (draw_button_icon(self, { /*"timelinepreviousframe"*/ STR(3700), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_FRAME_PREVIOUS, null_, sReal(timeline_playing), /*"tooltiptlpreviousframe"*/ STR(3701) }))
		...
		buttonsx += IntType(6);
		draw_button_icon(self, { /*"timelinestop"*/ STR(3702), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_STOP, ID_action_tl_play_stop, false, /*"tooltiptlstop"*/ STR(3703) });
		buttonsx += IntType(24) + IntType(6);
		tip_set_keybind(self, e_keybind_PLAY);
		draw_button_icon(self, { /*"timelineplay"*/ STR(3704), buttonsx, buttonsy, IntType(24), IntType(24), false, (sReal(timeline_playing) > 0) ? icons_PAUSE : icons_PLAY, ID_action_tl_play, false, (sReal(timeline_playing) > 0) ? /*"tooltiptlpause"*/ STR(3705) : /*"tooltiptlplay"*/ STR(3706) });
		buttonsx += IntType(24) + IntType(6);
		if (draw_button_icon(self, { /*"timelineplayregion"*/ STR(3707), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_PLAY_REGION, null_, sVar(timeline_region_start) == null_, /*"tooltiptlplayregion"*/ STR(3708) }))
		{
		...
		draw_divide_vertical(buttonsx, buttonsy + IntType(2), IntType(20));
		buttonsx += IntType(6);
		if (draw_button_icon(self, { /*"timelinenextframe"*/ STR(3709), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_FRAME_NEXT, null_, sReal(timeline_playing), /*"tooltiptlnextframe"*/ STR(3710) }))
		...
		buttonsx += IntType(24) + IntType(6);
		draw_button_icon(self, { /*"timelinenextkeyframe"*/ STR(3711), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_KEYFRAME_NEXT, ID_action_tl_keyframe_next, sReal(timeline_playing), /*"tooltiptlnextkeyframe"*/ STR(3712) });
		...
		{
			if (draw_button_icon(self, { /*"timelinewalkcycle"*/ STR(3713), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_WALK_CYCLE, null_, !file_exists_lib(sStr(timeline_settings_walk_fn)), /*"tooltiptlwalk"*/ STR(3714) }))
				action_tl_load_loop(self, sStr(timeline_settings_walk_fn));
			buttonsx += IntType(24) + IntType(6);
			if (draw_button_icon(self, { /*"timelineruncycle"*/ STR(3715), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_RUN_CYCLE, null_, !file_exists_lib(sStr(timeline_settings_run_fn)), /*"tooltiptlrun"*/ STR(3716) }))
		...
		draw_button_icon(self, { /*"timelineintervals"*/ STR(3717), buttonsx, buttonsy, IntType(24), IntType(24), sVar(timeline_intervals_show), icons_STOPWATCH, ID_action_tl_intervals_show, false, (sVar(timeline_intervals_show) > 0) ? /*"timelineintervalshide"*/ STR(3718) : /*"timelineintervalsshow"*/ STR(3719) });
		buttonsx += IntType(24);
		if (draw_button_icon(self, { /*"timelineintervalsettings"*/ STR(3720), buttonsx, buttonsy, IntType(16), IntType(24), sStr(settings_menu_name) == /*"timelineintervalsettings"*/ STR(3720), icons_CHEVRON_DOWN_TINY, null_, false }))
		{
			menu_settings_set(self, buttonsx, buttonsy, /*"timelineintervalsettings"*/ STR(3720), IntType(24));
			sInt(settings_menu_script) = ID_tl_interval_settings_draw;
		}
		if (sStr(settings_menu_name) == /*"timelineintervalsettings"*/ STR(3720) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
		...
		if (!(sVar(timeline_repeat) > 0) && !(sVar(timeline_seamless_repeat) > 0))
			tooltip = /*"tooltiptlenableloop"*/ STR(3721);
		else
			if (sVar(timeline_repeat) > 0 && !(sVar(timeline_seamless_repeat) > 0))
				tooltip = /*"tooltiptlenableseamlessloop"*/ STR(3722);
			else
				tooltip = /*"tooltiptldisableloop"*/ STR(3723);
		
		
		draw_button_icon(self, { /*"timelineloop"*/ STR(3724), buttonsx, buttonsy, IntType(24), IntType(24), sVar(timeline_repeat) > 0 || sVar(timeline_seamless_repeat) > 0, (sVar(timeline_seamless_repeat) > 0) ? icons_REPEAT_SEAMLESS : icons_REPEAT, ID_action_tl_play_repeat, false, tooltip });
		buttonsx += IntType(24) + IntType(4);
		draw_divide_vertical(buttonsx, buttonsy, IntType(24));
		buttonsx += IntType(4);
		if (window_get_current() == e_window_MAIN)
		{
			if (draw_button_icon(self, { /*"tabpopout"*/ STR(3725), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_EXTERNAL, null_, false, /*"tooltiptlpopout"*/ STR(3726) }))
			{
		...
		{
			if (draw_button_icon(self, { /*"tabpopback"*/ STR(3727), buttonsx, buttonsy, IntType(24), IntType(24), false, icons_INTERNAL, null_, false, /*"tooltiptlpopin"*/ STR(3728) }))
		...
		if (sVar(project_file) != /*""*/ STR(0) && !instance_exists(ID_obj_timeline) && sVar(content_height) > IntType(100) && (ds_list_size(sInt(timeline_marker_list)) == IntType(0)))
		{
			...
			textwid = string_width(text_get({ /*"timelineempty"*/ STR(3729) }));
			if (tlw > (textwid + IntType(32)))
				draw_label(self, { text_get({ /*"timelineempty"*/ STR(3729) }), floor(tlx + tlw / 2.0 - textwid / 2.0), floor(tly + tlh / 2.0), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		}
		sVar(dy) = tly;
		...
		if (sVar(window_busy) == /*"timelineselectkeyframes"*/ STR(3730))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (markerbarh != IntType(0))
		{
			...
			for (IntType i = IntType(0); i < ds_list_size(sInt(timeline_marker_list)); i++)
			{
				...
				name = idVar(marker, name) + (dev_mode_debug_names ? /*" ["*/ STR(1826) + idVar(marker, save_id) + /*"]"*/ STR(1252) : /*""*/ STR(0));
			...
			{
				context_menu_area(self, { markerbarx, markerbary, markerbarw, markerbarh, /*"timelinemarker"*/ STR(1584), markermouseon, null_, null_, null_ });
				if (sBool(mouse_left_pressed))
				{
					sVar(window_busy) = /*"timelinemovemarker"*/ STR(3731);
			...
			if (sVar(window_busy) == /*"timelinemovemarker"*/ STR(3731))
			{
				BoolType fail;
				RealType mousepos;
				fail = false;
		...
		if (sReal(setting_advanced_mode) > 0)
		{
			if (draw_button_icon(self, { /*"timelinefilter"*/ STR(3732), listx + IntType(8), bary + IntType(4), IntType(24), IntType(24), sVar(setting_timeline_hide_ghosts) > 0 || !array_equals(VarType::CreateRef(sVar(timeline_hide_color_tag)), VarType::CreateRef(array_create({ array_length(VarType::CreateRef(sVar(timeline_hide_color_tag))), false }))), icons_FILTER, null_, false, /*"tooltiptlfilter"*/ STR(3733) }))
			{
				menu_settings_set(self, listx + IntType(8), bary + IntType(4), /*"timelinefilter"*/ STR(3732), IntType(24));
				sInt(settings_menu_script) = ID_tl_filter_draw;
				sBool(settings_menu_above) = true;
			}
			if (sStr(settings_menu_name) == /*"timelinefilter"*/ STR(3732) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		}
		VarType searchx;
		RealType searchwid;
		...
		draw_textfield(self, /*"timelinesearch"*/ STR(3734), searchx, bary + IntType(4), searchwid, IntType(24), idInt(sVar(timeline), tbx_search), ID_action_tl_search, text_get({ /*"listsearch"*/ STR(3003) }), /*"none"*/ STR(919));
		if (mouseinnames)
			context_menu_area(self, { listx, listy, listw, listh, /*"timelinelist"*/ STR(1555), mousetl, null_, null_, null_ });
		sVar(dy) = listy;
		RealType itemmaxw;
		tlmaxw = IntType(0);
		for (RealType t = sReal(timeline_list_first); t < ds_list_size(sInt(tree_visible_list)); t++)
		{
			...
			if ((itemhover && sBool(mouse_left)) || idReal(tl, selected) > 0)
				...
				if (idReal(tl, selected) > 0 || itemhover || tl == sVar(context_menu_value) || (sVar(window_busy) == /*"timelineclick"*/ STR(3735) && sVar(timeline_select) == tl))
			...
			if (itemhover || idVar(tl, hide) > 0)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelinehide"*/ STR(260) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, hide), (idVar(tl, hide) > 0) ? icons_HIDDEN_SMALL : icons_VISIBLE_SMALL, null_, false, (idVar(tl, hide) > 0) ? /*"tooltiptlshow"*/ STR(3736) : /*"tooltiptlhide"*/ STR(3737) }))
						action_tl_hide(self, tl);
				}
				else
				{
					if (draw_button_icon(self, { /*"timelinehide"*/ STR(260) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, hide), (idVar(tl, hide) > 0) ? icons_MUTE_SMALL : icons_VOLUME_SMALL, null_, false, (idVar(tl, hide) > 0) ? /*"tooltiptlunmute"*/ STR(3738) : /*"tooltiptlmute"*/ STR(3739) }))
						action_tl_hide(self, tl);
				}
			...
			{
				if (draw_button_icon(self, { /*"timelinelock"*/ STR(3740) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, lock), (idVar(tl, lock) > 0) ? icons_LOCK_SMALL : icons_UNLOCK_SMALL, null_, false, ((idVar(tl, lock) > 0) ? /*"tooltiptlunlock"*/ STR(3741) : /*"tooltiptllock"*/ STR(3742)) }))
			...
			if (!(sVar(setting_timeline_hide_ghosts) > 0) && sReal(setting_advanced_mode) > 0)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelineghosttl"*/ STR(3743) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, ghost), icons_GHOST_SMALL, null_, false, ((idVar(tl, ghost) > 0) ? /*"tooltiptlunghost"*/ STR(3744) : /*"tooltiptlghost"*/ STR(3745)) }))
			...
			if (itemhover)
			{
				if (draw_button_icon(self, { /*"timelineselectkeyframes"*/ STR(3730) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, false, icons_KEYFRAME_SMALL, null_, false, /*"contextmenutlselectkeyframes"*/ STR(1557) }))
			...
			if (tlhierarchy)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelineexpand"*/ STR(3746) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, tree_extend), null_, null_, false, ((idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1720) : /*"tooltiptlexpand"*/ STR(1721)), ID_spr_chevron_ani }))
			...
			if (!(sReal(setting_timeline_compact) > 0))
			{
				VarType iconcolor;
				RealType iconalpha;
				if (idReal(tl, selected) > 0 || (sVar(window_busy) == /*"timelineclick"*/ STR(3735) && sVar(timeline_select) == tl) || ((itemhover && !buttonhover) && (sBool(mouse_left) || sBool(mouse_left_released))))
				{
					if (idVar(tl, color_tag) == null_)
			...
			else
			{
				...
				if (dev_mode_debug_saveid)
					name += /*" ["*/ STR(1826) + string(idVar(tl, save_id)) + /*"]"*/ STR(1252);
				if (name != /*""*/ STR(0))
				{
					if (idVar(tl, color_tag) != null_)
			...
			if (sVar(window_busy) == /*"timelineclick"*/ STR(3735))
			{
				sVar(window_busy) = /*""*/ STR(0);
				if (app_mouse_box(self, xx - IntType(28), itemy, string_width(name) + IntType(28), itemh))
					mousetlname = tl;
				sVar(window_busy) = /*"timelineclick"*/ STR(3735);
			}
		...
		if (app_mouse_box(self, tlx - IntType(5), tly - barh, IntType(5), sVar(content_height) - headerh))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"timelinelistresize"*/ STR(3747);
				sVar(timeline_list_resize_start) = listw;
			}
		}
		if (sVar(window_busy) == /*"timelinelistresize"*/ STR(3747))
		{
			sInt(mouse_cursor) = cr_size_we;
		...
		if (mousekfend > 0)
		{
			...
			if (sBool(mouse_left_pressed))
			{
				...
				sVar(window_busy) = /*"timelinesetsoundend"*/ STR(3748);
			}
		}
		if (sVar(window_busy) == /*""*/ STR(0) && mouseintl && !(mousekfstart > 0) && !(mousekfend > 0))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				...
				{
					sVar(window_busy) = /*"timelineclickkeyframes"*/ STR(3749);
			...
			{
				sVar(window_focus) = /*"timeline"*/ STR(119);
				sVar(window_busy) = /*"timelinedrag"*/ STR(3750);
			}
		}
		...
		if (sVar(window_busy) == /*"timelinesetsoundend"*/ STR(3748))
		{
		...
		if (sVar(window_busy) == /*"timelineselect"*/ STR(258))
		{
			RealType x1, y1, x2, y2;
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"timelineclick"*/ STR(3735))
		{
			...
			if (sReal(mouse_move) > IntType(5))
			{
				...
				else
				{
					...
					sVar(window_busy) = /*"timelineselect"*/ STR(258);
				}
		...
		if (sVar(window_busy) == /*"timelineclickkeyframes"*/ STR(3749))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (sReal(mouse_move) > IntType(5))
			{
				if (!keyboard_check(vk_shift) && !keyboard_check(vk_control))
					action_tl_deselect_all(self);
				sVar(window_busy) = /*"timelineselectkeyframes"*/ STR(3730);
			}
			if (!sBool(mouse_left))
			{
				if (sVar(timeline_select) > 0)
		...
		if (app_mouse_box(self, barx, bary + IntType(5) * (IntType)(idInt(sVar(tab), panel) == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(189))), barw, barh - IntType(5) * (IntType)(idInt(sVar(tab), panel) == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(189)))) && !sBool(popup_mouseon) && !sBool(context_menu_mouseon) && !sBool(toast_mouseon))
		{
			...
			if (sVar(timeline_region_start) != null_)
			{
				if (app_mouse_box(self, barx + regionx1 - IntType(8), bary, IntType(8), barh))
				{
					...
					{
						sVar(window_focus) = /*"timeline"*/ STR(119);
						sVar(window_busy) = /*"timelinesetregionstart"*/ STR(3751);
					...
					if (app_mouse_box(self, barx + regionx2, bary, IntType(8), barh))
					{
						...
						{
							sVar(window_focus) = /*"timeline"*/ STR(119);
							sVar(window_busy) = /*"timelinesetregionend"*/ STR(3752);
			...
			{
				sVar(window_focus) = /*"timeline"*/ STR(119);
				sVar(window_busy) = /*"timelinemarker"*/ STR(1584);
			}
			if (sBool(mouse_right_pressed))
			{
				sVar(window_focus) = /*"timeline"*/ STR(119);
				sVar(window_busy) = /*"timelinecreateregion"*/ STR(3753);
				sVar(timeline_region_pos) = sVar(timeline_mouse_pos);
				action_tl_play_break(self);
			}
		}
		if (sVar(window_busy) == /*"timelinecreateregion"*/ STR(3753) || sVar(window_busy) == /*"timelinesetregionstart"*/ STR(3751) || sVar(window_busy) == /*"timelinesetregionend"*/ STR(3752))
		{
			BoolType release;
			if (sVar(window_busy) == /*"timelinecreateregion"*/ STR(3753))
			{
				sInt(mouse_cursor) = cr_handpoint;
			...
			if (sVar(window_busy) == /*"timelinesetregionend"*/ STR(3752))
			{
		...
		if (sVar(window_busy) == /*"timelinemarker"*/ STR(1584))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sStr(window_scroll_focus_prev) == /*"timelinezoom"*/ STR(3754) && sVar(window_busy) == /*""*/ STR(0) && sReal(mouse_wheel) != IntType(0))
		{
		...
		if (sVar(window_busy) == /*"timelinedrag"*/ STR(3750))
		{
			sInt(mouse_cursor) = cr_size_all;
		...
		if (sVar(window_busy) == /*"timelinemove"*/ STR(48) || sVar(window_busy) == /*"timelineselect"*/ STR(258))
		{
		...
		if (sVar(window_busy) == /*"timelineselectkeyframes"*/ STR(3730) || sVar(window_busy) == /*"timelinemovekeyframes"*/ STR(45) || sVar(window_busy) == /*"timelinecreateregion"*/ STR(3753) || sVar(window_busy) == /*"timelinesetregionstart"*/ STR(3751) || sVar(window_busy) == /*"timelinesetregionend"*/ STR(3752) || sVar(window_busy) == /*"timelineresizesounds"*/ STR(46) || sVar(window_busy) == /*"timelinesetsoundend"*/ STR(3748) || sVar(window_busy) == /*"timelinemovemarker"*/ STR(3731))
		{
		...
		if (sBool(content_mouseon))
		{
			...
			if (mouseintl)
				sStr(shortcut_bar_state) = /*"timelinekeyframes"*/ STR(2973);
			if (mouseinnames)
				sStr(shortcut_bar_state) = /*"timelinenames"*/ STR(2979);
			if (mouseinbar)
				sStr(shortcut_bar_state) = /*"timelinebar"*/ STR(2985);
			...
			if (keyboard_check(vk_control))
				sStr(window_scroll_focus) = /*"timelinezoom"*/ STR(3754);
		}
	}
