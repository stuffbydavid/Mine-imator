...
namespace CppProject
{
	...
	void toast_draw(ScopeAny self, IntType toast)
	{
		...
		sVar(content_x) = floor(sReal(window_width) / 2.0 - ObjType(obj_toast, toast)->toast_width / 2.0) + (IntType(32) * ease(/*"easeoutcirc"*/ STR(565), ObjType(obj_toast, toast)->remove_alpha));
		...
		{
			if (draw_button_icon(self, { string(toast) + /*"close"*/ STR(1120), sVar(dx), sVar(dy), IntType(24), IntType(24), false, icons_CLOSE, null_, false }))
				ObjType(obj_toast, toast)->remove = true;
			hei = IntType(40);
			wid += IntType(32);
		}
	...
	void toolbar_draw(ScopeAny self)
	{
		...
		padding = IntType(0);
		draw_set_font(sInt(font_value));
		capwid = string_width(text_get({ /*"toolbarfile"*/ STR(1586) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarfile"*/ STR(1586), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		if (sStr(window_state) == /*""*/ STR(0))
		{
			capwid = string_width(text_get({ /*"toolbaredit"*/ STR(1595) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbaredit"*/ STR(1595), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
			capwid = string_width(text_get({ /*"toolbarrender"*/ STR(1604) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbarrender"*/ STR(1604), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
		}
		capwid = string_width(text_get({ /*"toolbarview"*/ STR(1607) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarview"*/ STR(1607), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		capwid = string_width(text_get({ /*"toolbarhelp"*/ STR(1616) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarhelp"*/ STR(1616), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		sVar(dx) += IntType(8);
	...
	BoolType toolbar_draw_button(ScopeAny self, StringType name, VarType xx, VarType yy, IntType wid, BoolType hasmenu)
	{
		...
		trigger = false;
		click = false;
		if (sVar(window_busy) == /*"contextmenu"*/ STR(1059))
	...
	StringType unzip_asset(VarType fn)
	{
		...
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miproject;.mproj;.mani;"*/ STR(3863));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory, /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3864));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3864));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	StringType unzip_model(StringType fn)
	{
		...
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"json"*/ STR(3865));
		if (!file_exists_lib(validfile))
	...
	RealType window_drop_enter(ScopeAny self, ArrType files)
	{
		...
		if (array_length(VarType::CreateRef(files)) > IntType(1))
			return false;
		return string_contains(asset_exts, /*"*"*/ STR(1128) + filename_ext(files.Value(IntType(0))) + /*";"*/ STR(3871));
	}
