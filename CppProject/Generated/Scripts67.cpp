...
namespace CppProject
{
	...
	void tl_value_set_start(ScopeAny self, IntType script, BoolType combine)
	{
		...
		else
		{
			...
			log({ /*"Action"*/ STR(1185), script_get_name(script) });
		}
	...
	void toast_draw(ScopeAny self, IntType toast)
	{
		...
		{
			if (draw_button_icon(self, { string(toast) + /*"close"*/ STR(1072), sVar(dx), sVar(dy), IntType(24), IntType(24), false, icons_CLOSE, null_, false }))
				ObjType(obj_toast, toast)->remove = true;
			hei = IntType(40);
			wid += IntType(32);
		}
	...
	IntType toast_new(ScopeAny self, IntType variant, VarType text)
	{
		...
		log({ /*"New toast"*/ STR(3751), ObjType(obj_toast, toast)->text, type });
		return toast;
	}
	...
	void toolbar_draw(ScopeAny self)
	{
		...
		padding = IntType(0);
		draw_set_font(sInt(font_value));
		capwid = string_width(text_get({ /*"toolbarfile"*/ STR(1527) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarfile"*/ STR(1527), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		if (sStr(window_state) == /*""*/ STR(0))
		{
			capwid = string_width(text_get({ /*"toolbaredit"*/ STR(1536) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbaredit"*/ STR(1536), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
			capwid = string_width(text_get({ /*"toolbarrender"*/ STR(1545) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbarrender"*/ STR(1545), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
		}
		capwid = string_width(text_get({ /*"toolbarview"*/ STR(1548) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarview"*/ STR(1548), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		capwid = string_width(text_get({ /*"toolbarhelp"*/ STR(1557) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarhelp"*/ STR(1557), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		sVar(dx) += IntType(8);
		draw_label(self, { text_get({ /*"toolbarbackup"*/ STR(3752) }), sVar(dx), sVar(dy) + IntType(22), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary * clamp(sReal(backup_text_ani), IntType(0), IntType(1)), sInt(font_value) });
		if (!(sReal(setting_advanced_mode) > 0))
		{
			if (draw_button_label(self, { /*"toolbarsimplemode"*/ STR(3753), sVar(content_x) + sVar(content_width) - IntType(10), sVar(dy), null_, null_, e_button_TOOLBAR, null_, fa_right }))
			{
	...
	BoolType toolbar_draw_button(ScopeAny self, StringType name, VarType xx, VarType yy, IntType wid, BoolType hasmenu)
	{
		...
		trigger = false;
		click = false;
		if (sVar(window_busy) == /*"contextmenu"*/ STR(1011))
		...
		if (trigger && hasmenu)
		{
			...
			sVar(window_busy) = /*"contextmenu"*/ STR(1011);
	...
	void trial_startup()
	{
		...
		{
			log({ /*"Found key_file"*/ STR(3754), key_file });
	...
	BoolType trial_upgrade(ScopeAny self, VarType argument0)
	{
		...
		settings_save(self);
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertupgraded"*/ STR(3755) }));
		return true;
	}
	...
	VarType unzip(VarArgs argument)
	{
		...
		log({ /*"Unzipping"*/ STR(3756), fn, dir });
		VarType num = lib_unzip(fn, dir);
		log({ string(num) + /*" files were extracted"*/ STR(3757) });
		return num;
	}
	...
	StringType unzip_asset(VarType fn)
	{
		...
		unzip({ fn });
		validfile = file_find_single(unzip_directory, /*"miproject;.mproj;.mani;"*/ STR(3758));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miproject;.mproj;.mani;"*/ STR(3758));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory, /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3759));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3759));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	StringType unzip_model(StringType fn)
	{
		...
		name = filename_new_ext(filename_name(fn), /*""*/ STR(0));
		unzip({ fn });
		validfile = file_find_single(unzip_directory, /*"json"*/ STR(3760));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"json"*/ STR(3760));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/item/"*/ STR(3761), /*".json;"*/ STR(3762));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/block/"*/ STR(3763), /*".json;"*/ STR(3762));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	RealType window_drop_enter(ScopeAny self, ArrType files)
	{
		...
		if (array_length(VarType::CreateRef(files)) > IntType(1))
			return false;
		return string_contains(asset_exts, /*"*"*/ STR(1080) + filename_ext(files.Value(IntType(0))) + /*";"*/ STR(3767));
	}
	...
	VarType value_get_real(VarArgs argument)
	{
		...
		if (is_int32(val) || is_int64(val) || is_real(val) || is_bool(val))
			...
			if (is_string(val) && val == /*"null"*/ STR(1199))
				return null_;
