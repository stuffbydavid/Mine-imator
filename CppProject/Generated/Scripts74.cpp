...
namespace CppProject
{
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(117)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3941) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
	...
	RealType window_draw(ScopeAny self)
	{
		switch ((IntType)window_get_current())
		{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3197) }) + /*" - Mine-imator"*/ STR(674));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3943) : /*"exporttimelefthours"*/ STR(3944)), string(timelefthours) }) + /*", "*/ STR(1027);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3945) : /*"exporttimeleftminutes"*/ STR(3946)), string(timeleftmins) }) + /*" "*/ STR(21) + text_get({ /*"exporttimeleftand"*/ STR(3947) }) + /*" "*/ STR(21);
		...
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
			text = text_get({ /*"exportframe"*/ STR(3953), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1110) + samplecount + /*")"*/ STR(1086)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
