...
namespace CppProject
{
					...
					if (app_mouse_box(self, sVar(content_x) + (nextcoord2d.Real(X_) - IntType(13)), sVar(content_y) + (nextcoord2d.Real(Y_) - IntType(13)), IntType(26), IntType(26)) && sBool(content_mouseon) && ObjType(obj_view, view)->control_mouseon_last == null_)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							...
							if (sVar(setting_timeline_select_jump) > 0)
								tl_jump(pointtl);
							sVar(window_busy) = /*"viewpathpointclick"*/ STR(3924);
						}
					}
				}
			}
	...
	RealType view_toolbar_draw(ScopeAny self, IntType view, RealType xx, RealType yy)
	{
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolselect"*/ STR(3925), xx, yy, IntType(24), IntType(24), sBool(setting_tool_select), icons_SELECT, null_, false, /*"viewtoolselecttip"*/ STR(3926) }))
			{
		...
		if (draw_button_icon(self, { /*"viewtoolmove"*/ STR(3927), xx, yy, IntType(24), IntType(24), sVar(setting_tool_move), icons_MOVE, null_, false, /*"viewtoolmovetip"*/ STR(3928) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolrotate"*/ STR(3929), xx, yy, IntType(24), IntType(24), sBool(setting_tool_rotate), icons_ROTATE, null_, false, /*"viewtoolrotatetip"*/ STR(3930) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolscale"*/ STR(3931), xx, yy, IntType(24), IntType(24), sBool(setting_tool_scale), icons_SCALE, null_, false, /*"viewtoolscaletip"*/ STR(3932) }))
			{
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolbend"*/ STR(3933), xx, yy, IntType(24), IntType(24), sBool(setting_tool_bend), icons_BEND, null_, false, /*"viewtoolbendtip"*/ STR(3934) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtooltransform"*/ STR(3935), xx, yy, IntType(24), IntType(24), sBool(setting_tool_transform), icons_MULTITRANSFORM, null_, false, /*"viewtooltransformtip"*/ STR(3936) }))
			{
				action_tools_disable_all(self);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(117)))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"viewclick"*/ STR(3937);
			...
			if ((!(cam > 0) || editcamobj) && sBool(mouse_right_pressed))
			{
				...
				sVar(window_busy) = /*"viewmovecamera"*/ STR(3938);
				sVar(window_focus) = string(view);
				if (cam > 0)
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(117)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3939) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		{
			if (sVar(window_busy) == /*"viewclick"*/ STR(3937))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if ((!(cam > 0) || editcamobj) && sReal(mouse_move) > IntType(5))
				{
					...
					{
						sVar(window_busy) = /*"viewpancamera"*/ STR(3940);
						sVar(window_focus) = string(view);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
					else
					{
						sInt(view_click_x) = display_mouse_get_x();
						sInt(view_click_y) = display_mouse_get_y();
						sVar(window_busy) = /*"viewrotatecamera"*/ STR(3939);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
			...
			if (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3939))
			{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3938))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2933);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2934);
			...
			if (sVar(window_busy) == /*"viewpancamera"*/ STR(3940))
			{
		...
		if (sVar(window_busy) == /*"viewpathpointclick"*/ STR(3924))
			sVar(window_busy) = /*""*/ STR(0);
	}
	...
	RealType window_draw(ScopeAny self)
	{
		switch ((IntType)window_get_current())
		{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3195) }) + /*" - Mine-imator"*/ STR(674));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3941) : /*"exporttimelefthours"*/ STR(3942)), string(timelefthours) }) + /*", "*/ STR(1027);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3943) : /*"exporttimeleftminutes"*/ STR(3944)), string(timeleftmins) }) + /*" "*/ STR(21) + text_get({ /*"exporttimeleftand"*/ STR(3945) }) + /*" "*/ STR(21);
		timeleftstr += text_get({ ((timeleftsecs == IntType(1)) ? /*"exporttimeleftsecond"*/ STR(3946) : /*"exporttimeleftseconds"*/ STR(3947)), string(timeleftsecs) });
		draw_label(self, { text_get({ /*"exporttimeleft"*/ STR(3948), timeleftstr }), framex + (RealType)framew / 2.0, framey + frameh + IntType(33), fa_center, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading_big) });
		VarType loadtext, loadw;
		RealType sw, sh;
		loadtext = text_get({ /*"exportloading"*/ STR(3949), string(floor(perc * IntType(100))) });
		loadw = framew;
		...
		if (usesamples > 0)
			samplecount = text_get({ /*"exportsamples"*/ STR(3950), string(max({ global::render_samples, IntType(1) })), string(sVar(project_render_samples)) });
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
			text = text_get({ /*"exportframe"*/ STR(3951), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1110) + samplecount + /*")"*/ STR(1086)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(53) ? text_get({ /*"exportmovietitle"*/ STR(3952) }) : text_get({ /*"exportimagetitle"*/ STR(3953) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
		draw_label(self, { text_get({ /*"exportstop"*/ STR(3954) }), framex + (RealType)framew / 2.0, framey - IntType(16), fa_center, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		return 0.0;
	}
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		{
			global::load_assets_stage = /*"exit"*/ STR(3955);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(3955))
			{
		...
		draw_label(self, { /*"Mine-imator "*/ STR(3956) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading) });
		...
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(3957), global::_app->setting_minecraft_assets_version, floor(global::load_assets_progress * IntType(100)) }), xoff + IntType(95), yoff + IntType(437), fa_middle, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		...
		if (global::load_assets_credits != /*""*/ STR(0))
			draw_label(self, { text_get({ /*"startupsplashauthor"*/ STR(3958), global::load_assets_credits }), xoff + IntType(95), yoff + IntType(289) + IntType(31), fa_middle, fa_top, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
	...
	RealType window_draw_new_assets(ScopeAny self)
	{
		...
		draw_label(self, { text_get({ /*"newassetstitle"*/ STR(3959), sStr(new_assets_version) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(227), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading) });
		draw_label(self, { text_get({ /*"newassetssubtitle"*/ STR(3960) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(244), fa_center, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(264);
		sVar(dx) += IntType(12);
		sVar(dw) -= 24.0;
		if (sStr(new_assets_stage) == /*"download"*/ STR(108))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), sReal(new_assets_download_progress), text_get({ /*"newassetsdownloading"*/ STR(3961) }));
			tab_next(self);
		}
		else
		{
			draw_set_font(sInt(font_button));
			IntType capwid = string_width(text_get({ /*"newassetsdownload"*/ STR(3962) })) + button_padding;
			if (draw_button_label(self, /*"newassetsdownload"*/ STR(3962), sVar(dx) + sVar(dw), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_PRIMARY, null_, fa_right))
			{
				if (sVar(new_assets_format) > minecraft_assets_format)
				{
					if (question(text_get({ /*"questionassetsnewer"*/ STR(3963) })))
					{
			...
			if (draw_button_label(self, /*"newassetslater"*/ STR(3964), (sVar(dx) + sVar(dw)) - (capwid + IntType(8)), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_SECONDARY, null_, fa_right))
			{
			...
			sVar(dy_start) = sVar(dy);
			sVar(dy) += IntType(16);
			draw_label(self, { text_get({ /*"newassetschangelog"*/ STR(3965) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(20);
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(sArr(new_assets_changes_lines))); i++)
			{
				...
				else
				{
					if (string_copy(text, IntType(1), IntType(2)) == /*"* "*/ STR(3967))
						text = /*"• "*/ STR(3968) + string_delete(text, IntType(1), IntType(2));
