...
namespace CppProject
{
	VarType block_load_model_file(StringType fname, IntType res)
	{
		...
		withOne (obj_block_load_model_file, (new obj_block_load_model_file)->id, noone)
		{
			...
			self->parent = null_;
			if (res == null_ && is_string(DsMap(map).Value(/*"parent"*/ STR(850))))
				self->parent = block_load_model_file(global::load_assets_dir + mc_models_directory + string_replace(DsMap(map).Value(/*"parent"*/ STR(850)), /*"minecraft:"*/ STR(847), /*""*/ STR(0)) + /*".json"*/ STR(736));
			self->texture_map = null_;
			if (is_real(DsMap(map).Value(/*"textures"*/ STR(851))))
			{
				self->texture_map = ds_map_create();
				if (ds_map_find_value((IntType)(DsMap(typemap).Value(map)), /*"textures"*/ STR(851)) == e_json_type_ARRAY && ds_list_valid(DsMap(map).Value(/*"textures"*/ STR(851))))
				{
					for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(map).Value(/*"textures"*/ STR(851)))); i++)
					{
						StringType texname = ds_list_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(851))), i);
						texname = string_replace(texname, /*"minecraft:"*/ STR(847), /*""*/ STR(0));
					...
					{
						VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"textures"*/ STR(851))));
						while (!is_undefined(key))
						{
							StringType texname = ds_map_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(851))), key);
							texname = string_replace(texname, /*"minecraft:"*/ STR(847), /*""*/ STR(0));
							DsMap(self->texture_map)[key] = block_load_model_file_texture(self, texname, res);
							key = ds_map_find_next((IntType)(DsMap(map).Value(/*"textures"*/ STR(851))), key);
						}
			...
			VarType elementslist = DsMap(map).Value(/*"elements"*/ STR(852));
			if (!is_undefined(elementslist))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(elementslist)); i++)
				{
					withOne (obj_block_load_element, (new obj_block_load_element)->id, self->id)
					{
						...
						self->from = value_get_point3D(DsMap(elementmap).Value(/*"from"*/ STR(853)));
						self->to = value_get_point3D(DsMap(elementmap).Value(/*"to"*/ STR(854)));
						self->size = point3D_sub(self->to, self->from);
						self->volume = self->size.Value(X_) * self->size.Value(Y_) * self->size.Value(Z_);
						VarType rotationmap = DsMap(elementmap).Value(/*"rotation"*/ STR(855));
						if (ds_map_valid(rotationmap))
						{
							VarType origin, angle;
							VecType rot, scale;
							origin = value_get_point3D(DsMap(rotationmap).Value(/*"origin"*/ STR(856)), point3D(IntType(8), IntType(8), IntType(8)));
							angle = IntType(0);
							rot = vec3(IntType(0));
							scale = vec3(IntType(1));
							if (is_real(DsMap(rotationmap).Value(/*"angle"*/ STR(857))))
								angle = snap(clamp(DsMap(rotationmap).Value(/*"angle"*/ STR(857)), -IntType(45), IntType(45)), 22.5);
							if (is_bool(DsMap(rotationmap).Value(/*"rescale"*/ STR(858))) && DsMap(rotationmap).Value(/*"rescale"*/ STR(858)) > 0)
								scale = vec3(1.0 / dcos(abs(angle)));
							if (is_string(DsMap(rotationmap).Value(/*"axis"*/ STR(841))))
							{
								switch (StringType(DsMap(rotationmap).Value(/*"axis"*/ STR(841))).id)
								{
									case 859: // x
									{
										rot.Real(X_) = angle;
						...
						VarType facesmap = DsMap(elementmap).Value(/*"faces"*/ STR(861));
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							if (!is_undefined(curmap))
							{
								...
								if (ds_list_valid(DsMap(curmap).Value(/*"uv"*/ STR(862))))
								{
									VarType uvlist = DsMap(curmap).Value(/*"uv"*/ STR(862));
								...
								self->face_texture[f] = DsMap(curmap).Value(/*"texture"*/ STR(540));
								self->face_rotation[f] = IntType(0);
								if (is_real(DsMap(curmap).Value(/*"rotation"*/ STR(855))))
									self->face_rotation[f] = DsMap(curmap).Value(/*"rotation"*/ STR(855));
							}
							else
	...
	VarType block_load_model_file_texture(Scope<obj_block_load_model_file> self, VarType name, IntType res)
	{
		...
		if (string_pos(/*"assets/minecraft_"*/ STR(863), name) == IntType(1))
			name = string_replace(name, /*"assets/minecraft_"*/ STR(863), /*"block/"*/ STR(830));
		if (res == null_)
			return name;
		if (string_pos(/*"blocks/"*/ STR(864), name) == IntType(1))
		{
			name = string_replace(name, /*"blocks/"*/ STR(864), /*"block/"*/ STR(830));
		...
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"blocks/"*/ STR(864), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"block/"*/ STR(830), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + filename_name(name) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/"*/ STR(865) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/item/"*/ STR(866) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/block/"*/ STR(867) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			return name;
					...
					withOne (obj_block_render_element, (new obj_block_render_element)->id, self->id)
					{
						...
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							else
							{
								...
								if (opaque)
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" opaque"*/ STR(870));
								if (slot < IntType(0))
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(871));
								if (slot < IntType(0))
								...
								if (slot < IntType(0))
								{
									...
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(871));
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list), texname + /*" opaque"*/ STR(870));
									if (slot < IntType(0))
									{
										self->face_render[nd] = false;
										continue;
									...
									if (!is_undefined(col) && col == /*"water"*/ STR(826))
										...
										if (alpha > IntType(0))
										{
											...
											else
											{
												...
												switch ((IntType)self->face_block_vbuffer.Value(nd))
												{
													...
													{
														colstr = /*"grass"*/ STR(872);
														break;
													}
													case e_block_vbuffer_FOLIAGE:
													{
														colstr = /*"foliage"*/ STR(873);
														break;
													}
													case e_block_vbuffer_WATER:
													{
														colstr = /*"water"*/ STR(826);
														break;
													}
