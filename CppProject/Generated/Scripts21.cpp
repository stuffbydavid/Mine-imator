...
namespace CppProject
{
	...
	IntType block_get_timeline(Scope<obj_builder_thread> self, VarType block, VarType stateid)
	{
		...
		withOne (obj_block_tl, (new obj_block_tl)->id, self->id)
		{
			...
			self->variant = block_get_state_id_value(block, stateid, /*"variant"*/ STR(826));
			return self->id;
		}
	...
	IntType block_load(VarType map, IntType typemap)
	{
		withOne (obj_block, (new obj_block)->id, noone)
		{
			...
			if (dev_mode_debug_names && !text_exists(/*"block"*/ STR(518) + self->name))
				log({ /*"block/"*/ STR(832) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"type"*/ STR(816))))
			{
				self->type = DsMap(map).Value(/*"type"*/ STR(816));
				self->set_script = asset_get_index(/*"block_set_"*/ STR(833) + self->type);
				self->generate_script = asset_get_index(/*"block_generate_"*/ STR(834) + self->type);
			}
			else
			{
				self->type = /*""*/ STR(0);
			...
			if (is_real(DsMap(map).Value(/*"emissive"*/ STR(835))))
				self->emissive = DsMap(map).Value(/*"emissive"*/ STR(835));
			else
				self->emissive = IntType(0);
			
			if (is_bool(DsMap(map).Value(/*"random_offset"*/ STR(836))))
				self->random_offset = DsMap(map).Value(/*"random_offset"*/ STR(836));
			else
				self->random_offset = false;
			
			if (is_bool(DsMap(map).Value(/*"random_offset_xz"*/ STR(837))))
				self->random_offset_xy = DsMap(map).Value(/*"random_offset_xz"*/ STR(837));
			else
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(838))))
			{
				...
				VarType curstate;
				curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(838))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"blockstate"*/ STR(806) + curstate))
						log({ /*"block/state/"*/ STR(839) + curstate + dev_mode_name_translation_message });
					withOne (obj_block_state, (new obj_block_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (ds_map_valid(curvalue))
							{
								...
								if (is_real(DsMap(curvalue).Value(/*"emissive"*/ STR(835))))
									self->value_emissive[v] = DsMap(curvalue).Value(/*"emissive"*/ STR(835));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset"*/ STR(836))))
									self->value_random_offset[v] = DsMap(curvalue).Value(/*"random_offset"*/ STR(836));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(837))))
									self->value_random_offset_xy[v] = DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(837));
							}
							DsMap(self->value_map)[self->value_name.Value(v)] = v;
							if (dev_mode_debug_names && string_length(self->value_name.Value(v)) > IntType(3) && !text_exists(/*"blockstatevalue"*/ STR(807) + self->value_name.Value(v)))
								log({ /*"block/state/value/"*/ STR(841) + self->value_name.Value(v) + dev_mode_name_translation_message });
						}
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(845))))
					self->wind_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(845));
			}
			if (is_bool(DsMap(map).Value(/*"waterlogged"*/ STR(846))))
				self->waterlogged = DsMap(map).Value(/*"waterlogged"*/ STR(846));
			else
				self->waterlogged = false;
			
			if (is_bool(DsMap(map).Value(/*"require_models"*/ STR(847))))
				self->require_models = DsMap(map).Value(/*"require_models"*/ STR(847));
			else
			...
			if (is_bool(DsMap(map).Value(/*"multithreaded"*/ STR(848))))
				self->multithreaded = DsMap(map).Value(/*"multithreaded"*/ STR(848));
			else
			...
			if (is_string(idmap))
			{
				self->mc_ids = ArrType::From({ string_replace(idmap, /*"minecraft:"*/ STR(849), /*""*/ STR(0)) });
				...
				if (ds_map_valid(idmap))
				{
					...
					while (!is_undefined(key))
					{
						array_add(VarType::CreateRef(self->mc_ids), string_replace(key, /*"minecraft:"*/ STR(849), /*""*/ STR(0)));
	...
	VarType block_load_model_file(StringType fname, IntType res)
	{
		...
		withOne (obj_block_load_model_file, (new obj_block_load_model_file)->id, noone)
		{
			...
			self->parent = null_;
			if (res == null_ && is_string(DsMap(map).Value(/*"parent"*/ STR(852))))
				self->parent = block_load_model_file(global::load_assets_dir + mc_models_directory + string_replace(DsMap(map).Value(/*"parent"*/ STR(852)), /*"minecraft:"*/ STR(849), /*""*/ STR(0)) + /*".json"*/ STR(738));
			self->texture_map = null_;
			if (is_real(DsMap(map).Value(/*"textures"*/ STR(853))))
			{
				self->texture_map = ds_map_create();
				if (ds_map_find_value((IntType)(DsMap(typemap).Value(map)), /*"textures"*/ STR(853)) == e_json_type_ARRAY && ds_list_valid(DsMap(map).Value(/*"textures"*/ STR(853))))
				{
					for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(map).Value(/*"textures"*/ STR(853)))); i++)
					{
						StringType texname = ds_list_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(853))), i);
						texname = string_replace(texname, /*"minecraft:"*/ STR(849), /*""*/ STR(0));
					...
					{
						VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"textures"*/ STR(853))));
						while (!is_undefined(key))
						{
							StringType texname = ds_map_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(853))), key);
							texname = string_replace(texname, /*"minecraft:"*/ STR(849), /*""*/ STR(0));
							DsMap(self->texture_map)[key] = block_load_model_file_texture(self, texname, res);
							key = ds_map_find_next((IntType)(DsMap(map).Value(/*"textures"*/ STR(853))), key);
						}
			...
			VarType elementslist = DsMap(map).Value(/*"elements"*/ STR(854));
			if (!is_undefined(elementslist))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(elementslist)); i++)
				{
					withOne (obj_block_load_element, (new obj_block_load_element)->id, self->id)
					{
						...
						self->from = value_get_point3D(DsMap(elementmap).Value(/*"from"*/ STR(855)));
						self->to = value_get_point3D(DsMap(elementmap).Value(/*"to"*/ STR(856)));
						self->size = point3D_sub(self->to, self->from);
						self->volume = self->size.Value(X_) * self->size.Value(Y_) * self->size.Value(Z_);
						VarType rotationmap = DsMap(elementmap).Value(/*"rotation"*/ STR(857));
						if (ds_map_valid(rotationmap))
						{
							VarType origin, angle;
							VecType rot, scale;
							origin = value_get_point3D(DsMap(rotationmap).Value(/*"origin"*/ STR(858)), point3D(IntType(8), IntType(8), IntType(8)));
							angle = IntType(0);
							rot = vec3(IntType(0));
							scale = vec3(IntType(1));
							if (is_real(DsMap(rotationmap).Value(/*"angle"*/ STR(859))))
								angle = snap(clamp(DsMap(rotationmap).Value(/*"angle"*/ STR(859)), -IntType(45), IntType(45)), 22.5);
							if (is_bool(DsMap(rotationmap).Value(/*"rescale"*/ STR(860))) && DsMap(rotationmap).Value(/*"rescale"*/ STR(860)) > 0)
								scale = vec3(1.0 / dcos(abs(angle)));
							if (is_string(DsMap(rotationmap).Value(/*"axis"*/ STR(843))))
							{
								switch (StringType(DsMap(rotationmap).Value(/*"axis"*/ STR(843))).id)
								{
									case 861: // x
									{
										rot.Real(X_) = angle;
						...
						VarType facesmap = DsMap(elementmap).Value(/*"faces"*/ STR(863));
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							if (!is_undefined(curmap))
							{
								...
								if (ds_list_valid(DsMap(curmap).Value(/*"uv"*/ STR(864))))
								{
									VarType uvlist = DsMap(curmap).Value(/*"uv"*/ STR(864));
								...
								self->face_texture[f] = DsMap(curmap).Value(/*"texture"*/ STR(542));
								self->face_rotation[f] = IntType(0);
								if (is_real(DsMap(curmap).Value(/*"rotation"*/ STR(857))))
									self->face_rotation[f] = DsMap(curmap).Value(/*"rotation"*/ STR(857));
							}
							else
	...
	VarType block_load_model_file_texture(Scope<obj_block_load_model_file> self, VarType name, IntType res)
	{
		...
		if (string_pos(/*"assets/minecraft_"*/ STR(865), name) == IntType(1))
			name = string_replace(name, /*"assets/minecraft_"*/ STR(865), /*"block/"*/ STR(832));
		if (res == null_)
			return name;
		if (string_pos(/*"blocks/"*/ STR(866), name) == IntType(1))
		{
			name = string_replace(name, /*"blocks/"*/ STR(866), /*"block/"*/ STR(832));
		...
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"blocks/"*/ STR(866), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"block/"*/ STR(832), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + filename_name(name) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/"*/ STR(867) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/item/"*/ STR(868) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/block/"*/ STR(869) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			return name;
					...
					withOne (obj_block_render_element, (new obj_block_render_element)->id, self->id)
					{
						...
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							else
							{
								...
								if (opaque)
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" opaque"*/ STR(872));
								if (slot < IntType(0))
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(873));
								if (slot < IntType(0))
								...
								if (slot < IntType(0))
								{
									...
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(873));
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list), texname + /*" opaque"*/ STR(872));
									if (slot < IntType(0))
									{
										self->face_render[nd] = false;
										continue;
									...
									if (!is_undefined(col) && col == /*"water"*/ STR(828))
										...
										if (alpha > IntType(0))
										{
											...
											else
											{
												...
												switch ((IntType)self->face_block_vbuffer.Value(nd))
												{
													...
													{
														colstr = /*"grass"*/ STR(874);
														break;
													}
													case e_block_vbuffer_FOLIAGE:
													{
														colstr = /*"foliage"*/ STR(875);
														break;
													}
													case e_block_vbuffer_WATER:
													{
														colstr = /*"water"*/ STR(828);
														break;
													}
