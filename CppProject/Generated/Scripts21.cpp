...
namespace CppProject
{
	...
	VarType block_load_model_file_texture(Scope<obj_block_load_model_file> self, VarType name, IntType res)
	{
		...
		if (string_pos(/*"assets/minecraft_"*/ STR(860), name) == IntType(1))
			name = string_replace(name, /*"assets/minecraft_"*/ STR(860), /*"block/"*/ STR(827));
		if (res == null_)
			return name;
		if (string_pos(/*"blocks/"*/ STR(861), name) == IntType(1))
		{
			name = string_replace(name, /*"blocks/"*/ STR(861), /*"block/"*/ STR(827));
		...
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"blocks/"*/ STR(861), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + string_replace(name, /*"block/"*/ STR(827), /*""*/ STR(0)) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(16) + filename_name(name) + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/"*/ STR(862) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/item/"*/ STR(863) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/block/"*/ STR(864) + name + /*".png"*/ STR(6);
		if (!file_exists_lib(fn))
			return name;
					...
					withOne (obj_block_render_element, (new obj_block_render_element)->id, self->id)
					{
						...
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							else
							{
								...
								if (opaque)
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" opaque"*/ STR(867));
								if (slot < IntType(0))
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(868));
								if (slot < IntType(0))
								...
								if (slot < IntType(0))
								{
									...
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(868));
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list), texname + /*" opaque"*/ STR(867));
									if (slot < IntType(0))
									{
										self->face_render[nd] = false;
										continue;
									...
									if (!is_undefined(col) && col == /*"water"*/ STR(823))
										...
										if (alpha > IntType(0))
										{
											...
											else
											{
												...
												switch ((IntType)self->face_block_vbuffer.Value(nd))
												{
													...
													{
														colstr = /*"grass"*/ STR(869);
														break;
													}
													case e_block_vbuffer_FOLIAGE:
													{
														colstr = /*"foliage"*/ STR(870);
														break;
													}
													case e_block_vbuffer_WATER:
													{
														colstr = /*"water"*/ STR(823);
														break;
													}
	...
	VarType block_load_state_file(StringType fname, IntType block, ArrType state)
	{
		...
		{
			log({ /*"Could not find state file"*/ STR(878), filename_name(fname) });
			return null_;
		}
		...
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse state file"*/ STR(879), filename_name(fname) });
			ds_map_destroy(jsontypemap);
			return null_;
		}
		VarType variantsmap, multipartlist;
		variantsmap = DsMap(map).Value(/*"variants"*/ STR(880));
		multipartlist = DsMap(map).Value(/*"multipart"*/ STR(881));
		if (!ds_map_valid(variantsmap) && !ds_list_valid(multipartlist))
		{
			log({ /*"No models in the states file"*/ STR(882), fname });
			ds_map_destroy(map);
			return null_;
		}
		withOne (obj_block_load_state_file, (new obj_block_load_state_file)->id, noone)
		{
			...
			if (ds_map_valid(DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_preview_map).Value(sVar(name))))
			{
				...
				if (DsMap(blockmap).Value(/*"colorZ"*/ STR(883)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorZ"*/ STR(883))))
						self->model_preview_color_yp = hex_to_color(DsMap(blockmap).Value(/*"colorZ"*/ STR(883)));
					else
					{
				...
				if (self->model_preview_alpha_yp != null_ && DsMap(blockmap).Value(/*"alphaZ"*/ STR(884)) != VarType())
					self->model_preview_alpha_yp = DsMap(blockmap).Value(/*"alphaZ"*/ STR(884));
				if (DsMap(blockmap).Value(/*"colorY"*/ STR(885)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorY"*/ STR(885))))
						self->model_preview_color_zp = hex_to_color(DsMap(blockmap).Value(/*"colorY"*/ STR(885)));
					else
					{
				...
				if (self->model_preview_alpha_zp != null_ && DsMap(blockmap).Value(/*"alphaY"*/ STR(886)) != VarType())
					self->model_preview_alpha_zp = DsMap(blockmap).Value(/*"alphaY"*/ STR(886));
			}
			...
			if (ds_map_valid(variantsmap))
			{
				...
				while (!is_undefined(variant))
				{
					...
					{
						if (variant != /*""*/ STR(0) && variant != /*"normal"*/ STR(640))
						{
				...
				if (ds_list_valid(multipartlist))
				{
					for (IntType c = IntType(0); c < ds_list_size((IntType)(multipartlist)); c++)
					{
						...
						{
							VarType whenmap = DsMap(mcase).Value(/*"when"*/ STR(887));
							if (ds_map_valid(whenmap) && ds_map_size((IntType)(whenmap)) > IntType(0))
							{
								VarType orlist = DsMap(whenmap).Value(/*"OR"*/ STR(888));
								VarType andlist = DsMap(whenmap).Value(/*"AND"*/ STR(889));
									...
									for (IntType oc = IntType(0); oc < ds_list_size((IntType)(orlist)); oc++)
									{
										...
										while (!is_undefined(cond))
										{
											...
											if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(curcondmap)), cond) == e_json_type_BOOL)
												val = ((val > 0) ? /*"true"*/ STR(783) : /*"false"*/ STR(784));
											if (string_contains(val, /*"|"*/ STR(890)))
												state_vars_set_value(VarType::CreateRef(condvars), cond, string_split(val, /*"|"*/ STR(890)));
											else
							...
							self->model_amount = IntType(0);
							self->total_weight = IntType(0);
							if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(mcase)), /*"apply"*/ STR(891)) == e_json_type_ARRAY)
							{
								VarType modellist = DsMap(mcase).Value(/*"apply"*/ STR(891));
								for (IntType i = IntType(0); i < ds_list_size((IntType)(modellist)); i++)
									if (!block_load_variant_model(ScopeAny(self), DsList(modellist).Value(i), ObjType(obj_block, block)->type))
										return false;
							}
							else
								if (!block_load_variant_model(ScopeAny(self), DsMap(mcase).Value(/*"apply"*/ STR(891)), ObjType(obj_block, block)->type))
									return null_;
