...
namespace CppProject
{
	...
	void app_update_tl_edit_select(ScopeAny self)
	{
		...
		if (sInt(timeline_settings_import_loop_tl) != null_)
		{
			...
			sStr(timeline_settings_walk_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_walk.miframes"*/ STR(678);
			sStr(timeline_settings_run_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_run.miframes"*/ STR(679);
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		if (ext == /*".zip"*/ STR(5))
		{
			...
			{
				if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(681)))
					action_res_pack_load(self, fn);
				else
					error(/*"erroropenassetzip"*/ STR(682));
		...
		switch (StringType(ext).id)
		{
			...
			case 693: // .blocks
			{
				log({ /*"Opening scenery"*/ STR(694), fn });
			...
			case 703: // .m4a
			{
				log({ /*"Opening audio"*/ STR(704), fn });
				action_res_sound_load(self, fn);
				return true;
			}
			case 705: // .ttf
			{
				log({ /*"Adding font"*/ STR(706), fn });
		...
		{
			log({ /*"Opening legacy asset"*/ STR(711), fn });
		...
		switch (StringType(ext).id)
		{
			...
			case 684: // .miparticles
			{
				...
				withOne (obj_template, temp, self->id)
				{
					...
					project_load_particles(ScopeAny(self), DsMap(rootmap).Value(/*"particles"*/ STR(509)));
	...
	void background_ground_startup()
	{
		...
		log({ /*"Ground vbuffer init"*/ STR(715) });
