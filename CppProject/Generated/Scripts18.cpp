...
namespace CppProject
{
	...
	void app_update_tl_edit_select(ScopeAny self)
	{
		...
		if (sInt(timeline_settings_import_loop_tl) != null_)
		{
			...
			sStr(timeline_settings_walk_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_walk.miframes"*/ STR(717);
			sStr(timeline_settings_run_fn) = global::load_assets_startup_dir + mc_loops_directory + name + /*"_run.miframes"*/ STR(718);
	...
	BoolType asset_load(ScopeAny self, VarArgs argument)
	{
		...
		if (ext == /*".zip"*/ STR(5))
		{
			...
			{
				if (directory_exists_lib(unzip_directory + /*"assets"*/ STR(720)))
					action_res_pack_load(self, fn);
				else
					error(/*"erroropenassetzip"*/ STR(721));
		...
		switch (StringType(ext).id)
		{
			...
			case 732: // .blocks
			{
				log({ /*"Opening scenery"*/ STR(733), fn });
			...
			case 742: // .m4a
			{
				log({ /*"Opening audio"*/ STR(743), fn });
				action_res_sound_load(self, fn);
				return true;
			}
			case 744: // .ttf
			{
				log({ /*"Adding font"*/ STR(745), fn });
		...
		{
			log({ /*"Opening legacy asset"*/ STR(750), fn });
		...
		switch (StringType(ext).id)
		{
			...
			case 723: // .miparticles
			{
				...
				withOne (obj_template, temp, self->id)
				{
					...
					project_load_particles(ScopeAny(self), DsMap(rootmap).Value(/*"particles"*/ STR(517)));
	...
	void background_ground_startup()
	{
		...
		log({ /*"Ground vbuffer init"*/ STR(754) });
