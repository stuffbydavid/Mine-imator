...
namespace CppProject
{
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1504) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1504) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1504), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		if (sInt(settings_menu_window) != window_get_current())
			return IntType(0);
		if (sStr(settings_menu_ani_type) == /*"hide"*/ STR(758))
		{
			...
			if (sStr(settings_menu_ani_type) == /*"show"*/ STR(759))
			{
				sVar(settings_menu_ani) += test_reduced_motion(IntType(1), (0.08 * global::delta));
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(759)) ? /*"easeoutexpo"*/ STR(562) : /*"easeinexpo"*/ STR(561)), sVar(settings_menu_ani));
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1056) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				...
				{
					sVar(settings_menu_ani) = IntType(1);
					sStr(settings_menu_ani_type) = /*"hide"*/ STR(758);
					sVar(window_busy) = /*"settingsmenu"*/ STR(1056);
		...
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(758))
			sVar(window_busy) = /*"settingsmenu"*/ STR(1056);
		return 0.0;
	}
	...
	void menu_settings_set(ScopeAny self, VarType xx, VarType yy, StringType name, RealType buttonheight)
	{
		...
		sVar(settings_menu_ani) = IntType(0);
		sStr(settings_menu_ani_type) = /*"show"*/ STR(759);
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenuother"*/ STR(1725) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (!string_contains(DsList(global::transition_list).Value(i), /*"ease"*/ STR(1096)))
			{
		...
		sVar(dx) = sVar(dx_start);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueasein"*/ STR(1726) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easein"*/ STR(1094)) && !string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1093)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseout"*/ STR(1727) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeout"*/ STR(1095)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseinout"*/ STR(1728) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1093)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		if (sVar(context_menu_name) != /*""*/ STR(0) && transition != null_)
		{
			...
			if (transition != null_)
			{
				...
				ObjType(obj_menu, sVar(menu_current))->menu_ani_type = /*"hide"*/ STR(758);
				sVar(window_busy) = /*""*/ STR(0);
				return true;
			}
			...
			switch (StringType(global::load_assets_stage).id)
			{
				case 1314: // unzip
				{
					...
					if (!(exists > 0))
					{
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1729) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1730));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1731), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1732);
				...
				case 1732: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1732)));
					...
					global::load_assets_stage = /*"textures"*/ STR(851);
				...
				case 851: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1733));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1734) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1735));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1736) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1737));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1738) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1739));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1740) });
						return false;
					}
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(868))
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1741));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1742));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1743) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1744));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1745) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(518))))
					{
						log({ /*"No particle list found"*/ STR(1746) });
						return false;
					}
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						res_load_pack_item_textures(ScopeAny(self), /*"diffuse"*/ STR(647), /*""*/ STR(0));
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(518)));
					...
					global::load_assets_stage = /*"misc"*/ STR(1747);
				...
				case 1747: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1748))))
					{
						log({ /*"No pattern designs list found"*/ STR(1749) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1748));
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(976))))
					{
						log({ /*"No sherd keys found"*/ STR(1750) });
						return false;
					}
					else
						ds_map_copy((IntType)(global::minecraft_sherd_map), (IntType)(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(976))));
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"armor"*/ STR(789))))
					{
						log({ /*"No armor data found"*/ STR(1751) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(789));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1752))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(652))));
					}
				...
				case 1753: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1754));
					...
					debug_timer_stop(/*"Load characters"*/ STR(1757));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1758));
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1760));
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(1029);
				...
				case 1029: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(1029));
					if (is_undefined(blockslist))
					{
						log({ /*"No block list found"*/ STR(1761) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1762);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1763) });
							continue;
						}
					...
					if (dev_mode_skip_blocks)
						global::load_assets_stage = /*"done"*/ STR(1762);
					if (global::load_assets_stage == /*"done"*/ STR(1762))
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1764)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(826));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1765)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1766));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(826)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1767));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1766)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1768));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1764)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1769));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1765)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1770));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1771)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1772));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(82)), /*"minecraft:"*/ STR(847), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1773))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1773)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1774));
						...
						{
							ArrType filesarr = file_find(global::load_assets_dir + mc_blockstates_directory, /*".json"*/ STR(736));
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1776) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(982);
								log({ str });
							}
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(830), /*".json"*/ STR(736));
						...
						log({ /*"Loaded assets successfully"*/ STR(1778) });
	...
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(828));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1779)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(873))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(873)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(873)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(872))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(872)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(826))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(826)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(824))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(824));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(828));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(873))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(873)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(873))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(873)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(872))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(872)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(826))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(826)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(847), /*""*/ STR(0));
					block = null_;
					if (is_string(newid) && !is_undefined(DsMap(self->block_id_map).Value(newid)))
						block = DsMap(self->block_id_map).Value(newid);
