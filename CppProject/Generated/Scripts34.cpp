...
namespace CppProject
{
	...
	void menu_grid_settings(ScopeAny self)
	{
		...
		IntType draggerwid;
		draggerwid = text_max_width({ /*"viewgridrows"*/ STR(1660), /*"viewgridcolumns"*/ STR(1661) }) + IntType(16) + dragger_width;
		tab_control_dragger(self);
		draw_dragger(self, { /*"viewgridrows"*/ STR(1660), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_rows), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_rows), ID_action_project_grid_rows, null_, true });
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, { /*"viewgridcolumns"*/ STR(1661), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_columns), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_columns), ID_action_project_grid_columns, null_, true });
		tab_next(self);
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1455) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1455) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1455), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1008) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
	...
	void menu_snap_settings(ScopeAny self)
	{
		...
		draggerwid = text_max_width({ /*"viewsnapmove"*/ STR(1662), /*"viewsnaprotate"*/ STR(1663), /*"viewsnapscale"*/ STR(1664) }) + IntType(16) + dragger_width;
		switchwid = text_max_width({ /*"viewsnapabsolute"*/ STR(1665) }) + IntType(28) + IntType(16) + IntType(24);
		tab_control_dragger(self);
		draw_dragger(self, { /*"viewsnapmove"*/ STR(1662), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_position), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_position), ID_action_setting_snap_size_position });
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, { /*"viewsnaprotate"*/ STR(1663), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_rotation), 0.01, snap_min, no_limit, IntType(15), snap_min, sInt(tbx_snap_rotation), ID_action_setting_snap_size_rotation });
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, { /*"viewsnapscale"*/ STR(1664), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_scale), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_scale), ID_action_setting_snap_size_scale });
		tab_next(self);
		draw_divide(sVar(content_x), sVar(dy), sVar(content_width));
		sVar(dy) += IntType(8);
		tab_control_switch(self);
		draw_switch(self, { /*"viewsnapabsolute"*/ STR(1665), sVar(dx), sVar(dy), sVar(setting_snap_absolute), ID_action_setting_snap_absolute, /*"viewsnapabsolutetip"*/ STR(1666) });
		tab_next(self);
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenuother"*/ STR(1667) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (!string_contains(DsList(global::transition_list).Value(i), /*"ease"*/ STR(1048)))
			{
		...
		sVar(dx) = sVar(dx_start);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueasein"*/ STR(1668) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easein"*/ STR(1046)) && !string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1045)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseout"*/ STR(1669) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeout"*/ STR(1047)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseinout"*/ STR(1670) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1045)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			switch (StringType(global::load_assets_stage).id)
			{
				...
				case 1689: // misc
				{
					...
					global::load_assets_stage = /*"models"*/ STR(1692);
				...
				case 1692: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1693));
					if (is_undefined(characterslist))
					{
						log({ /*"No character list found"*/ STR(1694) });
						return false;
					}
					debug_timer_start();
					for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
					{
						...
						if (!(model > 0))
						{
							log({ /*"Could not load model"*/ STR(1695) });
							continue;
						}
						DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
						ds_list_add({ self->char_list, model });
					}
					debug_timer_stop(/*"Load characters"*/ STR(1696));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1697));
					if (is_undefined(specialblockslist))
					{
						log({ /*"No special block list found"*/ STR(1698) });
						return false;
					}
					debug_timer_start();
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1699));
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(981);
				...
				case 981: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(981));
					if (is_undefined(blockslist))
					{
						log({ /*"No block list found"*/ STR(1700) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1701);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1702) });
							continue;
						}
					...
					if (dev_mode_skip_blocks)
						global::load_assets_stage = /*"done"*/ STR(1701);
					if (global::load_assets_stage == /*"done"*/ STR(1701))
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1703)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(785));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1704)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1705));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(785)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1706));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1705)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1707));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1703)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1708));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1704)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1709));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1710)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1711));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1712))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1712)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1713));
						...
						if (dev_mode_debug_unused)
						{
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1715) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(940);
								log({ str });
							}
						...
						log({ /*"Loaded assets successfully"*/ STR(1717) });
	...
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			withOne (obj_biome, (new obj_biome)->id, noone)
			{
				...
				self->group = value_get_real({ DsMap(biome).Value(/*"group"*/ STR(1718)), false });
