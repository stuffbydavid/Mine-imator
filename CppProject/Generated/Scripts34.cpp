				...
				if (idVar(tl, part_of) != null_)
				{
					if (idVar(idVar(tl, part_of), parent) != global::_app->id)
					{
						if (idVar(idVar(tl, part_of), parent) == root)
							...
							caption += /*"/.../"*/ STR(1670) + idVar(idVar(tl, part_of), display_name);
					...
					if (root != idVar(tl, parent))
						caption += /*"/.../"*/ STR(1670) + idVar(idVar(tl, parent), display_name);
				
			}
			list_item_add(string_remove_newline(idVar(tl, display_name)), tl, caption);
			if (ds_list_size(idInt(tl, tree_list)) && !sBool(menu_expose))
				list_item_add_action(global::list_item_last, string(tl) + /*"extend"*/ STR(1671), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(197), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1672) : /*"tooltiptlexpand"*/ STR(1673), ID_spr_chevron_ani);
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			if (!(sBool(menu_expose) && ObjType(obj_biome, biome)->group > 0))
				menu_add_item({ ObjType(obj_biome, biome)->name, minecraft_asset_get_name(/*"biome"*/ STR(1674), ObjType(obj_biome, biome)->name) });
			if (ObjType(obj_biome, biome)->biome_variants != null_ && ds_list_size(ObjType(obj_biome, biome)->biome_variants) > IntType(0))
			{
				if (!sBool(menu_expose))
					list_item_add_action(global::list_item_last, string(biome) + /*"extend"*/ STR(1671), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(197), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1672) : /*"tooltiptlexpand"*/ STR(1673), ID_spr_chevron_ani);
				if (ObjType(obj_biome, biome)->variants_extend > 0 || sBool(menu_expose))
				{
					for (IntType v = IntType(0); v < ds_list_size(ObjType(obj_biome, biome)->biome_variants); v++)
					{
						if (ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name == /*"normal"*/ STR(603) && !sBool(menu_expose))
							continue;
						menu_add_item({ ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name, minecraft_asset_get_name(/*"biome"*/ STR(1674), ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name) });
						if (!sBool(menu_expose))
