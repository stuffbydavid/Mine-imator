...
namespace CppProject
{
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1501) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1501) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1501), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		if (sInt(settings_menu_window) != window_get_current())
			return IntType(0);
		if (sStr(settings_menu_ani_type) == /*"hide"*/ STR(757))
		{
			...
			if (sStr(settings_menu_ani_type) == /*"show"*/ STR(758))
			{
				sVar(settings_menu_ani) += test_reduced_motion(IntType(1), (0.08 * global::delta));
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(758)) ? /*"easeoutexpo"*/ STR(561) : /*"easeinexpo"*/ STR(560)), sVar(settings_menu_ani));
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1053) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				...
				{
					sVar(settings_menu_ani) = IntType(1);
					sStr(settings_menu_ani_type) = /*"hide"*/ STR(757);
		...
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(757))
			sVar(window_busy) = /*"settingsmenu"*/ STR(1053);
		return 0.0;
	}
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenuother"*/ STR(1712) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (!string_contains(DsList(global::transition_list).Value(i), /*"ease"*/ STR(1093)))
			{
		...
		sVar(dx) = sVar(dx_start);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueasein"*/ STR(1713) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easein"*/ STR(1091)) && !string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1090)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseout"*/ STR(1714) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeout"*/ STR(1092)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseinout"*/ STR(1715) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1090)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		if (sVar(context_menu_name) != /*""*/ STR(0) && transition != null_)
		{
			...
			if (transition != null_)
			{
				...
				ObjType(obj_menu, sVar(menu_current))->menu_ani_type = /*"hide"*/ STR(757);
				sVar(window_busy) = /*""*/ STR(0);
				return true;
			}
			...
			switch (StringType(global::load_assets_stage).id)
			{
				case 1311: // unzip
				{
					...
					if (!(exists > 0))
					{
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1716) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1717));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1718), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1719);
				...
				case 1719: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1719)));
					...
					global::load_assets_stage = /*"textures"*/ STR(848);
				...
				case 848: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1720));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1721) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1722));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1723) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1724));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1725) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1726));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1727) });
						return false;
					}
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(865))
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1728));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1729));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1730) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1731));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1732) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(517))))
					{
						log({ /*"No particle list found"*/ STR(1733) });
						return false;
					}
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						res_load_pack_item_textures(ScopeAny(self), /*"diffuse"*/ STR(646), /*""*/ STR(0));
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(517)));
					...
					global::load_assets_stage = /*"misc"*/ STR(1734);
				...
				case 1734: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1735))))
					{
						log({ /*"No pattern designs list found"*/ STR(1736) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1735));
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(973))))
					{
						log({ /*"No sherd keys found"*/ STR(1737) });
						return false;
					}
					else
						ds_map_copy((IntType)(global::minecraft_sherd_map), (IntType)(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(973))));
					
					global::load_assets_stage = /*"models"*/ STR(1738);
				...
				case 1738: // models
				{
					...
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(1026);
				...
				case 1026: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(1026));
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1749)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(823));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1750)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1751));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(823)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1752));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(82)), /*"minecraft:"*/ STR(844), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
						...
						{
							ArrType filesarr = file_find(global::load_assets_dir + mc_blockstates_directory, /*".json"*/ STR(735));
							...
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(827), /*".json"*/ STR(735));
	...
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(825));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1764)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(870))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(870)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(870)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(869))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(869)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(823))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(823)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(821))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(821));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(825));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(870))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(870)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(870))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(870)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(869))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(869)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(823))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(823)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(844), /*""*/ STR(0));
					block = null_;
					if (is_string(newid) && !is_undefined(DsMap(self->block_id_map).Value(newid)))
						block = DsMap(self->block_id_map).Value(newid);
