...
namespace CppProject
{
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1454) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1454) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1454), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1007) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		{
			if (!string_contains(DsList(global::transition_list).Value(i), /*"ease"*/ STR(1047)))
			{
		...
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easein"*/ STR(1045)) && !string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1044)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeout"*/ STR(1046)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
			if (string_contains(DsList(global::transition_list).Value(i), /*"easeinout"*/ STR(1044)))
			{
				if (sVar(dx) + IntType(46) > xx + wid)
				{
					sVar(dx) = sVar(dx_start);
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			switch (StringType(global::load_assets_stage).id)
			{
				...
				case 1691: // models
				{
					...
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(980);
				...
				case 980: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(980));
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1702)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(784));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1703)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1704));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(784)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1705));
