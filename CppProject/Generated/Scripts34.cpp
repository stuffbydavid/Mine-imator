...
namespace CppProject
{
	...
	void marker_editor_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"timelinemarkerlabel"*/ STR(1663), sVar(dx), sVar(dy), sVar(settings_menu_w) - IntType(24), IntType(24), idInt(sVar(timeline), tbx_marker_name), null_))
	...
	void math_lib_startup(StringType path)
	{
		...
		global::lib_math_simplex1d = external_define({ path, /*"simplex1D"*/ STR(1664), dll_cdecl, ty_real, IntType(1), ty_real });
		global::lib_math_simplex2d = external_define({ path, /*"simplex2D"*/ STR(1665), dll_cdecl, ty_real, IntType(2), ty_real, ty_real });
		global::lib_math_simplex3d = external_define({ path, /*"simplex3D"*/ STR(1666), dll_cdecl, ty_real, IntType(3), ty_real, ty_real, ty_real });
		global::lib_math_simplex4d = external_define({ path, /*"simplex4D"*/ STR(1667), dll_cdecl, ty_real, IntType(4), ty_real, ty_real, ty_real, ty_real });
	}
	...
	RealType menu_add_timeline(ScopeAny self, VarType tl, VarType root, RealType level, VarType menu)
	{
		if (tl == global::tl_edit && !ObjType(obj_menu, menu)->menu_include_tl_edit)
			return IntType(0);
		if (tl == null_)
			list_item_add(text_get({ /*"timelinenone"*/ STR(1668) }), tl);
		else
		{
			VarType caption = /*""*/ STR(0);
				...
				if (idVar(tl, part_of) != null_)
				{
					if (idVar(idVar(tl, part_of), parent) != global::_app->id)
					{
						if (idVar(idVar(tl, part_of), parent) == root)
							...
							caption += /*"/.../"*/ STR(1669) + idVar(idVar(tl, part_of), display_name);
					...
					if (root != idVar(tl, parent))
						caption += /*"/.../"*/ STR(1669) + idVar(idVar(tl, parent), display_name);
				
			}
			list_item_add(string_remove_newline(idVar(tl, display_name)), tl, caption);
			if (ds_list_size(idInt(tl, tree_list)) && !sBool(menu_expose))
				list_item_add_action(global::list_item_last, string(tl) + /*"extend"*/ STR(1670), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(195), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1671) : /*"tooltiptlexpand"*/ STR(1672), ID_spr_chevron_ani);
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			{
				if (!sBool(menu_expose))
					list_item_add_action(global::list_item_last, string(biome) + /*"extend"*/ STR(1670), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(195), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1671) : /*"tooltiptlexpand"*/ STR(1672), ID_spr_chevron_ani);
