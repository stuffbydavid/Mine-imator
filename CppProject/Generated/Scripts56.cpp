...
namespace CppProject
{
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			{
				if (itemname == /*"packunzipped"*/ STR(532) || itemname == /*"legacyblocksheet"*/ STR(535) || itemname == /*"fromworld"*/ STR(537))
					continue;
			}
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(811) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(811) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
				...
				for (IntType p = IntType(0); p < ds_list_size(idInt(slist, display_list)); p++)
				{
					...
					if (sVar(setting_search_variants) > 0)
					{
						...
						for (IntType i = IntType(0); i < statelen; i += IntType(2))
						{
							...
							VarType statename = text_get({ /*"blockstate"*/ STR(801) + state });
							...
							{
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(802), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								{
									variantmatch = true;
									break;
								}
					...
					if (!match && string_count(check, string_lower(minecraft_asset_get_name(/*"block"*/ STR(515), baseblock))) > IntType(0))
						match = true;
					if (!match)
					{
						ds_list_delete(idInt(slist, display_list), p);
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1144), /*" "*/ STR(17));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1123), /*""*/ STR(0)), /*"."*/ STR(1121), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2944)))
			str = string_replace(str, /*"-."*/ STR(2944), /*"-0."*/ STR(2945));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	VarType string_get_state_vars(VarType argument0)
	{
		...
		{
			ArrType nameval = string_split(vars.Value(i), /*"="*/ STR(2946));
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2947)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(979))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2947)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2947);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(17) || char_ == /*", "*/ STR(1059) || char_ == /*"."*/ STR(1121) || char_ == /*"/"*/ STR(16) || char_ == /*"-"*/ STR(1123))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1147) + sep, str);
	...
	{
		StringType sep = /*":"*/ STR(793);
		return string_repeat(/*"0"*/ STR(1080), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1080), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1080), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(17), /*""*/ STR(0));
	}
