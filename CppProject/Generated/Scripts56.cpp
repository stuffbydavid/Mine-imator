...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		{
			if (draw_button_icon(self, { /*"listfilter"*/ STR(3001) + string(slist), xx, yy, IntType(24), IntType(24), !ds_list_empty(idInt(slist, filter_list)), icons_FILTER, null_, false, /*"tooltipfilterlist"*/ STR(3002) }))
			{
				menu_settings_set(self, xx, yy, /*"listfilter"*/ STR(3001) + string(slist), IntType(24));
				sInt(settings_menu_script) = ID_sortlist_filters_draw;
				sVar(settings_menu_sortlist) = slist;
				sVar(settings_menu_h_max) = IntType(256);
			}
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(3001) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(3003) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(3003) }), /*"none"*/ STR(919)))
		{
		...
		if (sVar(window_busy) == /*"sortlist_resize"*/ STR(3004) && sInt(sortlist_resize) == slist)
		{
		...
		for (IntType c = IntType(0); c < idInt(slist, columns); c++)
		{
			...
			if (c > IntType(0) && app_mouse_box(self, xx + dx - IntType(5), yy, IntType(10), colsh) && sBool(content_mouseon))
			{
				...
				if (sBool(mouse_left_pressed))
				{
					...
					sVar(window_busy) = /*"sortlist_resize"*/ STR(3004);
				}
			}
			icon = null_;
			if (idInt(slist, column_sort) == c)
				icon = ((idReal(slist, sort_asc) > 0) ? icons_SORT_UP : icons_SORT_DOWN);
			if (sortlist_draw_button(self, /*"column"*/ STR(3005) + idArr(slist, column_name).Value(c), xx + dx, yy + IntType(4), idArr(slist, column_w).Value(c), colsh, idInt(slist, column_sort) == c, icon, (c == IntType(0)), (c == idInt(slist, columns) - IntType(1)), colmouseon))
			{
				if (idInt(slist, column_sort) == c)
				{
		...
		if (sVar(window_focus) == string(idInt(slist, scroll)))
		{
			...
			if (!app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && sBool(mouse_left) && sVar(window_busy) != /*"scrollbar"*/ STR(2665))
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			{
				if (itemname == /*"packunzipped"*/ STR(535) || itemname == /*"legacyblocksheet"*/ STR(538) || itemname == /*"fromworld"*/ STR(540))
					continue;
			}
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(816) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(816) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
				...
				for (IntType p = IntType(0); p < ds_list_size(idInt(slist, display_list)); p++)
				{
					...
					if (sVar(setting_search_variants) > 0)
					{
						...
						for (IntType i = IntType(0); i < statelen; i += IntType(2))
						{
							...
							VarType statename = text_get({ /*"blockstate"*/ STR(806) + state });
							...
							{
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(807), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								{
									variantmatch = true;
									break;
								}
					...
					if (!match && string_count(check, string_lower(minecraft_asset_get_name(/*"block"*/ STR(518), baseblock))) > IntType(0))
						match = true;
					if (!match)
					{
						ds_list_delete(idInt(slist, display_list), p);
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1149), /*" "*/ STR(17));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1128), /*""*/ STR(0)), /*"."*/ STR(1126), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(3006)))
			str = string_replace(str, /*"-."*/ STR(3006), /*"-0."*/ STR(3007));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(3009)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(984))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(3009)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(3009);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(17) || char_ == /*", "*/ STR(1064) || char_ == /*"."*/ STR(1126) || char_ == /*"/"*/ STR(16) || char_ == /*"-"*/ STR(1128))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1152) + sep, str);
	...
	{
		StringType sep = /*":"*/ STR(798);
		return string_repeat(/*"0"*/ STR(1085), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1085), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1085), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(17), /*""*/ STR(0));
	}
