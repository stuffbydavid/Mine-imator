...
namespace CppProject
{
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1100), /*" "*/ STR(17));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1079), /*""*/ STR(0)), /*"."*/ STR(1077), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2896)))
			str = string_replace(str, /*"-."*/ STR(2896), /*"-0."*/ STR(2897));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2899)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(938))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2899)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2899);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(17) || char_ == /*", "*/ STR(1015) || char_ == /*"."*/ STR(1077) || char_ == /*"/"*/ STR(16) || char_ == /*"-"*/ STR(1079))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1103) + sep, str);
	...
	StringType string_time(RealType hours, RealType mnts, RealType secs)
	{
		...
		return string_repeat(/*"0"*/ STR(1036), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1036), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1036), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(17), /*""*/ STR(0));
	}
