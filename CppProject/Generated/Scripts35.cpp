...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1726), /*"b"*/ STR(1727), /*"c"*/ STR(1728), /*"d"*/ STR(1729), /*"e"*/ STR(1204), /*"f"*/ STR(1205), /*"g"*/ STR(1730), /*"h"*/ STR(1731), /*"i"*/ STR(1732), /*"j"*/ STR(1733), /*"k"*/ STR(1734), /*"l"*/ STR(1735), /*"m"*/ STR(1736), /*"n"*/ STR(1206), /*"o"*/ STR(1737), /*"p"*/ STR(1738), /*"q"*/ STR(1739), /*"r"*/ STR(1740), /*"s"*/ STR(1741), /*"t"*/ STR(1207), /*"u"*/ STR(1208), /*"v"*/ STR(1742), /*"w"*/ STR(1743), /*"x"*/ STR(818), /*"y"*/ STR(801), /*"z"*/ STR(819) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1744))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1744));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real({ DsMap(tempmap).Value(/*"size"*/ STR(977)), IntType(8) });
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real({ DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1745)), false });
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1099) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1099) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1266);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1746);
		...
		{
			log({ /*"Could not load "*/ STR(1747) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1748), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1749)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1749) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1750));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1751));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1752), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1753), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1754), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1755), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1756), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1757));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1099) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1758) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1755), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1693));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1697));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1759) + name + /*"]"*/ STR(1202) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1760) + name + /*"]"*/ STR(1202) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1564) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(784) ? /*"entity/banner_base"*/ STR(1761) : /*"entity/shield_base"*/ STR(1762));
		patterndir = (type == /*"banner"*/ STR(784) ? /*"entity/banner/"*/ STR(1763) : /*"entity/shield/"*/ STR(1764));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1765), fn });
		show_message(/*"The file "*/ STR(1766) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1767));
		return false;
	}
	...
	IntType model_file_load(VarArgs argument)
	{
		...
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1768) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1769))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1770) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1771))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1772) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(515) + self->name))
				log({ /*"model/"*/ STR(1773) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string({ DsMap(map).Value(/*"description"*/ STR(1774)), /*""*/ STR(0) });
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string({ DsMap(map).Value(/*"texture_material"*/ STR(1775)), /*""*/ STR(0) });
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string({ DsMap(map).Value(/*"texture_normal"*/ STR(1776)), /*""*/ STR(0) });
			...
			self->texture_size = value_get_point2D({ DsMap(map).Value(/*"texture_size"*/ STR(1769)) });
			...
			self->player_skin = value_get_real({ DsMap(map).Value(/*"player_skin"*/ STR(1777)), false });
			self->scale = value_get_point3D({ DsMap(map).Value(/*"scale"*/ STR(1778)), vec3(IntType(1), IntType(1), IntType(1)) });
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1771));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1779))) && !(DsMap(map).Value(/*"visible"*/ STR(1779)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(787))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(788) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1780) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1781), DsMap(map).Value(/*"name"*/ STR(787)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(748) + self->name))
				log({ /*"model/part/"*/ STR(1782) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real({ DsMap(map).Value(/*"color_inherit"*/ STR(1783)), true });
			self->color_blend = value_get_color({ DsMap(map).Value(/*"color_blend"*/ STR(1784)), c_white });
			self->color_alpha = value_get_real({ DsMap(map).Value(/*"color_alpha"*/ STR(1785)), IntType(1) });
			if (is_real(/*"color_emissive"*/ STR(1786)))
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_emissive"*/ STR(1786)), IntType(0) });
			else
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_brightness"*/ STR(1787)), IntType(0) });
			
			self->color_mix = value_get_color({ DsMap(map).Value(/*"color_mix"*/ STR(1788)), c_black });
			self->color_mix_percent = value_get_real({ DsMap(map).Value(/*"color_mix_percent"*/ STR(1789)), IntType(0) });
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real({ DsMap(map).Value(/*"show_position"*/ STR(1790)), false });
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1791))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1791))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1791));
				if (self->lock_bend > 0)
				{
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1792))))
			{
				...
				self->bend_size = value_get_real({ DsMap(bendmap).Value(/*"size"*/ STR(977)), null_ });
