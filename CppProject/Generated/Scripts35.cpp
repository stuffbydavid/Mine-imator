...
namespace CppProject
{
	...
	void menu_grid_settings(ScopeAny self)
	{
		...
		IntType draggerwid;
		draggerwid = text_max_width({ /*"viewgridrows"*/ STR(1675), /*"viewgridcolumns"*/ STR(1676) }) + IntType(16) + dragger_width;
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridrows"*/ STR(1675), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_rows), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_rows), ID_action_project_grid_rows, null_, true);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridcolumns"*/ STR(1676), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_columns), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_columns), ID_action_project_grid_columns, null_, true);
		tab_next(self);
	...
	void menu_snap_settings(ScopeAny self)
	{
		...
		draggerwid = text_max_width({ /*"viewsnapmove"*/ STR(1677), /*"viewsnaprotate"*/ STR(1678), /*"viewsnapscale"*/ STR(1679) }) + IntType(16) + dragger_width;
		switchwid = text_max_width({ /*"viewsnapabsolute"*/ STR(1680) }) + IntType(28) + IntType(16) + IntType(24);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapmove"*/ STR(1677), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_position), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_position), ID_action_setting_snap_size_position);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnaprotate"*/ STR(1678), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_rotation), 0.01, snap_min, no_limit, IntType(15), snap_min, sInt(tbx_snap_rotation), ID_action_setting_snap_size_rotation);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapscale"*/ STR(1679), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_scale), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_scale), ID_action_setting_snap_size_scale);
		tab_next(self);
		draw_divide(sVar(content_x), sVar(dy), sVar(content_width));
		sVar(dy) += IntType(8);
		tab_control_switch(self);
		draw_switch(self, /*"viewsnapabsolute"*/ STR(1680), sVar(dx), sVar(dy), sVar(setting_snap_absolute), ID_action_setting_snap_absolute, /*"viewsnapabsolutetip"*/ STR(1681));
		tab_next(self);
	...
	BoolType menu_swatches_draw(ScopeAny self)
	{
		...
		for (IntType s = IntType(0); s < array_length(VarType::CreateRef(global::minecraft_swatch_array)); s++)
		{
			...
			sVar(dy) += IntType(14);
			draw_label(self, { text_get({ /*"swatch"*/ STR(1682) + ObjType(obj_swatch, swatch)->name }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(9);
			VarType xx;
			xx = sVar(dx);
			for (IntType c = IntType(0); c < array_length(VarType::CreateRef(ObjType(obj_swatch, swatch)->colors)); c++)
			{
				if (draw_button_swatch(self, xx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1682) + ObjType(obj_swatch, swatch)->name + ObjType(obj_swatch, swatch)->color_names.Value(c), ObjType(obj_swatch, swatch)->colors.Value(c)))
				{
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1687) + name + /*"]"*/ STR(1213) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1688) + name + /*"]"*/ STR(1213) : /*""*/ STR(0));
			...
			switch (StringType(global::load_assets_stage).id)
			{
				case 1277: // unzip
				{
					...
					if (!(exists > 0))
					{
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1689) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1690));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1691), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1692);
				...
				case 1692: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1692)));
				...
				case 814: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1693));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1694) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1695));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1696) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1697));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1698) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1699));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1700) });
						return false;
					}
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1701));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1702));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1703) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1704));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1705) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(525))))
					{
						log({ /*"No particle list found"*/ STR(1706) });
						return false;
					}
					...
					global::load_assets_stage = /*"misc"*/ STR(1707);
				...
				case 1707: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1708))))
					{
						log({ /*"No pattern designs list found"*/ STR(1709) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1708));
					...
					{
						log({ /*"No sherd keys found"*/ STR(1710) });
						return false;
					}
					else
					...
					{
						log({ /*"No armor data found"*/ STR(1711) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(753));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1712))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(614))));
					}
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1713))))
					{
						log({ /*"No map colors found"*/ STR(1714) });
						return false;
					}
					VarType colors = DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1713));
					...
					global::load_assets_stage = /*"models"*/ STR(1715);
					global::load_assets_progress = 0.5;
					VarType swatches = DsMap(global::load_assets_map).Value(/*"swatches"*/ STR(1716));
					if (is_undefined(swatches))
					{
						log({ /*"No swatches found"*/ STR(1717) });
						return false;
					}
					for (IntType i = IntType(0); i < ds_list_size((IntType)(swatches)); i++)
					{
						...
						VarType colors = DsMap(swatch).Value(/*"colors"*/ STR(1718));
					...
					global::minecraft_swatch_dyes = minecraft_get_swatch(/*"dye"*/ STR(1719));
					if (global::minecraft_swatch_dyes == null_)
					{
						log({ /*"No dyes found"*/ STR(1720) });
						return false;
					}
					break;
				}
				...
				case 1715: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1721));
					...
					debug_timer_stop(/*"Load characters"*/ STR(1724));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1725));
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1727));
					debug_timer_start();
				...
				case 996: // blocks
				{
					...
					{
						log({ /*"No block list found"*/ STR(1728) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1729);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1730) });
							continue;
						}
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1731)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(789));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1732)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1733));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(789)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1734));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1733)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1735));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1731)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1736));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1732)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1737));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1738)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1739));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1740))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1740)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1741));
						...
						if (dev_mode_debug_unused)
						{
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1743) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(945);
								log({ str });
							}
						...
						log({ /*"Loaded assets successfully"*/ STR(1745) });
