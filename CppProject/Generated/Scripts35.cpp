...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1723), /*"b"*/ STR(1724), /*"c"*/ STR(1725), /*"d"*/ STR(1726), /*"e"*/ STR(1202), /*"f"*/ STR(1203), /*"g"*/ STR(1727), /*"h"*/ STR(1728), /*"i"*/ STR(1729), /*"j"*/ STR(1730), /*"k"*/ STR(1731), /*"l"*/ STR(1732), /*"m"*/ STR(1733), /*"n"*/ STR(1204), /*"o"*/ STR(1734), /*"p"*/ STR(1735), /*"q"*/ STR(1736), /*"r"*/ STR(1737), /*"s"*/ STR(1738), /*"t"*/ STR(1205), /*"u"*/ STR(1206), /*"v"*/ STR(1739), /*"w"*/ STR(1740), /*"x"*/ STR(817), /*"y"*/ STR(800), /*"z"*/ STR(818) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(786));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(531));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1741))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1741));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real({ DsMap(tempmap).Value(/*"size"*/ STR(975)), IntType(8) });
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real({ DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1742)), false });
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1097) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1097) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1264);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1743);
		...
		{
			log({ /*"Could not load "*/ STR(1744) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1745), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1746)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1746) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1747));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1748));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1749), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1750), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1751), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1752), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1753), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1754));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1097) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1755) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1752), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1690));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1694));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1756) + name + /*"]"*/ STR(1200) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1757) + name + /*"]"*/ STR(1200) : /*""*/ STR(0));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(783) ? /*"entity/banner_base"*/ STR(1758) : /*"entity/shield_base"*/ STR(1759));
		patterndir = (type == /*"banner"*/ STR(783) ? /*"entity/banner/"*/ STR(1760) : /*"entity/shield/"*/ STR(1761));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1762), fn });
		show_message(/*"The file "*/ STR(1763) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1764));
		return false;
	}
	...
	IntType model_file_load(VarArgs argument)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(806), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(807), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(786))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(787) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(531))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1765) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1766))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1767) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1768))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1769) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(786));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(514) + self->name))
				log({ /*"model/"*/ STR(1770) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string({ DsMap(map).Value(/*"description"*/ STR(1771)), /*""*/ STR(0) });
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string({ DsMap(map).Value(/*"texture_material"*/ STR(1772)), /*""*/ STR(0) });
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string({ DsMap(map).Value(/*"texture_normal"*/ STR(1773)), /*""*/ STR(0) });
			...
			self->texture_size = value_get_point2D({ DsMap(map).Value(/*"texture_size"*/ STR(1766)) });
			...
			self->player_skin = value_get_real({ DsMap(map).Value(/*"player_skin"*/ STR(1774)), false });
			self->scale = value_get_point3D({ DsMap(map).Value(/*"scale"*/ STR(1775)), vec3(IntType(1), IntType(1), IntType(1)) });
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1768));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1776))) && !(DsMap(map).Value(/*"visible"*/ STR(1776)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(786))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(787) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1777) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(786)))
			{
				log({ /*"Duplicate part name found"*/ STR(1778), DsMap(map).Value(/*"name"*/ STR(786)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(786));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(747) + self->name))
				log({ /*"model/part/"*/ STR(1779) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real({ DsMap(map).Value(/*"depth"*/ STR(610)), IntType(0) });
			IntType pos;
			...
			self->color_inherit = value_get_real({ DsMap(map).Value(/*"color_inherit"*/ STR(1780)), true });
			self->color_blend = value_get_color({ DsMap(map).Value(/*"color_blend"*/ STR(1781)), c_white });
			self->color_alpha = value_get_real({ DsMap(map).Value(/*"color_alpha"*/ STR(1782)), IntType(1) });
			if (is_real(/*"color_emissive"*/ STR(1783)))
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_emissive"*/ STR(1783)), IntType(0) });
			else
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_brightness"*/ STR(1784)), IntType(0) });
			
			self->color_mix = value_get_color({ DsMap(map).Value(/*"color_mix"*/ STR(1785)), c_black });
			self->color_mix_percent = value_get_real({ DsMap(map).Value(/*"color_mix_percent"*/ STR(1786)), IntType(0) });
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real({ DsMap(map).Value(/*"show_position"*/ STR(1787)), false });
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real({ DsMap(map).Value(/*"locked"*/ STR(906)), false });
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1788))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1788))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1788));
				if (self->lock_bend > 0)
				{
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1789))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1789));
				self->bend_inherit = value_get_real({ DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1790)), false });
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1791))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1792) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1791));
				self->bend_end_offset = value_get_real({ DsMap(bendmap).Value(/*"end_offset"*/ STR(1793)), IntType(0) });
				self->bend_size = value_get_real({ DsMap(bendmap).Value(/*"size"*/ STR(975)), null_ });
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(868))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1794) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(868))).id)
				{
					case 189: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1798) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(799))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(799))).id)
					{
						case 817: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1799) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(799))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(799)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(799))), i)).id)
							{
								case 817: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1800) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1801))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1801));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1801))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1801)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1801))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1802))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1802));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1802))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1802)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1802))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(895))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(895))).id)
					{
						case 1803: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1806) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(895))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(895)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(895))), i)).id)
							{
								case 1803: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1807))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1807)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1807)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1807))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1807)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1807))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(815))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(815)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(815))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(815)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(815))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1808));
