...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1725), /*"b"*/ STR(1726), /*"c"*/ STR(1727), /*"d"*/ STR(1728), /*"e"*/ STR(1203), /*"f"*/ STR(1204), /*"g"*/ STR(1729), /*"h"*/ STR(1730), /*"i"*/ STR(1731), /*"j"*/ STR(1732), /*"k"*/ STR(1733), /*"l"*/ STR(1734), /*"m"*/ STR(1735), /*"n"*/ STR(1205), /*"o"*/ STR(1736), /*"p"*/ STR(1737), /*"q"*/ STR(1738), /*"r"*/ STR(1739), /*"s"*/ STR(1740), /*"t"*/ STR(1206), /*"u"*/ STR(1207), /*"v"*/ STR(1741), /*"w"*/ STR(1742), /*"x"*/ STR(817), /*"y"*/ STR(800), /*"z"*/ STR(818) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real({ DsMap(tempmap).Value(/*"size"*/ STR(976)), IntType(8) });
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1098) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1098) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1265);
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1098) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1758) + name + /*"]"*/ STR(1201) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1759) + name + /*"]"*/ STR(1201) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1563) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(783) ? /*"entity/banner_base"*/ STR(1760) : /*"entity/shield_base"*/ STR(1761));
		patterndir = (type == /*"banner"*/ STR(783) ? /*"entity/banner/"*/ STR(1762) : /*"entity/shield/"*/ STR(1763));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	VarType model_file_load_part(Scope<obj_model_part> self, VarType map, IntType root, VarType res, IntType model)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1780), DsMap(map).Value(/*"name"*/ STR(786)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1791))))
			{
				...
				self->bend_size = value_get_real({ DsMap(bendmap).Value(/*"size"*/ STR(976)), null_ });
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(869))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1796) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(869))).id)
				{
					case 189: // right
					{
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(896))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(896))).id)
					{
						case 1805: // forward
						{
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(896))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(896)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(896))), i)).id)
							{
								case 1805: // forward
								{
