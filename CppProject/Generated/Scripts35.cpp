...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1772), /*"b"*/ STR(1773), /*"c"*/ STR(1774), /*"d"*/ STR(1775), /*"e"*/ STR(1249), /*"f"*/ STR(1250), /*"g"*/ STR(1776), /*"h"*/ STR(1777), /*"i"*/ STR(1778), /*"j"*/ STR(1779), /*"k"*/ STR(1780), /*"l"*/ STR(1781), /*"m"*/ STR(1782), /*"n"*/ STR(1251), /*"o"*/ STR(1783), /*"p"*/ STR(1784), /*"q"*/ STR(1785), /*"r"*/ STR(1786), /*"s"*/ STR(1787), /*"t"*/ STR(1252), /*"u"*/ STR(1253), /*"v"*/ STR(1788), /*"w"*/ STR(1789), /*"x"*/ STR(856), /*"y"*/ STR(839), /*"z"*/ STR(857) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(825));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(539));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(1022)), IntType(8));
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1144) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1144) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1311);
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1144) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1805) + name + /*"]"*/ STR(1247) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1806) + name + /*"]"*/ STR(1247) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1610) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(822) ? /*"entity/banner_base"*/ STR(1807) : /*"entity/shield_base"*/ STR(1808));
		patterndir = (type == /*"banner"*/ STR(822) ? /*"entity/banner/"*/ STR(1809) : /*"entity/shield/"*/ STR(1810));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1811), fn });
		show_message(/*"The file "*/ STR(1812) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1813));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(845), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(846), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(825))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(826) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(539))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1814) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1815))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1816) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1817))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1818) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(825));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(522) + self->name))
				log({ /*"model/"*/ STR(1819) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1820)), /*""*/ STR(0));
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(539));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1821)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1822)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1815)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1823)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(895)), vec3(IntType(1), IntType(1), IntType(1)));
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1817));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1824))) && !(DsMap(map).Value(/*"visible"*/ STR(1824)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(825))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(826) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1825) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(825)))
			{
				log({ /*"Duplicate part name found"*/ STR(1826), DsMap(map).Value(/*"name"*/ STR(825)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(825));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(786) + self->name))
				log({ /*"model/part/"*/ STR(1827) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(650)), IntType(0));
			IntType pos;
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1828)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1829)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1830)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1831)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1831)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1832)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1833)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1834)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(852)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(895)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1835)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(947)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1836))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1836))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1836));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1837)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1838))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1838));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1839)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1840))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1841) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1840));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1842)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(1022)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(909))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1843) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(909))).id)
				{
					case 190: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1845) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(838))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(838))).id)
					{
						case 856: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1846) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(838))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(838)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(838))), i)).id)
							{
								case 856: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1847) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1848))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1848));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1848))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1848)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1848))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1849))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1849));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1849))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1849)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1849))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(936))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(936))).id)
					{
						case 1850: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1853) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(936))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(936)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(936))), i)).id)
							{
								case 1850: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1854))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1854)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1854)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1854))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1854)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1854))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(854))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(854)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(854))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(854)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(854))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1855));
