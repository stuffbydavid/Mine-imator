...
namespace CppProject
{
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1023) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
			sVar(window_busy) = /*""*/ STR(0);
		sBool(content_mouseon) = app_mouse_box(self, sVar(content_x), sVar(content_y), sVar(content_width), sVar(content_height));
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				if (!app_mouse_box(self, sVar(content_x), sVar(content_y), sVar(content_width), sVar(content_height)))
				{
					...
					sVar(window_busy) = /*"settingsmenu"*/ STR(1023);
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			switch (StringType(global::load_assets_stage).id)
			{
				...
				case 1705: // misc
				{
					...
					{
						log({ /*"No sherd keys found"*/ STR(1708) });
						return false;
					}
					else
					...
					{
						log({ /*"No armor data found"*/ STR(1709) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(753));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1710))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(614))));
					}
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1711))))
					{
						log({ /*"No map colors found"*/ STR(1712) });
						return false;
					}
					VarType colors = DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1711));
					...
					global::load_assets_stage = /*"models"*/ STR(1713);
					global::load_assets_progress = 0.5;
					VarType swatches = DsMap(global::load_assets_map).Value(/*"swatches"*/ STR(1714));
					if (is_undefined(swatches))
					{
						log({ /*"No swatches found"*/ STR(1715) });
						return false;
					}
					for (IntType i = IntType(0); i < ds_list_size((IntType)(swatches)); i++)
					{
						...
						VarType colors = DsMap(swatch).Value(/*"colors"*/ STR(1716));
					...
					global::minecraft_swatch_dyes = minecraft_get_swatch(/*"dye"*/ STR(1717));
					if (global::minecraft_swatch_dyes == null_)
					{
						log({ /*"No dyes found"*/ STR(1718) });
						return false;
					}
					break;
				}
				...
				case 1713: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1719));
					...
					debug_timer_stop(/*"Load characters"*/ STR(1722));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1723));
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1725));
					debug_timer_start();
				...
				case 996: // blocks
				{
					...
					{
						log({ /*"No block list found"*/ STR(1726) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1727);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1728) });
							continue;
						}
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1729)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(789));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1730)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1731));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(789)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1732));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1731)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1733));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1729)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1734));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1730)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1735));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1736)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1737));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1738))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1738)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1739));
						...
						if (dev_mode_debug_unused)
						{
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1741) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(945);
								log({ str });
							}
						...
						log({ /*"Loaded assets successfully"*/ STR(1743) });
