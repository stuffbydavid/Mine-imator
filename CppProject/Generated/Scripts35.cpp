...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1787), /*"b"*/ STR(1788), /*"c"*/ STR(1789), /*"d"*/ STR(1790), /*"e"*/ STR(1252), /*"f"*/ STR(1253), /*"g"*/ STR(1791), /*"h"*/ STR(1792), /*"i"*/ STR(1793), /*"j"*/ STR(1794), /*"k"*/ STR(1795), /*"l"*/ STR(1796), /*"m"*/ STR(1797), /*"n"*/ STR(1254), /*"o"*/ STR(1798), /*"p"*/ STR(1799), /*"q"*/ STR(1800), /*"r"*/ STR(1801), /*"s"*/ STR(1802), /*"t"*/ STR(1255), /*"u"*/ STR(1256), /*"v"*/ STR(1803), /*"w"*/ STR(1804), /*"x"*/ STR(859), /*"y"*/ STR(842), /*"z"*/ STR(860) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(828));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(540));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1805))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1805));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real(DsMap(tempmap).Value(/*"size"*/ STR(1025)), IntType(8));
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1806)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1147) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1147) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1807);
		...
		{
			log({ /*"Could not load "*/ STR(1808) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1809), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1810)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1810) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1811));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1812));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1813), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1814), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1815), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1816), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1817), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1818));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1147) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1819) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1816), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1754));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1755) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1756) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1758));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1759) });
				break;
			}
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1820) + name + /*"]"*/ STR(1250) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1821) + name + /*"]"*/ STR(1250) : /*""*/ STR(0));
	...
	{
		if (global::armor_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1623) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
		...
		for (IntType i = IntType(0); i < IntType(4); i++)
		{
			...
			if (basemat == /*"none"*/ STR(917))
				continue;
			dye = data.Value(piece + IntType(1));
			trimpat = data.Value(piece + IntType(2));
			trimmat = data.Value(piece + IntType(3));
			layernum = ((i == IntType(2)) ? /*"2"*/ STR(1131) : /*"1"*/ STR(1130));
			basetex = /*"models/armor/"*/ STR(1822) + basemat + /*"_layer_"*/ STR(1823) + layernum;
			...
			surface_set_target(armorsurf);
		{
			...
			if (basemat == /*"leather"*/ STR(1824))
			{
				draw_texture({ basesprite, IntType(0), IntType(0), IntType(1), IntType(1), dye, IntType(1) });
				draw_texture({ DsMap(idInt(res, model_texture_map)).Value(basetex + /*"_overlay"*/ STR(1825)), IntType(0), IntType(0) });
			}
			else
			...
			if (trimpat != /*"none"*/ STR(917) && trimmat != /*"none"*/ STR(917))
			{
				VarType palette = DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/"*/ STR(1826) + trimmat + (trimmat == basemat ? /*"_darker"*/ STR(1827) : /*""*/ STR(0)));
				surface_set_target(armorsurf);
			{
				...
				withOne (Object, global::render_shader_obj, noone)
				{
					...
					shader_palette_set(self, palette, DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/trim_palette"*/ STR(1828)));
				}
				
				if (i == IntType(2))
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1829) + trimpat + /*"_leggings"*/ STR(1830))), IntType(0), IntType(0), IntType(0));
				else
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1829) + trimpat)), IntType(0), IntType(0), IntType(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1623) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(825) ? /*"entity/banner_base"*/ STR(1831) : /*"entity/shield_base"*/ STR(1832));
		patterndir = (type == /*"banner"*/ STR(825) ? /*"entity/banner/"*/ STR(1833) : /*"entity/shield/"*/ STR(1834));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1835), fn });
		show_message(/*"The file "*/ STR(1836) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1837));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Could not find model file"*/ STR(848), fname });
			return null_;
		}
		IntType map = json_load({ fname });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse model file"*/ STR(849), fname });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(828))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(829) });
			return null_;
		}
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(540))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1838) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1839))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1840) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1841))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1842) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(828));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(523) + self->name))
				log({ /*"model/"*/ STR(1843) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1844)), /*""*/ STR(0));
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(540));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1845)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1846)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1839)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1847)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(898)), vec3(IntType(1), IntType(1), IntType(1)));
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1841));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1848))) && !(DsMap(map).Value(/*"visible"*/ STR(1848)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(828))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(829) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1849) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(828)))
			{
				log({ /*"Duplicate part name found"*/ STR(1850), DsMap(map).Value(/*"name"*/ STR(828)) });
				return null_;
			}
		}
		...
		{
			self->name = DsMap(map).Value(/*"name"*/ STR(828));
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(787) + self->name))
				log({ /*"model/part/"*/ STR(1851) + self->name + dev_mode_name_translation_message });
			self->depth = value_get_real(DsMap(map).Value(/*"depth"*/ STR(651)), IntType(0));
			IntType pos;
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1852)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1853)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1854)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1855)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1855)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1856)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1857)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1858)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(855)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(898)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1859)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(950)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1860))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1860))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1860));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1861)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1862))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1862));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1863)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1864))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1865) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1864));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1866)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(1025)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(912))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1867) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(912))).id)
				{
					case 190: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1869) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(841))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(841))).id)
					{
						case 859: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1870) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(841))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(841)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(841))), i)).id)
							{
								case 859: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1871) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1872))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1872));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1872))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1872)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1872))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1873))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1873));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1873))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1873)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1873))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(939))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(939))).id)
					{
						case 1874: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1877) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(939))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(939)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(939))), i)).id)
							{
								case 1874: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1878))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1878)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1878)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1878))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1878)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1878))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(857))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(857)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(857))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(857)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(857))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1879));
