...
namespace CppProject
{
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(816) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(789), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3874) });
				...
				if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
				{
					if (sVar(model_file) != null_)
						sVar(type_name) = minecraft_asset_get_name(/*"model"*/ STR(525), idVar(sVar(model_file), name));
				}
				else
					if (sVar(type) == e_tl_type_BLOCK)
					{
						sVar(type_name) = minecraft_asset_get_name(/*"block"*/ STR(518), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					}
			
			
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3875), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3876), idVar(sVar(temp), display_name) });
	...
	void tl_update_values(ScopeAny self)
	{
		...
		if (sVar(keyframe_transition) == /*"bezier"*/ STR(547))
