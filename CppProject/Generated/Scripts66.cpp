...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3834) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2967) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3835) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2967) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3836) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2967) + text) : /*""*/ STR(0));
		return text;
	}
