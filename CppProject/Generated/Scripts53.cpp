...
namespace CppProject
{
		...
		if (fileslist != null_)
		{
			...
			if (ds_list_size(fileslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(fileslist); i++)
					str += /*"  "*/ STR(1741) + filename_name(DsList(fileslist).Value(i)) + /*"\n"*/ STR(945);
				log({ str });
			}
		...
		surface_set_target(surf);
	{
			...
			if (file_exists_lib(fname))
			{
				...
				if (ds_map_valid(map))
				{
					...
					if (!is_undefined(animation))
					{
						...
						if (!is_undefined(DsMap(animation).Value(/*"frames"*/ STR(1770))))
							framelist = DsMap(animation).Value(/*"frames"*/ STR(1770));
					}
				}
			}
			if (tex != null_)
		...
		log({ /*"Block textures"*/ STR(2575), type, /*"done"*/ STR(1727) });
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(614), /*"_s"*/ STR(2572));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(603), /*"_n"*/ STR(2573));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2594), /*"done"*/ STR(1727) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2595), /*"surface"*/ STR(529) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2595), type, /*"done"*/ STR(1727) });
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		log({ /*"Model textures"*/ STR(2600), /*"done"*/ STR(1727) });
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		log({ /*"Particle textures"*/ STR(2607), /*"done"*/ STR(1727) });
