...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"block/"*/ STR(791), /*".png"*/ STR(10));
		...
		log({ /*"Block textures"*/ STR(2574), type, /*"load static"*/ STR(2575) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(831), /*""*/ STR(0));
			name = string_replace(name, /*" noalpha"*/ STR(832), /*""*/ STR(0));
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2576), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2574), type, /*"load animated"*/ STR(2577) });
		...
		{
			StringType name, fname;
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(831), /*""*/ STR(0));
			fname = global::load_assets_dir + mc_textures_directory + name + suffix + /*".png"*/ STR(10);
			if (!file_exists_lib(fname) && !is_undefined(DsMap(global::legacy_block_texture_name_map).Value(name)))
				fname = global::load_assets_dir + mc_textures_directory + DsMap(global::legacy_block_texture_name_map).Value(name) + suffix + /*".png"*/ STR(10);
			if (file_exists_lib(fname))
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2578), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		if (fileslist != null_)
		{
			...
			if (ds_list_size(fileslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(fileslist); i++)
					str += /*"  "*/ STR(1740) + filename_name(DsList(fileslist).Value(i)) + /*"\n"*/ STR(943);
				log({ str });
			}
		...
		log({ /*"Block textures, blocksize"*/ STR(2580), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2574), /*"static block surface"*/ STR(2581) });
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			if (tex != null_)
			{
				...
				if (string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(831)))
				...
				if (type == /*"normal"*/ STR(601) || string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" noalpha"*/ STR(832)))
				{
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2574), /*"static block preview"*/ STR(2582) });
		...
		log({ /*"Block textures"*/ STR(2574), type, /*"animated block surfaces"*/ STR(2583) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2574), type, /*"animated frames"*/ STR(2584) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			frametime = IntType(1);
			framelist = null_;
			opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(831));
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2585);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2586));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2587))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2587));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2588))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2588));
						if (!is_undefined(DsMap(animation).Value(/*"frames"*/ STR(1769))))
							framelist = DsMap(animation).Value(/*"frames"*/ STR(1769));
					}
				}
			}
			if (tex != null_)
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2574), /*"animated block preview"*/ STR(2589) });
		...
		if (type == /*"diffuse"*/ STR(607))
		{
			...
			log({ /*"Block textures"*/ STR(2574), /*"find static block depths"*/ STR(2590) });
			...
			for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
			{
				...
				RealType opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*"leaves"*/ STR(866));
				if (opaque > 0)
				{
			...
			log({ /*"Block textures"*/ STR(2574), /*"find animated block depths"*/ STR(2591) });
		...
		log({ /*"Block textures"*/ STR(2574), type, /*"done"*/ STR(1726) });
		debug_timer_stop(/*"Block textures: "*/ STR(2592) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(612), /*"_s"*/ STR(2571));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2572));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2593), /*"done"*/ STR(1726) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2442), /*".png"*/ STR(10));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2594), type, /*"load"*/ STR(2595) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2596), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2594), /*"surface"*/ STR(527) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2594), type, /*"done"*/ STR(1726) });
		debug_timer_stop(/*"Item textures: "*/ STR(2598) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2599), /*"load"*/ STR(2595) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2600);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2601);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2602))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2603), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2604))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2605))
		...
		log({ /*"Model textures"*/ STR(2599), /*"done"*/ STR(1726) });
		debug_timer_stop(/*"Model textures"*/ STR(2599));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2606), /*"load"*/ STR(2595) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2607), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2608)))
				{
		...
		log({ /*"Particle textures"*/ STR(2606), /*"legacy sheets"*/ STR(2609) });
		...
		log({ /*"Particle textures"*/ STR(2606), /*"done"*/ STR(1726) });
		debug_timer_stop(/*"Particle textures"*/ STR(2606));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2610);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2611) });
				sVar(pack_format) = e_minecraft_pack_LATEST;
				return false;
			}
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(536))))
			{
				VarType packmap = DsMap(map).Value(/*"pack"*/ STR(536));
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2612)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
