...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		log({ /*"Block textures"*/ STR(2573), type, /*"load static"*/ STR(2574) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2575), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2573), type, /*"load animated"*/ STR(2576) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2577), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		log({ /*"Block textures, blocksize"*/ STR(2579), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2573), /*"static block surface"*/ STR(2580) });
		surface_set_target(surf);
	{
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2573), /*"static block preview"*/ STR(2581) });
		...
		log({ /*"Block textures"*/ STR(2573), type, /*"animated block surfaces"*/ STR(2582) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2573), type, /*"animated frames"*/ STR(2583) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2584);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2585));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2586))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2586));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2587))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2587));
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2573), /*"animated block preview"*/ STR(2588) });
		...
		if (type == /*"diffuse"*/ STR(607))
		{
			...
			log({ /*"Block textures"*/ STR(2573), /*"find static block depths"*/ STR(2589) });
			...
			log({ /*"Block textures"*/ STR(2573), /*"find animated block depths"*/ STR(2590) });
		...
		log({ /*"Block textures"*/ STR(2573), type, /*"done"*/ STR(1725) });
		debug_timer_stop(/*"Block textures: "*/ STR(2591) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(612), /*"_s"*/ STR(2570));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2571));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2592), /*"done"*/ STR(1725) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2441), /*".png"*/ STR(10));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2593), type, /*"load"*/ STR(2594) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2595), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2593), /*"surface"*/ STR(527) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2593), type, /*"done"*/ STR(1725) });
		debug_timer_stop(/*"Item textures: "*/ STR(2597) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2598), /*"load"*/ STR(2594) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2599);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2600);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2601))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2602), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2603))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2604))
		...
		log({ /*"Model textures"*/ STR(2598), /*"done"*/ STR(1725) });
		debug_timer_stop(/*"Model textures"*/ STR(2598));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2605), /*"load"*/ STR(2594) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2606), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2607)))
				{
		...
		log({ /*"Particle textures"*/ STR(2605), /*"legacy sheets"*/ STR(2608) });
		...
		log({ /*"Particle textures"*/ STR(2605), /*"done"*/ STR(1725) });
		debug_timer_stop(/*"Particle textures"*/ STR(2605));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2609);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2610) });
			...
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(536))))
			{
				...
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2611)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
