/*
	...
	[ Generated on 2023.10.25 12:21:04 ]
*/
...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (location != /*"full"*/ STR(2753) && location != /*"top"*/ STR(194) && location != /*"bottom"*/ STR(193))
		{
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			...
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3919))
			{
				...
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2753);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3884))
			{
				...
				if (gmlGlobal::mouse_y < sReal(view_area_y) + sReal(view_area_height) * 0.3)
					mouselocation += /*"_top"*/ STR(1933);
				else
					if (gmlGlobal::mouse_y >= sReal(view_area_y) + sReal(view_area_height) * 0.7)
						mouselocation += /*"_bottom"*/ STR(1934);
				
				if (mouselocation == /*"_top"*/ STR(1933))
					mouselocation = /*"top"*/ STR(194);
				if (mouselocation == /*"_bottom"*/ STR(1934))
		...
		if (view == sInt(view_second))
		{
			...
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3877) || sVar(window_busy) == /*"viewresizever"*/ STR(3878) || sVar(window_busy) == /*"viewresizehor"*/ STR(3876)))
			{
				...
				if (location == /*"right_bottom"*/ STR(2754))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3876) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3877));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3878) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3877));
				}
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2753);
		self->location_last = /*"full"*/ STR(2753);
		self->surface = null_;
