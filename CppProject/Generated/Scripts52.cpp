...
namespace CppProject
{
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2548), IntType(0));
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2553) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2554) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2555);
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1275: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2563), /*"unzip"*/ STR(1275) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2564) });
						error(/*"errorunzippack"*/ STR(2565));
						withOne (app, global::_app->id, self->id)
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2566) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2563), /*"modeltextures"*/ STR(1357) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2567);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2568) });
			...
			case 2567: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2563), /*"blocktextures"*/ STR(2567) });
				...
				sStr(load_stage) = /*"itemtextures"*/ STR(2569);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2570) });
			...
			case 2569: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2563), /*"itemtextures"*/ STR(2569) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2571), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(521));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(607), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(612), /*"_s"*/ STR(2572));
				res_load_pack_item_textures(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2573));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2574) });
