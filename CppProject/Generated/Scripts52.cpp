...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		log({ /*"Block textures"*/ STR(2488), type, /*"load static"*/ STR(2489) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2490), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2488), type, /*"load animated"*/ STR(2491) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2492), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		if (fileslist != null_)
		{
			...
			if (ds_list_size(fileslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(fileslist); i++)
					str += /*"  "*/ STR(1712) + filename_name(DsList(fileslist).Value(i)) + /*"\n"*/ STR(938);
				log({ str });
			}
		...
		log({ /*"Block textures, blocksize"*/ STR(2494), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2488), /*"static block surface"*/ STR(2495) });
		surface_set_target(surf);
	{
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2488), /*"static block preview"*/ STR(2496) });
		...
		log({ /*"Block textures"*/ STR(2488), type, /*"animated block surfaces"*/ STR(2497) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2488), type, /*"animated frames"*/ STR(2498) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2499);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2500));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2501))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2501));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2502))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2502));
						if (!is_undefined(DsMap(animation).Value(/*"frames"*/ STR(1741))))
							framelist = DsMap(animation).Value(/*"frames"*/ STR(1741));
					}
				}
			}
			if (tex != null_)
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2488), /*"animated block preview"*/ STR(2503) });
		...
		if (type == /*"diffuse"*/ STR(606))
		{
			...
			log({ /*"Block textures"*/ STR(2488), /*"find static block depths"*/ STR(2504) });
			...
			for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
			{
				...
				RealType opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*"leaves"*/ STR(861));
				if (opaque > 0)
				{
			...
			log({ /*"Block textures"*/ STR(2488), /*"find animated block depths"*/ STR(2505) });
		...
		log({ /*"Block textures"*/ STR(2488), type, /*"done"*/ STR(1698) });
		debug_timer_stop(/*"Block textures: "*/ STR(2506) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(611), /*"_s"*/ STR(2485));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(600), /*"_n"*/ STR(2486));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2507), /*"done"*/ STR(1698) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2364), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2508), type, /*"load"*/ STR(2509) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2510), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2508), /*"surface"*/ STR(513) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2508), type, /*"done"*/ STR(1698) });
		debug_timer_stop(/*"Item textures: "*/ STR(2512) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2513), /*"load"*/ STR(2509) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2514);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2515);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2516))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2517), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2518))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2519))
		...
		log({ /*"Model textures"*/ STR(2513), /*"done"*/ STR(1698) });
		debug_timer_stop(/*"Model textures"*/ STR(2513));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2520), /*"load"*/ STR(2509) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2521), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2522)))
				{
		...
		log({ /*"Particle textures"*/ STR(2520), /*"legacy sheets"*/ STR(2523) });
		...
		log({ /*"Particle textures"*/ STR(2520), /*"done"*/ STR(1698) });
		debug_timer_stop(/*"Particle textures"*/ STR(2520));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2524);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2525) });
			...
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(523))))
			{
				...
				sVar(pack_format) = value_get_real({ DsMap(packmap).Value(/*"pack_format"*/ STR(2526)) });
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
