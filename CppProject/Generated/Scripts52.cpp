...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"block/"*/ STR(830), /*".png"*/ STR(6));
		...
		log({ /*"Block textures"*/ STR(2586), type, /*"load static"*/ STR(2587) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(870), /*""*/ STR(0));
			name = string_replace(name, /*" noalpha"*/ STR(871), /*""*/ STR(0));
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2588), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2586), type, /*"load animated"*/ STR(2589) });
		...
		{
			StringType name, fname;
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(870), /*""*/ STR(0));
			fname = global::load_assets_dir + mc_textures_directory + name + suffix + /*".png"*/ STR(6);
			if (!file_exists_lib(fname) && !is_undefined(DsMap(global::legacy_block_texture_name_map).Value(name)))
				fname = global::load_assets_dir + mc_textures_directory + DsMap(global::legacy_block_texture_name_map).Value(name) + suffix + /*".png"*/ STR(6);
			if (file_exists_lib(fname))
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2590), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		if (fileslist != null_)
		{
			...
			if (ds_list_size(fileslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(fileslist); i++)
					str += /*"  "*/ STR(1776) + filename_name(DsList(fileslist).Value(i)) + /*"\n"*/ STR(982);
				log({ str });
			}
		...
		log({ /*"Block textures, blocksize"*/ STR(2592), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2586), /*"static block surface"*/ STR(2593) });
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			if (tex != null_)
			{
				...
				if (string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(870)))
				...
				if (type == /*"normal"*/ STR(641) || string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" noalpha"*/ STR(871)))
				{
			...
			else
			{
				if (type == /*"diffuse"*/ STR(647))
				{
					...
					if (type == /*"material"*/ STR(652))
						draw_box(dx, dy, blocksize, blocksize, false, c_black, IntType(1));
					else
						if (type == /*"normal"*/ STR(641))
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2586), /*"static block preview"*/ STR(2594) });
		...
		log({ /*"Block textures"*/ STR(2586), type, /*"animated block surfaces"*/ STR(2595) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2586), type, /*"animated frames"*/ STR(2596) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			frametime = IntType(1);
			framelist = null_;
			opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(870));
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2597);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2598));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2599))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2599));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2600))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2600));
						if (!is_undefined(DsMap(animation).Value(/*"frames"*/ STR(1805))))
							framelist = DsMap(animation).Value(/*"frames"*/ STR(1805));
					}
				}
			}
			if (tex != null_)
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2586), /*"animated block preview"*/ STR(2601) });
		...
		if (type == /*"diffuse"*/ STR(647))
		{
			...
			log({ /*"Block textures"*/ STR(2586), /*"find static block depths"*/ STR(2602) });
			...
			for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
			{
				...
				RealType opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*"leaves"*/ STR(905));
				if (opaque > 0)
				{
			...
			log({ /*"Block textures"*/ STR(2586), /*"find animated block depths"*/ STR(2603) });
		...
		log({ /*"Block textures"*/ STR(2586), type, /*"done"*/ STR(1762) });
		debug_timer_stop(/*"Block textures: "*/ STR(2604) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		res_load_pack_block_sheet(self, /*"diffuse"*/ STR(647), /*""*/ STR(0));
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(652), /*"_s"*/ STR(2583));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(641), /*"_n"*/ STR(2584));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2605), /*"done"*/ STR(1762) });
	}
	
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		if (type == /*"diffuse"*/ STR(647) && sInt(item_sheet_texture) != null_)
			texture_free(sInt(item_sheet_texture));
		if (type == /*"material"*/ STR(652) && sInt(item_sheet_texture_material) != null_)
			texture_free(sInt(item_sheet_texture_material));
		if (type == /*"normal"*/ STR(641) && sInt(item_sheet_tex_normal) != null_)
		...
		fileslist = null_;
		debug_timer_start();
		if (dev_mode_debug_unused && type == /*"diffuse"*/ STR(647))
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2463), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2606), type, /*"load"*/ STR(2607) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2608), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2606), /*"surface"*/ STR(522) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			{
				if (type == /*"diffuse"*/ STR(647) && self->id != global::mc_res)
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2606), type, /*"done"*/ STR(1762) });
		debug_timer_stop(/*"Item textures: "*/ STR(2610) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2611), /*"load"*/ STR(2607) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2612);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2613);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2614))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2615), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2616))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2617))
		...
		log({ /*"Model textures"*/ STR(2611), /*"done"*/ STR(1762) });
		debug_timer_stop(/*"Model textures"*/ STR(2611));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2618), /*"load"*/ STR(2607) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2619), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2620)))
				{
		...
		log({ /*"Particle textures"*/ STR(2618), /*"legacy sheets"*/ STR(2621) });
		...
		log({ /*"Particle textures"*/ STR(2618), /*"done"*/ STR(1762) });
		debug_timer_stop(/*"Particle textures"*/ STR(2618));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2622);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2623) });
				sVar(pack_format) = e_minecraft_pack_LATEST;
				return false;
			}
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(532))))
			{
				VarType packmap = DsMap(map).Value(/*"pack"*/ STR(532));
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2624)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
