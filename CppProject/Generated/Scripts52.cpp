...
namespace CppProject
{
		...
		surface_set_target(surf);
	{
		...
		log({ /*"Block textures"*/ STR(2493), type, /*"done"*/ STR(1700) });
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(611), /*"_s"*/ STR(2490));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(600), /*"_n"*/ STR(2491));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2512), /*"done"*/ STR(1700) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2369), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2513), /*"surface"*/ STR(513) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2513), type, /*"done"*/ STR(1700) });
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		log({ /*"Model textures"*/ STR(2518), /*"done"*/ STR(1700) });
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		log({ /*"Particle textures"*/ STR(2525), /*"done"*/ STR(1700) });
