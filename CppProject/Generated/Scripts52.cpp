...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"block/"*/ STR(832), /*".png"*/ STR(6));
		...
		log({ /*"Block textures"*/ STR(2600), type, /*"load static"*/ STR(2601) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(872), /*""*/ STR(0));
			name = string_replace(name, /*" noalpha"*/ STR(873), /*""*/ STR(0));
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2602), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2600), type, /*"load animated"*/ STR(2603) });
		...
		{
			StringType name, fname;
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(872), /*""*/ STR(0));
			fname = global::load_assets_dir + mc_textures_directory + name + suffix + /*".png"*/ STR(6);
			if (!file_exists_lib(fname) && !is_undefined(DsMap(global::legacy_block_texture_name_map).Value(name)))
				fname = global::load_assets_dir + mc_textures_directory + DsMap(global::legacy_block_texture_name_map).Value(name) + suffix + /*".png"*/ STR(6);
			if (file_exists_lib(fname))
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2604), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		log({ /*"Block textures, blocksize"*/ STR(2606), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2600), /*"static block surface"*/ STR(2607) });
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			if (tex != null_)
			{
				...
				if (string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(872)))
				...
				if (type == /*"normal"*/ STR(643) || string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" noalpha"*/ STR(873)))
				{
			...
			else
			{
				if (type == /*"diffuse"*/ STR(649))
				{
					...
					if (type == /*"material"*/ STR(654))
						draw_box(dx, dy, blocksize, blocksize, false, c_black, IntType(1));
					else
						if (type == /*"normal"*/ STR(643))
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2600), /*"static block preview"*/ STR(2608) });
		...
		log({ /*"Block textures"*/ STR(2600), type, /*"animated block surfaces"*/ STR(2609) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2600), type, /*"animated frames"*/ STR(2610) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			frametime = IntType(1);
			framelist = null_;
			opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(872));
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2611);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2612));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2613))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2613));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2614))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2614));
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2600), /*"animated block preview"*/ STR(2615) });
		...
		if (type == /*"diffuse"*/ STR(649))
		{
			...
			log({ /*"Block textures"*/ STR(2600), /*"find static block depths"*/ STR(2616) });
			...
			for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
			{
				...
				RealType opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*"leaves"*/ STR(907));
				if (opaque > 0)
				{
			...
			log({ /*"Block textures"*/ STR(2600), /*"find animated block depths"*/ STR(2617) });
		...
		log({ /*"Block textures"*/ STR(2600), type, /*"done"*/ STR(1767) });
		debug_timer_stop(/*"Block textures: "*/ STR(2618) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		res_load_pack_block_sheet(self, /*"diffuse"*/ STR(649), /*""*/ STR(0));
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(654), /*"_s"*/ STR(2597));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(643), /*"_n"*/ STR(2598));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2619), /*"done"*/ STR(1767) });
	}
	
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		if (type == /*"diffuse"*/ STR(649) && sInt(item_sheet_texture) != null_)
			texture_free(sInt(item_sheet_texture));
		if (type == /*"material"*/ STR(654) && sInt(item_sheet_texture_material) != null_)
			texture_free(sInt(item_sheet_texture_material));
		if (type == /*"normal"*/ STR(643) && sInt(item_sheet_tex_normal) != null_)
		...
		fileslist = null_;
		debug_timer_start();
		if (dev_mode_debug_unused && type == /*"diffuse"*/ STR(649))
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2471), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2620), type, /*"load"*/ STR(2621) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2622), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2620), /*"surface"*/ STR(524) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			{
				if (type == /*"diffuse"*/ STR(649) && self->id != global::mc_res)
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2620), type, /*"done"*/ STR(1767) });
		debug_timer_stop(/*"Item textures: "*/ STR(2624) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2625), /*"load"*/ STR(2621) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2626);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2627);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2628))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2629), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2630))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2631))
		...
		log({ /*"Model textures"*/ STR(2625), /*"done"*/ STR(1767) });
		debug_timer_stop(/*"Model textures"*/ STR(2625));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2632), /*"load"*/ STR(2621) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2633), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2634)))
				{
		...
		log({ /*"Particle textures"*/ STR(2632), /*"legacy sheets"*/ STR(2635) });
		...
		log({ /*"Particle textures"*/ STR(2632), /*"done"*/ STR(1767) });
		debug_timer_stop(/*"Particle textures"*/ STR(2632));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2636);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2637) });
				sVar(pack_format) = e_minecraft_pack_LATEST;
				return false;
			}
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(534))))
			{
				VarType packmap = DsMap(map).Value(/*"pack"*/ STR(534));
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2638)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
