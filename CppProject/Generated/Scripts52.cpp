...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"block/"*/ STR(827), /*".png"*/ STR(6));
		...
		log({ /*"Block textures"*/ STR(2544), type, /*"load static"*/ STR(2545) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(867), /*""*/ STR(0));
			name = string_replace(name, /*" noalpha"*/ STR(868), /*""*/ STR(0));
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2546), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2544), type, /*"load animated"*/ STR(2547) });
		...
		{
			StringType name, fname;
			name = string_replace(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(867), /*""*/ STR(0));
			fname = global::load_assets_dir + mc_textures_directory + name + suffix + /*".png"*/ STR(6);
			if (!file_exists_lib(fname) && !is_undefined(DsMap(global::legacy_block_texture_name_map).Value(name)))
				fname = global::load_assets_dir + mc_textures_directory + DsMap(global::legacy_block_texture_name_map).Value(name) + suffix + /*".png"*/ STR(6);
			if (file_exists_lib(fname))
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2548), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		log({ /*"Block textures, blocksize"*/ STR(2550), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2544), /*"static block surface"*/ STR(2551) });
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			if (tex != null_)
			{
				...
				if (string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" opaque"*/ STR(867)))
				...
				if (type == /*"normal"*/ STR(640) || string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*" noalpha"*/ STR(868)))
				{
			...
			else
			{
				if (type == /*"diffuse"*/ STR(646))
				{
					...
					if (type == /*"material"*/ STR(651))
						draw_box(dx, dy, blocksize, blocksize, false, c_black, IntType(1));
					else
						if (type == /*"normal"*/ STR(640))
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2544), /*"static block preview"*/ STR(2552) });
		...
		log({ /*"Block textures"*/ STR(2544), type, /*"animated block surfaces"*/ STR(2553) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2544), type, /*"animated frames"*/ STR(2554) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			frametime = IntType(1);
			framelist = null_;
			opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t), /*" opaque"*/ STR(867));
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2555);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2556));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2557))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2557));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2558))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2558));
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2544), /*"animated block preview"*/ STR(2559) });
		...
		if (type == /*"diffuse"*/ STR(646))
		{
			...
			log({ /*"Block textures"*/ STR(2544), /*"find static block depths"*/ STR(2560) });
			...
			for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
			{
				...
				RealType opaque = string_contains(DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t), /*"leaves"*/ STR(902));
				if (opaque > 0)
				{
			...
			log({ /*"Block textures"*/ STR(2544), /*"find animated block depths"*/ STR(2561) });
		...
		log({ /*"Block textures"*/ STR(2544), type, /*"done"*/ STR(1747) });
		debug_timer_stop(/*"Block textures: "*/ STR(2562) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		res_load_pack_block_sheet(self, /*"diffuse"*/ STR(646), /*""*/ STR(0));
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(651), /*"_s"*/ STR(2541));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(640), /*"_n"*/ STR(2542));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2563), /*"done"*/ STR(1747) });
	}
	
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		if (type == /*"diffuse"*/ STR(646) && sInt(item_sheet_texture) != null_)
			texture_free(sInt(item_sheet_texture));
		if (type == /*"material"*/ STR(651) && sInt(item_sheet_texture_material) != null_)
			texture_free(sInt(item_sheet_texture_material));
		if (type == /*"normal"*/ STR(640) && sInt(item_sheet_tex_normal) != null_)
		...
		fileslist = null_;
		debug_timer_start();
		if (dev_mode_debug_unused && type == /*"diffuse"*/ STR(646))
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2421), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		log({ /*"Item textures"*/ STR(2564), type, /*"load"*/ STR(2565) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2566), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2564), /*"surface"*/ STR(521) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texlist); t++)
		{
			...
			{
				if (type == /*"diffuse"*/ STR(646) && self->id != global::mc_res)
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2564), type, /*"done"*/ STR(1747) });
		debug_timer_stop(/*"Item textures: "*/ STR(2568) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2569), /*"load"*/ STR(2565) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2570);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2571);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2572))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2573), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2574))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2575))
		...
		log({ /*"Model textures"*/ STR(2569), /*"done"*/ STR(1747) });
		debug_timer_stop(/*"Model textures"*/ STR(2569));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2576), /*"load"*/ STR(2565) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2577), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2578)))
				{
		...
		log({ /*"Particle textures"*/ STR(2576), /*"legacy sheets"*/ STR(2579) });
		...
		log({ /*"Particle textures"*/ STR(2576), /*"done"*/ STR(1747) });
		debug_timer_stop(/*"Particle textures"*/ STR(2576));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2580);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2581) });
				sVar(pack_format) = e_minecraft_pack_LATEST;
				return false;
			}
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(531))))
			{
				VarType packmap = DsMap(map).Value(/*"pack"*/ STR(531));
				sVar(pack_format) = value_get_real(DsMap(packmap).Value(/*"pack_format"*/ STR(2582)));
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
