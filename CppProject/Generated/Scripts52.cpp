...
namespace CppProject
{
		...
		if (fileslist != null_)
		{
			...
			if (ds_list_size(fileslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(fileslist); i++)
					str += /*"  "*/ STR(1715) + filename_name(DsList(fileslist).Value(i)) + /*"\n"*/ STR(940);
				log({ str });
			}
		...
		surface_set_target(surf);
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				if (ds_map_valid(map))
				{
					...
					if (!is_undefined(animation))
					{
						...
						if (!is_undefined(DsMap(animation).Value(/*"frames"*/ STR(1744))))
							framelist = DsMap(animation).Value(/*"frames"*/ STR(1744));
					}
				}
			}
			if (tex != null_)
			...
			{
				RealType aniprogress = roundto(frac(f / (block_sheet_ani_frames / aniloops)), 1.0 / block_sheet_ani_frames);
		...
		log({ /*"Block textures"*/ STR(2495), type, /*"done"*/ STR(1701) });
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		log({ /*"Block textures all"*/ STR(2514), /*"done"*/ STR(1701) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		{
			ArrType filesarr = file_find(global::load_assets_dir + mc_textures_directory + /*"item/"*/ STR(2371), /*".png"*/ STR(6));
			fileslist = ds_list_create();
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(filesarr)); i++)
				ds_list_add({ fileslist, filesarr.Value(i) });
		}
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2515), type, /*"done"*/ STR(1701) });
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		log({ /*"Model textures"*/ STR(2520), /*"done"*/ STR(1701) });
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		log({ /*"Particle textures"*/ STR(2527), /*"done"*/ STR(1701) });
