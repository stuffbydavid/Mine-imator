...
namespace CppProject
{
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2546), IntType(0));
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2551) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2552) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2553);
			...
			case e_res_type_TEXTURE:
			{
				if (sVar(texture) > 0)
					texture_free(sVar(texture));
				sVar(texture) = null_;
				if (filename_ext(fn) == /*".dat"*/ STR(710))
				{
					IntType map = new_minecraft_map(fn);
					if (!(map > 0))
					{
						error(/*"errorloadmap"*/ STR(2554));
						break;
					}
					else
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1275: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2561), /*"unzip"*/ STR(1275) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2562) });
						error(/*"errorunzippack"*/ STR(2563));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1357);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2564) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2561), /*"modeltextures"*/ STR(1357) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2565);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2566) });
			...
			case 2565: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2561), /*"blocktextures"*/ STR(2565) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(994), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(4));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2567);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2568) });
			...
			case 2567: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2561), /*"itemtextures"*/ STR(2567) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2569), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(521));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(607), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(612), /*"_s"*/ STR(2570));
				res_load_pack_item_textures(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2571));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2572) });
