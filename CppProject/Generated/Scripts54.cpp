...
namespace CppProject
{
	RealType res_load_scenery(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 894: // open
			{
				debug({ /*"res_load_scenery"*/ STR(2613), /*"open"*/ STR(894) });
				if (sVar(type) == e_res_type_FROM_WORLD)
				{
					if (!res_load_scenery_world(self))
					{
						error(/*"errorloadworld"*/ STR(2614));
				...
				else
				{
					...
					if (ext == /*".schematic"*/ STR(692) || ext == /*".nbt"*/ STR(693))
					{
						...
						if (rootmap == null_)
							break;
						debug_timer_stop(/*"res_load_scenery, Parse NBT"*/ STR(2615));
						if (dev_mode_debug_schematics)
							nbt_debug_tag_compound(/*"root"*/ STR(2173), rootmap);
						if (ext == /*".schematic"*/ STR(692))
						{
							withOne (obj_builder, global::mc_builder, self->id)
							{
								openerr = !builder_read_schematic(self, DsMap(rootmap).Value(/*"Schematic"*/ STR(2616)));
								if (openerr)
									break;
					...
					{
						log({ /*"Loading .blocks"*/ STR(2617), fname });
				...
				debug_timer_start();
				openerr = false;
				sStr(load_stage) = /*"download"*/ STR(108);
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryblocks"*/ STR(2618) });
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaptionpieceof"*/ STR(2619), ObjType(obj_builder, global::mc_builder)->file_map });
					else
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaption"*/ STR(2620), idVar(self.otherId, filename) });
				...
				if (sVar(scenery_tl_add) == null_)
				{
					if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(500))
						...
						if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(20))
							sVar(scenery_tl_add) = question(text_get({ /*"loadsceneryaddtimelines"*/ STR(2621), ObjType(obj_builder, global::mc_builder)->sch_timeline_amount }));
						else
			...
			case 994: // blocks
			{
				...
				{
					debug_timer_stop(/*"res_load_scenery, Set models"*/ STR(2622));
					debug_timer_start();
					...
					ObjType(obj_builder, global::mc_builder)->build_pos = IntType(0);
					withOne (app, global::_app->id, self->id)
						ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerymodel"*/ STR(2623) });
			...
			case 8: // model
			{
				...
				if (ObjType(obj_builder, global::mc_builder)->build_pos == ObjType(obj_builder, global::mc_builder)->build_size_total)
				{
					...
					debug_timer_stop(/*"res_load_scenery, Generate models"*/ STR(2624));
					...
					sBool(ready) = true;
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						sVar(display_name) = text_get({ /*"loadscenerypieceof"*/ STR(2625), ObjType(obj_builder, global::mc_builder)->file_map });
					res_save_block_cache(self, global::_app->project_folder + /*"/"*/ STR(20) + sVar(filename) + /*".meshcache"*/ STR(2554));
		...
		if (openerr)
		{
			error(/*"errorloadschematic"*/ STR(2626));
			buffer_delete(global::buffer_current);
			withOne (app, global::_app->id, self->id)
				load_next(ScopeAny(self));
	...
	RealType res_load_skulls(ScopeAny self)
	{
		...
		ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerydownload"*/ STR(2627), ObjType(obj_builder, global::mc_builder)->block_skull_finish_count, ObjType(obj_builder, global::mc_builder)->block_skull_texture_count, ObjType(obj_builder, global::mc_builder)->block_skull_fail_count });
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			if (self->block_skull_texture != null_ || self->block_skull_texture_fail == true || (current_time() - self->block_skull_download_time > IntType(3000)))
			{
				...
				else
				{
					...
					log({ /*"Failed to download texture"*/ STR(2628), self->block_skull_texture_name });
	...
	void res_load_start(ScopeAny self)
	{
		switch ((IntType)sVar(type))
		{
			...
			{
				sStr(load_stage) = /*"open"*/ STR(894);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryopen"*/ STR(2629) });
			...
			case e_res_type_SOUND:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadaudioread"*/ STR(2630) });
			...
			{
				sStr(load_stage) = /*"unzip"*/ STR(1275);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackunzip"*/ STR(2631) });
					ObjType(obj_popup, global::_app->popup_loading)->caption = idVar(self.otherId, filename);
		...
		surface_set_target(surf);
	{
		...
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/stone"*/ STR(2632)), IntType(0), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/grass_side"*/ STR(2633)), IntType(16), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/tnt_side"*/ STR(2634)), IntType(0), IntType(16), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/sapling_oak"*/ STR(2635)), IntType(16), IntType(16), size, size, block_sheet_width, block_sheet_height);
	}
	...
	RealType res_update_colors(ScopeAny self, VarArgs argument)
	{
		...
		if (ObjType(obj_biome, biome)->name == /*"custom"*/ STR(600))
		{
		...
		else
		{
			...
			sVar(color_leaves_spruce) = hex_to_color(/*"62A857"*/ STR(2636));
	...
	void res_update_display_name(Scope<obj_resource> self)
	{
		...
		if (self->type == e_res_type_DOWNLOADED_SKIN)
			self->display_name = text_get({ /*"downloadskinname"*/ STR(2637), self->display_name });
	...
	VarType rotation_get_time(VarType rotation)
	{
		...
		str = (day != IntType(0) ? string(day) + /*":"*/ STR(758) : /*""*/ STR(0));
		str += ((day != IntType(0) && hours < IntType(10)) ? /*"0"*/ STR(1048) : /*""*/ STR(0)) + string(hours) + /*":"*/ STR(758);
	...
	RealType scrollbar_draw(ScopeAny self, IntType sb, IntType dir, VarType xx, VarType yy, VarType size, VarType maxsize)
	{
		...
		if (dir == e_scroll_HORIZONTAL)
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx, yy - nearsize, size, areasize + nearsize) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2638));
		else
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx - nearsize, yy, areasize + nearsize, size) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2638));
		...
		if (mouseinarea)
		{
			...
			if (!sBool(mouse_left_pressed) && sBool(mouse_left) && !mouseinbar && ((dir > 0) ? (gmlGlobal::mouse_x < (xx + barpos) || gmlGlobal::mouse_x > (xx + barpos + barsize)) : (gmlGlobal::mouse_y < (yy + barpos) || gmlGlobal::mouse_y > (yy + barpos + barsize))))
			{
				...
				if (sBool(mouse_left) && mouseinbar)
				{
					...
					sVar(window_busy) = /*"scrollbar"*/ STR(2638);
				}
		...
		{
			if (sVar(window_busy) == /*"scrollbar"*/ STR(2638))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if (!sBool(mouse_left))
				{
		...
		if (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2638))
			idVar(sb, value_goal) = idVar(sb, value);
		barpos = min({ size - barsize, floor(idVar(sb, value) * ((RealType)size / maxsize)) });
		pressed = (sVar(window_busy) == /*"scrollbar"*/ STR(2638) && sVar(window_focus) == string(sb));
		if (dir == e_scroll_HORIZONTAL)
		{
	...
	RealType settings_load(ScopeAny self)
	{
		...
		{
			log({ /*"Loading settings"*/ STR(2639), fn });
			...
			if (ds_map_valid(assetsmap))
			{
				...
				VarType newmap = DsMap(assetsmap).Value(/*"new"*/ STR(2640));
			...
			VarType toastslist = DsMap(map).Value(/*"closed_toasts"*/ STR(2641));
			if (ds_list_valid(toastslist))
				ds_list_copy(sInt(closed_toast_list), (IntType)(toastslist));
			VarType programmap = DsMap(map).Value(/*"program"*/ STR(2642));
			if (ds_map_valid(programmap))
			{
				sReal(setting_advanced_mode) = value_get_real(DsMap(programmap).Value(/*"advanced_mode"*/ STR(2643)), sReal(setting_advanced_mode));
				gmlGlobal::room_speed = value_get_real(DsMap(programmap).Value(/*"fps"*/ STR(2644)), gmlGlobal::room_speed);
				if (!dev_mode)
					sStr(setting_project_folder) = value_get_string(DsMap(programmap).Value(/*"project_folder"*/ STR(2645)), sStr(setting_project_folder)).Str();
				if (!directory_exists_lib(sStr(setting_project_folder)))
					sStr(setting_project_folder) = projects_directory;
				sVar(setting_backup) = value_get_real(DsMap(programmap).Value(/*"backup"*/ STR(118)), sVar(setting_backup));
				sReal(setting_backup_time) = value_get_real(DsMap(programmap).Value(/*"backup_time"*/ STR(2646)), sReal(setting_backup_time));
				sReal(setting_backup_amount) = value_get_real(DsMap(programmap).Value(/*"backup_amount"*/ STR(2647)), sReal(setting_backup_amount));
				sVar(setting_spawn_cameras) = value_get_real(DsMap(programmap).Value(/*"spawn_cameras"*/ STR(2648)), sVar(setting_spawn_cameras));
				sReal(setting_unlimited_values) = value_get_real(DsMap(programmap).Value(/*"unlimited_values"*/ STR(2649)), sReal(setting_unlimited_values));
				sVar(setting_watermark_custom) = value_get_real(DsMap(programmap).Value(/*"watermark_custom"*/ STR(2650)), sVar(setting_watermark_custom));
				sStr(setting_watermark_fn) = value_get_string(DsMap(programmap).Value(/*"watermark_fn"*/ STR(2651)), sStr(setting_watermark_fn)).Str();
				sVar(setting_watermark_halign) = value_get_string(DsMap(programmap).Value(/*"watermark_halign"*/ STR(2652)), sVar(setting_watermark_halign));
				sVar(setting_watermark_valign) = value_get_string(DsMap(programmap).Value(/*"watermark_valign"*/ STR(2653)), sVar(setting_watermark_valign));
				sReal(setting_watermark_padding) = value_get_real(DsMap(programmap).Value(/*"watermark_padding"*/ STR(2654)), sReal(setting_watermark_padding));
				sReal(setting_watermark_scale) = value_get_real(DsMap(programmap).Value(/*"watermark_scale"*/ STR(2655)), sReal(setting_watermark_scale));
				sReal(setting_watermark_opacity) = value_get_real(DsMap(programmap).Value(/*"watermark_opacity"*/ STR(2656)), sReal(setting_watermark_opacity));
				if (sStr(setting_watermark_fn) != /*""*/ STR(0))
					sInt(setting_watermark_image) = texture_create(sStr(setting_watermark_fn));
			}
			VarType interfacemap = DsMap(map).Value(/*"interface"*/ STR(2657));
			if (ds_map_valid(interfacemap))
			{
				sStr(setting_language_filename) = value_get_string(DsMap(interfacemap).Value(/*"language_filename"*/ STR(2658)), sStr(setting_language_filename)).Str();
				...
				themename = value_get_string(DsMap(interfacemap).Value(/*"theme"*/ STR(2659)), themename);
				...
				sInt(setting_accent) = value_get_real(DsMap(interfacemap).Value(/*"accent"*/ STR(2660)), sInt(setting_accent));
				sVar(setting_accent_custom) = value_get_color(DsMap(interfacemap).Value(/*"accent_custom"*/ STR(2661)), sVar(setting_accent_custom));
				sVar(setting_timeline_autoscroll) = value_get_real(DsMap(interfacemap).Value(/*"timeline_autoscroll"*/ STR(2662)), sVar(setting_timeline_autoscroll));
				sBool(setting_timeline_show_markers) = value_get_real(DsMap(interfacemap).Value(/*"timeline_show_markers"*/ STR(2663)), sBool(setting_timeline_show_markers));
				sVar(setting_interface_compact) = value_get_real(DsMap(interfacemap).Value(/*"interface_compact"*/ STR(2664)), sVar(setting_interface_compact));
				sReal(setting_timeline_compact) = value_get_real(DsMap(interfacemap).Value(/*"timeline_compact"*/ STR(2665)), sReal(setting_timeline_compact));
				sVar(setting_reduced_motion) = value_get_real(DsMap(interfacemap).Value(/*"reduced_motion"*/ STR(2666)), sVar(setting_reduced_motion));
				sVar(setting_timeline_select_jump) = value_get_real(DsMap(interfacemap).Value(/*"timeline_select_jump"*/ STR(2667)), sVar(setting_timeline_select_jump));
				sVar(setting_timeline_hide_ghosts) = value_get_real(DsMap(interfacemap).Value(/*"timeline_hide_ghosts"*/ STR(2668)), sVar(setting_timeline_hide_ghosts));
				sVar(setting_timeline_frame_snap) = value_get_real(DsMap(interfacemap).Value(/*"timeline_frame_snap"*/ STR(2669)), sVar(setting_timeline_frame_snap));
				sReal(setting_z_is_up) = value_get_real(DsMap(interfacemap).Value(/*"z_is_up"*/ STR(2670)), sReal(setting_z_is_up));
				sBool(setting_show_shortcuts_bar) = value_get_real(DsMap(interfacemap).Value(/*"show_shortcuts_bar"*/ STR(2671)), sBool(setting_show_shortcuts_bar));
				sVar(setting_gizmos_face_camera) = value_get_real(DsMap(interfacemap).Value(/*"gizmos_face_camera"*/ STR(2672)), sVar(setting_gizmos_face_camera));
				sVar(setting_fade_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"fade_gizmos"*/ STR(2673)), sVar(setting_fade_gizmos));
				sVar(setting_camera_lock_mouse) = value_get_real(DsMap(interfacemap).Value(/*"camera_lock_mouse"*/ STR(2674)), sVar(setting_camera_lock_mouse));
				window_mouse_set_permission(sVar(setting_camera_lock_mouse));
				sVar(setting_place_new) = value_get_real(DsMap(interfacemap).Value(/*"place_new"*/ STR(2675)), sVar(setting_place_new));
				sVar(setting_interface_scale_auto) = value_get_real(DsMap(interfacemap).Value(/*"scale_auto"*/ STR(2676)), sVar(setting_interface_scale_auto));
				if (sVar(setting_interface_scale_auto) > 0)
					sVar(setting_interface_scale) = interface_scale_default_get();
				else
					sVar(setting_interface_scale) = value_get_real(DsMap(interfacemap).Value(/*"scale"*/ STR(859)), sVar(setting_interface_scale));
				
				interface_scale_set(sVar(setting_interface_scale));
				sVar(setting_separate_tool_modes) = value_get_real(DsMap(interfacemap).Value(/*"separate_tool_modes"*/ STR(2677)), sVar(setting_separate_tool_modes));
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
					sBool(setting_tool_select) = true;
				}
				sVar(setting_panel_left_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_bottom_size"*/ STR(2678)), sVar(setting_panel_left_bottom_size));
				sVar(setting_panel_right_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_bottom_size"*/ STR(2679)), sVar(setting_panel_right_bottom_size));
				sVar(setting_panel_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_bottom_size"*/ STR(2680)), sVar(setting_panel_bottom_size));
				sVar(setting_panel_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_top_size"*/ STR(2681)), sVar(setting_panel_top_size));
				sVar(setting_panel_left_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_top_size"*/ STR(2682)), sVar(setting_panel_left_top_size));
				sVar(setting_panel_right_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_top_size"*/ STR(2683)), sVar(setting_panel_right_top_size));
				sVar(setting_properties_location) = value_get_string(DsMap(interfacemap).Value(/*"properties_location"*/ STR(2684)), sVar(setting_properties_location));
				sVar(setting_ground_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"ground_editor_location"*/ STR(2685)), sVar(setting_ground_editor_location));
				sVar(setting_template_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"template_editor_location"*/ STR(2686)), sVar(setting_template_editor_location));
				sVar(setting_timeline_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"timeline_editor_location"*/ STR(2687)), sVar(setting_timeline_editor_location));
				sVar(setting_frame_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"frame_editor_location"*/ STR(2688)), sVar(setting_frame_editor_location));
				sVar(setting_settings_location) = value_get_string(DsMap(interfacemap).Value(/*"settings_location"*/ STR(2689)), sVar(setting_settings_location));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2690))))
					window_state_restore(e_window_TIMELINE, DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2690)));
				sVar(setting_view_split) = value_get_real(DsMap(interfacemap).Value(/*"view_split"*/ STR(2691)), sVar(setting_view_split));
				sVar(setting_view_main_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_main_overlays"*/ STR(2692)), sVar(setting_view_main_overlays));
				sVar(setting_view_main_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_main_aspect_ratio"*/ STR(2693)), sVar(setting_view_main_aspect_ratio));
				sVar(setting_view_main_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_main_grid"*/ STR(2694)), sVar(setting_view_main_grid));
				sVar(setting_view_main_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_main_gizmos"*/ STR(2695)), sVar(setting_view_main_gizmos));
				sVar(setting_view_main_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_main_fog"*/ STR(2696)), sVar(setting_view_main_fog));
				sVar(setting_view_main_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_main_effects"*/ STR(2697)), sVar(setting_view_main_effects));
				sVar(setting_view_main_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_main_particles"*/ STR(2698)), sVar(setting_view_main_particles));
				sVar(setting_view_main_location) = value_get_string(DsMap(interfacemap).Value(/*"view_main_location"*/ STR(2699)), sVar(setting_view_main_location));
				sVar(setting_view_second_show) = value_get_real(DsMap(interfacemap).Value(/*"view_second_show"*/ STR(2700)), sVar(setting_view_second_show));
				sVar(setting_view_second_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_second_overlays"*/ STR(2701)), sVar(setting_view_second_overlays));
				sVar(setting_view_second_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_second_aspect_ratio"*/ STR(2702)), sVar(setting_view_second_aspect_ratio));
				sVar(setting_view_second_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_second_grid"*/ STR(2703)), sVar(setting_view_second_grid));
				sVar(setting_view_second_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_second_gizmos"*/ STR(2704)), sVar(setting_view_second_gizmos));
				sVar(setting_view_second_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_second_fog"*/ STR(2705)), sVar(setting_view_second_fog));
				sVar(setting_view_second_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_second_effects"*/ STR(2706)), sVar(setting_view_second_effects));
				sVar(setting_view_second_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_second_particles"*/ STR(2707)), sVar(setting_view_second_particles));
				sVar(setting_view_second_location) = value_get_string(DsMap(interfacemap).Value(/*"view_second_location"*/ STR(2708)), sVar(setting_view_second_location));
				sVar(setting_view_second_width) = value_get_real(DsMap(interfacemap).Value(/*"view_second_width"*/ STR(2709)), sVar(setting_view_second_width));
				sVar(setting_view_second_height) = value_get_real(DsMap(interfacemap).Value(/*"view_second_height"*/ STR(2710)), sVar(setting_view_second_height));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2711))))
					window_state_restore(e_window_VIEW_SECOND, DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2711)));
				sReal(setting_snap) = value_get_real(DsMap(interfacemap).Value(/*"snap"*/ STR(296)), sReal(setting_snap));
				sVar(setting_snap_absolute) = value_get_real(DsMap(interfacemap).Value(/*"snap_absolute"*/ STR(2712)), sVar(setting_snap_absolute));
				sReal(setting_snap_size_position) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_position"*/ STR(2713)), sReal(setting_snap_size_position));
				sReal(setting_snap_size_rotation) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_rotation"*/ STR(2714)), sReal(setting_snap_size_rotation));
				sReal(setting_snap_size_scale) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_scale"*/ STR(2715)), sReal(setting_snap_size_scale));
				sVar(setting_modelbench_popup_hidden) = value_get_real(DsMap(interfacemap).Value(/*"modelbench_popup_hidden"*/ STR(2716)), sVar(setting_modelbench_popup_hidden));
			}
			VarType controlsmap = DsMap(map).Value(/*"controls"*/ STR(2717));
			if (ds_map_valid(controlsmap))
			{
				...
				sReal(setting_move_speed) = value_get_real(DsMap(controlsmap).Value(/*"move_speed"*/ STR(2718)), sReal(setting_move_speed));
				sReal(setting_look_sensitivity) = value_get_real(DsMap(controlsmap).Value(/*"look_sensitivity"*/ STR(2719)), sReal(setting_look_sensitivity));
				sReal(setting_fast_modifier) = value_get_real(DsMap(controlsmap).Value(/*"fast_modifier"*/ STR(2720)), sReal(setting_fast_modifier));
				sReal(setting_slow_modifier) = value_get_real(DsMap(controlsmap).Value(/*"slow_modifier"*/ STR(2721)), sReal(setting_slow_modifier));
				keybinds_update_match();
			}
			VarType exportmap = DsMap(map).Value(/*"export"*/ STR(2722));
			if (ds_map_valid(exportmap))
			{
				sVar(setting_export_movie_format) = value_get_string(DsMap(map).Value(/*"exportmovie_format"*/ STR(2723)), sVar(setting_export_movie_format));
				sVar(setting_export_movie_frame_rate) = value_get_real(DsMap(map).Value(/*"exportmovie_frame_rate"*/ STR(2724)), sVar(setting_export_movie_frame_rate));
				sVar(setting_export_movie_framespersecond) = value_get_real(DsMap(map).Value(/*"exportmovie_framespersecond"*/ STR(2725)), sVar(setting_export_movie_framespersecond));
				sVar(setting_export_movie_bit_rate) = value_get_real(DsMap(map).Value(/*"exportmovie_bit_rate"*/ STR(2726)), sVar(setting_export_movie_bit_rate));
				sVar(setting_export_movie_include_audio) = value_get_real(DsMap(map).Value(/*"exportmovie_include_audio"*/ STR(2727)), sVar(setting_export_movie_include_audio));
				sVar(setting_export_movie_remove_background) = value_get_real(DsMap(map).Value(/*"exportmovie_remove_background"*/ STR(2728)), sVar(setting_export_movie_remove_background));
				sVar(setting_export_movie_include_hidden) = value_get_real(DsMap(map).Value(/*"exportmovie_remove_background"*/ STR(2728)), sVar(setting_export_movie_include_hidden));
				sVar(setting_export_movie_high_quality) = value_get_real(DsMap(map).Value(/*"exportmovie_high_quality"*/ STR(2729)), sVar(setting_export_movie_high_quality));
				sVar(setting_export_movie_watermark) = value_get_real(DsMap(map).Value(/*"exportmovie_watermark"*/ STR(2730)), sVar(setting_export_movie_watermark));
				sVar(setting_export_image_remove_background) = value_get_real(DsMap(map).Value(/*"exportimage_remove_background"*/ STR(2731)), sVar(setting_export_image_remove_background));
				sVar(setting_export_image_include_hidden) = value_get_real(DsMap(map).Value(/*"exportimage_include_hidden"*/ STR(2732)), sVar(setting_export_image_include_hidden));
				sVar(setting_export_image_high_quality) = value_get_real(DsMap(map).Value(/*"exportimage_high_quality"*/ STR(2733)), sVar(setting_export_image_high_quality));
				sVar(setting_export_image_watermark) = value_get_real(DsMap(map).Value(/*"exportimage_watermark"*/ STR(2734)), sVar(setting_export_image_watermark));
			}
			...
			if (ds_map_valid(DsMap(map).Value(/*"main_window"*/ STR(2735))))
			{
				VarType mainwindowmap = DsMap(map).Value(/*"main_window"*/ STR(2735));
				VarType rectlist = DsMap(mainwindowmap).Value(/*"rect"*/ STR(2736));
				...
				sVar(setting_main_window_maximized) = DsMap(mainwindowmap).Value(/*"maximized"*/ STR(2737));
			}
			VarType worldimportmap = DsMap(map).Value(/*"world_import"*/ STR(2738));
			if (ds_map_valid(worldimportmap))
			{
				sVar(setting_world_import_filter_enabled) = value_get_real(DsMap(worldimportmap).Value(/*"filter_enabled"*/ STR(2739)), sVar(setting_world_import_filter_enabled));
				sVar(setting_world_import_filter_mode) = value_get_real(DsMap(worldimportmap).Value(/*"filter_mode"*/ STR(2740)), sVar(setting_world_import_filter_mode));
				ds_list_merge(sInt(setting_world_import_filter_list), DsMap(worldimportmap).Value(/*"filter_list"*/ STR(2741)));
				sVar(setting_world_import_unload_regions) = value_get_real(DsMap(worldimportmap).Value(/*"unload_regions"*/ STR(2742)), sVar(setting_world_import_unload_regions));
			}
		}
		else
		{
			settings_load_legacy(self, fn);
			settings_load_legacy_recent(self, data_directory + /*"recent.file"*/ STR(2743));
		}
