/*
	...
	[ Generated on 2023.08.25 15:55:50 ]
*/
...
namespace CppProject
{
	RealType res_load_scenery(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 894: // open
			{
				debug({ /*"res_load_scenery"*/ STR(2612), /*"open"*/ STR(894) });
				if (sVar(type) == e_res_type_FROM_WORLD)
				{
					if (!res_load_scenery_world(self))
					{
						error(/*"errorloadworld"*/ STR(2613));
				...
				else
				{
					...
					{
						log({ /*"Loading "*/ STR(2551) + ext, fname });
						debug_timer_start();
						...
						if (rootmap == null_)
							break;
						debug_timer_stop(/*"res_load_scenery, Parse NBT"*/ STR(2614));
						if (dev_mode_debug_schematics)
							nbt_debug_tag_compound(/*"root"*/ STR(2172), rootmap);
						if (ext == /*".schematic"*/ STR(692))
						{
							withOne (obj_builder, global::mc_builder, self->id)
							{
								openerr = !builder_read_schematic(self, DsMap(rootmap).Value(/*"Schematic"*/ STR(2615)));
								if (openerr)
									break;
					...
					{
						log({ /*"Loading .blocks"*/ STR(2616), fname });
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryblocks"*/ STR(2617) });
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaptionpieceof"*/ STR(2618), ObjType(obj_builder, global::mc_builder)->file_map });
					else
						ObjType(obj_popup, global::_app->popup_loading)->caption = text_get({ /*"loadscenerycaption"*/ STR(2619), idVar(self.otherId, filename) });
				...
				if (sVar(scenery_tl_add) == null_)
				{
					if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(500))
						...
						if (ObjType(obj_builder, global::mc_builder)->sch_timeline_amount > IntType(20))
							sVar(scenery_tl_add) = question(text_get({ /*"loadsceneryaddtimelines"*/ STR(2620), ObjType(obj_builder, global::mc_builder)->sch_timeline_amount }));
						else
			...
			case 994: // blocks
			{
				...
				{
					debug_timer_stop(/*"res_load_scenery, Set models"*/ STR(2621));
					debug_timer_start();
					...
					ObjType(obj_builder, global::mc_builder)->build_pos = IntType(0);
					withOne (app, global::_app->id, self->id)
						ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerymodel"*/ STR(2622) });
			...
			case 8: // model
			{
				...
				if (ObjType(obj_builder, global::mc_builder)->build_pos == ObjType(obj_builder, global::mc_builder)->build_size_total)
				{
					...
					debug_timer_stop(/*"res_load_scenery, Generate models"*/ STR(2623));
					...
					sBool(ready) = true;
					if (ObjType(obj_builder, global::mc_builder)->file_map != /*""*/ STR(0))
						sVar(display_name) = text_get({ /*"loadscenerypieceof"*/ STR(2624), ObjType(obj_builder, global::mc_builder)->file_map });
					res_save_block_cache(self, global::_app->project_folder + /*"/"*/ STR(20) + sVar(filename) + /*".meshcache"*/ STR(2553));
		...
		if (openerr)
		{
			error(/*"errorloadschematic"*/ STR(2625));
			buffer_delete(global::buffer_current);
			withOne (app, global::_app->id, self->id)
				load_next(ScopeAny(self));
	...
	RealType res_load_skulls(ScopeAny self)
	{
		...
		ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadscenerydownload"*/ STR(2626), ObjType(obj_builder, global::mc_builder)->block_skull_finish_count, ObjType(obj_builder, global::mc_builder)->block_skull_texture_count, ObjType(obj_builder, global::mc_builder)->block_skull_fail_count });
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			if (self->block_skull_texture != null_ || self->block_skull_texture_fail == true || (current_time() - self->block_skull_download_time > IntType(3000)))
			{
				...
				else
				{
					...
					log({ /*"Failed to download texture"*/ STR(2627), self->block_skull_texture_name });
	...
	void res_load_start(ScopeAny self)
	{
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_FROM_WORLD:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadsceneryopen"*/ STR(2628) });
			...
			case e_res_type_SOUND:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadaudioread"*/ STR(2629) });
			...
			case e_res_type_PACK_UNZIPPED:
			{
				...
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackunzip"*/ STR(2630) });
					ObjType(obj_popup, global::_app->popup_loading)->caption = idVar(self.otherId, filename);
		...
		surface_set_target(surf);
	{
		...
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/stone"*/ STR(2631)), IntType(0), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/grass_side"*/ STR(2632)), IntType(16), IntType(0), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/tnt_side"*/ STR(2633)), IntType(0), IntType(16), size, size, block_sheet_width, block_sheet_height);
		draw_texture_slot(self->block_sheet_texture, ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), /*"block/sapling_oak"*/ STR(2634)), IntType(16), IntType(16), size, size, block_sheet_width, block_sheet_height);
	}
	...
	RealType res_update_colors(ScopeAny self, VarArgs argument)
	{
		...
		else
		{
			...
			sVar(color_leaves_spruce) = hex_to_color(/*"62A857"*/ STR(2635));
	...
	void res_update_display_name(Scope<obj_resource> self)
	{
		...
		if (self->type == e_res_type_DOWNLOADED_SKIN)
			self->display_name = text_get({ /*"downloadskinname"*/ STR(2636), self->display_name });
	...
	RealType scrollbar_draw(ScopeAny self, IntType sb, IntType dir, VarType xx, VarType yy, VarType size, VarType maxsize)
	{
		...
		if (dir == e_scroll_HORIZONTAL)
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx, yy - nearsize, size, areasize + nearsize) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2637));
		else
			ObjType(value_animation, idInt(sb, mousenear))->value = (app_mouse_box(self, xx - nearsize, yy, areasize + nearsize, size) && sBool(content_mouseon)) || (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2637));
		...
		if (mouseinarea)
		{
			...
			if (!sBool(mouse_left_pressed) && sBool(mouse_left) && !mouseinbar && ((dir > 0) ? (gmlGlobal::mouse_x < (xx + barpos) || gmlGlobal::mouse_x > (xx + barpos + barsize)) : (gmlGlobal::mouse_y < (yy + barpos) || gmlGlobal::mouse_y > (yy + barpos + barsize))))
			{
				...
				if (sBool(mouse_left) && mouseinbar)
				{
					...
					sVar(window_busy) = /*"scrollbar"*/ STR(2637);
				}
		...
		{
			if (sVar(window_busy) == /*"scrollbar"*/ STR(2637))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if (!sBool(mouse_left))
				{
		...
		if (sVar(window_focus) == string(sb) && sVar(window_busy) == /*"scrollbar"*/ STR(2637))
			idVar(sb, value_goal) = idVar(sb, value);
		barpos = min({ size - barsize, floor(idVar(sb, value) * ((RealType)size / maxsize)) });
		pressed = (sVar(window_busy) == /*"scrollbar"*/ STR(2637) && sVar(window_focus) == string(sb));
		if (dir == e_scroll_HORIZONTAL)
		{
	...
	RealType settings_load(ScopeAny self)
	{
		...
		{
			log({ /*"Loading settings"*/ STR(2638), fn });
			...
			if (ds_map_valid(assetsmap))
			{
				...
				VarType newmap = DsMap(assetsmap).Value(/*"new"*/ STR(2639));
			...
			VarType toastslist = DsMap(map).Value(/*"closed_toasts"*/ STR(2640));
			if (ds_list_valid(toastslist))
				ds_list_copy(sInt(closed_toast_list), (IntType)(toastslist));
			VarType programmap = DsMap(map).Value(/*"program"*/ STR(2641));
			if (ds_map_valid(programmap))
			{
				sReal(setting_advanced_mode) = value_get_real(DsMap(programmap).Value(/*"advanced_mode"*/ STR(2642)), sReal(setting_advanced_mode));
				gmlGlobal::room_speed = value_get_real(DsMap(programmap).Value(/*"fps"*/ STR(2643)), gmlGlobal::room_speed);
				if (!dev_mode)
					sStr(setting_project_folder) = value_get_string(DsMap(programmap).Value(/*"project_folder"*/ STR(2644)), sStr(setting_project_folder)).Str();
				...
				sReal(setting_backup_time) = value_get_real(DsMap(programmap).Value(/*"backup_time"*/ STR(2645)), sReal(setting_backup_time));
				sReal(setting_backup_amount) = value_get_real(DsMap(programmap).Value(/*"backup_amount"*/ STR(2646)), sReal(setting_backup_amount));
				sVar(setting_spawn_cameras) = value_get_real(DsMap(programmap).Value(/*"spawn_cameras"*/ STR(2647)), sVar(setting_spawn_cameras));
				sReal(setting_unlimited_values) = value_get_real(DsMap(programmap).Value(/*"unlimited_values"*/ STR(2648)), sReal(setting_unlimited_values));
				sVar(setting_watermark_custom) = value_get_real(DsMap(programmap).Value(/*"watermark_custom"*/ STR(2649)), sVar(setting_watermark_custom));
				sStr(setting_watermark_fn) = value_get_string(DsMap(programmap).Value(/*"watermark_fn"*/ STR(2650)), sStr(setting_watermark_fn)).Str();
				sVar(setting_watermark_halign) = value_get_string(DsMap(programmap).Value(/*"watermark_halign"*/ STR(2651)), sVar(setting_watermark_halign));
				sVar(setting_watermark_valign) = value_get_string(DsMap(programmap).Value(/*"watermark_valign"*/ STR(2652)), sVar(setting_watermark_valign));
				sReal(setting_watermark_padding) = value_get_real(DsMap(programmap).Value(/*"watermark_padding"*/ STR(2653)), sReal(setting_watermark_padding));
				sReal(setting_watermark_scale) = value_get_real(DsMap(programmap).Value(/*"watermark_scale"*/ STR(2654)), sReal(setting_watermark_scale));
				sReal(setting_watermark_opacity) = value_get_real(DsMap(programmap).Value(/*"watermark_opacity"*/ STR(2655)), sReal(setting_watermark_opacity));
				if (sStr(setting_watermark_fn) != /*""*/ STR(0))
					sInt(setting_watermark_image) = texture_create(sStr(setting_watermark_fn));
			}
			VarType interfacemap = DsMap(map).Value(/*"interface"*/ STR(2656));
			if (ds_map_valid(interfacemap))
			{
				sStr(setting_language_filename) = value_get_string(DsMap(interfacemap).Value(/*"language_filename"*/ STR(2657)), sStr(setting_language_filename)).Str();
				...
				themename = value_get_string(DsMap(interfacemap).Value(/*"theme"*/ STR(2658)), themename);
				...
				sInt(setting_accent) = value_get_real(DsMap(interfacemap).Value(/*"accent"*/ STR(2659)), sInt(setting_accent));
				sVar(setting_accent_custom) = value_get_color(DsMap(interfacemap).Value(/*"accent_custom"*/ STR(2660)), sVar(setting_accent_custom));
				sVar(setting_timeline_autoscroll) = value_get_real(DsMap(interfacemap).Value(/*"timeline_autoscroll"*/ STR(2661)), sVar(setting_timeline_autoscroll));
				sBool(setting_timeline_show_markers) = value_get_real(DsMap(interfacemap).Value(/*"timeline_show_markers"*/ STR(2662)), sBool(setting_timeline_show_markers));
				sVar(setting_interface_compact) = value_get_real(DsMap(interfacemap).Value(/*"interface_compact"*/ STR(2663)), sVar(setting_interface_compact));
				sReal(setting_timeline_compact) = value_get_real(DsMap(interfacemap).Value(/*"timeline_compact"*/ STR(2664)), sReal(setting_timeline_compact));
				sVar(setting_reduced_motion) = value_get_real(DsMap(interfacemap).Value(/*"reduced_motion"*/ STR(2665)), sVar(setting_reduced_motion));
				sVar(setting_timeline_select_jump) = value_get_real(DsMap(interfacemap).Value(/*"timeline_select_jump"*/ STR(2666)), sVar(setting_timeline_select_jump));
				sVar(setting_timeline_hide_ghosts) = value_get_real(DsMap(interfacemap).Value(/*"timeline_hide_ghosts"*/ STR(2667)), sVar(setting_timeline_hide_ghosts));
				sVar(setting_timeline_frame_snap) = value_get_real(DsMap(interfacemap).Value(/*"timeline_frame_snap"*/ STR(2668)), sVar(setting_timeline_frame_snap));
				sReal(setting_z_is_up) = value_get_real(DsMap(interfacemap).Value(/*"z_is_up"*/ STR(2669)), sReal(setting_z_is_up));
				sBool(setting_show_shortcuts_bar) = value_get_real(DsMap(interfacemap).Value(/*"show_shortcuts_bar"*/ STR(2670)), sBool(setting_show_shortcuts_bar));
				sVar(setting_gizmos_face_camera) = value_get_real(DsMap(interfacemap).Value(/*"gizmos_face_camera"*/ STR(2671)), sVar(setting_gizmos_face_camera));
				sVar(setting_fade_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"fade_gizmos"*/ STR(2672)), sVar(setting_fade_gizmos));
				sVar(setting_camera_lock_mouse) = value_get_real(DsMap(interfacemap).Value(/*"camera_lock_mouse"*/ STR(2673)), sVar(setting_camera_lock_mouse));
				window_mouse_set_permission(sVar(setting_camera_lock_mouse));
				sVar(setting_place_new) = value_get_real(DsMap(interfacemap).Value(/*"place_new"*/ STR(2674)), sVar(setting_place_new));
				sVar(setting_interface_scale_auto) = value_get_real(DsMap(interfacemap).Value(/*"scale_auto"*/ STR(2675)), sVar(setting_interface_scale_auto));
				...
				sVar(setting_separate_tool_modes) = value_get_real(DsMap(interfacemap).Value(/*"separate_tool_modes"*/ STR(2676)), sVar(setting_separate_tool_modes));
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
					sBool(setting_tool_select) = true;
				}
				sVar(setting_panel_left_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_bottom_size"*/ STR(2677)), sVar(setting_panel_left_bottom_size));
				sVar(setting_panel_right_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_bottom_size"*/ STR(2678)), sVar(setting_panel_right_bottom_size));
				sVar(setting_panel_bottom_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_bottom_size"*/ STR(2679)), sVar(setting_panel_bottom_size));
				sVar(setting_panel_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_top_size"*/ STR(2680)), sVar(setting_panel_top_size));
				sVar(setting_panel_left_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_left_top_size"*/ STR(2681)), sVar(setting_panel_left_top_size));
				sVar(setting_panel_right_top_size) = value_get_real(DsMap(interfacemap).Value(/*"panel_right_top_size"*/ STR(2682)), sVar(setting_panel_right_top_size));
				sVar(setting_properties_location) = value_get_string(DsMap(interfacemap).Value(/*"properties_location"*/ STR(2683)), sVar(setting_properties_location));
				sVar(setting_ground_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"ground_editor_location"*/ STR(2684)), sVar(setting_ground_editor_location));
				sVar(setting_template_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"template_editor_location"*/ STR(2685)), sVar(setting_template_editor_location));
				sVar(setting_timeline_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"timeline_editor_location"*/ STR(2686)), sVar(setting_timeline_editor_location));
				sVar(setting_frame_editor_location) = value_get_string(DsMap(interfacemap).Value(/*"frame_editor_location"*/ STR(2687)), sVar(setting_frame_editor_location));
				sVar(setting_settings_location) = value_get_string(DsMap(interfacemap).Value(/*"settings_location"*/ STR(2688)), sVar(setting_settings_location));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2689))))
					window_state_restore(e_window_TIMELINE, DsMap(interfacemap).Value(/*"timeline_window"*/ STR(2689)));
				sVar(setting_view_split) = value_get_real(DsMap(interfacemap).Value(/*"view_split"*/ STR(2690)), sVar(setting_view_split));
				sVar(setting_view_main_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_main_overlays"*/ STR(2691)), sVar(setting_view_main_overlays));
				sVar(setting_view_main_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_main_aspect_ratio"*/ STR(2692)), sVar(setting_view_main_aspect_ratio));
				sVar(setting_view_main_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_main_grid"*/ STR(2693)), sVar(setting_view_main_grid));
				sVar(setting_view_main_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_main_gizmos"*/ STR(2694)), sVar(setting_view_main_gizmos));
				sVar(setting_view_main_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_main_fog"*/ STR(2695)), sVar(setting_view_main_fog));
				sVar(setting_view_main_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_main_effects"*/ STR(2696)), sVar(setting_view_main_effects));
				sVar(setting_view_main_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_main_particles"*/ STR(2697)), sVar(setting_view_main_particles));
				sVar(setting_view_main_location) = value_get_string(DsMap(interfacemap).Value(/*"view_main_location"*/ STR(2698)), sVar(setting_view_main_location));
				sVar(setting_view_second_show) = value_get_real(DsMap(interfacemap).Value(/*"view_second_show"*/ STR(2699)), sVar(setting_view_second_show));
				sVar(setting_view_second_overlays) = value_get_real(DsMap(interfacemap).Value(/*"view_second_overlays"*/ STR(2700)), sVar(setting_view_second_overlays));
				sVar(setting_view_second_aspect_ratio) = value_get_real(DsMap(interfacemap).Value(/*"view_second_aspect_ratio"*/ STR(2701)), sVar(setting_view_second_aspect_ratio));
				sVar(setting_view_second_grid) = value_get_real(DsMap(interfacemap).Value(/*"view_second_grid"*/ STR(2702)), sVar(setting_view_second_grid));
				sVar(setting_view_second_gizmos) = value_get_real(DsMap(interfacemap).Value(/*"view_second_gizmos"*/ STR(2703)), sVar(setting_view_second_gizmos));
				sVar(setting_view_second_fog) = value_get_real(DsMap(interfacemap).Value(/*"view_second_fog"*/ STR(2704)), sVar(setting_view_second_fog));
				sVar(setting_view_second_effects) = value_get_real(DsMap(interfacemap).Value(/*"view_second_effects"*/ STR(2705)), sVar(setting_view_second_effects));
				sVar(setting_view_second_particles) = value_get_real(DsMap(interfacemap).Value(/*"view_second_particles"*/ STR(2706)), sVar(setting_view_second_particles));
				sVar(setting_view_second_location) = value_get_string(DsMap(interfacemap).Value(/*"view_second_location"*/ STR(2707)), sVar(setting_view_second_location));
				sVar(setting_view_second_width) = value_get_real(DsMap(interfacemap).Value(/*"view_second_width"*/ STR(2708)), sVar(setting_view_second_width));
				sVar(setting_view_second_height) = value_get_real(DsMap(interfacemap).Value(/*"view_second_height"*/ STR(2709)), sVar(setting_view_second_height));
				if (ds_map_valid(DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2710))))
					window_state_restore(e_window_VIEW_SECOND, DsMap(interfacemap).Value(/*"view_second_window"*/ STR(2710)));
				sReal(setting_snap) = value_get_real(DsMap(interfacemap).Value(/*"snap"*/ STR(296)), sReal(setting_snap));
				sVar(setting_snap_absolute) = value_get_real(DsMap(interfacemap).Value(/*"snap_absolute"*/ STR(2711)), sVar(setting_snap_absolute));
				sReal(setting_snap_size_position) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_position"*/ STR(2712)), sReal(setting_snap_size_position));
				sReal(setting_snap_size_rotation) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_rotation"*/ STR(2713)), sReal(setting_snap_size_rotation));
				sReal(setting_snap_size_scale) = value_get_real(DsMap(interfacemap).Value(/*"snap_size_scale"*/ STR(2714)), sReal(setting_snap_size_scale));
				sVar(setting_modelbench_popup_hidden) = value_get_real(DsMap(interfacemap).Value(/*"modelbench_popup_hidden"*/ STR(2715)), sVar(setting_modelbench_popup_hidden));
			}
			VarType controlsmap = DsMap(map).Value(/*"controls"*/ STR(2716));
			if (ds_map_valid(controlsmap))
			{
				...
				sReal(setting_move_speed) = value_get_real(DsMap(controlsmap).Value(/*"move_speed"*/ STR(2717)), sReal(setting_move_speed));
				sReal(setting_look_sensitivity) = value_get_real(DsMap(controlsmap).Value(/*"look_sensitivity"*/ STR(2718)), sReal(setting_look_sensitivity));
				sReal(setting_fast_modifier) = value_get_real(DsMap(controlsmap).Value(/*"fast_modifier"*/ STR(2719)), sReal(setting_fast_modifier));
				sReal(setting_slow_modifier) = value_get_real(DsMap(controlsmap).Value(/*"slow_modifier"*/ STR(2720)), sReal(setting_slow_modifier));
				keybinds_update_match();
			}
			VarType exportmap = DsMap(map).Value(/*"export"*/ STR(2721));
			if (ds_map_valid(exportmap))
			{
				sVar(setting_export_movie_format) = value_get_string(DsMap(map).Value(/*"exportmovie_format"*/ STR(2722)), sVar(setting_export_movie_format));
				sVar(setting_export_movie_frame_rate) = value_get_real(DsMap(map).Value(/*"exportmovie_frame_rate"*/ STR(2723)), sVar(setting_export_movie_frame_rate));
				sVar(setting_export_movie_framespersecond) = value_get_real(DsMap(map).Value(/*"exportmovie_framespersecond"*/ STR(2724)), sVar(setting_export_movie_framespersecond));
				sVar(setting_export_movie_bit_rate) = value_get_real(DsMap(map).Value(/*"exportmovie_bit_rate"*/ STR(2725)), sVar(setting_export_movie_bit_rate));
				sVar(setting_export_movie_include_audio) = value_get_real(DsMap(map).Value(/*"exportmovie_include_audio"*/ STR(2726)), sVar(setting_export_movie_include_audio));
				sVar(setting_export_movie_remove_background) = value_get_real(DsMap(map).Value(/*"exportmovie_remove_background"*/ STR(2727)), sVar(setting_export_movie_remove_background));
				sVar(setting_export_movie_include_hidden) = value_get_real(DsMap(map).Value(/*"exportmovie_remove_background"*/ STR(2727)), sVar(setting_export_movie_include_hidden));
				sVar(setting_export_movie_high_quality) = value_get_real(DsMap(map).Value(/*"exportmovie_high_quality"*/ STR(2728)), sVar(setting_export_movie_high_quality));
				sVar(setting_export_movie_watermark) = value_get_real(DsMap(map).Value(/*"exportmovie_watermark"*/ STR(2729)), sVar(setting_export_movie_watermark));
				sVar(setting_export_image_remove_background) = value_get_real(DsMap(map).Value(/*"exportimage_remove_background"*/ STR(2730)), sVar(setting_export_image_remove_background));
				sVar(setting_export_image_include_hidden) = value_get_real(DsMap(map).Value(/*"exportimage_include_hidden"*/ STR(2731)), sVar(setting_export_image_include_hidden));
				sVar(setting_export_image_high_quality) = value_get_real(DsMap(map).Value(/*"exportimage_high_quality"*/ STR(2732)), sVar(setting_export_image_high_quality));
				sVar(setting_export_image_watermark) = value_get_real(DsMap(map).Value(/*"exportimage_watermark"*/ STR(2733)), sVar(setting_export_image_watermark));
			}
			...
			if (ds_map_valid(DsMap(map).Value(/*"main_window"*/ STR(2734))))
			{
				VarType mainwindowmap = DsMap(map).Value(/*"main_window"*/ STR(2734));
				VarType rectlist = DsMap(mainwindowmap).Value(/*"rect"*/ STR(2735));
				...
				sVar(setting_main_window_maximized) = DsMap(mainwindowmap).Value(/*"maximized"*/ STR(2736));
			}
			VarType worldimportmap = DsMap(map).Value(/*"world_import"*/ STR(2737));
			if (ds_map_valid(worldimportmap))
			{
				sVar(setting_world_import_filter_enabled) = value_get_real(DsMap(worldimportmap).Value(/*"filter_enabled"*/ STR(2738)), sVar(setting_world_import_filter_enabled));
				sVar(setting_world_import_filter_mode) = value_get_real(DsMap(worldimportmap).Value(/*"filter_mode"*/ STR(2739)), sVar(setting_world_import_filter_mode));
				ds_list_merge(sInt(setting_world_import_filter_list), DsMap(worldimportmap).Value(/*"filter_list"*/ STR(2740)));
				sVar(setting_world_import_unload_regions) = value_get_real(DsMap(worldimportmap).Value(/*"unload_regions"*/ STR(2741)), sVar(setting_world_import_unload_regions));
			}
		}
		else
		{
			settings_load_legacy(self, fn);
			settings_load_legacy_recent(self, data_directory + /*"recent.file"*/ STR(2742));
		}
