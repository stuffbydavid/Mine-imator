/*
	...
	[ Generated on 2023.10.21 15:36:07 ]
*/
	...
	void tab_timeline(ScopeAny self)
	{
		...
		if (markerbarh != IntType(0))
		{
			...
			for (IntType i = IntType(0); i < ds_list_size(sInt(timeline_marker_list)); i++)
			{
				...
				name = idVar(marker, name) + (dev_mode_debug_names ? /*" ["*/ STR(1785) + idVar(marker, save_id) + /*"]"*/ STR(1211) : /*""*/ STR(0));
		...
		for (RealType t = sReal(timeline_list_first); t < ds_list_size(sInt(tree_visible_list)); t++)
		{
			...
			if (tlhierarchy)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelineexpand"*/ STR(3720) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, tree_extend), null_, null_, false, ((idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1671) : /*"tooltiptlexpand"*/ STR(1672)), ID_spr_chevron_ani }))
			...
			else
			{
				...
				if (dev_mode_debug_saveid)
					name += /*" ["*/ STR(1785) + string(idVar(tl, save_id)) + /*"]"*/ STR(1211);
				if (name != /*""*/ STR(0))
				{
					if (idVar(tl, color_tag) != null_)
