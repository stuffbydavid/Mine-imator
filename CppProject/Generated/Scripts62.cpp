...
namespace CppProject
{
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3690), string(ds_list_size(sInt(pc_type_list))) });
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3204), minecraft_asset_get_name(/*"modelpart"*/ STR(748), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(515), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3205) });
	...
	BoolType textbox_draw(ScopeAny self, VarArgs argument)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3692)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1051))))
			sVar(window_busy) = /*""*/ STR(0);
		if (sVar(window_focus) == string(tbx))
		{
			if (contextmenu > 0)
				context_menu_area(self, { xx, yy, w, h, /*"contextmenutextbox"*/ STR(1053), tbx, e_context_type_NONE, null_, null_ });
				...
				if (key_press.Value(vk_right) || key_press.Value(vk_left))
				{
					if (keyboard_check(vk_control))
					{
						...
						else
						{
							...
							if (char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(940) || sVar(textbox_select_mousepos) == IntType(0))
							{
								sVar(textbox_select_mousepos)--;
							...
							{
								char_ = string_char_at(idVar(tbx, text), (IntType)(sVar(textbox_select_mousepos)));
								if (char_ == /*""*/ STR(0) || char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(940) || (idArr(tbx, line).Value(sVar(textbox_select_mouseline)) != /*""*/ STR(0) && sVar(textbox_select_mousepos) == IntType(0)))
									break;
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1051)))
				sVar(window_busy) = /*""*/ STR(0);
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3692);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3693), idArr(tbx, line).Value(l));
						if (p == IntType(0))
				...
				for (l = IntType(0); l < idVar(tbx, lines); l++)
				{
					if (string_width(idArr(tbx, line).Value(l)) > w)
					{
						...
						for (p = string_length(idArr(tbx, line).Value(l)); p > IntType(1); p--)
						{
							...
							if ((a == /*" "*/ STR(17) || a == /*"-"*/ STR(1078)) && string_width(string_copy(idArr(tbx, line).Value(l), IntType(1), (IntType)(p - IntType(1)))) < w)
								break;
						}
						if (p == IntType(1))
						{
				...
				if ((gmlGlobal::mouse_x >= xx || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_x < xx + w || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_y >= yy + ly || (sVar(window_busy) == string(tbx) && ly == IntType(0))) && (gmlGlobal::mouse_y < yy + ly + hh || (sVar(window_busy) == string(tbx) && (ly + lineheight > h || l == idVar(tbx, lines) - IntType(1)))))
				{
					if (sBool(mouse_left))
					{
						if (idReal(tbx, select_on_focus) > 0 && sVar(textbox_lastfocus) != tbx)
						{
							...
							sVar(window_busy) = string(tbx) + /*"click"*/ STR(1051);
						}
						else
						{
								...
								if (sVar(textbox_select_clickline) == sVar(textbox_select_mouseline) && sVar(textbox_select_clickpos) == sVar(textbox_select_mousepos))
								{
									if (current_time() - sInt(textbox_click) < IntType(500))
									{
										...
										while (true)
										{
											...
											if (c == /*" "*/ STR(17) || c == /*", "*/ STR(1014) || c == /*"("*/ STR(1073) || c == /*")"*/ STR(1077) || c == /*"["*/ STR(962) || c == /*"]"*/ STR(1202) || c == /*"+"*/ STR(1079))
												break;
											sVar(textbox_select_startpos)--;
											if (sVar(textbox_select_startpos) <= IntType(0))
											{
