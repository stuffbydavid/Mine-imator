	...
	void tab_timeline(ScopeAny self)
	{
		...
		for (RealType t = sReal(timeline_list_first); t < ds_list_size(sInt(tree_visible_list)); t++)
		{
			...
			if ((itemhover && sBool(mouse_left)) || idReal(tl, selected) > 0)
				...
				if (idReal(tl, selected) > 0 || itemhover || tl == sVar(context_menu_value) || (sVar(window_busy) == /*"timelineclick"*/ STR(3708) && sVar(timeline_select) == tl))
			...
			if (tlhierarchy)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelineexpand"*/ STR(3719) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, tree_extend), null_, null_, false, ((idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1671) : /*"tooltiptlexpand"*/ STR(1672)), ID_spr_chevron_ani }))
