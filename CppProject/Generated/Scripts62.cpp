...
namespace CppProject
{
	...
	void tab_template_editor_particles_value(ScopeAny self, StringType name, VarType val, VarType israndom, VarType randommin, VarType randommax, RealType mul, RealType minval, RealType maxval, ArrType def, RealType snapval, ArrType tbx, ArrType scripts, VarType capwid, BoolType showcaption, VarType suffix)
	{
		...
		if (israndom > 0)
		{
			...
			textfield_group_add(self, { name + /*"min"*/ STR(1072), randommin, def.Value(IntType(1)), scripts.Value(IntType(2)), global::axis_edit, tbx.Value(IntType(0)), null_, mul, minval, randommax, /*"particleeditormin"*/ STR(3668) });
			textfield_group_add(self, { name + /*"max"*/ STR(1073), randommax, def.Value(IntType(2)), scripts.Value(IntType(3)), global::axis_edit, tbx.Value(IntType(1)), null_, mul, randommin, maxval, /*"particleeditormax"*/ STR(3669) });
	...
	void tab_timeline(ScopeAny self)
	{
		...
		{
			timelabel = /*" ("*/ STR(1112) + ((sReal(timeline_show_frames) > 0) ? VarType(string(sVar(timeline_region_end) - sVar(timeline_region_start))) : string_time_seconds((sVar(timeline_region_end) - sVar(timeline_region_start)) / sVar(project_tempo))) + /*")"*/ STR(1088);
		...
		if (sStr(settings_menu_name) == /*"timelineintervalsettings"*/ STR(3694) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
		...
		if (markerbarh != IntType(0))
		{
			...
			for (IntType i = IntType(0); i < ds_list_size(sInt(timeline_marker_list)); i++)
			{
				...
				name = idVar(marker, name) + (dev_mode_debug_names ? /*" ["*/ STR(1786) + idVar(marker, save_id) + /*"]"*/ STR(1213) : /*""*/ STR(0));
		...
		if (sReal(setting_advanced_mode) > 0)
		{
			...
			if (sStr(settings_menu_name) == /*"timelinefilter"*/ STR(3706) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		}
		VarType searchx;
		RealType searchwid;
		...
		draw_textfield(self, /*"timelinesearch"*/ STR(3708), searchx, bary + IntType(4), searchwid, IntType(24), idInt(sVar(timeline), tbx_search), ID_action_tl_search, text_get({ /*"listsearch"*/ STR(2977) }), /*"none"*/ STR(880));
		if (mouseinnames)
		...
		for (RealType t = sReal(timeline_list_first); t < ds_list_size(sInt(tree_visible_list)); t++)
		{
			...
			if ((itemhover && sBool(mouse_left)) || idReal(tl, selected) > 0)
				...
				if (idReal(tl, selected) > 0 || itemhover || tl == sVar(context_menu_value) || ((sVar(window_busy) == /*"timelineclick"*/ STR(3709)) && sVar(timeline_select) == tl) || ((sVar(window_busy) == /*"timelineclick"*/ STR(3709) && sVar(window_busy) == /*"place"*/ STR(119)) && sVar(timeline_select) == tl))
			...
			if (itemhover || idVar(tl, hide) > 0)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelinehide"*/ STR(266) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, hide), (idVar(tl, hide) > 0) ? icons_HIDDEN_SMALL : icons_VISIBLE_SMALL, null_, false, (idVar(tl, hide) > 0) ? /*"tooltiptlshow"*/ STR(3710) : /*"tooltiptlhide"*/ STR(3711) }))
						action_tl_hide(self, tl);
				}
				else
				{
					if (draw_button_icon(self, { /*"timelinehide"*/ STR(266) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, hide), (idVar(tl, hide) > 0) ? icons_MUTE_SMALL : icons_VOLUME_SMALL, null_, false, (idVar(tl, hide) > 0) ? /*"tooltiptlunmute"*/ STR(3712) : /*"tooltiptlmute"*/ STR(3713) }))
						action_tl_hide(self, tl);
				}
			...
			if (itemhover)
			{
				if (draw_button_icon(self, { /*"timelineselectkeyframes"*/ STR(3704) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, false, icons_KEYFRAME_SMALL, null_, false, /*"contextmenutlselectkeyframes"*/ STR(1523) }))
			...
			if (tlhierarchy)
			{
				...
				{
					if (draw_button_icon(self, { /*"timelineexpand"*/ STR(3720) + string(tl), xx, itemy + buttonpad, buttonsize, buttonsize, idVar(tl, tree_extend), null_, null_, false, ((idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1672) : /*"tooltiptlexpand"*/ STR(1673)), ID_spr_chevron_ani }))
			...
			else
			{
				...
				if (dev_mode_debug_saveid)
					name += /*" ["*/ STR(1786) + string(idVar(tl, save_id)) + /*"]"*/ STR(1213);
				if (name != /*""*/ STR(0))
				{
					if (idVar(tl, color_tag) != null_)
		...
		if (sVar(window_busy) == /*"timelinemove"*/ STR(52) || sVar(window_busy) == /*"timelineselect"*/ STR(264) || sVar(window_busy) == /*"place"*/ STR(119))
		{
