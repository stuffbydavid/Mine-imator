	...
	void tab_timeline(ScopeAny self)
	{
		...
		for (RealType t = sReal(timeline_list_first); t < ds_list_size(sInt(tree_visible_list)); t++)
		{
			...
			if ((itemhover && sBool(mouse_left)) || idReal(tl, selected) > 0)
				...
				if (idReal(tl, selected) > 0 || itemhover || tl == sVar(context_menu_value) || (sVar(window_busy) == /*"timelineclick"*/ STR(3709) && sVar(timeline_select) == tl))
		...
		if (app_mouse_box(self, barx, bary + IntType(5) * (IntType)(idInt(sVar(tab), panel) == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(193))), barw, barh - IntType(5) * (IntType)(idInt(sVar(tab), panel) == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(193)))) && !sBool(popup_mouseon) && !sBool(context_menu_mouseon) && !sBool(toast_mouseon))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (sVar(timeline_region_start) != null_)
			{
		...
		if (sVar(window_busy) == /*"timelinemove"*/ STR(52) || sVar(window_busy) == /*"timelineselect"*/ STR(262))
		{
