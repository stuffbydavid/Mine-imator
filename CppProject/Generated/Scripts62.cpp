...
namespace CppProject
{
	...
	void temp_particles_init(ScopeAny self)
	{
		...
		sVar(pc_spawn_region_type) = /*"sphere"*/ STR(512);
		...
		sVar(pc_bounding_box_type) = /*"none"*/ STR(873);
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3682), string(ds_list_size(sInt(pc_type_list))) });
	...
	void temp_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(772) + DsList(global::temp_type_name_list).Value(sVar(type)) });
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_SPECIAL_BLOCK:
				{
					if (sVar(model_file) != null_)
						sVar(display_name) = minecraft_asset_get_name(/*"model"*/ STR(514), idVar(sVar(model_file), name));
					break;
				}
				...
				{
					sVar(display_name) = minecraft_asset_get_name(/*"block"*/ STR(507), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					break;
				}
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3196), minecraft_asset_get_name(/*"modelpart"*/ STR(747), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(514), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3197) });
	...
	BoolType textbox_draw(ScopeAny self, VarArgs argument)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3684)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1049))))
			sVar(window_busy) = /*""*/ STR(0);
		if (sVar(window_focus) == string(tbx))
		{
			if (contextmenu > 0)
				context_menu_area(self, { xx, yy, w, h, /*"contextmenutextbox"*/ STR(1051), tbx, e_context_type_NONE, null_, null_ });
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1049)))
				sVar(window_busy) = /*""*/ STR(0);
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3684);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3685), idArr(tbx, line).Value(l));
						if (p == IntType(0))
				...
				for (l = IntType(0); l < idVar(tbx, lines); l++)
				{
					if (string_width(idArr(tbx, line).Value(l)) > w)
					{
						...
						for (p = string_length(idArr(tbx, line).Value(l)); p > IntType(1); p--)
						{
							...
							if ((a == /*" "*/ STR(17) || a == /*"-"*/ STR(1076)) && string_width(string_copy(idArr(tbx, line).Value(l), IntType(1), (IntType)(p - IntType(1)))) < w)
								break;
						}
						if (p == IntType(1))
						{
				...
				if ((gmlGlobal::mouse_x >= xx || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_x < xx + w || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_y >= yy + ly || (sVar(window_busy) == string(tbx) && ly == IntType(0))) && (gmlGlobal::mouse_y < yy + ly + hh || (sVar(window_busy) == string(tbx) && (ly + lineheight > h || l == idVar(tbx, lines) - IntType(1)))))
				{
					if (sBool(mouse_left))
					{
						if (idReal(tbx, select_on_focus) > 0 && sVar(textbox_lastfocus) != tbx)
						{
							...
							sVar(window_busy) = string(tbx) + /*"click"*/ STR(1049);
						}
						else
						{
								...
								if (sVar(textbox_select_clickline) == sVar(textbox_select_mouseline) && sVar(textbox_select_clickpos) == sVar(textbox_select_mousepos))
								{
									if (current_time() - sInt(textbox_click) < IntType(500))
									{
										...
										while (true)
										{
											...
											if (c == /*" "*/ STR(17) || c == /*", "*/ STR(1012) || c == /*"("*/ STR(1071) || c == /*")"*/ STR(1075) || c == /*"["*/ STR(960) || c == /*"]"*/ STR(1200) || c == /*"+"*/ STR(1077))
												break;
											sVar(textbox_select_startpos)--;
											if (sVar(textbox_select_startpos) <= IntType(0))
											{
