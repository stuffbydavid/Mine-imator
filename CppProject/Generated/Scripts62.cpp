...
namespace CppProject
{
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3201), minecraft_asset_get_name(/*"modelpart"*/ STR(747), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(514), idVar(sVar(model_file), name)) });
					else
	...
	BoolType textbox_draw(ScopeAny self, VarArgs argument)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3689)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1050))))
