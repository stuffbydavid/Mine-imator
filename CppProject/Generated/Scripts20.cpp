...
namespace CppProject
{
	RealType block_generate_liquid(ScopeAny self, VarArgs argument)
	{
		...
		if (idVar(sInt(block_current), name) == /*"water"*/ STR(784))
		...
		slot = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_liquid_slot_map).Value(/*"flowing_"*/ STR(785) + idVar(sInt(block_current), name));
	...
	IntType block_get_timeline(Scope<obj_builder_thread> self, VarType block, VarType stateid)
	{
		...
		withOne (obj_block_tl, (new obj_block_tl)->id, self->id)
		{
			...
			self->variant = block_get_state_id_value(block, stateid, /*"variant"*/ STR(782));
			return self->id;
		}
	...
	IntType block_load(VarType map, IntType typemap)
	{
		withOne (obj_block, (new obj_block)->id, noone)
		{
			...
			if (dev_mode_debug_names && !text_exists(/*"block"*/ STR(507) + self->name))
				log({ /*"block/"*/ STR(788) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"type"*/ STR(772))))
			{
				self->type = DsMap(map).Value(/*"type"*/ STR(772));
				self->set_script = asset_get_index(/*"block_set_"*/ STR(789) + self->type);
				self->generate_script = asset_get_index(/*"block_generate_"*/ STR(790) + self->type);
			}
			else
			{
				self->type = /*""*/ STR(0);
			...
			if (is_real(DsMap(map).Value(/*"emissive"*/ STR(791))))
				self->emissive = DsMap(map).Value(/*"emissive"*/ STR(791));
			else
				self->emissive = IntType(0);
			
			if (is_bool(DsMap(map).Value(/*"random_offset"*/ STR(792))))
				self->random_offset = DsMap(map).Value(/*"random_offset"*/ STR(792));
			else
				self->random_offset = false;
			
			if (is_bool(DsMap(map).Value(/*"random_offset_xz"*/ STR(793))))
				self->random_offset_xy = DsMap(map).Value(/*"random_offset_xz"*/ STR(793));
			else
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(794))))
			{
				...
				VarType curstate;
				IntType num;
				curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(794))));
				num = IntType(0);
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"blockstate"*/ STR(762) + curstate))
						log({ /*"block/state/"*/ STR(795) + curstate + dev_mode_name_translation_message });
					withOne (obj_block_state, (new obj_block_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (ds_map_valid(curvalue))
							{
								...
								if (is_real(DsMap(curvalue).Value(/*"emissive"*/ STR(791))))
									self->value_emissive[v] = DsMap(curvalue).Value(/*"emissive"*/ STR(791));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset"*/ STR(792))))
									self->value_random_offset[v] = DsMap(curvalue).Value(/*"random_offset"*/ STR(792));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(793))))
									self->value_random_offset_xy[v] = DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(793));
							}
							DsMap(self->value_map)[self->value_name.Value(v)] = v;
							if (dev_mode_debug_names && string_length(self->value_name.Value(v)) > IntType(3) && !text_exists(/*"blockstatevalue"*/ STR(763) + self->value_name.Value(v)))
								log({ /*"block/state/value/"*/ STR(797) + self->value_name.Value(v) + dev_mode_name_translation_message });
						}
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(801))))
					self->wind_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(801));
			}
			if (is_bool(DsMap(map).Value(/*"waterlogged"*/ STR(802))))
				self->waterlogged = DsMap(map).Value(/*"waterlogged"*/ STR(802));
			else
				self->waterlogged = false;
			
			if (is_bool(DsMap(map).Value(/*"require_models"*/ STR(803))))
				self->require_models = DsMap(map).Value(/*"require_models"*/ STR(803));
			else
			...
			if (is_bool(DsMap(map).Value(/*"multithreaded"*/ STR(804))))
				self->multithreaded = DsMap(map).Value(/*"multithreaded"*/ STR(804));
			else
			...
			if (is_string(idmap))
			{
				self->mc_ids = ArrType::From({ string_replace(idmap, /*"minecraft:"*/ STR(805), /*""*/ STR(0)) });
				...
				if (ds_map_valid(idmap))
				{
					...
					while (!is_undefined(key))
					{
						array_add(VarType::CreateRef(self->mc_ids), string_replace(key, /*"minecraft:"*/ STR(805), /*""*/ STR(0)));
	...
	VarType block_load_model_file(VarArgs argument)
	{
		...
		withOne (obj_block_load_model_file, (new obj_block_load_model_file)->id, noone)
		{
			...
			self->parent = null_;
			if (res == null_ && is_string(DsMap(map).Value(/*"parent"*/ STR(808))))
				self->parent = block_load_model_file({ global::load_assets_dir + mc_models_directory + string_replace(DsMap(map).Value(/*"parent"*/ STR(808)), /*"minecraft:"*/ STR(805), /*""*/ STR(0)) + /*".json"*/ STR(696) });
			self->texture_map = null_;
			if (is_real(DsMap(map).Value(/*"textures"*/ STR(809))))
			{
				self->texture_map = ds_map_create();
				if (ds_map_find_value((IntType)(DsMap(typemap).Value(map)), /*"textures"*/ STR(809)) == e_json_type_ARRAY && ds_list_valid(DsMap(map).Value(/*"textures"*/ STR(809))))
				{
					for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(map).Value(/*"textures"*/ STR(809)))); i++)
					{
						StringType texname = ds_list_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(809))), i);
						texname = string_replace(texname, /*"minecraft:"*/ STR(805), /*""*/ STR(0));
					...
					{
						VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"textures"*/ STR(809))));
						while (!is_undefined(key))
						{
							StringType texname = ds_map_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(809))), key);
							texname = string_replace(texname, /*"minecraft:"*/ STR(805), /*""*/ STR(0));
							DsMap(self->texture_map)[key] = block_load_model_file_texture(self, texname, res);
							key = ds_map_find_next((IntType)(DsMap(map).Value(/*"textures"*/ STR(809))), key);
						}
			...
			VarType elementslist = DsMap(map).Value(/*"elements"*/ STR(810));
			if (!is_undefined(elementslist))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(elementslist)); i++)
				{
					withOne (obj_block_load_element, (new obj_block_load_element)->id, self->id)
					{
						...
						self->from = value_get_point3D({ DsMap(elementmap).Value(/*"from"*/ STR(811)) });
						self->to = value_get_point3D({ DsMap(elementmap).Value(/*"to"*/ STR(812)) });
						self->size = point3D_sub(self->to, self->from);
						self->volume = self->size.Value(X_) * self->size.Value(Y_) * self->size.Value(Z_);
						VarType rotationmap = DsMap(elementmap).Value(/*"rotation"*/ STR(813));
						if (ds_map_valid(rotationmap))
						{
							VarType origin, angle;
							VecType rot, scale;
							origin = value_get_point3D({ DsMap(rotationmap).Value(/*"origin"*/ STR(814)), point3D(IntType(8), IntType(8), IntType(8)) });
							angle = IntType(0);
							rot = vec3(IntType(0));
							scale = vec3(IntType(1));
							if (is_real(DsMap(rotationmap).Value(/*"angle"*/ STR(815))))
								angle = snap(clamp(DsMap(rotationmap).Value(/*"angle"*/ STR(815)), -IntType(45), IntType(45)), 22.5);
							if (is_bool(DsMap(rotationmap).Value(/*"rescale"*/ STR(816))) && DsMap(rotationmap).Value(/*"rescale"*/ STR(816)) > 0)
								scale = vec3(1.0 / dcos(abs(angle)));
							if (is_string(DsMap(rotationmap).Value(/*"axis"*/ STR(799))))
							{
								switch (StringType(DsMap(rotationmap).Value(/*"axis"*/ STR(799))).id)
								{
									case 817: // x
									{
										rot.Real(X_) = angle;
						...
						VarType facesmap = DsMap(elementmap).Value(/*"faces"*/ STR(819));
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							if (!is_undefined(curmap))
							{
								...
								if (ds_list_valid(DsMap(curmap).Value(/*"uv"*/ STR(820))))
								{
									VarType uvlist = DsMap(curmap).Value(/*"uv"*/ STR(820));
								...
								self->face_texture[f] = DsMap(curmap).Value(/*"texture"*/ STR(531));
								self->face_rotation[f] = IntType(0);
								if (is_real(DsMap(curmap).Value(/*"rotation"*/ STR(813))))
									self->face_rotation[f] = DsMap(curmap).Value(/*"rotation"*/ STR(813));
							}
							else
