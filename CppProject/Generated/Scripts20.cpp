...
namespace CppProject
{
		...
		withOne (obj_timeline, (new obj_timeline)->id, self->id)
		{
			...
			else
			{
				...
				if (is_string(idVar(self.otherId, variant)))
					state_vars_add(VarType::CreateRef(self->block_state), ArrType::From({ /*"variant"*/ STR(824), idVar(self.otherId, variant) }));
				tl_update_scenery_part(ScopeAny(self));
			...
			if (idVar(self.otherId, pattern_type) != /*""*/ STR(0))
			{
				...
				if (self->pattern_type == /*"banner"*/ STR(825))
				{
					...
					ObjType(obj_timeline, banner)->pattern_type = /*"banner"*/ STR(825);
				}
	...
	RealType block_generate_liquid(ScopeAny self, RealType waterlogged)
	{
		...
		if (idVar(sInt(block_current), name) == /*"water"*/ STR(826))
		...
		slot = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_liquid_slot_map).Value(/*"flowing_"*/ STR(827) + idVar(sInt(block_current), name));
	...
	IntType block_get_timeline(Scope<obj_builder_thread> self, VarType block, VarType stateid)
	{
		...
		withOne (obj_block_tl, (new obj_block_tl)->id, self->id)
		{
			...
			self->variant = block_get_state_id_value(block, stateid, /*"variant"*/ STR(824));
			return self->id;
		}
	...
	IntType block_load(VarType map, IntType typemap)
	{
		withOne (obj_block, (new obj_block)->id, noone)
		{
			...
			if (dev_mode_debug_names && !text_exists(/*"block"*/ STR(516) + self->name))
				log({ /*"block/"*/ STR(830) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"type"*/ STR(814))))
			{
				self->type = DsMap(map).Value(/*"type"*/ STR(814));
				self->set_script = asset_get_index(/*"block_set_"*/ STR(831) + self->type);
				self->generate_script = asset_get_index(/*"block_generate_"*/ STR(832) + self->type);
			}
			else
			{
				self->type = /*""*/ STR(0);
			...
			if (is_real(DsMap(map).Value(/*"emissive"*/ STR(833))))
				self->emissive = DsMap(map).Value(/*"emissive"*/ STR(833));
			else
				self->emissive = IntType(0);
			
			if (is_bool(DsMap(map).Value(/*"random_offset"*/ STR(834))))
				self->random_offset = DsMap(map).Value(/*"random_offset"*/ STR(834));
			else
				self->random_offset = false;
			
			if (is_bool(DsMap(map).Value(/*"random_offset_xz"*/ STR(835))))
				self->random_offset_xy = DsMap(map).Value(/*"random_offset_xz"*/ STR(835));
			else
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(836))))
			{
				...
				VarType curstate;
				curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(836))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"blockstate"*/ STR(804) + curstate))
						log({ /*"block/state/"*/ STR(837) + curstate + dev_mode_name_translation_message });
					withOne (obj_block_state, (new obj_block_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (ds_map_valid(curvalue))
							{
								...
								if (is_real(DsMap(curvalue).Value(/*"emissive"*/ STR(833))))
									self->value_emissive[v] = DsMap(curvalue).Value(/*"emissive"*/ STR(833));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset"*/ STR(834))))
									self->value_random_offset[v] = DsMap(curvalue).Value(/*"random_offset"*/ STR(834));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(835))))
									self->value_random_offset_xy[v] = DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(835));
							}
							DsMap(self->value_map)[self->value_name.Value(v)] = v;
							if (dev_mode_debug_names && string_length(self->value_name.Value(v)) > IntType(3) && !text_exists(/*"blockstatevalue"*/ STR(805) + self->value_name.Value(v)))
								log({ /*"block/state/value/"*/ STR(839) + self->value_name.Value(v) + dev_mode_name_translation_message });
						}
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(843))))
					self->wind_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(843));
			}
			if (is_bool(DsMap(map).Value(/*"waterlogged"*/ STR(844))))
				self->waterlogged = DsMap(map).Value(/*"waterlogged"*/ STR(844));
			else
				self->waterlogged = false;
			
			if (is_bool(DsMap(map).Value(/*"require_models"*/ STR(845))))
				self->require_models = DsMap(map).Value(/*"require_models"*/ STR(845));
			else
			...
			if (is_bool(DsMap(map).Value(/*"multithreaded"*/ STR(846))))
				self->multithreaded = DsMap(map).Value(/*"multithreaded"*/ STR(846));
			else
			...
			if (is_string(idmap))
			{
				self->mc_ids = ArrType::From({ string_replace(idmap, /*"minecraft:"*/ STR(847), /*""*/ STR(0)) });
				...
				if (ds_map_valid(idmap))
				{
					...
					while (!is_undefined(key))
					{
						array_add(VarType::CreateRef(self->mc_ids), string_replace(key, /*"minecraft:"*/ STR(847), /*""*/ STR(0)));
