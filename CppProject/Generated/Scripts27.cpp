...
namespace CppProject
{
	...
	RealType draw_button_transition(ScopeAny self, VarType xx, VarType yy, VarType transition)
	{
		...
		tipname = transition;
		if (tipname != /*"linear"*/ STR(543) && tipname != /*"instant"*/ STR(544) && tipname != /*"bezier"*/ STR(545))
		{
			tipname = string_replace(tipname, /*"easeinout"*/ STR(1093), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easein"*/ STR(1094), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easeout"*/ STR(1095), /*""*/ STR(0));
			tipname = /*"ease"*/ STR(1096) + tipname;
		}
		if (transition == /*"bezier"*/ STR(545) && !(sReal(setting_advanced_mode) > 0))
			return IntType(0);
		return draw_button_icon(self, { /*"menu"*/ STR(1091) + transition, xx, yy, IntType(36), IntType(36), active, null_, null_, false, /*"transition"*/ STR(1097) + tipname, DsMap(sVar(transition_texture_map)).Value(transition) });
	}
	...
	RealType draw_dragger(ScopeAny self, StringType name, VarType xx, VarType yy, RealType wid, VarType value, RealType mul, RealType minval, VarType maxval, VarType def, RealType snapval, VarType tbx, IntType script, VarType capwidth, BoolType showcaption, BoolType disabled, StringType tip)
	{
		...
		if (!disabled)
			context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1090), value, e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
		...
		if (dragmouseon && sBool(mouse_left_pressed))
			sVar(window_focus) = name + /*"press"*/ STR(1098);
		if (sVar(window_focus) == name + /*"press"*/ STR(1098))
		{
		...
		if (sVar(window_busy) != name + /*"drag"*/ STR(30) && sVar(window_busy) != name + /*"press"*/ STR(1098) && sVar(window_focus) != string(tbx))
			idVar(tbx, text) = string_decimals(value);
		return 0.0;
	}
	...
	RealType draw_dragger_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1090), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
	...
	RealType draw_inputbox(ScopeAny self, VarArgs argument)
	{
		...
		if (xx + w < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || yy + h < sVar(content_y) || yy > sVar(content_y) + sVar(content_height))
			return IntType(0);
		BoolType mouseon;
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == string(tbx) + /*"click"*/ STR(1099)) && !(disabled > 0);
		...
		if (string_contains(inputname, /*"search"*/ STR(1100)))
		{
	...
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		name = /*"settingskey"*/ STR(1102) + idVar(keyobj, name);
		w = sVar(dw);
		...
		context_menu_area(self, { xx, yy, w, sReal(tab_control_h), /*"keybind"*/ STR(1103), keybindID, null_, null_, null_ });
		...
		draw_label(self, { text_get({ name }) + /*":"*/ STR(796), sVar(dx), sVar(dy) + (sReal(tab_control_h) / 2.0), fa_left, fa_middle, textcolor, textalpha, sInt(font_label) });
		...
		if (draw_button_icon(self, { name + /*"edit"*/ STR(116), sVar(dx) + w - IntType(24), sVar(dy) + IntType(2), IntType(24), IntType(24), sVar(window_busy) == name, icons_PENCIL, null_, false, /*"tooltipeditkeybind"*/ STR(1104) }))
		{
	...
	RealType draw_meter(ScopeAny self, StringType name, VarType xx, VarType yy, VarType wid, VarType value, RealType minval, RealType maxval, RealType def, RealType snapval, VarType tbx, IntType script, StringType tip)
	{
		...
		draw_dragger(self, name + /*"input"*/ STR(1105), (xx + wid - draggerwid) + ((IntType)!(compact > 0) * IntType(6)), yy, draggerwid, value, snapval * .1, minval, maxval, def, snapval, tbx, script, null_, false);
		inputfocus = global::microani_arr.Value(e_microani_ACTIVE);
		if (sVar(window_busy) == name || (sVar(window_busy) != name + /*"inputpress"*/ STR(1106) && sVar(window_focus) != string(tbx)))
			ObjType(obj_textbox, tbx)->text = string_decimals(value);
		microani_set(name, script, (sVar(window_busy) == name) || slidermouseon, slidermouseon && sBool(mouse_left), false, false, IntType(1), false);
		microani_update({ slidermouseon, slidermouseon && sBool(mouse_left), sVar(window_busy) == name, locked, mouseon || (sVar(window_busy) == name + /*"inputpress"*/ STR(1106) || sVar(window_focus) == string(tbx)) });
		IntType labelcolor;
		RealType labelalpha;
		labelcolor = merge_color((IntType)(global::c_text_secondary), (IntType)(global::c_text_main), global::microani_arr.Value(e_microani_HOVER));
		...
		context_menu_area(self, { linex, yy, linewid, thumbhei, /*"contextmenuvalue"*/ STR(1090), value, e_context_type_NUMBER, script, def });
		if (slidermouseon)
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (sBool(mouse_left_pressed))
	...
	RealType draw_meter_range(ScopeAny self, StringType name, VarType xx, VarType yy, RealType wid, RealType minrange, RealType maxrange, RealType snapval, VarType minval, VarType maxval, RealType mindef, RealType maxdef, IntType mintbx, IntType maxtbx, IntType minscript, IntType maxscript)
	{
		...
		textfield_group_add(self, { name + /*"mininput"*/ STR(1107), minval, mindef, minscript, X_, mintbx, null_, 0.1, minrange, min({ maxval, maxrange }) });
		textfield_group_add(self, { name + /*"maxinput"*/ STR(1108), maxval, maxdef, maxscript, X_, maxtbx, null_, 0.1, max({ minval, minrange }), maxrange });
		...
		microani_set(name, null_, sVar(window_busy) == name + /*"min"*/ STR(1109) || sVar(window_busy) == name + /*"max"*/ STR(1110) || slidermouseon, slidermouseon && sBool(mouse_left), false, false, IntType(1), false);
		microani_update({ sVar(window_busy) == name + /*"min"*/ STR(1109) || sVar(window_busy) == name + /*"max"*/ STR(1110) || slidermouseon, slidermouseon && sBool(mouse_left), sVar(window_busy) == name + /*"min"*/ STR(1109) || sVar(window_busy) == name + /*"max"*/ STR(1110), false, mouseon || textfocus > 0 });
		IntType labelcolor;
		RealType labelalpha;
		labelcolor = merge_color((IntType)(global::c_text_secondary), (IntType)(global::c_text_main), global::microani_arr.Value(e_microani_HOVER));
		...
		minthumbpos = (sVar(window_busy) == name + /*"min"*/ STR(1109) ? sVar(meter_drag_value) : minval);
		minthumbpos = trackx + floor(percent(minthumbpos, minrange, maxrange) * trackwid) - IntType(6);
		maxthumbpos = (sVar(window_busy) == name + /*"max"*/ STR(1110) ? sVar(meter_drag_value) : maxval);
		...
		if (slidermouseon && sVar(window_busy) == /*""*/ STR(0))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				if (minmouseon)
				{
					sVar(window_busy) = name + /*"min"*/ STR(1109);
					sVar(meter_drag_value) = minval;
				}
				else
					if (maxmouseon)
					{
						sVar(window_busy) = name + /*"max"*/ STR(1110);
		...
		if (sVar(window_busy) == name + /*"min"*/ STR(1109) || sVar(window_busy) == name + /*"max"*/ STR(1110))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (global::_app->setting_unlimited_values > 0)
			...
			if (sVar(window_busy) == name + /*"min"*/ STR(1109))
			{
		...
		if (sVar(window_busy) == name + /*"max"*/ STR(1110))
		...
		microani_set(name + /*"min"*/ STR(1109), minscript, (sVar(window_busy) == name + /*"min"*/ STR(1109)) || minmouseon, minmouseon && sBool(mouse_left), false);
		microani_update({ (sVar(window_busy) == name + /*"min"*/ STR(1109)) || minmouseon, minmouseon && sBool(mouse_left), false });
		color = merge_color((IntType)(global::c_accent), global::c_accent_hover, global::microani_arr.Value(e_microani_HOVER));
		color = merge_color(color, global::c_accent_pressed, max({ global::microani_arr.Value(e_microani_ACTIVE), global::microani_arr.Value(e_microani_PRESS) }));
		color = merge_color(color, (IntType)(global::c_text_tertiary), global::microani_arr.Value(e_microani_DISABLED));
		alpha = lerp(1.0, global::a_text_tertiary, global::microani_arr.Value(e_microani_DISABLED));
		...
		microani_set(name + /*"max"*/ STR(1110), maxscript, (sVar(window_busy) == name + /*"max"*/ STR(1110)) || maxmouseon, maxmouseon && sBool(mouse_left), false);
		microani_update({ (sVar(window_busy) == name + /*"max"*/ STR(1110)) || maxmouseon, maxmouseon && sBool(mouse_left), false });
		color = merge_color((IntType)(global::c_accent), global::c_accent_hover, global::microani_arr.Value(e_microani_HOVER));
		color = merge_color(color, global::c_accent_pressed, max({ global::microani_arr.Value(e_microani_ACTIVE), global::microani_arr.Value(e_microani_PRESS) }));
		color = merge_color(color, (IntType)(global::c_text_tertiary), global::microani_arr.Value(e_microani_DISABLED));
		alpha = lerp(1.0, global::a_text_tertiary, global::microani_arr.Value(e_microani_DISABLED));
