...
namespace CppProject
{
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		name = /*"settingskey"*/ STR(1054) + idVar(keyobj, name);
		w = sVar(dw);
		...
		context_menu_area(self, { xx, yy, w, sVar(tab_control_h), /*"keybind"*/ STR(1055), keybindID, null_, null_, null_ });
		...
		if (draw_button_icon(self, { name + /*"edit"*/ STR(116), sVar(dx) + w - IntType(24), sVar(dy) + IntType(2), IntType(24), IntType(24), sVar(window_busy) == name, icons_PENCIL, null_, false, /*"tooltipeditkeybind"*/ STR(1056) }))
		{
	...
	RealType draw_meter(ScopeAny self, StringType name, VarType xx, VarType yy, VarType wid, VarType value, RealType minval, RealType maxval, RealType def, RealType snapval, VarType tbx, IntType script, StringType tip)
	{
		...
		draw_dragger(self, { name + /*"input"*/ STR(1057), (xx + wid - draggerwid) + ((IntType)!(compact > 0) * IntType(6)), yy, draggerwid, value, snapval * .1, minval, maxval, def, snapval, tbx, script, null_, false });
		inputfocus = global::microani_arr.Value(e_microani_ACTIVE);
		if (sVar(window_busy) == name || (sVar(window_busy) != name + /*"inputpress"*/ STR(1058) && sVar(window_focus) != string(tbx)))
		...
		microani_update({ slidermouseon, slidermouseon && sBool(mouse_left), sVar(window_busy) == name, locked, mouseon || (sVar(window_busy) == name + /*"inputpress"*/ STR(1058) || sVar(window_focus) == string(tbx)) });
		IntType labelcolor;
		RealType labelalpha;
		...
		context_menu_area(self, { linex, yy, linewid, thumbhei, /*"contextmenuvalue"*/ STR(1042), value, e_context_type_NUMBER, script, def });
		if (slidermouseon)
		{
	...
	RealType draw_meter_range(ScopeAny self, StringType name, VarType xx, VarType yy, RealType wid, RealType minrange, RealType maxrange, RealType snapval, VarType minval, VarType maxval, RealType mindef, RealType maxdef, IntType mintbx, IntType maxtbx, IntType minscript, IntType maxscript)
	{
		...
		textfield_group_add(self, { name + /*"mininput"*/ STR(1059), minval, mindef, minscript, X_, mintbx, null_, 0.1, minrange, min({ maxval, maxrange }) });
		textfield_group_add(self, { name + /*"maxinput"*/ STR(1060), maxval, maxdef, maxscript, X_, maxtbx, null_, 0.1, max({ minval, minrange }), maxrange });
		...
		microani_set(name, null_, sVar(window_busy) == name + /*"min"*/ STR(1061) || sVar(window_busy) == name + /*"max"*/ STR(1062) || slidermouseon, slidermouseon && sBool(mouse_left), false, false, IntType(1), false);
		microani_update({ sVar(window_busy) == name + /*"min"*/ STR(1061) || sVar(window_busy) == name + /*"max"*/ STR(1062) || slidermouseon, slidermouseon && sBool(mouse_left), sVar(window_busy) == name + /*"min"*/ STR(1061) || sVar(window_busy) == name + /*"max"*/ STR(1062), false, mouseon || textfocus > 0 });
		IntType labelcolor;
		RealType labelalpha;
		labelcolor = merge_color((IntType)(global::c_text_secondary), (IntType)(global::c_text_main), global::microani_arr.Value(e_microani_HOVER));
		...
		minthumbpos = (sVar(window_busy) == name + /*"min"*/ STR(1061) ? sVar(meter_drag_value) : minval);
		minthumbpos = trackx + floor(percent(minthumbpos, minrange, maxrange) * trackwid) - IntType(6);
		maxthumbpos = (sVar(window_busy) == name + /*"max"*/ STR(1062) ? sVar(meter_drag_value) : maxval);
		...
		if (slidermouseon && sVar(window_busy) == /*""*/ STR(0))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				if (minmouseon)
				{
					sVar(window_busy) = name + /*"min"*/ STR(1061);
					sVar(meter_drag_value) = minval;
				}
				else
					if (maxmouseon)
					{
						sVar(window_busy) = name + /*"max"*/ STR(1062);
		...
		if (sVar(window_busy) == name + /*"min"*/ STR(1061) || sVar(window_busy) == name + /*"max"*/ STR(1062))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (global::_app->setting_unlimited_values > 0)
			...
			if (sVar(window_busy) == name + /*"min"*/ STR(1061))
			{
		...
		if (sVar(window_busy) == name + /*"max"*/ STR(1062))
		...
		microani_set(name + /*"min"*/ STR(1061), minscript, (sVar(window_busy) == name + /*"min"*/ STR(1061)) || minmouseon, minmouseon && sBool(mouse_left), false);
		microani_update({ (sVar(window_busy) == name + /*"min"*/ STR(1061)) || minmouseon, minmouseon && sBool(mouse_left), false });
		color = merge_color((IntType)(global::c_accent), global::c_accent_hover, global::microani_arr.Value(e_microani_HOVER));
		color = merge_color(color, global::c_accent_pressed, max({ global::microani_arr.Value(e_microani_ACTIVE), global::microani_arr.Value(e_microani_PRESS) }));
		color = merge_color(color, (IntType)(global::c_text_tertiary), global::microani_arr.Value(e_microani_DISABLED));
		alpha = lerp(1.0, global::a_text_tertiary, global::microani_arr.Value(e_microani_DISABLED));
		...
		microani_set(name + /*"max"*/ STR(1062), maxscript, (sVar(window_busy) == name + /*"max"*/ STR(1062)) || maxmouseon, maxmouseon && sBool(mouse_left), false);
		microani_update({ (sVar(window_busy) == name + /*"max"*/ STR(1062)) || maxmouseon, maxmouseon && sBool(mouse_left), false });
		color = merge_color((IntType)(global::c_accent), global::c_accent_hover, global::microani_arr.Value(e_microani_HOVER));
		color = merge_color(color, global::c_accent_pressed, max({ global::microani_arr.Value(e_microani_ACTIVE), global::microani_arr.Value(e_microani_PRESS) }));
		color = merge_color(color, (IntType)(global::c_text_tertiary), global::microani_arr.Value(e_microani_DISABLED));
		alpha = lerp(1.0, global::a_text_tertiary, global::microani_arr.Value(e_microani_DISABLED));
	...
	RealType draw_recent(ScopeAny self, VarArgs argument)
	{
		...
		if (mode == /*"simple"*/ STR(1063))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				...
				if (hover)
				{
					if (draw_button_icon(self, { /*"recentdelete"*/ STR(1064) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1065) }))
				...
				microani_set(/*"recent"*/ STR(1066) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
		...
		if (mode == /*"list"*/ STR(1067))
		{
			...
			draw_label(self, { text_get({ /*"recentfilename"*/ STR(1068) }), namex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			draw_label(self, { text_get({ /*"recentlastopened"*/ STR(1069) }), timex, recenty + IntType(14), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
			recenty += IntType(28);
			draw_set_font(sInt(font_value));
			for (VarType i = liststart; i < sReal(recent_list_amount); i++)
			{
				...
				{
					if (draw_button_icon(self, { /*"recentpin"*/ STR(1070) + string(item), iconx, recenty + IntType(8), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1071) }))
		...
		if (mode == /*"grid"*/ STR(613))
		{
			...
			for (IntType i = IntType(0); i < sReal(recent_list_amount); i++)
			{
				if (cardy < yy + hei || cardy + IntType(240) + IntType(16) > yy)
				{
					...
					if (hover || idVar(item, pinned) > 0)
					{
						if (draw_button_icon(self, { /*"recentpin"*/ STR(1070) + string(item), iconx, cardy + IntType(12), IntType(24), IntType(24), idVar(item, pinned), icons_PIN, null_, false, /*"tooltippin"*/ STR(1071), null_ }))
							action_recent_pin(item);
					}
					iconx -= 28.0;
					if (hover)
					{
						if (draw_button_icon(self, { /*"recentdelete"*/ STR(1064), iconx, cardy + IntType(12), IntType(24), IntType(24), false, icons_DELETE, null_, false, /*"tooltipremove"*/ STR(1065), null_ }))
							action_recent_remove(item);
					}
					microani_set(/*"recent"*/ STR(1066) + string(item), null_, mouseon, mouseon && sBool(mouse_left), false);
	...
	void draw_subheader(ScopeAny self, VarType cat, VarType xx, VarType yy, RealType w, RealType h)
	{
		...
		microani_set(ObjType(obj_category, cat)->name + /*"close"*/ STR(1072), null_, false, false, ObjType(obj_category, cat)->show, false);
		IntType color;
