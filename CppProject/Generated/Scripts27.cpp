...
namespace CppProject
{
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1049))) + /*"/"*/ STR(20);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(871);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(872);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(888);
	...
	BoolType draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, VarType def, BoolType hsvmode, IntType script)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		click = context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1055), color, e_context_type_COLOR, script, def });
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(745), name);
			name = /*"modelstate"*/ STR(745) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(766), name);
				name = /*"blockstate"*/ STR(766) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(720);
		...
		draw_box_hover({ xx, yy, wid, hei, global::microani_arr.Value(e_microani_PRESS) });
		mouseon = (app_mouse_box(self, xx, yy, wid, hei) && !(disabled > 0) && sBool(content_mouseon)) || (menuactive && app_mouse_box(self, xx, yy, wid, hei, /*"menu"*/ STR(1056)));
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (showsearch)
		{
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1056))
			{
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			sVar(window_focus) = string(sInt(menu_search_tbx));
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(721);
			app_mouse_clear(self);
		}
		if (!menuactive && mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_value = value;
			ObjType(obj_menu, m)->menu_ani = IntType(0);
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(721);
