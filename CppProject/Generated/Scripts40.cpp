...
namespace CppProject
{
	...
	void popup_about_draw(ScopeAny self)
	{
		...
		VarType version = text_get({ /*"aboutversion"*/ STR(1984), mineimator_version_full }) + ((global::trial_version > 0) ? /*" "*/ STR(17) + text_get({ /*"startuptrial"*/ STR(1986) }) : /*""*/ STR(0));
		...
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutcreatedby"*/ STR(1995) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_button_text(self, { /*"David Andrei"*/ STR(1996), sVar(dx), sVar(dy), ID_popup_open_url, link_david, link_david, sInt(font_label) });
		sVar(dy) += IntType(34);
		...
		sVar(dy) += IntType(34);
		draw_label(self, { text_get({ /*"aboutuibranding"*/ STR(2002) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_label(self, { /*"Voxy"*/ STR(2003), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dx) += IntType(130) + IntType(24);
		sVar(dy) = sVar(content_y) + IntType(176);
		ArrType list = ArrType::From({ /*"9redwoods"*/ STR(2004), /*"Alpha Toostrr"*/ STR(2005), /*"AnxiousCynic"*/ STR(2006), /*"Cade [CaZaKoJa]"*/ STR(2007), /*"Hozq"*/ STR(2008), /*"Jnick"*/ STR(2009), /*"Jossamations"*/ STR(2010), /*"KeepOnChucking"*/ STR(2011), /*"Rollo"*/ STR(2012), /*"SKIBBZ"*/ STR(2013), /*"SoundsDotZip"*/ STR(2014), /*"UpgradedMoon"*/ STR(2015), /*"Vash"*/ STR(2016), /*"__Mine__"*/ STR(2017) });
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutbetatesting"*/ STR(2018) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"advancedinfo"*/ STR(2019) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		IntType buttonx = string_width(text_get({ /*"advancedenable"*/ STR(2020) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"advancedenable"*/ STR(2020), sVar(dx) + sVar(dw) - buttonx, sVar(dy)))
		{
			action_setting_program_mode(self, true);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"advancednotnow"*/ STR(2021) })) + button_padding);
		if (draw_button_label(self, /*"advancednotnow"*/ STR(2021), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
			popup_close(self);
		tab_next(self);
	}
	...
	VarType popup_armor_editor_draw_piece_type(ScopeAny self, StringType piece, IntType pieceid, RealType capwid)
	{
		...
		for (IntType i = IntType(0); i < statelen; i += IntType(2))
		{
			...
			VarType model = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->model_name_map).Value(/*"armor"*/ STR(789));
			if (state != piece)
				continue;
			...
			if (type == /*"leather"*/ STR(1824))
			{
				w -= (28.0 + 8.0);
				if (draw_button_color(self, /*"armoreditordye"*/ STR(2022) + piece, sVar(dx) + sVar(dw) - IntType(24), sVar(dy), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(1)), hex_to_color(/*"#A06540"*/ STR(2023)), false, ID_action_armor_editor))
				{
			...
			if (idVar(sInt(popup), armor_edit) == sInt(bench_settings))
				draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), w, IntType(24), type, minecraft_asset_get_name(/*"modelstatevalue"*/ STR(786), type), ID_action_bench_model_state, false, null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
			else
				draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), w, IntType(24), type, minecraft_asset_get_name(/*"modelstatevalue"*/ STR(786), type), (idVar(idVar(sInt(popup), armor_edit), type) == e_temp_type_BODYPART) ? ID_action_lib_bodypart_model_state : ID_action_lib_model_state, false, null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
	...
	void popup_armor_editor_draw_piece(ScopeAny self, StringType piece, IntType pieceid, RealType capwid)
	{
		...
		if (sVar(context_menu_name) != /*""*/ STR(0))
			sVar(context_menu_busy_prev) = /*"popup"*/ STR(1623) + idVar(sInt(popup), name);
		if (ds_list_size(sInt(menu_list)) > IntType(0))
			ObjType(obj_menu, DsList(sInt(menu_list))[IntType(0)])->menu_busy_prev = /*"popup"*/ STR(1623) + idVar(sInt(popup), name);
		...
		tab_control(self, IntType(24));
		draw_button_menu(self, { /*"armoreditorpattern"*/ STR(1529) + piece, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)), text_get({ /*"armoreditorpattern"*/ STR(1529) + idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)) }), ID_action_armor_editor, piecetype == /*"none"*/ STR(917), null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
		tab_next(self);
		ObjType(obj_popup, sInt(popup_armor_editor))->piece_data_id = IntType(3);
		tab_control(self, IntType(24));
		draw_button_menu(self, { /*"armoreditormaterial"*/ STR(1534) + piece, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(3)), text_get({ /*"armoreditormaterial"*/ STR(1534) + idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(3)) }), ID_action_armor_editor, (piecetype == /*"none"*/ STR(917) || idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)) == /*"none"*/ STR(917)), null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
		tab_next(self);
	}
	...
	void popup_armor_editor_draw(ScopeAny self)
	{
		...
		popup_armor_editor_draw_piece(self, /*"helmet"*/ STR(2024), IntType(0), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"chestplate"*/ STR(2025), IntType(4), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"leggings"*/ STR(2026), IntType(8), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"boots"*/ STR(2027), IntType(12), capwid);
	}
	...
	void popup_downloadskin_draw(ScopeAny self)
	{
		...
		draw_textfield(self, /*"downloadskinusername"*/ STR(2028), sVar(dx), sVar(dy), sVar(dw) - IntType(28), IntType(24), idInt(sInt(popup), tbx_username), null_);
		BoolType download = keyboard_check_pressed(vk_enter);
		if (draw_button_icon(self, { /*"downloadskindownload"*/ STR(2029), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(18), IntType(24), IntType(24), false, icons_DOWNLOAD, null_, ObjType(obj_textbox, idInt(sInt(popup), tbx_username))->text == /*""*/ STR(0) || sInt(http_downloadskin) != null_, /*"tooltipdownloadskin"*/ STR(2030) }))
			download = true;
		...
		{
			draw_label(self, { text_get({ /*"downloadskindownloading"*/ STR(2031) }), sVar(dx), sVar(dy) + IntType(8), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"downloadskindone"*/ STR(2032), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT, !(idVar(sInt(popup), texture) > 0)))
		{
			popup_close(self);
	...
	RealType popup_draw(ScopeAny self)
	{
		...
		if (sStr(popup_ani_type) == /*"show"*/ STR(759))
		{
			...
			if (sStr(popup_ani_type) == /*"hide"*/ STR(758))
			{
		...
		if (!(sInt(popup) > 0) || (sStr(popup_ani_type) == /*"hide"*/ STR(758) && sInt(popup_switch_to) == null_))
		{
			sVar(popup_block_ani) = max({ IntType(0), test_reduced_motion(IntType(0), (sVar(popup_block_ani) - 0.075 * global::delta)) });
			sStr(popup_block_ani_ease) = /*"easeincirc"*/ STR(564);
			sBool(popup_mouseon) = false;
			if (!(sInt(popup) > 0))
				return IntType(0);
		}
		else
			if (idVar(sInt(popup), block) > 0 || (sInt(popup_switch_to) != null_ && idVar(sInt(popup_switch_to), block) > 0 && sStr(popup_ani_type) == /*"hide"*/ STR(758)))
			{
		...
		draw_set_alpha(ease(/*"easeoutcirc"*/ STR(565), sVar(popup_ani)));
		if (sVar(window_busy) == /*"popupmove"*/ STR(2033))
			draw_set_alpha(0.5);
		if (sVar(window_busy) == /*"popup"*/ STR(1623) + idVar(sInt(popup), name))
			sVar(window_busy) = /*""*/ STR(0);
		...
		if (sVar(window_busy) == /*"popupclick"*/ STR(2034))
		{
			if (sReal(mouse_move) > IntType(10))
			{
				...
				sVar(window_busy) = /*"popupmove"*/ STR(2033);
			}
			else
				if (!sBool(mouse_left))
					sVar(window_busy) = /*""*/ STR(0);
		...
		if (idVar(sInt(popup), close_button) > 0)
		{
			...
			if (draw_button_icon(self, { idVar(sInt(popup), name) + /*"close"*/ STR(1120), closex, closey, IntType(24), IntType(24), false, (revert > 0) ? icons_ARROW_LEFT : icons_CLOSE, null_, false }))
			{
				if (revert > 0)
		...
		if (sBool(popup_mouseon) && sInt(mouse_cursor) == cr_default && sBool(mouse_left_pressed))
			sVar(window_busy) = /*"popupclick"*/ STR(2034);
	...
	void popup_exportimage_draw(ScopeAny self)
	{
		...
		draw_button_menu(self, { /*"exportimageimagesize"*/ STR(1467), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sVar(project_video_template), sVar(text), ID_action_project_video_template });
		tab_next(self);
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportimageimagesizecustomwidth"*/ STR(2035), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_image_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportimageimagesizecustomheight"*/ STR(2036), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_image_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, false);
			draw_textfield_group(self, /*"exportimageimagesizecustom"*/ STR(2037), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false);
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, /*"exportimageimagesizecustomkeepaspectratio"*/ STR(2038), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio);
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageremovebackground"*/ STR(2039), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportimage_remove_background);
		tab_next(self);
		if (idBool(sInt(popup), remove_background))
			draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(2040), icons_WARNING_TRIANGLE, e_toast_WARNING);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageincludehidden"*/ STR(2041), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportimage_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagehighquality"*/ STR(2042), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportimage_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagewatermark"*/ STR(2043), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportimage_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, /*"exportimagesave"*/ STR(2044), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportimage_save, e_anchor_RIGHT);
		tab_next(self);
	}
	...
	void popup_exportmovie_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
			...
			text = text_get({ /*"projectvideosizetemplate"*/ STR(1470) + ObjType(obj_videotemplate, sVar(project_video_template))->name }) + /*" ("*/ STR(1149) + string(ObjType(obj_videotemplate, sVar(project_video_template))->width) + /*"x"*/ STR(859) + string(ObjType(obj_videotemplate, sVar(project_video_template))->height) + /*")"*/ STR(1125);
		
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovievideosize"*/ STR(1466), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sVar(project_video_template), text, ID_action_project_video_template });
		tab_next(self);
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportmovievideosizecustomwidth"*/ STR(2045), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_video_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportmovievideosizecustomheight"*/ STR(2046), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_video_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, false);
			draw_textfield_group(self, /*"exportmovievideosizecustom"*/ STR(2047), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false);
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, /*"exportmovievideosizecustomkeepaspectratio"*/ STR(2048), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio);
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovieformat"*/ STR(1472), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idStr(sInt(popup), format), text_get({ /*"exportmovieformat"*/ STR(1472) + idStr(sInt(popup), format) }), ID_action_toolbar_exportmovie_format });
		tab_next(self);
		if (idStr(sInt(popup), format) != /*"png"*/ STR(60))
		{
			if (idInt(sInt(popup), video_quality) == IntType(0))
				text = text_get({ /*"exportmovievideoqualitycustom"*/ STR(1478) });
			else
				text = text_get({ /*"exportmovievideoquality"*/ STR(1477) + ObjType(obj_videoquality, idInt(sInt(popup), video_quality))->name });
			
			tab_control_menu(self);
			draw_button_menu(self, { /*"exportmovievideoquality"*/ STR(1477), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), video_quality), text, ID_action_toolbar_exportmovie_video_quality });
			tab_next(self);
			if (idInt(sInt(popup), video_quality) == IntType(0))
			{
				tab_control_dragger(self);
				draw_dragger(self, /*"exportmoviebitrate"*/ STR(2049), sVar(dx), sVar(dy), dragger_width, idReal(sInt(popup), bit_rate), IntType(500), IntType(1), no_limit, IntType(2500000), IntType(1), idInt(sInt(popup), tbx_bit_rate), ID_action_toolbar_exportmovie_bit_rate);
				tab_next(self);
			}
		}
		if (idVar(sInt(popup), frame_rate) == IntType(0))
			text = text_get({ /*"exportmovieframeratecustom"*/ STR(1483) });
		else
			text = string(idVar(sInt(popup), frame_rate));
		
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovieframerate"*/ STR(1479), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idVar(sInt(popup), frame_rate), text, ID_action_toolbar_exportmovie_frame_rate });
		tab_next(self);
		if (idVar(sInt(popup), frame_rate) == IntType(0))
		{
			tab_control_dragger(self);
			draw_dragger(self, /*"exportmovieframespersecond"*/ STR(2050), sVar(dx), sVar(dy), dragger_width, idReal(sInt(popup), framespersecond), IntType(1), IntType(1), IntType(120), IntType(30), IntType(1), idInt(sInt(popup), tbx_framespersecond), ID_action_toolbar_exportmovie_framespersecond);
			tab_next(self);
		}
		if (idStr(sInt(popup), format) == /*"png"*/ STR(60))
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieremovebackground"*/ STR(2051), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportmovie_remove_background);
			tab_next(self);
			if (idBool(sInt(popup), remove_background))
				draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(2040), icons_WARNING_TRIANGLE, e_toast_WARNING);
		}
		...
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieincludeaudio"*/ STR(2052), sVar(dx), sVar(dy), idBool(sInt(popup), include_audio), ID_action_toolbar_exportmovie_include_audio);
			tab_next(self);
		}
		
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmovieincludehidden"*/ STR(2053), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportmovie_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviehighquality"*/ STR(2054), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportmovie_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviewatermark"*/ STR(2055), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportmovie_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, /*"exportmoviesave"*/ STR(2056), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportmovie_save, e_anchor_RIGHT);
		tab_next(self);
	}
	
	void popup_importimage_draw(ScopeAny self)
	{
		draw_label(self, { text_get({ /*"importimagetype"*/ STR(2057) }) + /*":"*/ STR(796), sVar(dx), sVar(dy) + IntType(14), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(28);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageskin"*/ STR(2058), sVar(dx), sVar(dy), e_res_type_SKIN, idVar(sInt(popup), type) == e_res_type_SKIN, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageitemsheet"*/ STR(2059), sVar(dx), sVar(dy), e_res_type_ITEM_SHEET, idVar(sInt(popup), type) == e_res_type_ITEM_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageblocksheet"*/ STR(2060), sVar(dx), sVar(dy), e_res_type_BLOCK_SHEET, idVar(sInt(popup), type) == e_res_type_BLOCK_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageparticlesheet"*/ STR(2061), sVar(dx), sVar(dy), e_res_type_PARTICLE_SHEET, idVar(sInt(popup), type) == e_res_type_PARTICLE_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimagetexture"*/ STR(2062), sVar(dx), sVar(dy), e_res_type_TEXTURE, idVar(sInt(popup), type) == e_res_type_TEXTURE, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"importimageok"*/ STR(2063), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
	...
	void popup_importitemsheet_draw(ScopeAny self)
	{
		...
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, /*"importitemsheetissheet"*/ STR(2064), sVar(dx), sVar(dy), idBool(sInt(popup), is_sheet), ID_action_toolbar_importitemsheet_is_sheet);
		tab_next(self);
		if (idBool(sInt(popup), is_sheet))
		{
			...
			draw_dragger(self, /*"importitemsheetrows"*/ STR(2065), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(X_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(X_), IntType(1), idInt(sInt(popup), tbx_sheet_width), ID_action_toolbar_importitemsheet_sheet_size);
			tab_next(self);
			global::axis_edit = Y_;
			tab_control(self, IntType(28));
			draw_dragger(self, /*"importitemsheetcolumns"*/ STR(2066), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(Y_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(Y_), IntType(1), idInt(sInt(popup), tbx_sheet_height), ID_action_toolbar_importitemsheet_sheet_size);
			tab_next(self);
		}
		tab_control_button_label(self);
		if (draw_button_label(self, /*"importimageok"*/ STR(2063), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_loading_draw(ScopeAny self)
	{
		...
		if (idReal(sInt(popup), load_amount) > IntType(1))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), progress, text_get({ /*"loadingresources"*/ STR(2067) }), text_get({ /*"loadingpercent"*/ STR(2068), string(floor(progress * IntType(100))) }));
			tab_next(self);
		}
		tab_control_loading(self);
	...
	void popup_modelbench_draw(ScopeAny self)
	{
		...
		draw_label(self, { string_limit_ext(text_get({ /*"modelbenchinfo"*/ STR(2069) }), sVar(dw) + IntType(24), sVar(dh)), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy), fa_center, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(34) + IntType(30);
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"modelbenchdownload"*/ STR(2070) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"modelbenchdownload"*/ STR(2070), sVar(dx) + sVar(dw) - buttonx, sVar(dy)))
		{
			open_url(link_modelbench);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"modelbenchnotnow"*/ STR(2071) })) + button_padding);
		if (draw_button_label(self, /*"modelbenchnotnow"*/ STR(2071), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
			popup_close(self);
		if (draw_checkbox(self, /*"modelbenchdontshow"*/ STR(2072), sVar(dx), sVar(dy) + IntType(4), ObjType(obj_popup, sInt(popup_modelbench))->hidden, null_))
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_newproject))->tbx_name)->text = text_get({ /*"newprojectnamedefault"*/ STR(2073) });
	...
	void popup_newproject_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2074), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, idVar(sInt(popup), folder), /*"top"*/ STR(188)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"newprojectnamedefault"*/ STR(2073) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2079), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, false, /*"tooltipchangefolder"*/ STR(2080) }))
		{
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"newprojectcreate"*/ STR(2081), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
	...
	void popup_pattern_editor_draw(ScopeAny self)
	{
		...
		if (draw_button_label(self, /*"patterneditoraddlayer"*/ STR(2082), sVar(dx), sVar(dy), listw, icons_PLUS, e_button_SECONDARY))
		{
		...
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"patterneditordone"*/ STR(2083) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"patterneditordone"*/ STR(2083), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy)))
		{
		...
		buttonx += IntType(12) + (string_width(text_get({ /*"patterneditorcancel"*/ STR(2084) })) + button_padding);
		if (draw_button_label(self, /*"patterneditorcancel"*/ STR(2084), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
		{
			array_add(VarType::CreateRef(global::pattern_update), idVar(sInt(popup), pattern_edit));
			instance_destroy(self, { idInt(sInt(popup), pattern_edit_preview), false });
			idInt(sInt(popup), pattern_edit_preview) = null_;
