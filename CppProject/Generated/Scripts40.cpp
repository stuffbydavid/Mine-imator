...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(7))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1948));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*".miparticles;*.particles"*/ STR(1948));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1949));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		{
			log({ /*"Opening particles"*/ STR(1950), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1951), fn });
	...
	RealType particles_save(ScopeAny self)
	{
		...
		log({ /*"Saving particles"*/ STR(1953), fn });
