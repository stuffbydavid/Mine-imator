...
namespace CppProject
{
	...
	void popup_about_draw(ScopeAny self)
	{
		...
		draw_button_text(self, { text_get({ /*"aboutminecraft"*/ STR(1916) }), mctextx, sVar(dy) + IntType(98) + IntType(19), ID_popup_open_url, /*"https://www.minecraft.net"*/ STR(1620), /*"https://www.minecraft.net"*/ STR(1620) });
		...
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutcreatedby"*/ STR(1923) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_button_text(self, { /*"David Andrei"*/ STR(1924), sVar(dx), sVar(dy), ID_popup_open_url, /*"https://www.stuffbydavid.com"*/ STR(1621), /*"https://www.stuffbydavid.com"*/ STR(1621), sInt(font_label) });
		sVar(dy) += IntType(34);
		...
		sVar(dy) += IntType(26);
		draw_button_text(self, { /*"David"*/ STR(1926), sVar(dx), sVar(dy), ID_popup_open_url, /*"https://www.stuffbydavid.com"*/ STR(1621), /*"https://www.stuffbydavid.com"*/ STR(1621), sInt(font_label) });
		sVar(dy) += IntType(19);
		...
		sVar(dy) += IntType(34);
		draw_label(self, { text_get({ /*"aboutuibranding"*/ STR(1929) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_label(self, { /*"Voxy"*/ STR(1930), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dx) += IntType(130) + IntType(24);
		sVar(dy) = sVar(content_y) + IntType(176);
		ArrType list = ArrType::From({ /*"9redwoods"*/ STR(1931), /*"Alpha Toostrr"*/ STR(1932), /*"AnxiousCynic"*/ STR(1933), /*"Cade [CaZaKoJa]"*/ STR(1934), /*"Hozq"*/ STR(1935), /*"Jnick"*/ STR(1936), /*"Jossamations"*/ STR(1937), /*"KeepOnChucking"*/ STR(1938), /*"mbanders"*/ STR(1939), /*"Rollo"*/ STR(1940), /*"SKIBBZ"*/ STR(1941), /*"SoundsDotZip"*/ STR(1942), /*"UpgradedMoon"*/ STR(1943), /*"Vash"*/ STR(1944), /*"__Mine__"*/ STR(1945) });
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutbetatesting"*/ STR(1946) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"advancedinfo"*/ STR(1947) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		IntType buttonx = string_width(text_get({ /*"advancedenable"*/ STR(1948) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"advancedenable"*/ STR(1948), sVar(dx) + sVar(dw) - buttonx, sVar(dy) }))
		{
			action_setting_program_mode(self, true);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"advancednotnow"*/ STR(1949) })) + button_padding);
		if (draw_button_label(self, { /*"advancednotnow"*/ STR(1949), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY }))
			popup_close(self);
		tab_next(self);
	}
	...
	void popup_downloadskin_draw(ScopeAny self)
	{
		...
		draw_textfield(self, { /*"downloadskinusername"*/ STR(1950), sVar(dx), sVar(dy), sVar(dw) - IntType(28), IntType(24), idInt(sInt(popup), tbx_username), null_ });
		BoolType download = keyboard_check_pressed(vk_enter);
		if (draw_button_icon(self, { /*"downloadskindownload"*/ STR(1951), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(18), IntType(24), IntType(24), false, icons_DOWNLOAD, null_, ObjType(obj_textbox, idInt(sInt(popup), tbx_username))->text == /*""*/ STR(0) || sInt(http_downloadskin) != null_, /*"tooltipdownloadskin"*/ STR(1952) }))
			download = true;
		...
		{
			draw_label(self, { text_get({ /*"downloadskindownloading"*/ STR(1953) }), sVar(dx), sVar(dy) + IntType(8), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"downloadskindone"*/ STR(1954), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT, !(idVar(sInt(popup), texture) > 0) }))
		{
			popup_close(self);
	...
	RealType popup_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"popupmove"*/ STR(1955))
			draw_set_alpha(0.5);
		...
		if (sVar(window_busy) == /*"popupclick"*/ STR(1956))
		{
			if (sReal(mouse_move) > IntType(10))
			{
				...
				sVar(window_busy) = /*"popupmove"*/ STR(1955);
			}
			else
				if (!sBool(mouse_left))
					sVar(window_busy) = /*""*/ STR(0);
		...
		if (sBool(popup_mouseon) && sInt(mouse_cursor) == cr_default && sBool(mouse_left_pressed))
			sVar(window_busy) = /*"popupclick"*/ STR(1956);
	...
	void popup_exportimage_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportimageimagesizecustomwidth"*/ STR(1957), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_image_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportimageimagesizecustomheight"*/ STR(1958), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_image_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, { false });
			draw_textfield_group(self, { /*"exportimageimagesizecustom"*/ STR(1959), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false });
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, { /*"exportimageimagesizecustomkeepaspectratio"*/ STR(1960), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio });
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageremovebackground"*/ STR(1961), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportimage_remove_background);
		tab_next(self);
		if (idBool(sInt(popup), remove_background))
			draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(1962), icons_WARNING_TRIANGLE, e_toast_WARNING);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageincludehidden"*/ STR(1963), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportimage_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagehighquality"*/ STR(1964), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportimage_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagewatermark"*/ STR(1965), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportimage_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, { /*"exportimagesave"*/ STR(1966), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportimage_save, e_anchor_RIGHT });
		tab_next(self);
	}
	...
	void popup_exportmovie_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportmovievideosizecustomwidth"*/ STR(1967), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_video_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportmovievideosizecustomheight"*/ STR(1968), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_video_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, { false });
			draw_textfield_group(self, { /*"exportmovievideosizecustom"*/ STR(1969), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false });
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, { /*"exportmovievideosizecustomkeepaspectratio"*/ STR(1970), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio });
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_menu(self);
		...
		if (idStr(sInt(popup), format) != /*"png"*/ STR(60))
		{
			...
			if (idInt(sInt(popup), video_quality) == IntType(0))
			{
				...
				draw_dragger(self, { /*"exportmoviebitrate"*/ STR(1971), sVar(dx), sVar(dy), dragger_width, idReal(sInt(popup), bit_rate), IntType(500), IntType(1), no_limit, IntType(2500000), IntType(1), idInt(sInt(popup), tbx_bit_rate), ID_action_toolbar_exportmovie_bit_rate });
				tab_next(self);
			}
		}
		tab_control_menu(self);
		...
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieremovebackground"*/ STR(1972), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportmovie_remove_background);
			tab_next(self);
			if (idBool(sInt(popup), remove_background))
				draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(1962), icons_WARNING_TRIANGLE, e_toast_WARNING);
		}
		...
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieincludeaudio"*/ STR(1973), sVar(dx), sVar(dy), idBool(sInt(popup), include_audio), ID_action_toolbar_exportmovie_include_audio);
			tab_next(self);
		}
		
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmovieincludehidden"*/ STR(1974), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportmovie_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviehighquality"*/ STR(1975), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportmovie_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviewatermark"*/ STR(1976), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportmovie_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, { /*"exportmoviesave"*/ STR(1977), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportmovie_save, e_anchor_RIGHT });
		tab_next(self);
	}
	
	void popup_importimage_draw(ScopeAny self)
	{
		draw_label(self, { text_get({ /*"importimagetype"*/ STR(1978) }) + /*":"*/ STR(754), sVar(dx), sVar(dy) + IntType(14), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(28);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageskin"*/ STR(1979), sVar(dx), sVar(dy), e_res_type_SKIN, idVar(sInt(popup), type) == e_res_type_SKIN, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageitemsheet"*/ STR(1980), sVar(dx), sVar(dy), e_res_type_ITEM_SHEET, idVar(sInt(popup), type) == e_res_type_ITEM_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageblocksheet"*/ STR(1981), sVar(dx), sVar(dy), e_res_type_BLOCK_SHEET, idVar(sInt(popup), type) == e_res_type_BLOCK_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageparticlesheet"*/ STR(1982), sVar(dx), sVar(dy), e_res_type_PARTICLE_SHEET, idVar(sInt(popup), type) == e_res_type_PARTICLE_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimagetexture"*/ STR(1983), sVar(dx), sVar(dy), e_res_type_TEXTURE, idVar(sInt(popup), type) == e_res_type_TEXTURE, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"importimageok"*/ STR(1984), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT }))
		{
	...
	void popup_importitemsheet_draw(ScopeAny self)
	{
		...
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, { /*"importitemsheetissheet"*/ STR(1985), sVar(dx), sVar(dy), idBool(sInt(popup), is_sheet), ID_action_toolbar_importitemsheet_is_sheet });
		tab_next(self);
		if (idBool(sInt(popup), is_sheet))
		{
			...
			draw_dragger(self, { /*"importitemsheetrows"*/ STR(1986), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(X_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(X_), IntType(1), idInt(sInt(popup), tbx_sheet_width), ID_action_toolbar_importitemsheet_sheet_size });
			tab_next(self);
			global::axis_edit = Y_;
			tab_control(self, IntType(28));
			draw_dragger(self, { /*"importitemsheetcolumns"*/ STR(1987), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(Y_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(Y_), IntType(1), idInt(sInt(popup), tbx_sheet_height), ID_action_toolbar_importitemsheet_sheet_size });
			tab_next(self);
		}
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"importimageok"*/ STR(1984), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT }))
	...
	void popup_loading_draw(ScopeAny self)
	{
		...
		if (idReal(sInt(popup), load_amount) > IntType(1))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), progress, text_get({ /*"loadingresources"*/ STR(1988) }), text_get({ /*"loadingpercent"*/ STR(1989), string(floor(progress * IntType(100))) }));
			tab_next(self);
		}
		tab_control_loading(self);
	...
	void popup_modelbench_draw(ScopeAny self)
	{
		...
		draw_label(self, { string_limit_ext(text_get({ /*"modelbenchinfo"*/ STR(1990) }), sVar(dw) + IntType(24), sVar(dh)), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy), fa_center, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(34) + IntType(30);
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"modelbenchdownload"*/ STR(1991) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"modelbenchdownload"*/ STR(1991), sVar(dx) + sVar(dw) - buttonx, sVar(dy) }))
		{
			open_url(link_modelbench);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"modelbenchnotnow"*/ STR(1992) })) + button_padding);
		if (draw_button_label(self, { /*"modelbenchnotnow"*/ STR(1992), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY }))
			popup_close(self);
		if (draw_checkbox(self, /*"modelbenchdontshow"*/ STR(1993), sVar(dx), sVar(dy) + IntType(4), ObjType(obj_popup, sInt(popup_modelbench))->hidden, null_))
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_newproject))->tbx_name)->text = text_get({ /*"newprojectnamedefault"*/ STR(1994) });
	...
	void popup_newproject_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, { /*"newprojectname"*/ STR(1995), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, idVar(sInt(popup), folder), /*"top"*/ STR(187) }) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"newprojectnamedefault"*/ STR(1994) });
		...
		if (draw_textfield(self, { /*"newprojectdescription"*/ STR(1996), sVar(dx), sVar(dy), sVar(dw), IntType(76), idInt(sInt(popup), tbx_description), null_, /*""*/ STR(0), /*"top"*/ STR(187) }))
		{
			idVar(sInt(popup), description) = ObjType(obj_textbox, idInt(sInt(popup), tbx_description))->text;
		}
		tab_next(self);
		StringType directory = /*"../"*/ STR(1997) + directory_name(sStr(setting_project_folder)) + string_remove_newline(idVar(sInt(popup), folder));
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(1999), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, false, /*"tooltipchangefolder"*/ STR(2000) }))
		{
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"newprojectcreate"*/ STR(2001), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT }))
		{
	...
	void popup_pattern_editor_draw(ScopeAny self)
	{
		...
		if (draw_button_label(self, { /*"patterneditoraddlayer"*/ STR(2002), sVar(dx), sVar(dy), listw, icons_PLUS, e_button_SECONDARY }))
		{
		...
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"patterneditordone"*/ STR(2003) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"patterneditordone"*/ STR(2003), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy) }))
		{
		...
		buttonx += IntType(12) + (string_width(text_get({ /*"patterneditorcancel"*/ STR(2004) })) + button_padding);
		if (draw_button_label(self, { /*"patterneditorcancel"*/ STR(2004), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY }))
		{
			array_add(VarType::CreateRef(global::pattern_update), idVar(sInt(popup), pattern_edit));
			instance_destroy(self, { idInt(sInt(popup), pattern_edit_preview), false });
			idInt(sInt(popup), pattern_edit_preview) = null_;
	...
	void popup_pattern_editor_draw_layer(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, VarType index, RealType base)
	{
		...
		editname = /*"patterneditoreditlayer"*/ STR(2005) + string(index);
		...
		VarType name = ((base > 0) ? text_get({ /*"patterneditorbase"*/ STR(2006) }) : text_get({ /*"patterneditorlayer"*/ STR(2007), index + IntType(1) }));
		...
		if (!(base > 0))
		{
			...
			if (draghover)
			{
				...
				if (sReal(mouse_move) > IntType(0) && !dragging)
				{
					...
					sVar(window_busy) = /*"patterneditordraglayer"*/ STR(2008);
				}
			}
			...
			if (mouseon && !dragging)
			{
				if (draw_button_icon(self, { /*"patterneditorremovelayer"*/ STR(2009) + string(index), actionx - IntType(24), yy + height / 2.0 - IntType(14), IntType(24), IntType(24), false, icons_DELETE }))
	...
	{
		draw_label(self, { text_get({ /*"patterneditorcolors"*/ STR(2010) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(18);
		VarType colorsx;
		colorsx = sVar(dx);
		...
		{
			sVar(dy) += IntType(8);
			draw_label(self, { text_get({ /*"patterneditorpatterns"*/ STR(2011) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(18);
