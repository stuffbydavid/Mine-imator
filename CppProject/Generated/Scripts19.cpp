...
namespace CppProject
{
	void background_sky_update()
	{
		...
		RealType alphay;
		if (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(755))
		...
		global::_app->background_clouds_alpha = (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(755) ? IntType(1) - min({ global::_app->background_night_alpha, 0.95 }) : .8 - min({ global::_app->background_night_alpha, 0.75 })) * alphay;
	...
	RealType background_sky_update_clouds(ScopeAny self)
	{
		...
		hei = IntType(0);
		if (sVar(background_sky_clouds_mode) == /*"faded"*/ STR(755))
		{
		...
		if (sVar(background_sky_clouds_mode) != /*"flat"*/ STR(756))
		{
		...
		col = (sVar(background_sky_clouds_mode) == /*"flat"*/ STR(756) ? coltop : colbottom);
	...
	RealType bench_click(ScopeAny self, VarType type)
	{
		if (ObjType(obj_bench_settings, sInt(bench_settings))->type == type && sStr(bench_show_ani_type) == /*""*/ STR(0))
		{
			...
			sStr(bench_show_ani_type) = /*"hide"*/ STR(757);
			return IntType(0);
		}
	...
	RealType bench_draw(ScopeAny self)
	{
		...
		func = /*""*/ STR(0);
		if (sStr(bench_show_ani_type) == /*"show"*/ STR(758))
		{
			...
			func = /*"easeoutcirc"*/ STR(564);
		}
		else
			if (sStr(bench_show_ani_type) == /*"hide"*/ STR(757))
			{
				...
				if (sVar(bench_show_ani) == IntType(0))
					sStr(bench_show_ani_type) = /*""*/ STR(0);
				func = /*"easeincirc"*/ STR(563);
			}
		
		if (sVar(bench_show_ani) == IntType(0))
		{
			if (sVar(window_busy) == /*"bench"*/ STR(759))
		...
		clip_end();
		draw_set_alpha(1.0);
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(bench_show_ani_type) != /*"hide"*/ STR(757))
			sVar(window_busy) = /*"bench"*/ STR(759);
		return 0.0;
	}
	...
	void bench_draw_settings(ScopeAny self, VarType bx, VarType by, VarType bw, VarType bh)
	{
		...
		VarType aniease = ease(/*"easeoutcirc"*/ STR(564), sVar(bench_settings_ani));
		...
		if (type_is_timeline(ObjType(obj_bench_settings, sInt(bench_settings))->type))
		{
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_LIGHT_SOURCE)
			{
				...
				togglebutton_add(self, { /*"typepointlight"*/ STR(760), null_, e_tl_type_POINT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT, ID_action_bench_light_type });
				togglebutton_add(self, { /*"typespotlight"*/ STR(761), null_, e_tl_type_SPOT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_SPOT_LIGHT, ID_action_bench_light_type });
				draw_togglebutton(self, /*"benchlighttype"*/ STR(762), sVar(dx), sVar(dy));
				tab_next(self);
				if (ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT)
					draw_tooltip_label(self, /*"benchpointlighttip"*/ STR(763), icons_LIGHT_POINT, e_toast_INFO);
				else
					draw_tooltip_label(self, /*"benchspotlighttip"*/ STR(764), icons_LIGHT_SPOT, e_toast_INFO);
			...
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_CAMERA)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchcameratip"*/ STR(765), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_BACKGROUND)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchbackgroundtip"*/ STR(766), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_AUDIO)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchaudiotip"*/ STR(767), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchpathtip"*/ STR(768), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH_POINT)
				draw_tooltip_label(self, /*"benchpathpointtip"*/ STR(769), icons_INFO, e_toast_INFO);
		}
		else
		{
			...
			switch ((IntType)ObjType(obj_bench_settings, sInt(bench_settings))->type)
			{
				...
				case e_temp_type_BODYPART:
				{
					...
					{
						labeltext = text_get({ /*"benchmodel"*/ STR(770) });
						list = ObjType(obj_bench_settings, sInt(bench_settings))->char_list;
						texcap = /*"benchskin"*/ STR(771);
						texmatcap = /*"benchskinmaterial"*/ STR(772);
						texnormcap = /*"benchskinnormal"*/ STR(773);
						...
						{
							labeltext = text_get({ /*"benchblock"*/ STR(774) });
							list = ObjType(obj_bench_settings, sInt(bench_settings))->special_block_list;
							texcap = /*"benchspblocktex"*/ STR(775);
							texmatcap = /*"benchspblocktexmaterial"*/ STR(776);
							texnormcap = /*"benchspblocktexnormal"*/ STR(777);
							capwid = text_caption_width(self, { texcap, texmatcap, texnormcap });
							part = ObjType(obj_bench_settings, sInt(bench_settings))->model_file;
						}
						else
							if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_temp_type_BODYPART)
							{
								...
								texcap = /*"benchbodypartskin"*/ STR(778);
								texmatcap = /*"benchbodypartskinmaterial"*/ STR(779);
								texnormcap = /*"benchbodypartskinnormal"*/ STR(780);
								capwid = text_caption_width(self, { /*"benchbodypart"*/ STR(781), texcap, texmatcap, texnormcap });
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"modelstate"*/ STR(782), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"true"*/ STR(783) && ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"false"*/ STR(784))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i);
						...
						if (draw_checkbox(self, /*"modelstate"*/ STR(782) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(783), null_))
						{
							sInt(menu_model_state) = sInt(menu_model_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(783))
								action_bench_model_state(self, /*"false"*/ STR(784));
							else
								action_bench_model_state(self, /*"true"*/ STR(783));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_model_current) = model;
						sInt(menu_model_state_current) = DsMap(ObjType(obj_model, model)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"modelstatevalue"*/ STR(785), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1))), ID_action_bench_model_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					...
					{
						draw_button_menu(self, { /*"benchbodypart"*/ STR(781), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name, minecraft_asset_get_name(/*"modelpart"*/ STR(786), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name), ID_action_bench_model_part_name, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					VarType text, tex;
				...
				case e_temp_type_SCENERY:
				{
					IntType capwid;
					VarType text, tex;
					capwid = text_caption_width(self, { /*"benchscenery"*/ STR(788), /*"benchblocktex"*/ STR(789), /*"benchblocktexmaterial"*/ STR(790), /*"benchblocktexnormal"*/ STR(791) });
					...
					draw_button_menu(self, { /*"benchscenery"*/ STR(788), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->scenery, text, ID_action_bench_scenery, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					draw_button_menu(self, { /*"benchblocktex"*/ STR(789), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->block_tex, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex, display_name), ID_action_bench_block_tex, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						draw_button_menu(self, { /*"benchblocktexmaterial"*/ STR(790), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_material, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_material, display_name), ID_action_bench_block_tex_material, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_material, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						draw_button_menu(self, { /*"benchblocktexnormal"*/ STR(791), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_normal, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_normal, display_name), ID_action_bench_block_tex_normal, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->block_tex_normal, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					break;
				}
				...
				case e_temp_type_ITEM:
				{
					...
					if (!res_is_ready(res))
						res = global::mc_res;
					draw_label(self, { text_get({ /*"typeitem"*/ STR(168) }) + /*":"*/ STR(793), sVar(dx), sVar(dy) + IntType(12), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
					...
					draw_checkbox(self, /*"benchitem3d"*/ STR(794), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_3d, ID_action_bench_item_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemfacecamera"*/ STR(795), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_face_camera, ID_action_bench_item_face_camera);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitembounce"*/ STR(796), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_bounce, ID_action_bench_item_bounce);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemspin"*/ STR(797), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_spin, ID_action_bench_item_spin);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					capwid = text_caption_width(self, { /*"benchitemtex"*/ STR(798), /*"benchitemtexmaterial"*/ STR(799), /*"benchitemtexnormal"*/ STR(800) });
					VarType tex = idInt(res, block_preview_texture);
					if (tex == null_)
						tex = idVar(res, texture);
					draw_button_menu(self, { /*"benchitemtex"*/ STR(798), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, display_name), ID_action_bench_item_tex, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						VarType tex = idInt(res, block_preview_texture);
				...
				case e_temp_type_BLOCK:
				{
					...
					IntType capwid;
					VarType text, sprite;
					capwid = text_caption_width(self, { /*"benchblocktex"*/ STR(789), /*"benchblocktexmaterial"*/ STR(790), /*"benchblocktexnormal"*/ STR(791) });
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, ObjType(obj_bench_settings, sInt(bench_settings))->block_list, sVar(dx), sVar(dy), sVar(dw), sReal(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->block_name, false, text_get({ /*"benchblock"*/ STR(774) }));
					tab_next(self);
					VarType block;
					IntType statelen;
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"blockstate"*/ STR(801), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"true"*/ STR(783) && ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"false"*/ STR(784))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i);
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						tab_control(self, sReal(ui_small_height));
						if (draw_checkbox(self, /*"blockstate"*/ STR(801) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(783), null_))
						{
							sInt(menu_block_state) = sInt(menu_block_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(783))
								action_bench_block_state(self, /*"false"*/ STR(784));
							else
								action_bench_block_state(self, /*"true"*/ STR(783));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"blockstatevalue"*/ STR(802), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1))), ID_action_bench_block_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
				...
				{
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, ObjType(obj_bench_settings, sInt(bench_settings))->particles_list, sVar(dx), sVar(dy), sVar(dw), sReal(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->particle_preset, false, text_get({ /*"benchparticlespreset"*/ STR(803) }));
					tab_next(self);
				...
				{
					IntType capwid;
					capwid = text_caption_width(self, { /*"benchtextfont"*/ STR(804) });
					if (sReal(setting_advanced_mode) > 0)
					{
						draw_button_menu(self, { /*"benchtextfont"*/ STR(804), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->text_font, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->text_font, display_name), ID_action_bench_text_font, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					VarType sx;
					sx = sVar(dx_start);
					...
					tab_set_collumns(self, { true, IntType(2) });
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtext3d"*/ STR(805), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_3d, ID_action_bench_text_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtextfacecamera"*/ STR(806), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_face_camera, ID_action_bench_text_face_camera);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					break;
				...
				case e_tl_type_SHAPE:
				{
					IntType capwid;
					VarType text;
					capwid = text_caption_width(self, { /*"benchshapetype"*/ STR(807), /*"benchshapetex"*/ STR(808), /*"benchshapetexmaterial"*/ STR(809), /*"benchshapetexnormal"*/ STR(810) });
					text = text_get({ /*"type"*/ STR(811) + DsList(global::tl_type_name_list).Value(e_tl_type_CUBE + ObjType(obj_bench_settings, sInt(bench_settings))->shape_type) });
					draw_button_menu(self, { /*"benchshapetype"*/ STR(807), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->shape_type, text, ID_action_bench_shape_type, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += IntType(32);
					VarType tex;
					text = text_get({ /*"listnone"*/ STR(792) });
					tex = null_;
					...
					draw_button_menu(self, { /*"benchshapetex"*/ STR(808), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex, text, ID_action_bench_shape_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						text = text_get({ /*"listnone"*/ STR(792) });
						...
						draw_button_menu(self, { /*"benchshapetexmaterial"*/ STR(809), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_material, text, ID_action_bench_shape_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						text = text_get({ /*"listnone"*/ STR(792) });
						tex = null_;
						...
						draw_button_menu(self, { /*"benchshapetexnormal"*/ STR(810), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_normal, text, ID_action_bench_shape_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					if (ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CUBE || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CYLINDER || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CONE)
					{
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapetexmap"*/ STR(812), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_mapped, ID_action_bench_shape_tex_map, /*"benchshapetexmaptip"*/ STR(813));
							tab_next(self);
						}
					}
					else
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapefacecamera"*/ STR(814), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_face_camera, ID_action_bench_shape_face_camera);
							tab_next(self);
						}
				...
				{
					IntType capwid = text_caption_width(self, { /*"benchmodel"*/ STR(770), /*"benchmodeltex"*/ STR(815), /*"benchmodeltexmaterial"*/ STR(816), /*"benchmodeltexnormal"*/ STR(817) });
					VarType text;
					...
					draw_button_menu(self, { /*"benchmodel"*/ STR(770), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model, text, ID_action_bench_model, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					VarType texobj, tex;
					...
					draw_button_menu(self, { /*"benchmodeltex"*/ STR(815), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex, text, ID_action_bench_model_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material == null_)
							text = text_get({ /*"listdefault"*/ STR(818), text });
						draw_button_menu(self, { /*"benchmodeltexmaterial"*/ STR(816), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material, text, ID_action_bench_model_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						{
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal == null_)
							text = text_get({ /*"listdefault"*/ STR(818), text });
						draw_button_menu(self, { /*"benchmodeltexnormal"*/ STR(817), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal, text, ID_action_bench_model_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					break;
				}
		...
		if (draw_button_label(self, /*"benchcreate"*/ STR(819), (edit > 0) ? (sVar(dx) + wid + 8.0) : sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_ASSET_ADD))
		{
			action_bench_create(self);
			sStr(bench_show_ani_type) = /*"hide"*/ STR(757);
		}
		if (edit > 0)
		{
			if (draw_button_label(self, /*"benchcreateedit"*/ STR(820), sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_PENCIL, e_button_SECONDARY))
			{
	...
	{
		ArrType files = file_find(particles_directory, /*".miparticles"*/ STR(723));
		...
		withOne (obj_timeline, (new obj_timeline)->id, self->id)
		{
			...
			else
			{
				...
				if (is_string(idVar(self.otherId, variant)))
					state_vars_add(VarType::CreateRef(self->block_state), ArrType::From({ /*"variant"*/ STR(821), idVar(self.otherId, variant) }));
				tl_update_scenery_part(ScopeAny(self));
			...
			if (idVar(self.otherId, pattern_type) != /*""*/ STR(0))
			{
				...
				if (self->pattern_type == /*"banner"*/ STR(822))
				{
					...
					ObjType(obj_timeline, banner)->pattern_type = /*"banner"*/ STR(822);
				}
