...
namespace CppProject
{
	void background_sky_update()
	{
		...
		RealType alphay;
		if (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(756))
		...
		global::_app->background_clouds_alpha = (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(756) ? IntType(1) - min({ global::_app->background_night_alpha, 0.95 }) : .8 - min({ global::_app->background_night_alpha, 0.75 })) * alphay;
	...
	RealType background_sky_update_clouds(ScopeAny self)
	{
		...
		hei = IntType(0);
		if (sVar(background_sky_clouds_mode) == /*"faded"*/ STR(756))
		{
		...
		if (sVar(background_sky_clouds_mode) != /*"flat"*/ STR(757))
		{
		...
		col = (sVar(background_sky_clouds_mode) == /*"flat"*/ STR(757) ? coltop : colbottom);
	...
	RealType bench_click(ScopeAny self, VarType type)
	{
		if (ObjType(obj_bench_settings, sInt(bench_settings))->type == type && sStr(bench_show_ani_type) == /*""*/ STR(0))
		{
			...
			sStr(bench_show_ani_type) = /*"hide"*/ STR(758);
			return IntType(0);
		}
	...
	RealType bench_draw(ScopeAny self)
	{
		...
		func = /*""*/ STR(0);
		if (sStr(bench_show_ani_type) == /*"show"*/ STR(759))
		{
			...
			func = /*"easeoutcirc"*/ STR(565);
		}
		else
			if (sStr(bench_show_ani_type) == /*"hide"*/ STR(758))
			{
				...
				if (sVar(bench_show_ani) == IntType(0))
					sStr(bench_show_ani_type) = /*""*/ STR(0);
				func = /*"easeincirc"*/ STR(564);
			}
		
		if (sVar(bench_show_ani) == IntType(0))
		{
			if (sVar(window_busy) == /*"bench"*/ STR(760))
		...
		for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(bench_type_list), item)); i++)
		{
			...
			{
				if (ObjType(obj_list_item, DsList(idInt(sInt(bench_type_list), item)).Value(i))->name == /*"typemodel"*/ STR(167) || ObjType(obj_list_item, DsList(idInt(sInt(bench_type_list), item)).Value(i))->name == /*"typebackground"*/ STR(179))
					skipasset = true;
			}
			if (!skipasset)
			{
		...
		clip_end();
		draw_set_alpha(1.0);
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(bench_show_ani_type) != /*"hide"*/ STR(758))
			sVar(window_busy) = /*"bench"*/ STR(760);
		return 0.0;
	}
	...
	void bench_draw_settings(ScopeAny self, VarType bx, VarType by, VarType bw, VarType bh)
	{
		...
		VarType aniease = ease(/*"easeoutcirc"*/ STR(565), sVar(bench_settings_ani));
		...
		if (type_is_timeline(ObjType(obj_bench_settings, sInt(bench_settings))->type))
		{
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_LIGHT_SOURCE)
			{
				...
				togglebutton_add(self, { /*"typepointlight"*/ STR(761), null_, e_tl_type_POINT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT, ID_action_bench_light_type });
				togglebutton_add(self, { /*"typespotlight"*/ STR(762), null_, e_tl_type_SPOT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_SPOT_LIGHT, ID_action_bench_light_type });
				draw_togglebutton(self, /*"benchlighttype"*/ STR(763), sVar(dx), sVar(dy));
				tab_next(self);
				if (ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT)
					draw_tooltip_label(self, /*"benchpointlighttip"*/ STR(764), icons_LIGHT_POINT, e_toast_INFO);
				else
					draw_tooltip_label(self, /*"benchspotlighttip"*/ STR(765), icons_LIGHT_SPOT, e_toast_INFO);
			...
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_CAMERA)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchcameratip"*/ STR(766), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_BACKGROUND)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchbackgroundtip"*/ STR(767), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_AUDIO)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchaudiotip"*/ STR(768), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchpathtip"*/ STR(769), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH_POINT)
				draw_tooltip_label(self, /*"benchpathpointtip"*/ STR(770), icons_INFO, e_toast_INFO);
		}
		else
		{
			...
			switch ((IntType)ObjType(obj_bench_settings, sInt(bench_settings))->type)
			{
				...
				case e_temp_type_BODYPART:
				{
					...
					{
						labeltext = text_get({ /*"benchmodel"*/ STR(771) });
						list = ObjType(obj_bench_settings, sInt(bench_settings))->char_list;
						texcap = /*"benchskin"*/ STR(772);
						texmatcap = /*"benchskinmaterial"*/ STR(773);
						texnormcap = /*"benchskinnormal"*/ STR(774);
						...
						{
							labeltext = text_get({ /*"benchblock"*/ STR(775) });
							list = ObjType(obj_bench_settings, sInt(bench_settings))->special_block_list;
							texcap = /*"benchspblocktex"*/ STR(776);
							texmatcap = /*"benchspblocktexmaterial"*/ STR(777);
							texnormcap = /*"benchspblocktexnormal"*/ STR(778);
							capwid = text_caption_width(self, { texcap, texmatcap, texnormcap });
							part = ObjType(obj_bench_settings, sInt(bench_settings))->model_file;
						}
						else
							if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_temp_type_BODYPART)
							{
								...
								texcap = /*"benchbodypartskin"*/ STR(779);
								texmatcap = /*"benchbodypartskinmaterial"*/ STR(780);
								texnormcap = /*"benchbodypartskinnormal"*/ STR(781);
								capwid = text_caption_width(self, { /*"benchbodypart"*/ STR(782), texcap, texmatcap, texnormcap });
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"modelstate"*/ STR(783), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"true"*/ STR(784) && ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"false"*/ STR(785))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i);
						...
						if (draw_checkbox(self, /*"modelstate"*/ STR(783) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(784), null_))
						{
							sInt(menu_model_state) = sInt(menu_model_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(784))
								action_bench_model_state(self, /*"false"*/ STR(785));
							else
								action_bench_model_state(self, /*"true"*/ STR(784));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_model_current) = model;
						sInt(menu_model_state_current) = DsMap(ObjType(obj_model, model)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"modelstatevalue"*/ STR(786), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1))), ID_action_bench_model_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					...
					{
						draw_button_menu(self, { /*"benchbodypart"*/ STR(782), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name, minecraft_asset_get_name(/*"modelpart"*/ STR(787), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name), ID_action_bench_model_part_name, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					VarType text, tex;
					...
					if (ObjType(obj_bench_settings, sInt(bench_settings))->model_name == /*"armor"*/ STR(789))
					{
						tab_control_button_label(self);
				...
				case e_temp_type_SCENERY:
				{
					IntType capwid;
					VarType text, tex;
					capwid = text_caption_width(self, { /*"benchscenery"*/ STR(791), /*"benchblocktex"*/ STR(792), /*"benchblocktexmaterial"*/ STR(793), /*"benchblocktexnormal"*/ STR(794) });
				...
				case e_temp_type_ITEM:
				{
					...
					capwid = text_caption_width(self, { /*"typeitem"*/ STR(168) });
					res = ObjType(obj_bench_settings, sInt(bench_settings))->item_tex;
					if (!res_is_ready(res))
						res = global::mc_res;
					draw_label(self, { text_get({ /*"typeitem"*/ STR(168) }) + /*":"*/ STR(796), sVar(dx), sVar(dy) + IntType(12), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
					...
					draw_checkbox(self, /*"benchitem3d"*/ STR(797), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_3d, ID_action_bench_item_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemfacecamera"*/ STR(798), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_face_camera, ID_action_bench_item_face_camera);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitembounce"*/ STR(799), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_bounce, ID_action_bench_item_bounce);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemspin"*/ STR(800), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_spin, ID_action_bench_item_spin);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					capwid = text_caption_width(self, { /*"benchitemtex"*/ STR(801), /*"benchitemtexmaterial"*/ STR(802), /*"benchitemtexnormal"*/ STR(803) });
					VarType tex = idInt(res, block_preview_texture);
					if (tex == null_)
						tex = idVar(res, texture);
					draw_button_menu(self, { /*"benchitemtex"*/ STR(801), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, display_name), ID_action_bench_item_tex, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						VarType tex = idInt(res, block_preview_texture);
				...
				case e_temp_type_BLOCK:
				{
					...
					IntType capwid;
					VarType text, sprite;
					capwid = text_caption_width(self, { /*"benchblocktex"*/ STR(792), /*"benchblocktexmaterial"*/ STR(793), /*"benchblocktexnormal"*/ STR(794) });
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, ObjType(obj_bench_settings, sInt(bench_settings))->block_list, sVar(dx), sVar(dy), sVar(dw), sReal(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->block_name, false, text_get({ /*"benchblock"*/ STR(775) }));
					tab_next(self);
					VarType block;
					IntType statelen;
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"blockstate"*/ STR(804), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"true"*/ STR(784) && ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"false"*/ STR(785))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i);
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						tab_control(self, sReal(ui_small_height));
						if (draw_checkbox(self, /*"blockstate"*/ STR(804) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(784), null_))
						{
							sInt(menu_block_state) = sInt(menu_block_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(784))
								action_bench_block_state(self, /*"false"*/ STR(785));
							else
								action_bench_block_state(self, /*"true"*/ STR(784));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"blockstatevalue"*/ STR(805), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1))), ID_action_bench_block_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
				...
				{
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, ObjType(obj_bench_settings, sInt(bench_settings))->particles_list, sVar(dx), sVar(dy), sVar(dw), sReal(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->particle_preset, false, text_get({ /*"benchparticlespreset"*/ STR(806) }));
					tab_next(self);
				...
				{
					IntType capwid;
					capwid = text_caption_width(self, { /*"benchtextfont"*/ STR(807) });
					if (sReal(setting_advanced_mode) > 0)
					{
						draw_button_menu(self, { /*"benchtextfont"*/ STR(807), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->text_font, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->text_font, display_name), ID_action_bench_text_font, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					VarType sx;
					sx = sVar(dx_start);
					...
					tab_set_collumns(self, { true, IntType(2) });
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtext3d"*/ STR(808), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_3d, ID_action_bench_text_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtextfacecamera"*/ STR(809), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_face_camera, ID_action_bench_text_face_camera);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					break;
				...
				case e_tl_type_SHAPE:
				{
					IntType capwid;
					VarType text;
					capwid = text_caption_width(self, { /*"benchshapetype"*/ STR(810), /*"benchshapetex"*/ STR(811), /*"benchshapetexmaterial"*/ STR(812), /*"benchshapetexnormal"*/ STR(813) });
					text = text_get({ /*"type"*/ STR(814) + DsList(global::tl_type_name_list).Value(e_tl_type_CUBE + ObjType(obj_bench_settings, sInt(bench_settings))->shape_type) });
					draw_button_menu(self, { /*"benchshapetype"*/ STR(810), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->shape_type, text, ID_action_bench_shape_type, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += IntType(32);
					VarType tex;
					text = text_get({ /*"listnone"*/ STR(795) });
					tex = null_;
					...
					draw_button_menu(self, { /*"benchshapetex"*/ STR(811), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex, text, ID_action_bench_shape_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						text = text_get({ /*"listnone"*/ STR(795) });
						...
						draw_button_menu(self, { /*"benchshapetexmaterial"*/ STR(812), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_material, text, ID_action_bench_shape_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						text = text_get({ /*"listnone"*/ STR(795) });
						tex = null_;
						...
						draw_button_menu(self, { /*"benchshapetexnormal"*/ STR(813), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_normal, text, ID_action_bench_shape_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					if (ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CUBE || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CYLINDER || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CONE)
					{
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapetexmap"*/ STR(815), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_mapped, ID_action_bench_shape_tex_map, /*"benchshapetexmaptip"*/ STR(816));
							tab_next(self);
						}
					}
					else
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapefacecamera"*/ STR(817), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_face_camera, ID_action_bench_shape_face_camera);
							tab_next(self);
						}
				...
				{
					IntType capwid = text_caption_width(self, { /*"benchmodel"*/ STR(771), /*"benchmodeltex"*/ STR(818), /*"benchmodeltexmaterial"*/ STR(819), /*"benchmodeltexnormal"*/ STR(820) });
					VarType text;
					...
					draw_button_menu(self, { /*"benchmodel"*/ STR(771), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model, text, ID_action_bench_model, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					VarType texobj, tex;
					...
					draw_button_menu(self, { /*"benchmodeltex"*/ STR(818), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex, text, ID_action_bench_model_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material == null_)
							text = text_get({ /*"listdefault"*/ STR(821), text });
						draw_button_menu(self, { /*"benchmodeltexmaterial"*/ STR(819), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material, text, ID_action_bench_model_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						{
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal == null_)
							text = text_get({ /*"listdefault"*/ STR(821), text });
						draw_button_menu(self, { /*"benchmodeltexnormal"*/ STR(820), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal, text, ID_action_bench_model_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					break;
				}
		...
		if (draw_button_label(self, /*"benchcreate"*/ STR(822), (edit > 0) ? (sVar(dx) + wid + 8.0) : sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_ASSET_ADD))
		{
			action_bench_create(self);
			sStr(bench_show_ani_type) = /*"hide"*/ STR(758);
		}
		if (edit > 0)
		{
			if (draw_button_label(self, /*"benchcreateedit"*/ STR(823), sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_PENCIL, e_button_SECONDARY))
			{
	...
	{
		ArrType files = file_find(particles_directory, /*".miparticles"*/ STR(724));
