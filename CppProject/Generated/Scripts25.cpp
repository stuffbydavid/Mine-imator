...
namespace CppProject
{
	...
	VarType debug(VarArgs argument)
	{
		...
		cap = string_repeat(/*"\t"*/ STR(1012), global::debug_indent) + string(argument[IntType(0)]);
		valstr = /*""*/ STR(0);
	...
	void debug_ds_list(VarType argument0)
	{
		...
		debug({ /*"Elements"*/ STR(1015), ds_list_size((IntType)(list)) });
		for (IntType i = IntType(0); i < ds_list_size(list); i++)
			debug({ /*"    "*/ STR(1016) + string(DsList(list).Value(i)) });
	}
	...
	void debug_ds_map(IntType map)
	{
		...
		debug({ /*"Elements"*/ STR(1015), ds_list_size(keylist) });
		for (IntType i = IntType(0); i < ds_list_size(keylist); i++)
			debug({ /*"   "*/ STR(1017) + string(DsList(keylist).Value(i)) + /*" => "*/ STR(1018) + string(DsMap(map).Value(DsList(keylist).Value(i))) });
	...
	RealType debug_info_draw(ScopeAny self)
	{
		...
		str += /*"fps: "*/ STR(1020) + string(gmlGlobal::fps) + /*" \n"*/ STR(1021);
		str += /*"fps_real: "*/ STR(1022) + string(gmlGlobal::fps_real) + /*" \n"*/ STR(1021);
		str += /*"delta: "*/ STR(1023) + string(global::delta) + /*" \n"*/ STR(1021);
		str += /*"DPI: "*/ STR(1024) + string(display_get_dpi_x()) + /*","*/ STR(1025) + string(display_get_dpi_y()) + /*" \n"*/ STR(1021);
		str += /*"instance_count: "*/ STR(1026) + string(gmlGlobal::instance_count) + /*" \n"*/ STR(1021);
		str += /*"render_world() calls: "*/ STR(1027) + string(global::render_world_count) + /*" \n"*/ STR(1021);
		str += /*"Vertex buffer triangles: "*/ STR(1028) + string(get_vertex_buffer_triangles()) + /*" \n"*/ STR(1021);
		str += /*"Vertex buffer render calls: "*/ STR(1029) + string(get_vertex_buffer_render_calls()) + /*" \n"*/ STR(1021);
		str += /*"Primitive lines: "*/ STR(1030) + string(get_primitive_lines()) + /*" \n"*/ STR(1021);
		str += /*"Primitive triangles: "*/ STR(1031) + string(get_primitive_triangles()) + /*" \n"*/ STR(1021);
		str += /*"Primitive render calls: "*/ STR(1032) + string(get_primitive_render_calls()) + /*" \n"*/ STR(1021);
		str += /*"[F12 to disable]"*/ STR(1033);
	...
	{
		debug({ str, string(current_time() - global::debug_timer) + /*" msec"*/ STR(1034) });
	}
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1036))) + /*"/"*/ STR(16);
	}
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, VarType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1037);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1038);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1037))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1038))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1039) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, { text_get({ /*"tooltipcustomaccentcolor"*/ STR(1040) }), xx, yy, width, height });
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1041), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	void draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, IntType def, BoolType hsvmode, IntType script)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1042), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(998);
		}
	}
