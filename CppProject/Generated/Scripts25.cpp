...
namespace CppProject
{
	...
	RealType context_menu_draw_level(ScopeAny self, IntType argument0)
	{
		...
		if (sStr(context_menu_ani) == /*""*/ STR(0) || sStr(context_menu_ani) == /*"open"*/ STR(890))
		{
	...
	VarType debug(VarArgs argument)
	{
		...
		cap = string_repeat(/*"\t"*/ STR(1011), global::debug_indent) + string(argument[IntType(0)]);
		valstr = /*""*/ STR(0);
	...
	void debug_ds_list(VarType argument0)
	{
		...
		debug({ /*"Elements"*/ STR(1014), ds_list_size((IntType)(list)) });
		for (IntType i = IntType(0); i < ds_list_size(list); i++)
			debug({ /*"    "*/ STR(1015) + string(DsList(list).Value(i)) });
	}
	...
	void debug_ds_map(IntType map)
	{
		...
		debug({ /*"Elements"*/ STR(1014), ds_list_size(keylist) });
		for (IntType i = IntType(0); i < ds_list_size(keylist); i++)
			debug({ /*"   "*/ STR(1016) + string(DsList(keylist).Value(i)) + /*" => "*/ STR(1017) + string(DsMap(map).Value(DsList(keylist).Value(i))) });
	...
	RealType debug_info_draw(ScopeAny self)
	{
		...
		str += /*"fps: "*/ STR(1019) + string(gmlGlobal::fps) + /*" \n"*/ STR(1020);
		str += /*"fps_real: "*/ STR(1021) + string(gmlGlobal::fps_real) + /*" \n"*/ STR(1020);
		str += /*"delta: "*/ STR(1022) + string(global::delta) + /*" \n"*/ STR(1020);
		str += /*"DPI: "*/ STR(1023) + string(display_get_dpi_x()) + /*","*/ STR(1024) + string(display_get_dpi_y()) + /*" \n"*/ STR(1020);
		str += /*"instance_count: "*/ STR(1025) + string(gmlGlobal::instance_count) + /*" \n"*/ STR(1020);
		str += /*"render_world() calls: "*/ STR(1026) + string(global::render_world_count) + /*" \n"*/ STR(1020);
		str += /*"Vertex buffer triangles: "*/ STR(1027) + string(get_vertex_buffer_triangles()) + /*" \n"*/ STR(1020);
		str += /*"Vertex buffer render calls: "*/ STR(1028) + string(get_vertex_buffer_render_calls()) + /*" \n"*/ STR(1020);
		str += /*"Primitive lines: "*/ STR(1029) + string(get_primitive_lines()) + /*" \n"*/ STR(1020);
		str += /*"Primitive triangles: "*/ STR(1030) + string(get_primitive_triangles()) + /*" \n"*/ STR(1020);
		str += /*"Primitive render calls: "*/ STR(1031) + string(get_primitive_render_calls()) + /*" \n"*/ STR(1020);
		str += /*"[F12 to disable]"*/ STR(1032);
	...
	{
		debug({ str, string(current_time() - global::debug_timer) + /*" msec"*/ STR(1033) });
	}
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1035))) + /*"/"*/ STR(16);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(867);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(868);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(884);
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, VarType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1036);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1037);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1036))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1037))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1038) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, { text_get({ /*"tooltipcustomaccentcolor"*/ STR(1039) }), xx, yy, width, height });
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1040), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	void draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, IntType def, BoolType hsvmode, IntType script)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1041), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(997);
		}
	}
