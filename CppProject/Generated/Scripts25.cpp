...
namespace CppProject
{
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		if (!ds_map_valid(map))
			return IntType(0);
		texlist = DsMap(map).Value(/*"textures"*/ STR(812));
		...
		if (!ds_map_valid(DsMap(map).Value(/*"textures"*/ STR(812))))
			return IntType(0);
		map = DsMap(map).Value(/*"textures"*/ STR(812));
	...
	RealType builder_generate(ScopeAny self)
	{
		...
		if (builder_get_waterlogged(Scope<obj_builder_thread>(self), sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z)))
		{
			...
			sInt(block_pos_y) = sInt(build_pos_y) * block_size_;
			sInt(block_pos_z) = sInt(build_pos_z) * block_size_;
			sInt(block_current) = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(/*"water"*/ STR(787));
	...
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		...
		log({ /*"Size"*/ STR(59), string(self->build_size_x) + /*" x "*/ STR(967) + string(self->build_size_y) + /*" x "*/ STR(967) + string(self->build_size_z) });
				...
				if (!string_contains(key, /*"_NBT_"*/ STR(974)))
				{
					...
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(805)) != /*"false"*/ STR(747))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(805)) == /*"true"*/ STR(746))
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1000) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(808), /*""*/ STR(0)));
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(820));
						ey = DsMap(entity).Value(/*"z"*/ STR(821));
						ez = DsMap(entity).Value(/*"y"*/ STR(803));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1000) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(808), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
