...
namespace CppProject
{
	...
	void nbt_debug_tag_compound(VarType name, VarType map)
	{
		...
		debug({ /*"[TAG_Compound] "*/ STR(1838) + name });
		debug({ /*"{"*/ STR(1201) });
		global::debug_indent++;
		while (!is_undefined(key))
		{
			...
			{
				switch ((IntType)DsMap(map).Value(key + /*"_NBT_type"*/ STR(1839)))
				{
					case e_nbt_TAG_BYTE:
					{
						debug({ /*"[TAG_Byte] "*/ STR(1840) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_SHORT:
					{
						debug({ /*"[TAG_Short] "*/ STR(1841) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_INT:
					{
						debug({ /*"[TAG_Int] "*/ STR(1842) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LONG:
					{
						debug({ /*"[TAG_Long] "*/ STR(1843) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_FLOAT:
					{
						debug({ /*"[TAG_Float] "*/ STR(1844) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_DOUBLE:
					{
						debug({ /*"[TAG_Double] "*/ STR(1845) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_BYTE_ARRAY:
					{
						debug({ /*"[TAG_Byte_Array] "*/ STR(1846) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(953), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1847)) });
						global::debug_indent--;
						break;
					}
					case e_nbt_TAG_STRING:
					{
						debug({ /*"[TAG_String] "*/ STR(1848) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LIST:
					{
						nbt_debug_tag_list(key, DsMap(map).Value(key), DsMap(map).Value(key + /*"_NBT_listtype"*/ STR(1849)));
						break;
					}
					...
					{
						debug({ /*"[TAG_Int_Array] "*/ STR(1850) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(953), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1847)) });
						global::debug_indent--;
						break;
					...
					{
						debug({ /*"[TAG_Long_Array] "*/ STR(1851) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(953), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1847)) });
						global::debug_indent--;
						break;
					...
					default:
						debug({ /*"[TAG_Empty] "*/ STR(1852) + key, DsMap(map).Value(key) });
						break;
				}
	...
	void nbt_debug_tag_list(VarType name, VarType list, VarType listtype)
	{
		...
		switch ((IntType)listtype)
		{
			case e_nbt_TAG_BYTE:
			{
				listtypestr = /*"TAG_Byte"*/ STR(1853);
				break;
			}
			case e_nbt_TAG_SHORT:
			{
				listtypestr = /*"TAG_Short"*/ STR(1854);
				break;
			}
			case e_nbt_TAG_INT:
			{
				listtypestr = /*"TAG_Int"*/ STR(1855);
				break;
			}
			case e_nbt_TAG_LONG:
			{
				listtypestr = /*"TAG_Long"*/ STR(1856);
				break;
			}
			case e_nbt_TAG_FLOAT:
			{
				listtypestr = /*"TAG_Float"*/ STR(1857);
				break;
			}
			case e_nbt_TAG_DOUBLE:
			{
				listtypestr = /*"TAG_Double"*/ STR(1858);
				break;
			}
			case e_nbt_TAG_BYTE_ARRAY:
			{
				listtypestr = /*"TAG_Byte_Array"*/ STR(1859);
				break;
			}
			case e_nbt_TAG_STRING:
			{
				listtypestr = /*"TAG_String"*/ STR(1860);
				break;
			}
			case e_nbt_TAG_LIST:
			{
				listtypestr = /*"TAG_List"*/ STR(1861);
				break;
			}
			case e_nbt_TAG_COMPOUND:
			{
				listtypestr = /*"TAG_Compound"*/ STR(1862);
				break;
			}
			case e_nbt_TAG_INT_ARRAY:
			{
				listtypestr = /*"TAG_Int_Array"*/ STR(1863);
				break;
			}
			case e_nbt_TAG_LONG_ARRAY:
			{
				listtypestr = /*"TAG_Long_Array"*/ STR(1864);
				break;
			}
			default:
				listtypestr = /*"TAG_Empty"*/ STR(1865);
				break;
		}
		
		debug({ /*"[TAG_List of "*/ STR(1866) + listtypestr + /*"] "*/ STR(1867) + name });
		debug({ /*"{"*/ STR(1201) });
		global::debug_indent++;
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_STRING:
				{
					debug({ string(DsList(list).Value(i)) + /*","*/ STR(1026) });
					break;
				}
				case e_nbt_TAG_BYTE_ARRAY:
				{
	...
	IntType nbt_read_tag_compound()
	{
		...
		while (!buffer_is_eof())
		{
			...
			{
				log({ /*"TAG_Compound error"*/ STR(1868), /*"Invalid tag"*/ STR(1869) });
				ds_map_destroy(map);
				return null_;
			}
			StringType name = buffer_read_string_short_be();
			DsMap(map)[name + /*"_NBT_type"*/ STR(1839)] = type;
			switch ((IntType)type)
			{
				case e_nbt_TAG_BYTE:
				{
				...
				case e_nbt_TAG_BYTE_ARRAY:
				{
					...
					DsMap(map)[name + /*"_NBT_length"*/ STR(1847)] = len;
					buffer_skip(len);
					break;
				}
				case e_nbt_TAG_STRING:
				...
				case e_nbt_TAG_LIST:
				{
					...
					DsMap(map)[name + /*"_NBT_listtype"*/ STR(1849)] = listtype;
					break;
				}
