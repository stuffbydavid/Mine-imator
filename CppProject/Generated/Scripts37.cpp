...
namespace CppProject
{
	...
	{
		return vec3(bend.Real(X_) * ease(/*"easeinoutquint"*/ STR(548), weight), bend.Real(Y_) * ease(/*"easeinoutquint"*/ STR(548), weight), bend.Real(Z_) * weight);
	}
	...
	VecType model_shape_get_bend_scale(ScopeAny self, RealType bendstart, RealType bendend, RealType weight, BoolType start, RealType bendpos, VecType bend)
	{
		...
		if (bendpos > bendstart && bendpos < bendend)
		{
			...
			for (IntType i = X_; i <= Z_; i++)
				bendscale.Real(i) = ease(/*"easeincubic"*/ STR(540), bendscale.Real(i));
	...
	void nbt_debug_tag_compound(VarType name, VarType map)
	{
		...
		debug({ /*"[TAG_Compound] "*/ STR(1833) + name });
		debug({ /*"{"*/ STR(1198) });
		global::debug_indent++;
		while (!is_undefined(key))
		{
			...
			{
				switch ((IntType)DsMap(map).Value(key + /*"_NBT_type"*/ STR(1834)))
				{
					case e_nbt_TAG_BYTE:
					{
						debug({ /*"[TAG_Byte] "*/ STR(1835) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_SHORT:
					{
						debug({ /*"[TAG_Short] "*/ STR(1836) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_INT:
					{
						debug({ /*"[TAG_Int] "*/ STR(1837) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LONG:
					{
						debug({ /*"[TAG_Long] "*/ STR(1838) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_FLOAT:
					{
						debug({ /*"[TAG_Float] "*/ STR(1839) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_DOUBLE:
					{
						debug({ /*"[TAG_Double] "*/ STR(1840) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_BYTE_ARRAY:
					{
						debug({ /*"[TAG_Byte_Array] "*/ STR(1841) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(950), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1842)) });
						global::debug_indent--;
						break;
					}
					case e_nbt_TAG_STRING:
					{
						debug({ /*"[TAG_String] "*/ STR(1843) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LIST:
					{
						nbt_debug_tag_list(key, DsMap(map).Value(key), DsMap(map).Value(key + /*"_NBT_listtype"*/ STR(1844)));
						break;
					}
					...
					{
						debug({ /*"[TAG_Int_Array] "*/ STR(1845) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(950), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1842)) });
						global::debug_indent--;
						break;
					...
					{
						debug({ /*"[TAG_Long_Array] "*/ STR(1846) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(950), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1842)) });
						global::debug_indent--;
						break;
	...
	void nbt_debug_tag_list(VarType name, VarType list, VarType listtype)
	{
		...
		debug({ /*"[TAG_List of "*/ STR(1859) + listtypestr + /*"] "*/ STR(1860) + name });
		debug({ /*"{"*/ STR(1198) });
		global::debug_indent++;
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_STRING:
				{
					debug({ string(DsList(list).Value(i)) + /*","*/ STR(1023) });
					break;
				}
				case e_nbt_TAG_BYTE_ARRAY:
				{
	...
	IntType nbt_read_tag_compound()
	{
		...
		while (!buffer_is_eof())
		{
			...
			{
				log({ /*"TAG_Compound error"*/ STR(1861), /*"Invalid tag"*/ STR(1862) });
				ds_map_destroy(map);
				return null_;
			}
			StringType name = buffer_read_string_short_be();
			DsMap(map)[name + /*"_NBT_type"*/ STR(1834)] = type;
			switch ((IntType)type)
			{
				case e_nbt_TAG_BYTE:
				{
				...
				case e_nbt_TAG_BYTE_ARRAY:
				{
					...
					DsMap(map)[name + /*"_NBT_length"*/ STR(1842)] = len;
					buffer_skip(len);
					break;
				}
				case e_nbt_TAG_STRING:
				...
				case e_nbt_TAG_LIST:
				{
					...
					DsMap(map)[name + /*"_NBT_listtype"*/ STR(1844)] = listtype;
					break;
				}
