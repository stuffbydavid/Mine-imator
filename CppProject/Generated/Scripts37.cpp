...
namespace CppProject
{
	...
	{
		return vec3(bend.Real(X_) * ease(/*"easeinoutquint"*/ STR(559), weight), bend.Real(Y_) * ease(/*"easeinoutquint"*/ STR(559), weight), bend.Real(Z_) * weight);
	}
	...
	VecType model_shape_get_bend_scale(ScopeAny self, RealType bendstart, RealType bendend, RealType weight, BoolType start, RealType bendpos, VecType bend)
	{
		if (bendpos > bendstart && bendpos < bendend)
		{
			...
			for (IntType i = X_; i <= Z_; i++)
				bendscale.Real(i) = ease(/*"easeincubic"*/ STR(551), bendscale.Real(i));
	...
	void nbt_debug_tag_compound(VarType name, VarType map)
	{
		...
		debug({ /*"[TAG_Compound] "*/ STR(1909) + name });
		debug({ /*"{"*/ STR(1250) });
		global::debug_indent++;
		while (!is_undefined(key))
		{
			...
			{
				switch ((IntType)DsMap(map).Value(key + /*"_NBT_type"*/ STR(1910)))
				{
					case e_nbt_TAG_BYTE:
					{
						debug({ /*"[TAG_Byte] "*/ STR(1911) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_SHORT:
					{
						debug({ /*"[TAG_Short] "*/ STR(1912) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_INT:
					{
						debug({ /*"[TAG_Int] "*/ STR(1913) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LONG:
					{
						debug({ /*"[TAG_Long] "*/ STR(1914) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_FLOAT:
					{
						debug({ /*"[TAG_Float] "*/ STR(1915) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_DOUBLE:
					{
						debug({ /*"[TAG_Double] "*/ STR(1916) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_BYTE_ARRAY:
					{
						debug({ /*"[TAG_Byte_Array] "*/ STR(1917) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(1002), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1918)) });
						global::debug_indent--;
						break;
					}
					case e_nbt_TAG_STRING:
					{
						debug({ /*"[TAG_String] "*/ STR(1919) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LIST:
					{
						nbt_debug_tag_list(key, DsMap(map).Value(key), DsMap(map).Value(key + /*"_NBT_listtype"*/ STR(1920)));
						break;
					}
					...
					{
						debug({ /*"[TAG_Int_Array] "*/ STR(1921) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(1002), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1918)) });
						global::debug_indent--;
						break;
					...
					{
						debug({ /*"[TAG_Long_Array] "*/ STR(1922) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(1002), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1918)) });
						global::debug_indent--;
						break;
					...
					default:
						debug({ /*"[TAG_Empty] "*/ STR(1923) + key, DsMap(map).Value(key) });
						break;
				}
	...
	void nbt_debug_tag_list(VarType name, VarType list, VarType listtype)
	{
		...
		switch ((IntType)listtype)
		{
			case e_nbt_TAG_BYTE:
			{
				listtypestr = /*"TAG_Byte"*/ STR(1924);
				break;
			}
			case e_nbt_TAG_SHORT:
			{
				listtypestr = /*"TAG_Short"*/ STR(1925);
				break;
			}
			case e_nbt_TAG_INT:
			{
				listtypestr = /*"TAG_Int"*/ STR(1926);
				break;
			}
			case e_nbt_TAG_LONG:
			{
				listtypestr = /*"TAG_Long"*/ STR(1927);
				break;
			}
			case e_nbt_TAG_FLOAT:
			{
				listtypestr = /*"TAG_Float"*/ STR(1928);
				break;
			}
			case e_nbt_TAG_DOUBLE:
			{
				listtypestr = /*"TAG_Double"*/ STR(1929);
				break;
			}
			case e_nbt_TAG_BYTE_ARRAY:
			{
				listtypestr = /*"TAG_Byte_Array"*/ STR(1930);
				break;
			}
			case e_nbt_TAG_STRING:
			{
				listtypestr = /*"TAG_String"*/ STR(1931);
				break;
			}
			case e_nbt_TAG_LIST:
			{
				listtypestr = /*"TAG_List"*/ STR(1932);
				break;
			}
			case e_nbt_TAG_COMPOUND:
			{
				listtypestr = /*"TAG_Compound"*/ STR(1933);
				break;
			}
			case e_nbt_TAG_INT_ARRAY:
			{
				listtypestr = /*"TAG_Int_Array"*/ STR(1934);
				break;
			}
			case e_nbt_TAG_LONG_ARRAY:
			{
				listtypestr = /*"TAG_Long_Array"*/ STR(1935);
				break;
			}
			default:
				listtypestr = /*"TAG_Empty"*/ STR(1936);
				break;
		}
		
		debug({ /*"[TAG_List of "*/ STR(1937) + listtypestr + /*"] "*/ STR(1938) + name });
		debug({ /*"{"*/ STR(1250) });
		global::debug_indent++;
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_STRING:
				{
					debug({ string(DsList(list).Value(i)) + /*","*/ STR(1075) });
					break;
				}
				case e_nbt_TAG_BYTE_ARRAY:
				{
	...
	IntType nbt_read_tag_compound()
	{
		...
		while (!buffer_is_eof())
		{
			...
			{
				log({ /*"TAG_Compound error"*/ STR(1939), /*"Invalid tag"*/ STR(1940) });
				ds_map_destroy(map);
				return null_;
			}
			StringType name = buffer_read_string_short_be();
			DsMap(map)[name + /*"_NBT_type"*/ STR(1910)] = type;
			switch ((IntType)type)
			{
				case e_nbt_TAG_BYTE:
				{
				...
				case e_nbt_TAG_BYTE_ARRAY:
				{
					...
					DsMap(map)[name + /*"_NBT_length"*/ STR(1918)] = len;
					buffer_skip(len);
					break;
				}
				case e_nbt_TAG_STRING:
				...
				case e_nbt_TAG_LIST:
				{
					...
					DsMap(map)[name + /*"_NBT_listtype"*/ STR(1920)] = listtype;
					break;
				}
