...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		...
		{
			log({ /*"Missing parameter \"type\""*/ STR(1845) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(814))))
		{
			log({ /*"Missing array \"from\""*/ STR(1846) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(815))))
		{
			log({ /*"Missing array \"to\""*/ STR(1847) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(823))))
		{
			log({ /*"Missing array \"uv\""*/ STR(1848) });
			return null_;
		}
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			...
			self->use_model_color = value_get_real(DsMap(map).Value(/*"use_model_color"*/ STR(1849)), false);
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1850)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1843)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1851)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1852)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1853))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1853));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1854)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1855)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1856)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(814)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(815)));
			if (self->type == /*"plane"*/ STR(1857))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1858)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1857))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1857))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1859)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1827)), true);
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(823)));
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(804))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(804));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1860))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1860));
			}
			if (self->type == /*"block"*/ STR(4))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(874) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1861), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(10) && ext != /*".jpg"*/ STR(708) && ext != /*".jpeg"*/ STR(709))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1862) });
				return null_;
			}
			...
			if (is_string(DsMap(map).Value(/*"file"*/ STR(119))))
				self->file = model_file_load(dir + DsMap(map).Value(/*"file"*/ STR(119)));
			else
				self->file = null_;
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(544))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(544));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(544))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(544))));
				}
				else
			...
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(861)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(797))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(797))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(745) + curstate))
						log({ /*"model/state/"*/ STR(1863) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(9) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1864) + self->value_name.Value(v) + dev_mode_name_translation_message });
							if (!is_undefined(DsMap(curvalue).Value(/*"file"*/ STR(119))))
								self->value_file[v] = model_file_load(dir + DsMap(curvalue).Value(/*"file"*/ STR(119)));
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(544))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(544));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(544))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(544)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(544)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(544)), true);
								}
							
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1865)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(720))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(720))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1866))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1866))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1867))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1867)), true);
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(861))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(861));
						}
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1857) && self->is3d > 0)
				{
