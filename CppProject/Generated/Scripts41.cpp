...
namespace CppProject
{
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2014), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2014), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, { /*"newprojectname"*/ STR(1997), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2014), sVar(project_name) }), /*"top"*/ STR(187) }) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2014), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2001), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2002) }))
		{
		...
		if (draw_button_label(self, { /*"saveassave"*/ STR(2015), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT }))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2016), idVar(sInt(popup), name) });
	...
	RealType popup_switch(ScopeAny self, IntType argument0)
	{
		...
		sVar(window_busy) = /*"popup"*/ STR(1563) + idVar(sInt(popup_switch_to), name);
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2017), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
		...
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2018), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
		...
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2019) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2020) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(106), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2021), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2022), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2023) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"upgradecontinue"*/ STR(2024), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT }))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2025);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2026) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"welcomecontinue"*/ STR(2027), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right }))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2028) : /*"previewmove"*/ STR(2029));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2028))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2029))
			{
		...
		withOne (Object, preview, self->id)
		{
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uColorsExt"*/ STR(2030), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2031), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2032), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2033), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2034), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2035), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2036), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2037), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2038), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2039) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2038), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2039) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2040), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2041) : /*"tooltipplay"*/ STR(2042) }))
			{
				if (isplaying > 0)
				{
	...
	void preview_event_create(ScopeAny self)
	{
		...
		sStr(pack_image) = /*"preview"*/ STR(1345);
		sStr(pack_image_material) = /*"diffuse"*/ STR(606);
		sStr(pack_model_texture) = /*"entity/player/wide/steve"*/ STR(2043);
	...
	void project_backup(ScopeAny self)
	{
		...
		log({ /*"Backup"*/ STR(2044), fn });
		for (RealType b = sReal(setting_backup_amount) - IntType(1); b > IntType(0); b--)
			if (file_exists_lib(fn + /*".backup"*/ STR(2045) + string(b)))
				file_rename_lib(fn + /*".backup"*/ STR(2045) + string(b), fn + /*".backup"*/ STR(2045) + string(b + IntType(1)));
		project_save(self, { fn + /*".backup"*/ STR(2045) + ((sReal(setting_backup_amount) > IntType(1)) ? /*"1"*/ STR(1081) : /*""*/ STR(0)) });
		project_reset_backup(self);
		log({ /*"Backup saved"*/ STR(2046) });
	}
	...
	RealType project_create(ScopeAny self)
	{
		...
		log({ /*"Creating project"*/ STR(2048), dirname });
		project_reset(self);
