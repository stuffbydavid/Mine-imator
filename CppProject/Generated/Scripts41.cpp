...
namespace CppProject
{
	...
	void popup_exportmovie_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
			...
			text = text_get({ /*"projectvideosizetemplate"*/ STR(1433) + ObjType(obj_videotemplate, sVar(project_video_template))->name }) + /*" ("*/ STR(1112) + string(ObjType(obj_videotemplate, sVar(project_video_template))->width) + /*"x"*/ STR(822) + string(ObjType(obj_videotemplate, sVar(project_video_template))->height) + /*")"*/ STR(1088);
	...
	{
		draw_label(self, { text_get({ /*"importimagetype"*/ STR(2030) }) + /*":"*/ STR(760), sVar(dx), sVar(dy) + IntType(14), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(28);
		tab_control_checkbox(self);
	...
	void popup_newproject_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2047), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, idVar(sInt(popup), folder), /*"top"*/ STR(196)) || issue)
		{
		...
		tab_next(self);
		tab_control_textfield(self, true);
		if (draw_textfield(self, /*"newprojectauthor"*/ STR(2048), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_author), null_, /*""*/ STR(0), /*"top"*/ STR(196)))
		{
			idVar(sInt(popup), author) = ObjType(obj_textbox, idInt(sInt(popup), tbx_author))->text;
		}
		tab_next(self);
		tab_control_textfield(self, true, IntType(76));
		if (draw_textfield(self, /*"newprojectdescription"*/ STR(2049), sVar(dx), sVar(dy), sVar(dw), IntType(76), idInt(sInt(popup), tbx_description), null_, /*""*/ STR(0), /*"top"*/ STR(196)))
		{
