...
namespace CppProject
{
	...
	{
		draw_label(self, { text_get({ /*"patterneditorcolors"*/ STR(2060) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(18);
		VarType colorsx;
		colorsx = sVar(dx);
		...
		{
			sVar(dy) += IntType(8);
			draw_label(self, { text_get({ /*"patterneditorpatterns"*/ STR(2061) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(18);
	...
	void popup_pattern_editor_show(ScopeAny self, VarType obj)
	{
		withOne (obj_popup, sInt(popup_pattern_editor), self->id)
		{
			...
			if (self->pattern_resource != res || array_length(VarType::CreateRef(self->pattern_sprites)) == IntType(0))
			{
				...
				self->res_ratio = ceil((RealType)sprite_get_width((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1807)))) / sprite_get_width((IntType)(DsMap(ObjType(obj_resource, global::mc_res)->model_texture_map).Value(/*"entity/banner_base"*/ STR(1807)))));
				...
				for (IntType i = IntType(0); i < ds_list_size(global::minecraft_pattern_list); i++)
				{
					...
					array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner/"*/ STR(1809) + bannername), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
				}
				global::shader_mask = false;
				array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1807)), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2062), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2062), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2044), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2062), sVar(project_name) }), /*"top"*/ STR(188)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2062), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2049), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2050) }))
		{
		...
		if (draw_button_label(self, /*"saveassave"*/ STR(2063), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2064), idVar(sInt(popup), name) });
	...
	RealType popup_switch(ScopeAny self, IntType argument0)
	{
		...
		sVar(window_busy) = /*"popup"*/ STR(1610) + idVar(sInt(popup_switch_to), name);
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		pagealpha = ease(/*"easeoutcirc"*/ STR(564), idReal(sInt(popup), page_ani));
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2065), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
		...
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2066), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
		...
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2067) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2068) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(106), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2069), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2070), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2071) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, /*"upgradecontinue"*/ STR(2072), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2073);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2074) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"welcomecontinue"*/ STR(2075), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2076) : /*"previewmove"*/ STR(2077));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2076))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2077))
			{
		...
		withOne (Object, preview, self->id)
		{
			...
			if (isplaying > 0)
				sBool(update) = true;
			if (idVar(sVar(select), type) == e_res_type_PACK && sStr(pack_image) == /*"blocksheet"*/ STR(536) && sReal(pack_block_sheet_ani) > 0)
				sBool(update) = true;
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uFogShow"*/ STR(1929), IntType(0));
					render_set_uniform_int(/*"uColorsExt"*/ STR(2078), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2079), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2080), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2081), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2082), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2083), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2084), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2085), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2086), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2087) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2086), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2087) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2088), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2089) : /*"tooltipplay"*/ STR(2090) }))
			{
				if (isplaying > 0)
				{
