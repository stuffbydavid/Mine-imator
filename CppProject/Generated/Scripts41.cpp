...
namespace CppProject
{
	void popup_pattern_editor_draw_layer(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, VarType index, RealType base)
	{
		...
		editname = /*"patterneditoreditlayer"*/ STR(2091) + string(index);
		...
		VarType name = ((base > 0) ? text_get({ /*"patterneditorbase"*/ STR(2092) }) : text_get({ /*"patterneditorlayer"*/ STR(2093), index + IntType(1) }));
		...
		if (!(base > 0))
		{
			...
			if (draghover)
			{
				...
				if (sReal(mouse_move) > IntType(0) && !dragging)
				{
					...
					sVar(window_busy) = /*"patterneditordraglayer"*/ STR(2094);
				}
			}
			...
			if (mouseon && !dragging)
			{
				if (draw_button_icon(self, { /*"patterneditorremovelayer"*/ STR(2095) + string(index), actionx - IntType(24), yy + height / 2.0 - IntType(14), IntType(24), IntType(24), false, icons_DELETE }))
		...
		if (mouseon && !dragging)
		{
			...
			if (sStr(settings_menu_name) == editname && sStr(settings_menu_ani_type) != /*"hide"*/ STR(760))
	...
	{
		draw_label(self, { text_get({ /*"patterneditorcolors"*/ STR(2096) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(18);
		VarType colorsx;
		colorsx = sVar(dx);
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2098), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2098), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2080), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2098), sVar(project_name) }), /*"top"*/ STR(190)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2098), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2085), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2086) }))
		{
		...
		if (draw_button_label(self, /*"saveassave"*/ STR(2099), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2100), idVar(sInt(popup), name) });
	...
	RealType popup_switch(ScopeAny self, IntType argument0)
	{
		...
		sVar(window_busy) = /*"popup"*/ STR(1626) + idVar(sInt(popup_switch_to), name);
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		pagealpha = ease(/*"easeoutcirc"*/ STR(567), idReal(sInt(popup), page_ani));
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2101), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
		...
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2102), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
		...
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2103) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2104) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(106), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2105), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2106), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2107) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, /*"upgradecontinue"*/ STR(2108), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2109);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2110) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"welcomecontinue"*/ STR(2111), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2112) : /*"previewmove"*/ STR(2113));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2112))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2113))
			{
				sInt(mouse_cursor) = cr_size_all;
		...
		withOne (Object, preview, self->id)
		{
			...
			if (isplaying > 0)
				sBool(update) = true;
			if (idVar(sVar(select), type) == e_res_type_PACK && sStr(pack_image) == /*"blocksheet"*/ STR(539) && sReal(pack_block_sheet_ani) > 0)
				sBool(update) = true;
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uFogShow"*/ STR(1959), IntType(0));
					render_set_uniform_int(/*"uColorsExt"*/ STR(2114), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2115), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2116), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2117), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2118), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2119), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2120), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2121), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2122), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2123) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2122), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2123) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2124), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2125) : /*"tooltipplay"*/ STR(2126) }))
			{
				if (isplaying > 0)
				{
