...
namespace CppProject
{
	...
	void popup_about_draw(ScopeAny self)
	{
		...
		text = text_get({ /*"aboutversion"*/ STR(1956), mineimator_version_full }) + text_get({ /*"aboutreleasedate"*/ STR(1957), mineimator_version_date });
		width = string_width(text);
		textx = floor(sVar(dx) + (RealType)sVar(dw) / 2.0 - width / 2.0);
		VarType version = text_get({ /*"aboutversion"*/ STR(1956), mineimator_version_full }) + ((global::trial_version > 0) ? /*" "*/ STR(21) + text_get({ /*"startuptrial"*/ STR(1958) }) : /*""*/ STR(0));
		...
		draw_label(self, { text_get({ /*"aboutreleasedate"*/ STR(1957), mineimator_version_date }), textx, sVar(dy) + IntType(98), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary });
		RealType mctext, mctextx;
		mctext = string_width(text_get({ /*"aboutminecraftpre"*/ STR(1959) }) + text_get({ /*"aboutminecraft"*/ STR(1960) }));
		mctextx = floor(sVar(content_x) + ((RealType)sVar(content_width) / 2.0) - (mctext / 2.0));
		draw_label(self, { text_get({ /*"aboutminecraftpre"*/ STR(1959) }), mctextx, sVar(dy) + IntType(98) + IntType(19), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary });
		mctextx += string_width(text_get({ /*"aboutminecraftpre"*/ STR(1959) }));
		draw_button_text(self, { text_get({ /*"aboutminecraft"*/ STR(1960) }), mctextx, sVar(dy) + IntType(98) + IntType(19), ID_popup_open_url, link_minecraft, link_minecraft });
		...
		if (draw_button_icon(self, { /*"aboutsite"*/ STR(1961), buttonx, buttony, IntType(24), IntType(24), false, icons_WORLD, null_, false, /*"aboutsite"*/ STR(1961) }))
			open_url(link_website);
		if (draw_button_icon(self, { /*"aboutforums"*/ STR(1962), buttonx + (IntType(30)), buttony, IntType(24), IntType(24), false, icons_COMMENTS, null_, false, /*"aboutforums"*/ STR(1962) }))
			open_url(link_forums);
		if (draw_button_icon(self, { /*"abouttwitter"*/ STR(1963), buttonx + (IntType(30) * IntType(2)), buttony, IntType(24), IntType(24), false, icons_TWITTER, null_, false, /*"abouttwitter"*/ STR(1963) }))
			open_url(link_twitter);
		if (draw_button_icon(self, { /*"aboutdiscord"*/ STR(1964), buttonx + (IntType(30) * IntType(3)), buttony, IntType(24), IntType(24), false, icons_DISCORD, null_, false, /*"aboutdiscord"*/ STR(1964) }))
			open_url(link_discord);
		if (global::trial_version > 0)
		{
			if (draw_button_label(self, /*"aboutupgrade"*/ STR(1965), sVar(content_x) + sVar(content_width) - IntType(13), sVar(content_y) + sVar(content_height) - (IntType(12) + IntType(32)), null_, icons_KEY, e_button_PRIMARY, null_, fa_right))
			{
		...
		{
			if (draw_button_label(self, /*"aboutdonate"*/ STR(1966), sVar(content_x) + sVar(content_width) - IntType(13), sVar(content_y) + sVar(content_height) - (IntType(12) + IntType(32)), null_, icons_DONATE, e_button_PRIMARY, null_, fa_right))
				open_url(link_donate);
		}
		
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutcreatedby"*/ STR(1967) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_button_text(self, { /*"David Andrei"*/ STR(1968), sVar(dx), sVar(dy), ID_popup_open_url, link_david, link_david, sInt(font_label) });
		sVar(dy) += IntType(34);
		draw_label(self, { text_get({ /*"aboutdevelopment"*/ STR(1969) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_button_text(self, { /*"David"*/ STR(1970), sVar(dx), sVar(dy), ID_popup_open_url, link_david, link_david, sInt(font_label) });
		sVar(dy) += IntType(19);
		draw_label(self, { /*"Nimi"*/ STR(1971), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(19);
		draw_label(self, { /*"Marvin"*/ STR(1972), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(19);
		draw_label(self, { /*"mbanders"*/ STR(1973), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(34);
		draw_label(self, { text_get({ /*"aboutuibranding"*/ STR(1974) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
		draw_label(self, { /*"Voxy"*/ STR(1975), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dx) += IntType(130) + IntType(24);
		sVar(dy) = sVar(content_y) + IntType(176);
		ArrType list = ArrType::From({ /*"9redwoods"*/ STR(1976), /*"Alpha Toostrr"*/ STR(1977), /*"AnxiousCynic"*/ STR(1978), /*"Cade [CaZaKoJa]"*/ STR(1979), /*"Hozq"*/ STR(1980), /*"Jnick"*/ STR(1981), /*"Jossamations"*/ STR(1982), /*"KeepOnChucking"*/ STR(1983), /*"Rollo"*/ STR(1984), /*"SKIBBZ"*/ STR(1985), /*"SoundsDotZip"*/ STR(1986), /*"Swooplezz"*/ STR(1987), /*"UpgradedMoon"*/ STR(1988), /*"Vash"*/ STR(1989), /*"__Mine__"*/ STR(1990) });
		sVar(dy) += IntType(12);
		draw_label(self, { text_get({ /*"aboutbetatesting"*/ STR(1991) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(26);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"advancedinfo"*/ STR(1992) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		IntType buttonx = string_width(text_get({ /*"advancedenable"*/ STR(1993) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"advancedenable"*/ STR(1993), sVar(dx) + sVar(dw) - buttonx, sVar(dy)))
		{
			action_setting_program_mode(self, true);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"advancednotnow"*/ STR(1994) })) + button_padding);
		if (draw_button_label(self, /*"advancednotnow"*/ STR(1994), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
			popup_close(self);
		tab_next(self);
	}
	...
	VarType popup_armor_editor_draw_piece_type(ScopeAny self, StringType piece, IntType pieceid, RealType capwid)
	{
		...
		for (IntType i = IntType(0); i < statelen; i += IntType(2))
		{
			...
			{
				w -= (28.0 + 8.0);
				if (draw_button_color(self, /*"armoreditordye"*/ STR(1995) + piece, sVar(dx) + sVar(dw) - IntType(24), sVar(dy), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(1)), minecraft_get_color(/*"other:leather"*/ STR(1996)), false, ID_action_armor_editor))
				{
	...
	void popup_armor_editor_draw(ScopeAny self)
	{
		...
		popup_armor_editor_draw_piece(self, /*"helmet"*/ STR(1997), IntType(0), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"chestplate"*/ STR(1998), IntType(4), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"leggings"*/ STR(1999), IntType(8), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"boots"*/ STR(2000), IntType(12), capwid);
	}
	...
	void popup_downloadskin_draw(ScopeAny self)
	{
		...
		draw_textfield(self, /*"downloadskinusername"*/ STR(2001), sVar(dx), sVar(dy), sVar(dw) - IntType(28), IntType(24), idInt(sInt(popup), tbx_username), null_);
		BoolType download = keyboard_check_pressed(vk_enter);
		if (draw_button_icon(self, { /*"downloadskindownload"*/ STR(2002), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(18), IntType(24), IntType(24), false, icons_DOWNLOAD, null_, ObjType(obj_textbox, idInt(sInt(popup), tbx_username))->text == /*""*/ STR(0) || sInt(http_downloadskin) != null_, /*"tooltipdownloadskin"*/ STR(2003) }))
			download = true;
		...
		{
			draw_label(self, { text_get({ /*"downloadskindownloading"*/ STR(2004) }), sVar(dx), sVar(dy) + IntType(8), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"downloadskindone"*/ STR(2005), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT, !(idVar(sInt(popup), texture) > 0)))
		{
			popup_close(self);
	...
	RealType popup_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"popupmove"*/ STR(2006))
			draw_set_alpha(0.5);
		...
		if (sVar(window_busy) == /*"popupclick"*/ STR(2007))
		{
			if (sReal(mouse_move) > IntType(10))
			{
				...
				sVar(window_busy) = /*"popupmove"*/ STR(2006);
			}
			else
				if (!sBool(mouse_left))
					sVar(window_busy) = /*""*/ STR(0);
		...
		if (sBool(popup_mouseon) && sInt(mouse_cursor) == cr_default && sBool(mouse_left_pressed))
			sVar(window_busy) = /*"popupclick"*/ STR(2007);
	...
	void popup_exportimage_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportimageimagesizecustomwidth"*/ STR(2008), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_image_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportimageimagesizecustomheight"*/ STR(2009), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_image_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, false);
			draw_textfield_group(self, /*"exportimageimagesizecustom"*/ STR(2010), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false);
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, /*"exportimageimagesizecustomkeepaspectratio"*/ STR(2011), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio);
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageremovebackground"*/ STR(2012), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportimage_remove_background);
		tab_next(self);
		if (idBool(sInt(popup), remove_background))
			draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(2013), icons_WARNING_TRIANGLE, e_toast_WARNING);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimageincludehidden"*/ STR(2014), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportimage_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagehighquality"*/ STR(2015), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportimage_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportimagewatermark"*/ STR(2016), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportimage_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, /*"exportimagesave"*/ STR(2017), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportimage_save, e_anchor_RIGHT);
		tab_next(self);
	}
	...
	void popup_exportmovie_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
		{
			textfield_group_add(self, { /*"exportmovievideosizecustomwidth"*/ STR(2018), sVar(project_video_width), IntType(1280), ID_action_project_video_width, X_, idInt(sInt(popup), tbx_video_size_custom_width), null_, IntType(1), IntType(1), surface_get_max_size() });
			textfield_group_add(self, { /*"exportmovievideosizecustomheight"*/ STR(2019), sVar(project_video_height), IntType(720), ID_action_project_video_height, X_, idInt(sInt(popup), tbx_video_size_custom_height), null_, IntType(1), IntType(1), surface_get_max_size() });
			tab_control_textfield_group(self, false);
			draw_textfield_group(self, /*"exportmovievideosizecustom"*/ STR(2020), sVar(dx), sVar(dy), sVar(dw), IntType(1), IntType(1), no_limit, IntType(1), false);
			tab_next(self);
			tab_control_switch(self);
			draw_switch(self, /*"exportmovievideosizecustomkeepaspectratio"*/ STR(2021), sVar(dx), sVar(dy), sVar(project_video_keep_aspect_ratio), ID_action_project_video_keep_aspect_ratio);
			tab_next(self);
			sVar(dy) += IntType(8);
		}
		tab_control_menu(self);
		...
		if (idStr(sInt(popup), format) != /*"png"*/ STR(64))
		{
			...
			if (idInt(sInt(popup), video_quality) == IntType(0))
			{
				...
				draw_dragger(self, /*"exportmoviebitrate"*/ STR(2022), sVar(dx), sVar(dy), dragger_width, idReal(sInt(popup), bit_rate), IntType(500), IntType(1), no_limit, IntType(2500000), IntType(1), idInt(sInt(popup), tbx_bit_rate), ID_action_toolbar_exportmovie_bit_rate);
				tab_next(self);
			}
		}
		...
		{
			tab_control_dragger(self);
			draw_dragger(self, /*"exportmovieframespersecond"*/ STR(2023), sVar(dx), sVar(dy), dragger_width, idReal(sInt(popup), framespersecond), IntType(1), IntType(1), IntType(120), IntType(30), IntType(1), idInt(sInt(popup), tbx_framespersecond), ID_action_toolbar_exportmovie_framespersecond);
			tab_next(self);
		}
		if (idStr(sInt(popup), format) == /*"png"*/ STR(64))
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieremovebackground"*/ STR(2024), sVar(dx), sVar(dy), idBool(sInt(popup), remove_background), ID_action_toolbar_exportmovie_remove_background);
			tab_next(self);
			if (idBool(sInt(popup), remove_background))
				draw_tooltip_label(self, /*"exportimageblendmodewarning"*/ STR(2013), icons_WARNING_TRIANGLE, e_toast_WARNING);
		}
		...
		{
			tab_control_checkbox(self);
			draw_checkbox(self, /*"exportmovieincludeaudio"*/ STR(2025), sVar(dx), sVar(dy), idBool(sInt(popup), include_audio), ID_action_toolbar_exportmovie_include_audio);
			tab_next(self);
		}
		
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmovieincludehidden"*/ STR(2026), sVar(dx), sVar(dy), idBool(sInt(popup), include_hidden), ID_action_toolbar_exportmovie_include_hidden);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviehighquality"*/ STR(2027), sVar(dx), sVar(dy), idReal(sInt(popup), high_quality), ID_action_toolbar_exportmovie_high_quality);
		tab_next(self);
		tab_control_checkbox(self);
		draw_checkbox(self, /*"exportmoviewatermark"*/ STR(2028), sVar(dx), sVar(dy), idBool(sInt(popup), watermark), ID_action_toolbar_exportmovie_watermark);
		tab_next(self);
		tab_control_button_label(self);
		draw_button_label(self, /*"exportmoviesave"*/ STR(2029), sVar(dx) + sVar(dw), sVar(dy), null_, icons_SAVE, e_button_PRIMARY, ID_action_toolbar_exportmovie_save, e_anchor_RIGHT);
		tab_next(self);
	}
	
	void popup_importimage_draw(ScopeAny self)
	{
		draw_label(self, { text_get({ /*"importimagetype"*/ STR(2030) }) + /*":"*/ STR(758), sVar(dx), sVar(dy) + IntType(14), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(28);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageskin"*/ STR(2031), sVar(dx), sVar(dy), e_res_type_SKIN, idVar(sInt(popup), type) == e_res_type_SKIN, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageitemsheet"*/ STR(2032), sVar(dx), sVar(dy), e_res_type_ITEM_SHEET, idVar(sInt(popup), type) == e_res_type_ITEM_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageblocksheet"*/ STR(2033), sVar(dx), sVar(dy), e_res_type_BLOCK_SHEET, idVar(sInt(popup), type) == e_res_type_BLOCK_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimageparticlesheet"*/ STR(2034), sVar(dx), sVar(dy), e_res_type_PARTICLE_SHEET, idVar(sInt(popup), type) == e_res_type_PARTICLE_SHEET, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_checkbox(self);
		draw_radiobutton(self, /*"importimagetexture"*/ STR(2035), sVar(dx), sVar(dy), e_res_type_TEXTURE, idVar(sInt(popup), type) == e_res_type_TEXTURE, ID_action_toolbar_importimage_type);
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"importimageok"*/ STR(2036), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
	...
	void popup_importitemsheet_draw(ScopeAny self)
	{
		...
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, /*"importitemsheetissheet"*/ STR(2037), sVar(dx), sVar(dy), idBool(sInt(popup), is_sheet), ID_action_toolbar_importitemsheet_is_sheet);
		tab_next(self);
		if (idBool(sInt(popup), is_sheet))
		{
			...
			draw_dragger(self, /*"importitemsheetrows"*/ STR(2038), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(X_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(X_), IntType(1), idInt(sInt(popup), tbx_sheet_width), ID_action_toolbar_importitemsheet_sheet_size);
			tab_next(self);
			global::axis_edit = Y_;
			tab_control(self, IntType(28));
			draw_dragger(self, /*"importitemsheetcolumns"*/ STR(2039), sVar(dx), sVar(dy), dragger_width, idVec(sInt(popup), sheet_size).Real(Y_), 1.0 / 10.0, IntType(1), no_limit, idVec(sInt(popup), sheet_size_def).Real(Y_), IntType(1), idInt(sInt(popup), tbx_sheet_height), ID_action_toolbar_importitemsheet_sheet_size);
			tab_next(self);
		}
		tab_control_button_label(self);
		if (draw_button_label(self, /*"importimageok"*/ STR(2036), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_loading_draw(ScopeAny self)
	{
		...
		if (idReal(sInt(popup), load_amount) > IntType(1))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), progress, text_get({ /*"loadingresources"*/ STR(2040) }), text_get({ /*"loadingpercent"*/ STR(2041), string(floor(progress * IntType(100))) }));
			tab_next(self);
		}
		tab_control_loading(self);
	...
	void popup_modelbench_draw(ScopeAny self)
	{
		...
		draw_label(self, { string_limit_ext(text_get({ /*"modelbenchinfo"*/ STR(2042) }), sVar(dw) + IntType(24), sVar(dh)), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy), fa_center, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(34) + IntType(30);
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"modelbenchdownload"*/ STR(2043) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"modelbenchdownload"*/ STR(2043), sVar(dx) + sVar(dw) - buttonx, sVar(dy)))
		{
			open_url(link_modelbench);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"modelbenchnotnow"*/ STR(2044) })) + button_padding);
		if (draw_button_label(self, /*"modelbenchnotnow"*/ STR(2044), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
			popup_close(self);
		if (draw_checkbox(self, /*"modelbenchdontshow"*/ STR(2045), sVar(dx), sVar(dy) + IntType(4), ObjType(obj_popup, sInt(popup_modelbench))->hidden, null_))
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_newproject))->tbx_name)->text = text_get({ /*"newprojectnamedefault"*/ STR(2046) });
	...
	void popup_newproject_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2047), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, idVar(sInt(popup), folder), /*"top"*/ STR(194)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"newprojectnamedefault"*/ STR(2046) });
			idVar(sInt(popup), folder) = filename_name(filename_get_unique(sStr(setting_project_folder) + idVar(sInt(popup), folder)));
		}
		tab_next(self);
		tab_control_textfield(self, true);
		if (draw_textfield(self, /*"newprojectauthor"*/ STR(2048), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_author), null_, /*""*/ STR(0), /*"top"*/ STR(194)))
		{
			idVar(sInt(popup), author) = ObjType(obj_textbox, idInt(sInt(popup), tbx_author))->text;
		}
		tab_next(self);
		tab_control_textfield(self, true, IntType(76));
		if (draw_textfield(self, /*"newprojectdescription"*/ STR(2049), sVar(dx), sVar(dy), sVar(dw), IntType(76), idInt(sInt(popup), tbx_description), null_, /*""*/ STR(0), /*"top"*/ STR(194)))
		{
			idVar(sInt(popup), description) = ObjType(obj_textbox, idInt(sInt(popup), tbx_description))->text;
		}
		tab_next(self);
		StringType directory = /*"../"*/ STR(2050) + directory_name(sStr(setting_project_folder)) + string_remove_newline(idVar(sInt(popup), folder));
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2052), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, false, /*"tooltipchangefolder"*/ STR(2053) }))
		{
		...
		tab_next(self);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"newprojectcreate"*/ STR(2054), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
	...
	void popup_pattern_editor_draw(ScopeAny self)
	{
		...
		if (draw_button_label(self, /*"patterneditoraddlayer"*/ STR(2055), sVar(dx), sVar(dy), listw, icons_PLUS, e_button_SECONDARY))
		{
		...
		draw_set_font(sInt(font_button));
		IntType buttonx = string_width(text_get({ /*"patterneditordone"*/ STR(2056) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"patterneditordone"*/ STR(2056), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy)))
		{
		...
		buttonx += IntType(12) + (string_width(text_get({ /*"patterneditorcancel"*/ STR(2057) })) + button_padding);
		if (draw_button_label(self, /*"patterneditorcancel"*/ STR(2057), sVar(dx_start) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
		{
			array_add(VarType::CreateRef(global::pattern_update), idVar(sInt(popup), pattern_edit));
			instance_destroy(self, { idInt(sInt(popup), pattern_edit_preview), false });
			idInt(sInt(popup), pattern_edit_preview) = null_;
