...
namespace CppProject
{
	...
	void popup_pattern_editor_edit_layer(ScopeAny self)
	{
		...
		{
			if (draw_button_swatch(self, colorsx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1681) + ObjType(obj_swatch, global::minecraft_swatch_dyes)->name + ObjType(obj_swatch, global::minecraft_swatch_dyes)->color_names.Value(c), ObjType(obj_swatch, global::minecraft_swatch_dyes)->colors.Value(c)))
			{
	...
	void popup_pattern_editor_show(ScopeAny self, VarType obj)
	{
		withOne (obj_popup, sInt(popup_pattern_editor), self->id)
		{
			...
			if (self->pattern_resource != res || array_length(VarType::CreateRef(self->pattern_sprites)) == IntType(0))
			{
				...
				self->res_ratio = ceil((RealType)sprite_get_width((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1795)))) / sprite_get_width((IntType)(DsMap(ObjType(obj_resource, global::mc_res)->model_texture_map).Value(/*"entity/banner_base"*/ STR(1795)))));
				...
				for (IntType i = IntType(0); i < ds_list_size(global::minecraft_pattern_list); i++)
				{
					...
					array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner/"*/ STR(1797) + bannername), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
				}
				global::shader_mask = false;
				array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1795)), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
