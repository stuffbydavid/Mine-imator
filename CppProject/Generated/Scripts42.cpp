...
namespace CppProject
{
	void preview_event_create(ScopeAny self)
	{
		...
		sStr(pack_image) = /*"preview"*/ STR(1396);
		sStr(pack_image_material) = /*"diffuse"*/ STR(649);
		sStr(pack_model_texture) = /*"entity/player/wide/steve"*/ STR(2127);
	...
	void project_backup(ScopeAny self)
	{
		...
		log({ /*"Backup"*/ STR(2128), fn });
		for (RealType b = sReal(setting_backup_amount) - IntType(1); b > IntType(0); b--)
			if (file_exists_lib(fn + /*".backup"*/ STR(2129) + string(b)))
				file_rename_lib(fn + /*".backup"*/ STR(2129) + string(b), fn + /*".backup"*/ STR(2129) + string(b + IntType(1)));
		project_save(self, { fn + /*".backup"*/ STR(2129) + ((sReal(setting_backup_amount) > IntType(1)) ? /*"1"*/ STR(1132) : /*""*/ STR(0)) });
		project_reset_backup(self);
		log({ /*"Backup saved"*/ STR(2130) });
	}
	...
	RealType project_create(ScopeAny self)
	{
		...
		log({ /*"Creating project"*/ STR(2132), dirname });
		project_reset(self);
	...
	RealType project_load(ScopeAny self, VarArgs argument)
	{
		...
		{
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miproject;.mproj;.mani"*/ STR(2135));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miproject;.mproj;.mani"*/ STR(2135));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenprojectzip"*/ STR(2136));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		BoolType legacy;
		ext = filename_ext(fn);
		if (ext == /*".miproject"*/ STR(727) || string_contains(ext, /*".backup"*/ STR(2129)))
		{
			log({ /*"Opening project"*/ STR(2137), fn });
		...
		{
			log({ /*"Opening legacy project"*/ STR(2138), fn });
		...
		sVar(project_file) = filename_new_ext(fn, /*".miproject"*/ STR(727));
		...
		if (!legacy)
		{
			...
			project_load_markers(DsMap(rootmap).Value(/*"markers"*/ STR(2139)));
		}
		else
		{
		...
		project_reset_loaded(self);
		log({ /*"Project loaded"*/ STR(2140) });
		if (global::load_format < e_project_FORMAT_110_PRE_1)
		{
			if (!dev_mode)
				file_rename_lib(fn, fn + /*".old"*/ STR(2141));
			project_save(self);
		}
	...
	RealType project_load_background(ScopeAny self, VarType map)
	{
		...
		sVar(background_image_show) = value_get_real(DsMap(map).Value(/*"image_show"*/ STR(2142)), sVar(background_image_show));
		...
		sVar(background_image_type) = value_get_string(DsMap(map).Value(/*"image_type"*/ STR(2143)), sVar(background_image_type));
		sVar(background_image_stretch) = value_get_real(DsMap(map).Value(/*"image_stretch"*/ STR(2144)), sVar(background_image_stretch));
		sVar(background_image_box_mapped) = value_get_real(DsMap(map).Value(/*"image_box_mapped"*/ STR(2145)), sVar(background_image_box_mapped));
		sVar(background_image_rotation) = value_get_real(DsMap(map).Value(/*"image_rotation"*/ STR(2146)), sVar(background_image_rotation));
		idInt(sVar(background_sky_sun_tex), count)--;
		sVar(background_sky_sun_tex) = value_get_save_id(DsMap(map).Value(/*"sky_sun_tex"*/ STR(2147)), sVar(background_sky_sun_tex));
		sVar(background_sky_sun_angle) = value_get_real(DsMap(map).Value(/*"sky_sun_angle"*/ STR(2148)), sVar(background_sky_sun_angle));
		sVar(background_sky_sun_scale) = value_get_real(DsMap(map).Value(/*"sky_sun_scale"*/ STR(2149)), sVar(background_sky_sun_scale));
		idInt(sVar(background_sky_moon_tex), count)--;
		sVar(background_sky_moon_tex) = value_get_save_id(DsMap(map).Value(/*"sky_moon_tex"*/ STR(2150)), sVar(background_sky_moon_tex));
		sVar(background_sky_moon_phase) = value_get_real(DsMap(map).Value(/*"sky_moon_phase"*/ STR(2151)), sVar(background_sky_moon_phase));
		sVar(background_sky_moon_angle) = value_get_real(DsMap(map).Value(/*"sky_moon_angle"*/ STR(2152)), sVar(background_sky_moon_angle));
		sVar(background_sky_moon_scale) = value_get_real(DsMap(map).Value(/*"sky_moon_scale"*/ STR(2153)), sVar(background_sky_moon_scale));
		sVar(background_sky_time) = value_get_real(DsMap(map).Value(/*"sky_time"*/ STR(2154)), sVar(background_sky_time));
		sVar(background_sky_rotation) = value_get_real(DsMap(map).Value(/*"sky_rotation"*/ STR(2155)), sVar(background_sky_rotation));
		sVar(background_sunlight_strength) = value_get_real(DsMap(map).Value(/*"sunlight_strength"*/ STR(2156)), sVar(background_sunlight_strength));
		if (global::load_format < e_project_FORMAT_200_PRE_5)
			sVar(background_sunlight_strength) += IntType(1);
		sVar(background_sunlight_angle) = value_get_real(DsMap(map).Value(/*"sunlight_angle"*/ STR(2157)), sVar(background_sunlight_angle));
		sVar(background_twilight) = value_get_real(DsMap(map).Value(/*"twilight"*/ STR(2158)), sVar(background_twilight));
		sVar(background_sky_clouds_show) = value_get_real(DsMap(map).Value(/*"sky_clouds_show"*/ STR(2159)), sVar(background_sky_clouds_show));
		sVar(background_sky_clouds_mode) = value_get_real(DsMap(map).Value(/*"sky_clouds_mode"*/ STR(2160)), sVar(background_sky_clouds_mode));
		if (global::load_format < e_project_FORMAT_200_PRE_5)
		{
			VarType faded, flat;
			faded = false;
			flat = false;
			faded = value_get_real(DsMap(map).Value(/*"sky_clouds_story_mode"*/ STR(2161)), faded);
			flat = value_get_real(DsMap(map).Value(/*"sky_clouds_flat"*/ STR(2162)), flat);
			if (faded > 0)
				sVar(background_sky_clouds_mode) = /*"faded"*/ STR(758);
			else
				if (flat > 0)
					sVar(background_sky_clouds_mode) = /*"flat"*/ STR(759);
			
			sVar(background_sky_clouds_height) = value_get_real(DsMap(map).Value(/*"sky_clouds_z"*/ STR(2163)), sVar(background_sky_clouds_height));
			sVar(background_sky_clouds_thickness) = value_get_real(DsMap(map).Value(/*"sky_clouds_height"*/ STR(2164)), sVar(background_sky_clouds_thickness));
		}
		else
		{
			sVar(background_sky_clouds_mode) = value_get_string(DsMap(map).Value(/*"sky_clouds_mode"*/ STR(2160)), sVar(background_sky_clouds_mode));
			sVar(background_sky_clouds_height) = value_get_real(DsMap(map).Value(/*"sky_clouds_height"*/ STR(2164)), sVar(background_sky_clouds_height));
			sVar(background_sky_clouds_thickness) = value_get_real(DsMap(map).Value(/*"sky_clouds_thickness"*/ STR(2165)), sVar(background_sky_clouds_thickness));
		}
		
		idInt(sVar(background_sky_clouds_tex), count)--;
		sVar(background_sky_clouds_tex) = value_get_save_id(DsMap(map).Value(/*"sky_clouds_tex"*/ STR(2166)), sVar(background_sky_clouds_tex));
		sVar(background_sky_clouds_size) = value_get_real(DsMap(map).Value(/*"sky_clouds_size"*/ STR(2167)), sVar(background_sky_clouds_size));
		sVar(background_sky_clouds_speed) = value_get_real(DsMap(map).Value(/*"sky_clouds_speed"*/ STR(2168)), sVar(background_sky_clouds_speed));
		sVar(background_sky_clouds_offset) = value_get_real(DsMap(map).Value(/*"sky_clouds_offset"*/ STR(2169)), sVar(background_sky_clouds_offset));
		if (global::load_format < e_project_FORMAT_200_PRE_5)
		{
			if (global::_app->background_sky_clouds_tex == /*"default"*/ STR(98))
				global::_app->background_sky_clouds_size *= 8.0;
		}
		sVar(background_ground_show) = value_get_real(DsMap(map).Value(/*"ground_show"*/ STR(2170)), sVar(background_ground_show));
		sVar(background_ground_name) = value_get_string(DsMap(map).Value(/*"ground_name"*/ STR(2171)), sVar(background_ground_name));
		if (global::load_format < e_project_FORMAT_120_PRE_1)
		{
			sVar(background_ground_name) = string_replace(sVar(background_ground_name), /*"blocks/"*/ STR(866), /*"block/"*/ STR(832));
		...
		sVar(background_ground_tex) = value_get_save_id(DsMap(map).Value(/*"ground_tex"*/ STR(2172)), sVar(background_ground_tex));
		idInt(sVar(background_ground_tex_material), count)--;
		sVar(background_ground_tex_material) = value_get_save_id(DsMap(map).Value(/*"ground_tex_material"*/ STR(2173)), sVar(background_ground_tex_material));
		idInt(sVar(background_ground_tex_normal), count)--;
		sVar(background_ground_tex_normal) = value_get_save_id(DsMap(map).Value(/*"ground_tex_normal"*/ STR(2174)), sVar(background_ground_tex_normal));
		sVar(background_biome) = value_get_string(DsMap(map).Value(/*"biome"*/ STR(1722)), sVar(background_biome));
		if (sVar(background_biome) == /*""*/ STR(0))
			sVar(background_biome) = ObjType(obj_biome, DsList(global::biome_list).Value(IntType(2)))->name;
		sVar(background_sky_color) = value_get_color(DsMap(map).Value(/*"sky_color"*/ STR(2175)), sVar(background_sky_color));
		sVar(background_sky_clouds_color) = value_get_color(DsMap(map).Value(/*"sky_clouds_color"*/ STR(2176)), sVar(background_sky_clouds_color));
		sVar(background_sunlight_color) = value_get_color(DsMap(map).Value(/*"sunlight_color"*/ STR(2177)), sVar(background_sunlight_color));
		sVar(background_ambient_color) = value_get_color(DsMap(map).Value(/*"ambient_color"*/ STR(2178)), sVar(background_ambient_color));
		sVar(background_night_color) = value_get_color(DsMap(map).Value(/*"night_color"*/ STR(2179)), sVar(background_night_color));
		sVar(background_water_color) = value_get_color(DsMap(map).Value(/*"water_color"*/ STR(2180)), sVar(background_water_color));
		sVar(background_grass_color) = value_get_color(DsMap(map).Value(/*"grass_color"*/ STR(2181)), sVar(background_grass_color));
		sVar(background_foliage_color) = value_get_color(DsMap(map).Value(/*"foliage_color"*/ STR(2182)), sVar(background_foliage_color));
		sVar(background_leaves_oak_color) = value_get_color(DsMap(map).Value(/*"leaves_oak_color"*/ STR(2183)), sVar(background_leaves_oak_color));
		sVar(background_leaves_spruce_color) = value_get_color(DsMap(map).Value(/*"leaves_spruce_color"*/ STR(2184)), sVar(background_leaves_spruce_color));
		sVar(background_leaves_birch_color) = value_get_color(DsMap(map).Value(/*"leaves_birch_color"*/ STR(2185)), sVar(background_leaves_birch_color));
		sVar(background_leaves_jungle_color) = value_get_color(DsMap(map).Value(/*"leaves_jungle_color"*/ STR(2186)), sVar(background_leaves_jungle_color));
		sVar(background_leaves_acacia_color) = value_get_color(DsMap(map).Value(/*"leaves_acacia_color"*/ STR(2187)), sVar(background_leaves_acacia_color));
		sVar(background_leaves_dark_oak_color) = value_get_color(DsMap(map).Value(/*"leaves_dark_oak_color"*/ STR(2188)), sVar(background_leaves_dark_oak_color));
		sVar(background_leaves_mangrove_color) = value_get_color(DsMap(map).Value(/*"leaves_mangrove_color"*/ STR(2189)), sVar(background_leaves_mangrove_color));
		sVar(background_fog_show) = value_get_real(DsMap(map).Value(/*"fog_show"*/ STR(2190)), sVar(background_fog_show));
		sVar(background_fog_sky) = value_get_real(DsMap(map).Value(/*"fog_sky"*/ STR(2191)), sVar(background_fog_sky));
		sVar(background_fog_color_custom) = value_get_real(DsMap(map).Value(/*"fog_color_custom"*/ STR(2192)), sVar(background_fog_color_custom));
		sVar(background_fog_color) = value_get_color(DsMap(map).Value(/*"fog_color"*/ STR(2193)), sVar(background_fog_color));
		sVar(background_fog_custom_object_color) = value_get_real(DsMap(map).Value(/*"fog_object_color_custom"*/ STR(2194)), sVar(background_fog_custom_object_color));
		sVar(background_fog_object_color) = value_get_color(DsMap(map).Value(/*"fog_object_color"*/ STR(2195)), sVar(background_fog_object_color));
		sVar(background_fog_distance) = value_get_real(DsMap(map).Value(/*"fog_distance"*/ STR(2196)), sVar(background_fog_distance));
		sVar(background_fog_size) = value_get_real(DsMap(map).Value(/*"fog_size"*/ STR(2197)), sVar(background_fog_size));
		sVar(background_fog_height) = value_get_real(DsMap(map).Value(/*"fog_height"*/ STR(2198)), sVar(background_fog_height));
		sVar(background_wind) = value_get_real(DsMap(map).Value(/*"wind"*/ STR(138)), sVar(background_wind));
		sVar(background_wind_speed) = value_get_real(DsMap(map).Value(/*"wind_speed"*/ STR(2199)), sVar(background_wind_speed));
		sVar(background_wind_strength) = value_get_real(DsMap(map).Value(/*"wind_strength"*/ STR(2200)), sVar(background_wind_strength));
		sVar(background_wind_direction) = value_get_real(DsMap(map).Value(/*"wind_direction"*/ STR(2201)), sVar(background_wind_direction));
		sVar(background_wind_directional_speed) = value_get_real(DsMap(map).Value(/*"wind_directional_speed"*/ STR(2202)), sVar(background_wind_directional_speed));
		sVar(background_wind_directional_strength) = value_get_real(DsMap(map).Value(/*"wind_directional_strength"*/ STR(2203)), sVar(background_wind_directional_strength));
		sVar(background_texture_animation_speed) = value_get_real(DsMap(map).Value(/*"texture_animation_speed"*/ STR(2204)), sVar(background_texture_animation_speed));
		return 0.0;
	}
	...
	void project_load_find_save_ids(ScopeAny self)
	{
		...
		{
			if (DsMap(global::save_id_map).Value(key) != /*"root"*/ STR(2205) && DsMap(global::save_id_map).Value(key) != /*"default"*/ STR(98))
			{
		...
		DsMap(global::save_id_map)[/*"root"*/ STR(2205)] = /*"root"*/ STR(2205);
		...
		withAll (obj_keyframe, self->id)
		{
			...
			if (self->value.Value(e_value_TEXTURE_OBJ) == /*"none"*/ STR(919))
			...
			if (self->value.Value(e_value_TEXTURE_MATERIAL_OBJ) == /*"none"*/ STR(919))
			...
			if (self->value.Value(e_value_TEXTURE_NORMAL_OBJ) == /*"none"*/ STR(919))
	...
	void project_load_legacy_background(ScopeAny self)
	{
		...
		ArrType imagetype = ArrType::From({ /*"image"*/ STR(90), /*"sphere"*/ STR(523), /*"box"*/ STR(1366) });
		...
		sVar(background_sky_clouds_mode) = ((flat > 0) ? /*"flat"*/ STR(759) : /*"faded"*/ STR(758));
	...
	void project_load_legacy_beta(ScopeAny self, BoolType loadbackground)
	{
		...
		withOne (obj_data, load, self->id)
		{
			...
			for (IntType a = IntType(0); a < self->lib_amount; a++)
			{
				...
				if (is_string(self->lib_char_model_legacy_name.Value(a)))
				{
					...
					{
						self->lib_char_model_name[a] = DsMap(modelmap).Value(/*"name"*/ STR(830));
						if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(1034))))
							self->lib_char_model_state[a] = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(1034)));
						else
					...
					else
					{
						...
						self->lib_char_model_state[a] = ArrType();
						log({ /*"Could not convert model name"*/ STR(2206), self->lib_char_model_legacy_name.Value(a) });
					}
					
				}
				else
					log({ /*"Could not convert model ID"*/ STR(2207), modelid });
			...
			for (IntType a = IntType(0); a < self->tl_amount; a++)
			{
				...
				if (self->tl_type.Value(a) == /*"char"*/ STR(514) || self->tl_type.Value(a) == /*"spblock"*/ STR(515))
				{
					VarType modelname = self->lib_char_model_name.Value(self->tl_lib.Value(a));
					if (modelname == /*"spider"*/ STR(2208) || modelname == /*"cave_spider"*/ STR(2209))
						self->tl_parts[a] = IntType(7);
					else
						if (modelname == /*"ghast"*/ STR(2210))
							self->tl_parts[a] = IntType(8);
						else
							if (modelname == /*"squid"*/ STR(2211))
								self->tl_parts[a] = IntType(6);
							else
								if (modelname == /*"wither"*/ STR(2212))
									self->tl_parts[a] = IntType(6);
								else
								{
									...
									if (!is_undefined(modelpartlist))
										...
										log({ /*"Could get number of parts for "*/ STR(2213), self->lib_char_model_legacy_name.Value(self->tl_lib.Value(a)) });
		...
		{
			if (ObjType(obj_data, load)->lib_type.Value(a) == /*"light"*/ STR(656))
				continue;
		...
		for (IntType a = IntType(0); a < ObjType(obj_data, load)->tl_amount; a++)
		{
			...
			if (a == IntType(0))
			{
				...
				if (ObjType(obj_data, load)->tl_type.Value(a) == /*"light"*/ STR(656))
				{
					tl = (new obj_timeline)->id;
