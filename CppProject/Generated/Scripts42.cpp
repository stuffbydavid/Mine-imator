...
namespace CppProject
{
	RealType project_load(ScopeAny self, VarArgs argument)
	{
		...
		{
			unzip({ fn });
			fn = file_find_single(unzip_directory, /*".miproject;.mproj;.mani"*/ STR(2053));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miproject;.mproj;.mani"*/ STR(2053));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenprojectzip"*/ STR(2054));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		BoolType legacy;
		ext = filename_ext(fn);
		if (ext == /*".miproject"*/ STR(686) || string_contains(ext, /*".backup"*/ STR(2047)))
		{
			log({ /*"Opening project"*/ STR(2055), fn });
		...
		{
			log({ /*"Opening legacy project"*/ STR(2056), fn });
		...
		if (!legacy)
		{
			...
			project_load_markers(DsMap(rootmap).Value(/*"markers"*/ STR(2057)));
		}
		else
		{
		...
		project_reset_loaded(self);
		log({ /*"Project loaded"*/ STR(2058) });
		if (global::load_format < e_project_FORMAT_110_PRE_1)
		{
			if (!dev_mode)
				file_rename_lib(fn, fn + /*".old"*/ STR(2059));
			project_save(self);
		}
	...
	RealType project_load_background(ScopeAny self, VarType map)
	{
		...
		sVar(background_image_show) = value_get_real({ DsMap(map).Value(/*"image_show"*/ STR(2060)), sVar(background_image_show) });
		...
		sVar(background_image_type) = value_get_string({ DsMap(map).Value(/*"image_type"*/ STR(2061)), sVar(background_image_type) });
		sVar(background_image_stretch) = value_get_real({ DsMap(map).Value(/*"image_stretch"*/ STR(2062)), sVar(background_image_stretch) });
		sVar(background_image_box_mapped) = value_get_real({ DsMap(map).Value(/*"image_box_mapped"*/ STR(2063)), sVar(background_image_box_mapped) });
		sVar(background_image_rotation) = value_get_real({ DsMap(map).Value(/*"image_rotation"*/ STR(2064)), sVar(background_image_rotation) });
		idInt(sVar(background_sky_sun_tex), count)--;
		sVar(background_sky_sun_tex) = value_get_save_id({ DsMap(map).Value(/*"sky_sun_tex"*/ STR(2065)), sVar(background_sky_sun_tex) });
		idInt(sVar(background_sky_moon_tex), count)--;
		sVar(background_sky_moon_tex) = value_get_save_id({ DsMap(map).Value(/*"sky_moon_tex"*/ STR(2066)), sVar(background_sky_moon_tex) });
		sVar(background_sky_moon_phase) = value_get_real({ DsMap(map).Value(/*"sky_moon_phase"*/ STR(2067)), sVar(background_sky_moon_phase) });
		sVar(background_sky_time) = value_get_real({ DsMap(map).Value(/*"sky_time"*/ STR(2068)), sVar(background_sky_time) });
		sVar(background_sky_rotation) = value_get_real({ DsMap(map).Value(/*"sky_rotation"*/ STR(2069)), sVar(background_sky_rotation) });
		sVar(background_sunlight_strength) = value_get_real({ DsMap(map).Value(/*"sunlight_strength"*/ STR(2070)), sVar(background_sunlight_strength) });
		if (global::load_format < e_project_FORMAT_200_PRE_5)
			sVar(background_sunlight_strength) += IntType(1);
		sVar(background_sunlight_angle) = value_get_real({ DsMap(map).Value(/*"sunlight_angle"*/ STR(2071)), sVar(background_sunlight_angle) });
		sVar(background_twilight) = value_get_real({ DsMap(map).Value(/*"twilight"*/ STR(2072)), sVar(background_twilight) });
		sVar(background_sky_clouds_show) = value_get_real({ DsMap(map).Value(/*"sky_clouds_show"*/ STR(2073)), sVar(background_sky_clouds_show) });
		sVar(background_sky_clouds_mode) = value_get_real({ DsMap(map).Value(/*"sky_clouds_mode"*/ STR(2074)), sVar(background_sky_clouds_mode) });
		if (global::load_format < e_project_FORMAT_200_PRE_5)
		{
			VarType faded, flat;
			faded = false;
			flat = false;
			faded = value_get_real({ DsMap(map).Value(/*"sky_clouds_story_mode"*/ STR(2075)), faded });
			flat = value_get_real({ DsMap(map).Value(/*"sky_clouds_flat"*/ STR(2076)), flat });
			if (faded > 0)
			...
			sVar(background_sky_clouds_height) = value_get_real({ DsMap(map).Value(/*"sky_clouds_z"*/ STR(2077)), sVar(background_sky_clouds_height) });
			sVar(background_sky_clouds_thickness) = value_get_real({ DsMap(map).Value(/*"sky_clouds_height"*/ STR(2078)), sVar(background_sky_clouds_thickness) });
		}
		else
		{
			sVar(background_sky_clouds_mode) = value_get_string({ DsMap(map).Value(/*"sky_clouds_mode"*/ STR(2074)), sVar(background_sky_clouds_mode) });
			sVar(background_sky_clouds_height) = value_get_real({ DsMap(map).Value(/*"sky_clouds_height"*/ STR(2078)), sVar(background_sky_clouds_height) });
			sVar(background_sky_clouds_thickness) = value_get_real({ DsMap(map).Value(/*"sky_clouds_thickness"*/ STR(2079)), sVar(background_sky_clouds_thickness) });
		}
		
		idInt(sVar(background_sky_clouds_tex), count)--;
		sVar(background_sky_clouds_tex) = value_get_save_id({ DsMap(map).Value(/*"sky_clouds_tex"*/ STR(2080)), sVar(background_sky_clouds_tex) });
		sVar(background_sky_clouds_size) = value_get_real({ DsMap(map).Value(/*"sky_clouds_size"*/ STR(2081)), sVar(background_sky_clouds_size) });
		sVar(background_sky_clouds_speed) = value_get_real({ DsMap(map).Value(/*"sky_clouds_speed"*/ STR(2082)), sVar(background_sky_clouds_speed) });
		sVar(background_sky_clouds_offset) = value_get_real({ DsMap(map).Value(/*"sky_clouds_offset"*/ STR(2083)), sVar(background_sky_clouds_offset) });
		if (global::load_format < e_project_FORMAT_200_PRE_5)
		{
			if (global::_app->background_sky_clouds_tex == /*"default"*/ STR(98))
				global::_app->background_sky_clouds_size *= 8.0;
		}
		sVar(background_ground_show) = value_get_real({ DsMap(map).Value(/*"ground_show"*/ STR(2084)), sVar(background_ground_show) });
		sVar(background_ground_name) = value_get_string({ DsMap(map).Value(/*"ground_name"*/ STR(2085)), sVar(background_ground_name) });
		...
		sVar(background_ground_tex) = value_get_save_id({ DsMap(map).Value(/*"ground_tex"*/ STR(2086)), sVar(background_ground_tex) });
		idInt(sVar(background_ground_tex_material), count)--;
		sVar(background_ground_tex_material) = value_get_save_id({ DsMap(map).Value(/*"ground_tex_material"*/ STR(2087)), sVar(background_ground_tex_material) });
		idInt(sVar(background_ground_tex_normal), count)--;
		sVar(background_ground_tex_normal) = value_get_save_id({ DsMap(map).Value(/*"ground_tex_normal"*/ STR(2088)), sVar(background_ground_tex_normal) });
		sVar(background_biome) = value_get_string({ DsMap(map).Value(/*"biome"*/ STR(1659)), sVar(background_biome) });
		if (sVar(background_biome) == /*""*/ STR(0))
			sVar(background_biome) = ObjType(obj_biome, DsList(global::biome_list).Value(IntType(2)))->name;
		sVar(background_sky_color) = value_get_color({ DsMap(map).Value(/*"sky_color"*/ STR(2089)), sVar(background_sky_color) });
		sVar(background_sky_clouds_color) = value_get_color({ DsMap(map).Value(/*"sky_clouds_color"*/ STR(2090)), sVar(background_sky_clouds_color) });
		sVar(background_sunlight_color) = value_get_color({ DsMap(map).Value(/*"sunlight_color"*/ STR(2091)), sVar(background_sunlight_color) });
		sVar(background_ambient_color) = value_get_color({ DsMap(map).Value(/*"ambient_color"*/ STR(2092)), sVar(background_ambient_color) });
		sVar(background_night_color) = value_get_color({ DsMap(map).Value(/*"night_color"*/ STR(2093)), sVar(background_night_color) });
		sVar(background_water_color) = value_get_color({ DsMap(map).Value(/*"water_color"*/ STR(2094)), sVar(background_water_color) });
		sVar(background_grass_color) = value_get_color({ DsMap(map).Value(/*"grass_color"*/ STR(2095)), sVar(background_grass_color) });
		sVar(background_foliage_color) = value_get_color({ DsMap(map).Value(/*"foliage_color"*/ STR(2096)), sVar(background_foliage_color) });
		sVar(background_leaves_oak_color) = value_get_color({ DsMap(map).Value(/*"leaves_oak_color"*/ STR(2097)), sVar(background_leaves_oak_color) });
		sVar(background_leaves_spruce_color) = value_get_color({ DsMap(map).Value(/*"leaves_spruce_color"*/ STR(2098)), sVar(background_leaves_spruce_color) });
		sVar(background_leaves_birch_color) = value_get_color({ DsMap(map).Value(/*"leaves_birch_color"*/ STR(2099)), sVar(background_leaves_birch_color) });
		sVar(background_leaves_jungle_color) = value_get_color({ DsMap(map).Value(/*"leaves_jungle_color"*/ STR(2100)), sVar(background_leaves_jungle_color) });
		sVar(background_leaves_acacia_color) = value_get_color({ DsMap(map).Value(/*"leaves_acacia_color"*/ STR(2101)), sVar(background_leaves_acacia_color) });
		sVar(background_leaves_dark_oak_color) = value_get_color({ DsMap(map).Value(/*"leaves_dark_oak_color"*/ STR(2102)), sVar(background_leaves_dark_oak_color) });
		sVar(background_leaves_mangrove_color) = value_get_color({ DsMap(map).Value(/*"leaves_mangrove_color"*/ STR(2103)), sVar(background_leaves_mangrove_color) });
		sVar(background_fog_show) = value_get_real({ DsMap(map).Value(/*"fog_show"*/ STR(2104)), sVar(background_fog_show) });
		sVar(background_fog_sky) = value_get_real({ DsMap(map).Value(/*"fog_sky"*/ STR(2105)), sVar(background_fog_sky) });
		sVar(background_fog_color_custom) = value_get_real({ DsMap(map).Value(/*"fog_color_custom"*/ STR(2106)), sVar(background_fog_color_custom) });
		sVar(background_fog_color) = value_get_color({ DsMap(map).Value(/*"fog_color"*/ STR(2107)), sVar(background_fog_color) });
		sVar(background_fog_custom_object_color) = value_get_real({ DsMap(map).Value(/*"fog_custom_object_color"*/ STR(2108)), sVar(background_fog_custom_object_color) });
		sVar(background_fog_object_color) = value_get_color({ DsMap(map).Value(/*"fog_object_color"*/ STR(2109)), sVar(background_fog_object_color) });
		sVar(background_fog_distance) = value_get_real({ DsMap(map).Value(/*"fog_distance"*/ STR(2110)), sVar(background_fog_distance) });
		sVar(background_fog_size) = value_get_real({ DsMap(map).Value(/*"fog_size"*/ STR(2111)), sVar(background_fog_size) });
		sVar(background_fog_height) = value_get_real({ DsMap(map).Value(/*"fog_height"*/ STR(2112)), sVar(background_fog_height) });
		sVar(background_wind) = value_get_real({ DsMap(map).Value(/*"wind"*/ STR(137)), sVar(background_wind) });
		sVar(background_wind_speed) = value_get_real({ DsMap(map).Value(/*"wind_speed"*/ STR(2113)), sVar(background_wind_speed) });
		sVar(background_wind_strength) = value_get_real({ DsMap(map).Value(/*"wind_strength"*/ STR(2114)), sVar(background_wind_strength) });
		sVar(background_wind_direction) = value_get_real({ DsMap(map).Value(/*"wind_direction"*/ STR(2115)), sVar(background_wind_direction) });
		sVar(background_wind_directional_speed) = value_get_real({ DsMap(map).Value(/*"wind_directional_speed"*/ STR(2116)), sVar(background_wind_directional_speed) });
		sVar(background_wind_directional_strength) = value_get_real({ DsMap(map).Value(/*"wind_directional_strength"*/ STR(2117)), sVar(background_wind_directional_strength) });
		sVar(background_texture_animation_speed) = value_get_real({ DsMap(map).Value(/*"texture_animation_speed"*/ STR(2118)), sVar(background_texture_animation_speed) });
		return 0.0;
	}
	...
	void project_load_find_save_ids(ScopeAny self)
	{
		...
		{
			if (DsMap(global::save_id_map).Value(key) != /*"root"*/ STR(2119) && DsMap(global::save_id_map).Value(key) != /*"default"*/ STR(98))
			{
		...
		DsMap(global::save_id_map)[/*"root"*/ STR(2119)] = /*"root"*/ STR(2119);
	...
	void project_load_legacy_background(ScopeAny self)
	{
		...
		ArrType imagetype = ArrType::From({ /*"image"*/ STR(90), /*"sphere"*/ STR(513), /*"box"*/ STR(1316) });
		...
		withOne (obj_data, load, self->id)
		{
			...
			for (IntType a = IntType(0); a < self->lib_amount; a++)
			{
				...
				if (is_string(self->lib_char_model_legacy_name.Value(a)))
				{
					...
					if (!is_undefined(modelmap))
					{
						...
						if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(984))))
							self->lib_char_model_state[a] = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(984)));
						else
					...
					else
					{
						...
						self->lib_char_model_state[a] = ArrType();
						log({ /*"Could not convert model name"*/ STR(2120), self->lib_char_model_legacy_name.Value(a) });
					}
					
				}
				else
					log({ /*"Could not convert model ID"*/ STR(2121), modelid });
			...
			for (IntType a = IntType(0); a < self->tl_amount; a++)
			{
				...
				if (self->tl_type.Value(a) == /*"char"*/ STR(504) || self->tl_type.Value(a) == /*"spblock"*/ STR(505))
				{
					...
					if (modelname == /*"spider"*/ STR(2122) || modelname == /*"cave_spider"*/ STR(2123))
						self->tl_parts[a] = IntType(7);
					else
						if (modelname == /*"ghast"*/ STR(2124))
							self->tl_parts[a] = IntType(8);
						else
							if (modelname == /*"squid"*/ STR(2125))
								self->tl_parts[a] = IntType(6);
							else
								if (modelname == /*"wither"*/ STR(2126))
									self->tl_parts[a] = IntType(6);
								else
								{
									...
									if (!is_undefined(modelpartlist))
										...
										log({ /*"Could get number of parts for "*/ STR(2127), self->lib_char_model_legacy_name.Value(self->tl_lib.Value(a)) });
