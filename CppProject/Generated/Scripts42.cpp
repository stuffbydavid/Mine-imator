...
namespace CppProject
{
	RealType popup_pattern_editor_draw_layer(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, VarType index, RealType base)
	{
		...
		if (ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_edit_preview == null_)
			return IntType(0);
		editname = /*"patterneditoreditlayer"*/ STR(2057) + string(index);
		...
		layercolor = ((base > 0) ? minecraft_get_color(/*"dye:black"*/ STR(2058)) : DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_color_list_edit).Value(index));
		if (ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_edit_preview != null_)
		...
		if (base > 0)
		{
			...
			name = text_get({ /*"patterneditorbase"*/ STR(2059), text_get({ /*"swatchdye"*/ STR(2060) + colorname }) });
		}
		else
		{
			...
			name = text_get({ /*"patterneditorlayer"*/ STR(2061), text_get({ /*"swatchdye"*/ STR(2060) + colorname }), text_get({ /*"patterneditorpatterns"*/ STR(2062) + DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_list_edit).Value(index) }) });
		}
		...
		if (!(base > 0))
		{
			...
			if (draghover)
			{
				...
				if (sReal(mouse_move) > IntType(0) && !dragging)
				{
					...
					sVar(window_busy) = /*"patterneditordraglayer"*/ STR(2063);
				}
			}
			...
			if (mouseon && !dragging)
			{
				if (draw_button_icon(self, { /*"patterneditorremovelayer"*/ STR(2064) + string(index), actionx - IntType(24), yy + height / 2.0 - IntType(14), IntType(24), IntType(24), false, icons_DELETE }))
	...
	{
		draw_label(self, { text_get({ /*"patterneditorcolors"*/ STR(2065) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(18);
		VarType colorsx;
		RealType size;
		colorsx = sVar(dx);
		...
		{
			sVar(dy) += IntType(8);
			draw_label(self, { text_get({ /*"patterneditorpatterns"*/ STR(2062) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(18);
			...
			for (; p < ds_list_size(global::minecraft_pattern_list); p++)
			{
				...
				if (draw_button_icon(self, { /*"patterneditorpatterns"*/ STR(2062) + string(p), patternsx, sVar(dy), IntType(20), IntType(40), active, null_, null_, false, /*"patterneditorpatterns"*/ STR(2062) + DsList(global::minecraft_pattern_list).Value(p), ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_sprites.Value(p) }))
				{
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2066), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2066), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2046), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2066), sVar(project_name) }), /*"top"*/ STR(194)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2066), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2051), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2052) }))
		{
		...
		if (draw_button_label(self, /*"saveassave"*/ STR(2067), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2068), idVar(sInt(popup), name) });
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2069), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
		...
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2070), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
		...
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2071) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2072) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(110), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2073), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2074), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2075) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, /*"upgradecontinue"*/ STR(2076), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2077);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2078) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"welcomecontinue"*/ STR(2079), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2080) : /*"previewmove"*/ STR(2081));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2080))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2081))
			{
				sInt(mouse_cursor) = cr_size_all;
		...
		withOne (Object, preview, self->id)
		{
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uFogShow"*/ STR(1924), IntType(0));
					render_set_uniform_int(/*"uColorsExt"*/ STR(2082), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2083), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2084), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2085), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2086), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2087), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2088), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2089), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2090), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2091) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2090), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2091) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2092), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2093) : /*"tooltipplay"*/ STR(2094) }))
			{
				if (isplaying > 0)
				{
