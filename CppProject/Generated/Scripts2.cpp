	...
	RealType action_background_sky_moon_scale(ScopeAny self, VarType val, VarType add)
	{
		if (!(sReal(history_undo) > 0) && !(sReal(history_redo) > 0))
		{
			if (action_tl_select_single(self, { null_, e_tl_type_BACKGROUND }))
			{
				...
				tl_value_set(self, { e_value_BG_SKY_MOON_SCALE, val, add });
				tl_value_set_done();
				return IntType(0);
			}
			history_set_var(self, ID_action_background_sky_moon_scale, sVar(background_sky_moon_scale), sVar(background_sky_moon_scale) * add + val, true);
		}
		sVar(background_sky_moon_scale) = sVar(background_sky_moon_scale) * add + val;
		return 0.0;
	}
	...
	RealType action_background_sky_sun_scale(ScopeAny self, VarType val, VarType add)
	{
		if (!(sReal(history_undo) > 0) && !(sReal(history_redo) > 0))
		{
			if (action_tl_select_single(self, { null_, e_tl_type_BACKGROUND }))
			{
				...
				tl_value_set(self, { e_value_BG_SKY_SUN_SCALE, val, add });
				tl_value_set_done();
				return IntType(0);
			}
			history_set_var(self, ID_action_background_sky_sun_scale, sVar(background_sky_sun_scale), sVar(background_sky_sun_scale) * add + val, true);
		}
		sVar(background_sky_sun_scale) = sVar(background_sky_sun_scale) * add + val;
		return 0.0;
	}
