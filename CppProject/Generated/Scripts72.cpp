...
namespace CppProject
{
	void window_draw_startup(ScopeAny self)
	{
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			{
				if (sStr(recent_display_mode) == /*"list"*/ STR(1067))
					sStr(recent_display_mode) = /*"grid"*/ STR(613);
				else
					sStr(recent_display_mode) = /*"list"*/ STR(1067);
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			if (draw_button_label(self, { /*"startupsortby"*/ STR(1448), sVar(dx) + labelwid + IntType(16), sVar(dy), null_, icons_SORT_DOWN, e_button_TERTIARY }))
				menu_settings_set(self, sVar(dx) + labelwid + IntType(16), sVar(dy), /*"startupsortby"*/ STR(1448), IntType(32));
			if (sStr(settings_menu_name) == /*"startupsortby"*/ STR(1448) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(719))
	...
	void window_draw_toasts(ScopeAny self)
	{
		...
		if (sInt(popup) > 0 && busy == /*"popup"*/ STR(1564) + idVar(sInt(popup), name))
	...
	void app_startup_interface_world_import()
	{
		...
		sortlist_column_add(global::_app->world_import_settings_block_list, /*"blockfilter"*/ STR(2885), IntType(0));
		...
		sortlist_column_add(global::_app->world_import_settings_filter_list, /*"blockfilter"*/ STR(2885), IntType(0));
	...
	{
		sStr(window_state) = /*"world_import"*/ STR(2661);
	...
	void window_draw_world_import(ScopeAny self)
	{
		...
		sVar(dw) = IntType(256);
		capwid = IntType(50);
		draw_button_menu(self, { /*"worldimportworld"*/ STR(1460), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_world_root), sVar(world_import_world_name), ID_world_import_select_world, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = IntType(208);
		capwid = IntType(80);
		draw_button_menu(self, { /*"worldimportdimension"*/ STR(1461), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_dimension), text_get({ /*"worldimport"*/ STR(2877) + sStr(world_import_dimension) }), ID_world_import_select_dimension, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		draw_divide_vertical(sVar(dx), sVar(content_y) + IntType(6), sVar(content_height) - IntType(12));
		sVar(dx) += IntType(12);
		...
		spacing = IntType(12);
		sVar(dw) = string_width(text_get({ /*"worldimportselection"*/ STR(2880) }) + /*":"*/ STR(755));
		draw_label(self, { text_get({ /*"worldimportselection"*/ STR(2880) }) + /*":"*/ STR(755), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		sVar(dx) += sVar(dw) + spacing;
		draw_set_font(sInt(font_button));
	...
	void popup_worldsettings_draw(ScopeAny self)
	{
		...
		if (sVar(setting_world_import_filter_enabled) > 0)
		{
			...
			sortlist_draw(self, { sInt(world_import_settings_block_list), sVar(dx), sVar(dy), listdw / 2.0 - IntType(20), sVar(tab_control_h), sInt(world_import_settings_block_select), false, text_get({ /*"worldsettingsfilterblocks"*/ STR(3914) }) });
			sortlist_draw(self, { sInt(world_import_settings_filter_list), sVar(dx) + listdw / 2.0 + IntType(20), sVar(dy), listdw / 2.0 - IntType(20), sVar(tab_control_h), sInt(world_import_settings_filter_select), false, text_get({ /*"worldsettingsfilterfiltered"*/ STR(3915) }) });
			if (draw_button_icon(self, { /*"worldsettingsfilterright"*/ STR(3916), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + ((RealType)sVar(tab_control_h) / 2.0) - IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_RIGHT_TINY, null_, sInt(world_import_settings_block_select) == null_ }))
			{
			...
			if (draw_button_icon(self, { /*"worldsettingsfilterleft"*/ STR(3917), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + ((RealType)sVar(tab_control_h) / 2.0) + IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_LEFT_TINY, null_, sInt(world_import_settings_filter_select) == null_ }))
			{
