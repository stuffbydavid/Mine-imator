...
namespace CppProject
{
	void window_draw_startup(ScopeAny self)
	{
		...
		StringType trial = ((global::trial_version > 0) ? /*" "*/ STR(17) + text_get({ /*"startuptrial"*/ STR(1986) }) : /*""*/ STR(0));
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			if (draw_button_icon(self, { /*"startuprecentdisplay"*/ STR(3972), sVar(dx) - IntType(24) - IntType(8), sVar(dy) + IntType(4), IntType(24), IntType(24), false, sStr(recent_display_mode) == /*"grid"*/ STR(653) ? icons_VIEW_LIST : icons_VIEW_GRID, null_, false, sStr(recent_display_mode) == /*"grid"*/ STR(653) ? /*"tooltipviewlist"*/ STR(3973) : /*"tooltipviewgrid"*/ STR(3974) }))
			{
				if (sStr(recent_display_mode) == /*"list"*/ STR(1115))
					sStr(recent_display_mode) = /*"grid"*/ STR(653);
				else
					sStr(recent_display_mode) = /*"list"*/ STR(1115);
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			if (draw_button_label(self, /*"startupsortby"*/ STR(1497), sVar(dx) + labelwid + IntType(16), sVar(dy), null_, icons_SORT_DOWN, e_button_TERTIARY))
				menu_settings_set(self, sVar(dx) + labelwid + IntType(16), sVar(dy), /*"startupsortby"*/ STR(1497), IntType(32));
			if (sStr(settings_menu_name) == /*"startupsortby"*/ STR(1497) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(758))
	...
	void window_draw_toasts(ScopeAny self)
	{
		...
		if (sInt(popup) > 0 && busy == /*"popup"*/ STR(1623) + idVar(sInt(popup), name))
			sVar(window_busy) = /*""*/ STR(0);
		for (RealType i = sInt(toast_amount) - IntType(1); i >= IntType(0); i--)
		{
			toast = DsList(sInt(toast_list)).Value(i);
			draw_set_alpha(ease(/*"easeoutcirc"*/ STR(565), ObjType(obj_toast, toast)->remove_alpha));
	...
	void app_startup_interface_world_import()
	{
		...
		sortlist_column_add(global::_app->world_import_settings_block_list, /*"blockfilter"*/ STR(2981), IntType(0));
		...
		sortlist_column_add(global::_app->world_import_settings_filter_list, /*"blockfilter"*/ STR(2981), IntType(0));
	...
	{
		sStr(window_state) = /*"world_import"*/ STR(2752);
	...
	void window_draw_world_import(ScopeAny self)
	{
		...
		sVar(dw) = IntType(256);
		capwid = IntType(50);
		draw_button_menu(self, { /*"worldimportworld"*/ STR(1509), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_world_root), sVar(world_import_world_name), ID_world_import_select_world, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = IntType(208);
		capwid = IntType(80);
		draw_button_menu(self, { /*"worldimportdimension"*/ STR(1510), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_dimension), text_get({ /*"worldimport"*/ STR(2973) + sStr(world_import_dimension) }), ID_world_import_select_dimension, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		draw_divide_vertical(sVar(dx), sVar(content_y) + IntType(6), sVar(content_height) - IntType(12));
		sVar(dx) += IntType(12);
		...
		spacing = IntType(12);
		sVar(dw) = string_width(text_get({ /*"worldimportselection"*/ STR(2976) }) + /*":"*/ STR(796));
		draw_label(self, { text_get({ /*"worldimportselection"*/ STR(2976) }) + /*":"*/ STR(796), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		sVar(dx) += sVar(dw) + spacing;
		draw_set_font(sInt(font_button));
		...
		if (draw_button_label(self, /*"worldimportselectionmedium"*/ STR(4008), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"medium"*/ STR(589));
		sVar(dx) += sVar(dw) + spacing;
