...
namespace CppProject
{
	void window_draw_startup(ScopeAny self)
	{
		...
		StringType trial = ((global::trial_version > 0) ? /*" "*/ STR(17) + text_get({ /*"startuptrial"*/ STR(1911) }) : /*""*/ STR(0));
		draw_button_text(self, { text_get({ /*"startupversion"*/ STR(3855), mineimator_version_full + trial }), (sReal(window_width) / 2.0) + IntType(259), floor((headersize / 2.0) + ((RealType)sprite_get_height(ID_spr_logo) / 2.0)) + IntType(3), ID_popup_switch, sInt(popup_about) });
		...
		{
			draw_label(self, { text_get({ /*"recentnone"*/ STR(3856) }), sReal(window_width) / 2.0, sVar(dy), fa_center, fa_middle, global::c_accent, IntType(1), sInt(font_heading_big) });
			sVar(dy) += IntType(48);
		}
		draw_set_font(sInt(font_button));
		RealType newprojectwidth, browsewidth, centerx;
		newprojectwidth = string_width(text_get({ /*"startupnewproject"*/ STR(3857) })) + button_icon_padding;
		browsewidth = string_width(text_get({ /*"startupbrowse"*/ STR(3858) })) + button_icon_padding;
		...
		if (draw_button_label(self, { /*"startupnewproject"*/ STR(3857), sVar(dx), sVar(dy), null_, icons_FILE }))
		{
		...
		if (draw_button_label(self, { /*"startupbrowse"*/ STR(3858), sVar(dx), sVar(dy), null_, icons_FOLDER, e_button_SECONDARY }))
		{
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			if (draw_button_icon(self, { /*"startuprecentdisplay"*/ STR(3859), sVar(dx) - IntType(24) - IntType(8), sVar(dy) + IntType(4), IntType(24), IntType(24), false, sStr(recent_display_mode) == /*"grid"*/ STR(612) ? icons_VIEW_LIST : icons_VIEW_GRID, null_, false, sStr(recent_display_mode) == /*"grid"*/ STR(612) ? /*"tooltipviewlist"*/ STR(3860) : /*"tooltipviewgrid"*/ STR(3861) }))
			{
				if (sStr(recent_display_mode) == /*"list"*/ STR(1065))
					sStr(recent_display_mode) = /*"grid"*/ STR(612);
				else
					sStr(recent_display_mode) = /*"list"*/ STR(1065);
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			draw_label(self, { text_get({ /*"startuprecentprojects"*/ STR(3862) }), sVar(dx), sVar(dy) + IntType(16), fa_left, fa_middle, global::c_accent, IntType(1) });
			IntType labelwid = string_width(text_get({ /*"startuprecentprojects"*/ STR(3862) }));
			...
			if (sStr(settings_menu_name) == /*"startupsortby"*/ STR(1446) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
	...
	{
		global::_app->world_import_settings_popup = new_popup({ /*"worldsettings"*/ STR(3863), ID_popup_worldsettings_draw, IntType(600), IntType(532), true, false, false, true, ID_world_import_apply_settings });
		...
		sortlist_column_add(global::_app->world_import_settings_block_list, /*"blockfilter"*/ STR(2878), IntType(0));
		...
		sortlist_column_add(global::_app->world_import_settings_filter_list, /*"blockfilter"*/ STR(2878), IntType(0));
	...
	{
		sStr(window_state) = /*"world_import"*/ STR(2654);
		sStr(popup_ani_type) = /*""*/ STR(0);
		sStr(world_import_world_root) = /*""*/ STR(0);
		sVar(world_import_world_name) = text_get({ /*"worldimportnoworld"*/ STR(3864) });
		sStr(world_import_dimension) = /*"overworld"*/ STR(3865);
	...
	{
		menu_add_item({ /*"overworld"*/ STR(3865), text_get({ /*"worldimportoverworld"*/ STR(3875) }) });
		if (world_import_has_dimension(/*"nether"*/ STR(3876)))
			menu_add_item({ /*"nether"*/ STR(3876), text_get({ /*"worldimportnether"*/ STR(3877) }) });
		if (world_import_has_dimension(/*"end"*/ STR(3878)))
			menu_add_item({ /*"end"*/ STR(3878), text_get({ /*"worldimportend"*/ STR(3879) }) });
	}
	...
	void window_draw_world_import(ScopeAny self)
	{
		...
		if (hasselection)
		{
			...
			confirmw = string_width(text_get({ /*"worldimportconfirm"*/ STR(3880) })) + IntType(70);
		}
		else
			confirmw = IntType(0);
		...
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportcancel"*/ STR(3881) })) + IntType(24);
		if (draw_button_label(self, { /*"worldimportcancel"*/ STR(3881), sVar(dx), sVar(content_y) + IntType(4), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT }))
			world_import_cancel();
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		...
		sVar(dw) = IntType(208);
		capwid = IntType(80);
		draw_button_menu(self, { /*"worldimportdimension"*/ STR(1459), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_dimension), text_get({ /*"worldimport"*/ STR(2870) + sStr(world_import_dimension) }), ID_world_import_select_dimension, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		draw_divide_vertical(sVar(dx), sVar(content_y) + IntType(6), sVar(content_height) - IntType(12));
		sVar(dx) += IntType(12);
		sVar(dw) = IntType(24);
		spacing = IntType(4);
		if (draw_button_icon(self, { /*"worldimportbrowse"*/ STR(3882), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_FOLDER, null_, false, /*"worldimportbrowsetip"*/ STR(3883) }))
		{
			StringType leveldat = file_dialog_open(text_get({ /*"worldimportbrowseworlds"*/ STR(3884) }) + /*" (level.dat)|level.dat;"*/ STR(3885), /*""*/ STR(0), world_import_get_saves_dir(), text_get({ /*"worldimportbrowsecaption"*/ STR(3886) }));
			if (file_exists_lib(leveldat))
				world_import_select_world(self, filename_dir(leveldat));
		}
		sVar(dx) += sVar(dw) + spacing;
		if (draw_button_icon(self, { /*"worldimportsettings"*/ STR(3887), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_SETTINGS, null_, false, /*"worldimportsettingstip"*/ STR(3888) }))
			popup_show(self, sInt(world_import_settings_popup));
		sVar(dx) += sVar(dw) + spacing;
		BoolType worldpicked = sStr(world_import_world_root) != /*""*/ STR(0);
		if (draw_button_icon(self, { /*"worldimportreload"*/ STR(3889), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_REFRESH, null_, !worldpicked, /*"worldimportreloadtip"*/ STR(3890) }))
			world_import_select_world(self, sStr(world_import_world_root), sStr(world_import_dimension));
		sVar(dx) += sVar(dw) + spacing;
		if (draw_button_icon(self, { /*"worldimportgotoplayer"*/ STR(3891), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_PATH_POINT, null_, !worldpicked, /*"worldimportgotoplayertip"*/ STR(3892) }))
		...
		spacing = IntType(12);
		sVar(dw) = string_width(text_get({ /*"worldimportselection"*/ STR(2873) }) + /*":"*/ STR(754));
		draw_label(self, { text_get({ /*"worldimportselection"*/ STR(2873) }) + /*":"*/ STR(754), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		sVar(dx) += sVar(dw) + spacing;
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportselectionsmall"*/ STR(3893) })) + IntType(24);
		if (draw_button_label(self, { /*"worldimportselectionsmall"*/ STR(3893), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked }))
			world_import_set_selection(/*"small"*/ STR(3894));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionmedium"*/ STR(3895) })) + IntType(24);
		if (draw_button_label(self, { /*"worldimportselectionmedium"*/ STR(3895), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked }))
			world_import_set_selection(/*"medium"*/ STR(580));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionlarge"*/ STR(3896) })) + IntType(24);
		if (draw_button_label(self, { /*"worldimportselectionlarge"*/ STR(3896), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked }))
			world_import_set_selection(/*"large"*/ STR(3897));
		if (world_import_has_selection())
		{
			sVar(dx) += sVar(dw) + IntType(20);
			VecType size = world_import_get_selection_size();
			draw_label(self, { text_get({ /*"worldimportblocks"*/ STR(3898), size.Real(X_), size.Real(Y_), size.Real(Z_) }), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_main, global::a_text_main, sInt(font_value) });
		}
		sVar(content_x) = IntType(0);
		sVar(content_y) = IntType(0);
		sVar(content_width) = sReal(window_width);
		sVar(content_height) = sReal(window_height);
		if (hasselection)
			if (draw_button_label(self, { /*"worldimportconfirm"*/ STR(3880), confirmx, confirmy, confirmw, null_, e_button_BIG, null_, e_anchor_LEFT }))
		...
		{
			VarType filtertext = text_get({ /*"worldimportfilteractive"*/ STR(3899) });
	...
	void popup_worldsettings_draw(ScopeAny self)
	{
		...
		draw_switch(self, { /*"worldsettingsunloadregions"*/ STR(3900), sVar(dx), sVar(dy), sVar(setting_world_import_unload_regions), ID_action_world_import_settings_unload_regions, /*"worldsettingsunloadregionstip"*/ STR(3901) });
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, { /*"worldsettingsfilterenabled"*/ STR(3902), sVar(dx), sVar(dy), sVar(setting_world_import_filter_enabled), ID_action_world_import_settings_filter_enabled });
		tab_next(self);
		if (sVar(setting_world_import_filter_enabled) > 0)
		{
			tab_control_togglebutton(self);
			togglebutton_add(self, { /*"worldsettingsfilterremove"*/ STR(3903), null_, IntType(0), sVar(setting_world_import_filter_mode) == IntType(0), ID_action_world_import_settings_filter_mode });
			togglebutton_add(self, { /*"worldsettingsfilterkeep"*/ STR(3904), null_, IntType(1), sVar(setting_world_import_filter_mode) == IntType(1), ID_action_world_import_settings_filter_mode });
			draw_togglebutton(self, { /*"worldsettingsfiltermode"*/ STR(3905), sVar(dx), sVar(dy) });
			tab_next(self);
			RealType listdw = sVar(content_width) - IntType(24);
			sVar(dy) += IntType(8);
			tab_control_sortlist(self, IntType(10));
			sortlist_draw(self, { sInt(world_import_settings_block_list), sVar(dx), sVar(dy), listdw / 2.0 - IntType(20), sVar(tab_control_h), sInt(world_import_settings_block_select), false, text_get({ /*"worldsettingsfilterblocks"*/ STR(3906) }) });
			sortlist_draw(self, { sInt(world_import_settings_filter_list), sVar(dx) + listdw / 2.0 + IntType(20), sVar(dy), listdw / 2.0 - IntType(20), sVar(tab_control_h), sInt(world_import_settings_filter_select), false, text_get({ /*"worldsettingsfilterfiltered"*/ STR(3907) }) });
			if (draw_button_icon(self, { /*"worldsettingsfilterright"*/ STR(3908), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + ((RealType)sVar(tab_control_h) / 2.0) - IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_RIGHT_TINY, null_, sInt(world_import_settings_block_select) == null_ }))
			{
			...
			if (draw_button_icon(self, { /*"worldsettingsfilterleft"*/ STR(3909), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + ((RealType)sVar(tab_control_h) / 2.0) + IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_LEFT_TINY, null_, sInt(world_import_settings_filter_select) == null_ }))
			{
			...
			sVar(dy) += IntType(8);
			draw_tooltip_label(self, /*"worldsettingsfilterhelp"*/ STR(3910), icons_INFO, e_toast_INFO);
		}
	}
	
	StringType yesno(RealType val)
	{
		return ((val > 0) ? /*"yes"*/ STR(3911) : /*"no"*/ STR(3912));
	}
