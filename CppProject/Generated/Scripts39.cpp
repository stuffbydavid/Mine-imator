...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1983));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1983));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1984));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		IntType rootmap;
		BoolType legacy;
		if (string_contains(filename_ext(fn), /*".miparticles"*/ STR(726)))
		{
			log({ /*"Opening particles"*/ STR(1985), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1986), fn });
		...
		if (!legacy)
		{
			...
			{
				temp_particles_type_clear(self);
				project_load_particles(self, DsMap(rootmap).Value(/*"particles"*/ STR(520)));
				temp_particles_restart(self);
			}
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miparticles"*/ STR(726));
		log({ /*"Saving particles"*/ STR(1988), fn });
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(530) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(919) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
							...
							else
							{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1372))
								{
									if (!(idVar(temp, pc_spawn_region_use) > 0))
										continue;
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(523))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1366))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(530) && idVar(temp, pc_spawn_region_path) != null_)
												{
