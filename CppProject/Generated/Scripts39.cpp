...
namespace CppProject
{
	...
	IntType new_minecraft_map(StringType fn)
	{
		...
		{
			log({ /*"Minecraft map error"*/ STR(1907), /*"Not a map file"*/ STR(1908) });
			return IntType(0);
		}
		...
		{
			log({ /*"Minecraft map error"*/ STR(1907), /*"Data not available"*/ STR(1909) });
			return IntType(0);
		}
		colors = DsMap(data).Value(/*"colors"*/ STR(1714));
		if (is_undefined(colors))
		{
			log({ /*"Minecraft map error"*/ STR(1907), /*"Colors not found"*/ STR(1910) });
			return IntType(0);
		}
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1911) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1912), DsList(global::res_type_name_list).Value(type) });
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1913));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1914));
			new_shader_uniform(self, /*"uTextureSize"*/ STR(1915));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1916));
			new_shader_uniform(self, /*"uTime"*/ STR(1917));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1918));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1919));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1920));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1921));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1922));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1923));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1924));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1925));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1926));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1927));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1928));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1929));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1930));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1931));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1932));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1933));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1934), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1935), /*"_secondary"*/ STR(1936));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1937) + /*"+-/*^()"*/ STR(1938));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1939));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1022) + /*"+-/*^()"*/ STR(1938));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1940) + /*"+-/*^()"*/ STR(1938));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1941) + /*"+-/*^()"*/ STR(1938));
	}
	...
	RealType object_save(ScopeAny self)
	{
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1943) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1944), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1945))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1946))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1947) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1946))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1946);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1945);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1948);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1948))
		{
