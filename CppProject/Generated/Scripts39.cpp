...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1911));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1911));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1912));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		{
			log({ /*"Opening particles"*/ STR(1913), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1914), fn });
	...
	RealType particles_save(ScopeAny self)
	{
		...
		log({ /*"Saving particles"*/ STR(1916), fn });
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1323))
								{
									...
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(513))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1317))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
