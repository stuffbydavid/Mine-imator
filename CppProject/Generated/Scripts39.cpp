...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1977));
			if (!file_exists_lib(fn))
			{
		...
		IntType rootmap;
		BoolType legacy;
		if (string_contains(filename_ext(fn), /*".miparticles"*/ STR(724)))
		{
		...
		if (!legacy)
		{
			...
			{
				temp_particles_type_clear(self);
				project_load_particles(self, DsMap(rootmap).Value(/*"particles"*/ STR(518)));
				temp_particles_restart(self);
			}
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miparticles"*/ STR(724));
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(528) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(917) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
							...
							else
							{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1370))
								{
									if (!(idVar(temp, pc_spawn_region_use) > 0))
										continue;
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(521))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1364))
											{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(528) && idVar(temp, pc_spawn_region_path) != null_)
												{
