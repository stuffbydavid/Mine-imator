...
namespace CppProject
{
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1674), name);
	...
	{
		StringType charlist = /*" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿỳ"*/ STR(1905);
	...
	IntType new_minecraft_map(StringType fn)
	{
		...
		data = DsMap(data).Value(/*"data"*/ STR(1738));
		if (is_undefined(data))
		{
			log({ /*"Minecraft map error"*/ STR(1907), /*"Data not available"*/ STR(1909) });
			return IntType(0);
		}
		colors = DsMap(data).Value(/*"colors"*/ STR(1716));
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		log({ /*"filename"*/ STR(1234), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1024) + /*"+-/*^()"*/ STR(1938));
	}
