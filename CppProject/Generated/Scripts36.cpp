...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1750), /*"b"*/ STR(1751), /*"c"*/ STR(1752), /*"d"*/ STR(1753), /*"e"*/ STR(1213), /*"f"*/ STR(1214), /*"g"*/ STR(1754), /*"h"*/ STR(1755), /*"i"*/ STR(1756), /*"j"*/ STR(1757), /*"k"*/ STR(1758), /*"l"*/ STR(1759), /*"m"*/ STR(1760), /*"n"*/ STR(1215), /*"o"*/ STR(1761), /*"p"*/ STR(1762), /*"q"*/ STR(1763), /*"r"*/ STR(1764), /*"s"*/ STR(1765), /*"t"*/ STR(1216), /*"u"*/ STR(1217), /*"v"*/ STR(1766), /*"w"*/ STR(1767), /*"x"*/ STR(820), /*"y"*/ STR(803), /*"z"*/ STR(821) });
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1783) + name + /*"]"*/ STR(1211) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1784) + name + /*"]"*/ STR(1211) : /*""*/ STR(0));
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
		...
		for (IntType i = IntType(0); i < IntType(4); i++)
		{
			...
			basetex = /*"models/armor/"*/ STR(1785) + basemat + /*"_layer_"*/ STR(1786) + layernum;
			...
			surface_set_target(armorsurf);
		{
			...
			if (basemat == /*"leather"*/ STR(1787))
			{
				draw_texture({ basesprite, IntType(0), IntType(0), IntType(1), IntType(1), dye, IntType(1) });
				draw_texture({ DsMap(idInt(res, model_texture_map)).Value(basetex + /*"_overlay"*/ STR(1788)), IntType(0), IntType(0) });
			}
			else
			...
			{
				VarType palette = DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/"*/ STR(1789) + trimmat + (trimmat == basemat ? /*"_darker"*/ STR(1790) : /*""*/ STR(0)));
				surface_set_target(armorsurf);
			{
				...
				withOne (Object, global::render_shader_obj, noone)
				{
					...
					shader_palette_set(self, palette, DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/trim_palette"*/ STR(1791)));
				}
				
				if (i == IntType(2))
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1792) + trimpat + /*"_leggings"*/ STR(1793))), IntType(0), IntType(0), IntType(0));
				else
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1792) + trimpat)), IntType(0), IntType(0), IntType(0));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(786) ? /*"entity/banner_base"*/ STR(1794) : /*"entity/shield_base"*/ STR(1795));
		patterndir = (type == /*"banner"*/ STR(786) ? /*"entity/banner/"*/ STR(1796) : /*"entity/shield/"*/ STR(1797));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1798), fn });
		show_message(/*"The file "*/ STR(1799) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1800));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1801) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1802))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1803) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1804))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1805) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1806) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1807)), /*""*/ STR(0));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1808)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1809)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1802)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1810)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(859)), vec3(IntType(1), IntType(1), IntType(1)));
			self->model_color = value_get_string(DsMap(map).Value(/*"model_color"*/ STR(1811)), /*"none"*/ STR(878));
			...
			self->floor_box_uvs = value_get_real(DsMap(map).Value(/*"floor_box_uvs"*/ STR(1812)), false);
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1804));
