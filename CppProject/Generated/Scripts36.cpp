...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		...
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1818)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1812)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1819)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1820)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1821))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1821));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1822)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1823)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1824)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			if (self->type == /*"plane"*/ STR(1825))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1826)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1825))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1825))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1827)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1794)), true);
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1828))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1828));
			}
			if (self->type == /*"block"*/ STR(508))
			{
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1829), self->type });
					return null_;
				}
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1830) });
				return null_;
			}
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(795))))
			{
				...
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(744) + curstate))
						log({ /*"model/state/"*/ STR(1831) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(747) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1832) + self->value_name.Value(v) + dev_mode_name_translation_message });
							...
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1833)), true);
								}
							...
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1834))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1834))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1835))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1835)), true);
							}
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1825) && self->is3d > 0)
				{
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = global::_app->project_bend_style == /*"blocky"*/ STR(1836) && sVar(bend_size) == null_ && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && sVec(bend_axis).Real(Z_)));
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1837) ? 4.0 : 1.0)) : sVar(bend_size));
