...
namespace CppProject
{
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			withOne (obj_biome, (new obj_biome)->id, noone)
			{
				...
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1746)), false);
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1754), /*"b"*/ STR(1755), /*"c"*/ STR(1756), /*"d"*/ STR(1757), /*"e"*/ STR(1215), /*"f"*/ STR(1216), /*"g"*/ STR(1758), /*"h"*/ STR(1759), /*"i"*/ STR(1760), /*"j"*/ STR(1761), /*"k"*/ STR(1762), /*"l"*/ STR(1763), /*"m"*/ STR(1764), /*"n"*/ STR(1217), /*"o"*/ STR(1765), /*"p"*/ STR(1766), /*"q"*/ STR(1767), /*"r"*/ STR(1768), /*"s"*/ STR(1769), /*"t"*/ STR(1218), /*"u"*/ STR(1219), /*"v"*/ STR(1770), /*"w"*/ STR(1771), /*"x"*/ STR(822), /*"y"*/ STR(805), /*"z"*/ STR(823) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1772))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1772));
			...
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1773)), false);
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1774);
		...
		{
			log({ /*"Could not load "*/ STR(1775) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1776), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1777)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1777) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1778));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1779));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1780), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1781), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1782), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1783), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1784), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1785));
		if (!is_real(patch))
			patch = IntType(1);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1786) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1783), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1721));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1722) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1723) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1725));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1726) });
				break;
			}
