...
namespace CppProject
{
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1812))) && !(DsMap(map).Value(/*"visible"*/ STR(1812)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(789))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(790) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1813) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1814), DsMap(map).Value(/*"name"*/ STR(789)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(748) + self->name))
				log({ /*"model/part/"*/ STR(1815) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1816)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1817)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1818)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1819)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1819)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1820)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1821)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1822)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1823)), false);
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1824))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1824))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1824));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1825)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1826))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1826));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1827)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1828))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1829) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1828));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1830)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(990)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(873))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1831) });
					return null_;
				}
