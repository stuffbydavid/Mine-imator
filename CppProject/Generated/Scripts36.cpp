...
namespace CppProject
{
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			{
				biomeid = self->id;
				self->name = DsMap(biome).Value(/*"name"*/ STR(789));
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1743)), false);
				self->txy = vec2(IntType(0));
				if (is_string(DsMap(biome).Value(/*"foliage"*/ STR(834))))
				{
					self->color_foliage = hex_to_color(DsMap(biome).Value(/*"foliage"*/ STR(834)));
				...
				else
				{
					...
					self->txy = value_get_point2D(DsMap(biome).Value(/*"foliage"*/ STR(834)), vec2(IntType(0), IntType(0)));
					self->hardcoded = false;
				}
				
				if (is_string(DsMap(biome).Value(/*"grass"*/ STR(833))))
					self->color_grass = hex_to_color(DsMap(biome).Value(/*"grass"*/ STR(833)));
				if (is_string(DsMap(biome).Value(/*"water"*/ STR(787))))
					self->color_water = hex_to_color(DsMap(biome).Value(/*"water"*/ STR(787)));
				self->biome_base = null_;
				self->biome_variants = null_;
				self->variants_extend = false;
				if (ds_list_valid(DsMap(biome).Value(/*"variant"*/ STR(785))))
				{
					self->biome_variants = ds_list_create();
					VarType biomevariants = DsMap(biome).Value(/*"variant"*/ STR(785));
					for (IntType v = IntType(0); v < ds_list_size((IntType)(biomevariants)); v++)
					{
						...
						{
							self->name = DsMap(variant).Value(/*"name"*/ STR(789));
							self->group = false;
							...
							if (is_string(DsMap(variant).Value(/*"foliage"*/ STR(834))))
							{
								self->color_foliage = hex_to_color(DsMap(variant).Value(/*"foliage"*/ STR(834)));
								self->hardcoded = true;
							}
							else
								if (ds_list_valid(DsMap(variant).Value(/*"foliage"*/ STR(834))))
								{
									self->color_foliage = c_white;
									self->txy = value_get_point2D(DsMap(variant).Value(/*"foliage"*/ STR(834)), vec2(IntType(0), IntType(0)));
									self->hardcoded = false;
								}
							
							if (is_string(DsMap(variant).Value(/*"grass"*/ STR(833))))
								self->color_grass = hex_to_color(DsMap(variant).Value(/*"grass"*/ STR(833)));
							else
								self->color_grass = ObjType(obj_biome, self.otherId)->color_grass;
							
							if (is_string(DsMap(variant).Value(/*"water"*/ STR(787))))
								self->color_water = hex_to_color(DsMap(variant).Value(/*"water"*/ STR(787)));
							else
		...
		while (!is_undefined(key))
		{
			switch (StringType(key).id)
			{
				...
				default:
				{
					...
					if (newid != null_)
						newidnomc = string_replace(newid, /*"minecraft:"*/ STR(808), /*""*/ STR(0));
					block = null_;
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1751), /*"b"*/ STR(1752), /*"c"*/ STR(1753), /*"d"*/ STR(1754), /*"e"*/ STR(1213), /*"f"*/ STR(1214), /*"g"*/ STR(1755), /*"h"*/ STR(1756), /*"i"*/ STR(1757), /*"j"*/ STR(1758), /*"k"*/ STR(1759), /*"l"*/ STR(1760), /*"m"*/ STR(1761), /*"n"*/ STR(1215), /*"o"*/ STR(1762), /*"p"*/ STR(1763), /*"q"*/ STR(1764), /*"r"*/ STR(1765), /*"s"*/ STR(1766), /*"t"*/ STR(1216), /*"u"*/ STR(1217), /*"v"*/ STR(1767), /*"w"*/ STR(1768), /*"x"*/ STR(820), /*"y"*/ STR(803), /*"z"*/ STR(821) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			ObjType(obj_particle_template, ptemp)->name = DsMap(tempmap).Value(/*"name"*/ STR(789));
			ObjType(obj_particle_template, ptemp)->texture_name = DsMap(tempmap).Value(/*"texture"*/ STR(544));
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1769))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1769));
			...
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1770)), false);
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1108) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1108) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1275);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1771);
		...
		{
			log({ /*"Could not load "*/ STR(1772) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1773), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1774)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1774) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1775));
			splash = DsList(splashlist).Value(irandom((IntType)(ds_list_size((IntType)(splashlist)) - IntType(1))));
			splashfile = splash_directory + DsMap(splash).Value(/*"file"*/ STR(119));
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1776));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1777), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1778), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1779), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1780), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1781), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1782));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1108) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(20);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1783) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1780), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1718));
			if (is_undefined(characterslist))
			{
				log({ /*"No character list found"*/ STR(1719) });
				break;
			}
			for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
			{
				...
				if (!(model > 0))
				{
					log({ /*"Could not load model"*/ STR(1720) });
					continue;
				}
				DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
				ds_list_add({ self->char_list, model });
			}
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1722));
			if (is_undefined(specialblockslist))
			{
				log({ /*"No special block list found"*/ STR(1723) });
				break;
			}
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1784) + name + /*"]"*/ STR(1211) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1785) + name + /*"]"*/ STR(1211) : /*""*/ STR(0));
	...
	{
		ArrType keys = string_split(name, /*":"*/ STR(758));
		...
		if (string_contains(name, /*"#"*/ STR(829)))
	...
	IntType minecraft_mix_colors(ArrType arr)
	{
		ArrType rgb, total;
		VecType avg;
		RealType maxavg, gain;
		total = ArrType::From({ IntType(0), IntType(0), IntType(0), IntType(0) });
		for (IntType i = IntType(0); i < array_length(VarType::CreateRef(arr)); i++)
		{
			...
			total[IntType(0)] += rgb.Value(IntType(0));
			total[IntType(1)] += rgb.Value(IntType(1));
			total[IntType(2)] += rgb.Value(IntType(2));
			total[IntType(3)] += max({ rgb.Value(IntType(0)), rgb.Value(IntType(1)), rgb.Value(IntType(2)) });
		}
		avg = vec4_div(total, array_length(VarType::CreateRef(arr)));
		maxavg = max({ avg.Real(IntType(0)), avg.Real(IntType(1)), avg.Real(IntType(2)) });
		gain = avg.Real(IntType(3)) / maxavg;
		return make_color_rgb((IntType)(avg.Real(IntType(0)) * gain), (IntType)(avg.Real(IntType(1)) * gain), (IntType)(avg.Real(IntType(2)) * gain));
	}
	
	void minecraft_update_armor()
	{
		if (array_length(VarType::CreateRef(global::armor_update)) > IntType(0) && global::_app->window_busy != /*"popup"*/ STR(1591) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
			VarType obj = null_;
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
		...
		for (IntType i = IntType(0); i < IntType(4); i++)
		{
			...
			if (basemat == /*"none"*/ STR(878))
				continue;
			dye = data.Value(piece + IntType(1));
			trimpat = data.Value(piece + IntType(2));
			trimmat = data.Value(piece + IntType(3));
			layernum = ((i == IntType(2)) ? /*"2"*/ STR(1092) : /*"1"*/ STR(1091));
			basetex = /*"models/armor/"*/ STR(1786) + basemat + /*"_layer_"*/ STR(1787) + layernum;
			...
			surface_set_target(armorsurf);
		{
			...
			if (basemat == /*"leather"*/ STR(1788))
			{
				draw_texture({ basesprite, IntType(0), IntType(0), IntType(1), IntType(1), dye, IntType(1) });
				draw_texture({ DsMap(idInt(res, model_texture_map)).Value(basetex + /*"_overlay"*/ STR(1789)), IntType(0), IntType(0) });
			}
			else
			...
			if (trimpat != /*"none"*/ STR(878) && trimmat != /*"none"*/ STR(878))
			{
				VarType palette = DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/"*/ STR(1790) + trimmat + (trimmat == basemat ? /*"_darker"*/ STR(1791) : /*""*/ STR(0)));
				surface_set_target(armorsurf);
			{
				...
				withOne (Object, global::render_shader_obj, noone)
				{
					...
					shader_palette_set(self, palette, DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/trim_palette"*/ STR(1792)));
				}
				
				if (i == IntType(2))
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1793) + trimpat + /*"_leggings"*/ STR(1794))), IntType(0), IntType(0), IntType(0));
				else
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1793) + trimpat)), IntType(0), IntType(0), IntType(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1591) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(786) ? /*"entity/banner_base"*/ STR(1795) : /*"entity/shield_base"*/ STR(1796));
		patterndir = (type == /*"banner"*/ STR(786) ? /*"entity/banner/"*/ STR(1797) : /*"entity/shield/"*/ STR(1798));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1799), fn });
		show_message(/*"The file "*/ STR(1800) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1801));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		if (!is_string(DsMap(map).Value(/*"texture"*/ STR(544))))
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1802) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1803))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1804) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1805))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1806) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1807) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1808)), /*""*/ STR(0));
			self->texture_name = DsMap(map).Value(/*"texture"*/ STR(544));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1809)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1810)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1803)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1811)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(859)), vec3(IntType(1), IntType(1), IntType(1)));
			self->model_color = value_get_string(DsMap(map).Value(/*"model_color"*/ STR(1812)), /*"none"*/ STR(878));
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1805));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1813))) && !(DsMap(map).Value(/*"visible"*/ STR(1813)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(789))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(790) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1814) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			if (ObjType(obj_model_part, DsList(ObjType(obj_model_file, root)->file_part_list).Value(i))->name == DsMap(map).Value(/*"name"*/ STR(789)))
			{
				log({ /*"Duplicate part name found"*/ STR(1815), DsMap(map).Value(/*"name"*/ STR(789)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(748) + self->name))
				log({ /*"model/part/"*/ STR(1816) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1817)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1818)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1819)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1820)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1820)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1821)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1822)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1823)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(816)), vec3(IntType(0), IntType(0), IntType(0)));
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(859)), vec3(IntType(1), IntType(1), IntType(1)));
			self->scale = vec3_mul(self->scale, idVar(self.otherId, scale));
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1824)), false);
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real(DsMap(map).Value(/*"locked"*/ STR(911)), false);
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1825))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1825))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1825));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1826)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1827))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1827));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1828)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1829))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1830) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1829));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1831)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(990)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(873))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1832) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(873))).id)
				{
					case 196: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1834) });
						return null_;
				}
				
				self->bend_axis = ArrType::From({ false, false, false });
				ArrType axis = ArrType();
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(802))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(802))).id)
					{
						case 820: // x
						{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1835) });
							return null_;
					}
					
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"axis"*/ STR(802))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(802)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"axis"*/ STR(802))), i)).id)
							{
								case 820: // x
								{
									self->bend_axis.Real(X_) = true;
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1836) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1837))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1837));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1837))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1837)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1837))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1838))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1838));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1838))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1838)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1838))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(900))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(900))).id)
					{
						case 1839: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1842) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(900))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(900)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(900))), i)).id)
							{
								case 1839: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1843))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1843)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1843)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1843))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1843)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1843))), i);
					}
					else
				...
				if (is_real(DsMap(bendmap).Value(/*"angle"*/ STR(818))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_default_angle = vec3(DsMap(bendmap).Value(/*"angle"*/ STR(818)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"angle"*/ STR(818))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(818)))); i++)
							self->bend_default_angle.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"angle"*/ STR(818))), i);
					}
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1844));
