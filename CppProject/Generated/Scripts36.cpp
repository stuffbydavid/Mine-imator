...
namespace CppProject
{
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = (global::_app->project_bend_style == /*"blocky"*/ STR(1834)) && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_))) && sVar(bend_size) == null_;
