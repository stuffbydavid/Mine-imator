...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1824))) && !(DsMap(map).Value(/*"visible"*/ STR(1824)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"type"*/ STR(811))))
		{
			log({ /*"Missing parameter \"type\""*/ STR(1856) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(850))))
		{
			log({ /*"Missing array \"from\""*/ STR(1857) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(851))))
		{
			log({ /*"Missing array \"to\""*/ STR(1858) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(859))))
		{
			log({ /*"Missing array \"uv\""*/ STR(1859) });
			return null_;
		}
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			self->type = DsMap(map).Value(/*"type"*/ STR(811));
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1860)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1854)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1861)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1862)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1863))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1863));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1864)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1865)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1866)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(850)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(851)));
			if (self->type == /*"plane"*/ STR(1867))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1868)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1867))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1867))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1869)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1838)), true);
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(859)));
			...
			{
				if (is_string(DsMap(windmap).Value(/*"axis"*/ STR(838))))
				{
					if (DsMap(windmap).Value(/*"axis"*/ STR(838)) == /*"y"*/ STR(839))
						self->wind_wave = e_vertex_wave_Z_ONLY;
					else
						self->wind_wave = e_vertex_wave_ALL;
					
				}
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(840))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(840));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1870))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1870));
			}
			if (self->type == /*"block"*/ STR(515))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(910) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1871), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(6) && ext != /*".jpg"*/ STR(746) && ext != /*".jpeg"*/ STR(747))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1872) });
				return null_;
			}
			
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(522) + self->name))
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(539))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(539));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(539))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(539))));
				}
				else
			...
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(897)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(833))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(833))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(782) + curstate))
						log({ /*"model/state/"*/ STR(1873) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						self->name = curstate;
						IntType valuelist = ds_map_find_value((IntType)(DsMap(map).Value(/*"states"*/ STR(833))), curstate);
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							self->value_name[v] = DsMap(curvalue).Value(/*"value"*/ STR(835));
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(785) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1874) + self->value_name.Value(v) + dev_mode_name_translation_message });
							if (!is_undefined(DsMap(curvalue).Value(/*"file"*/ STR(115))))
								self->value_file[v] = model_file_load(dir + DsMap(curvalue).Value(/*"file"*/ STR(115)));
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(539))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(539));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(539))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(539)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(539)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(539)), true);
								}
							
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1875)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(757))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(757))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1876))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1876))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1877))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1877)), true);
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(897))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(897));
						}
						DsMap(ObjType(obj_model, self.otherId)->states_map)[curstate] = self->id;
						curstate = ds_map_find_next((IntType)(DsMap(map).Value(/*"states"*/ STR(833))), curstate);
					}
					
				}
				
			}
			if (is_string(DsMap(map).Value(/*"default_state"*/ STR(837))))
				self->default_state = string_get_state_vars(DsMap(map).Value(/*"default_state"*/ STR(837)));
			else
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1867) && self->is3d > 0)
				{
	...
	RealType model_part_fill_shape_vbuffer_map(VarType part, IntType vbufmap, IntType alphamap, VecType bend)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			withOne (obj_model_shape, DsList(idInt(part, shape_list)).Value(s), noone)
			{
				...
				if (self->type == /*"block"*/ STR(515) && self->bend_shape > 0 && isbent)
