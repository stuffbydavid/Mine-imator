...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, VarType res)
	{
		...
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			...
			self->texture_mirror = value_get_real({ DsMap(map).Value(/*"texture_mirror"*/ STR(1813)), false });
			self->invert = value_get_real({ DsMap(map).Value(/*"invert"*/ STR(1807)), false });
			self->hide_front = value_get_real({ DsMap(map).Value(/*"hide_front"*/ STR(1814)), false });
			self->hide_back = value_get_real({ DsMap(map).Value(/*"hide_back"*/ STR(1815)), false });
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1816))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1816));
			self->face_camera = value_get_real({ DsMap(map).Value(/*"face_camera"*/ STR(1817)), false });
			self->item_bounce = value_get_real({ DsMap(map).Value(/*"item_bounce"*/ STR(1818)), false });
			self->move_required_array = value_get_point3D({ DsMap(map).Value(/*"move_required"*/ STR(1819)), vec3(-IntType(1), -IntType(1), -IntType(1)) });
			...
			if (self->type == /*"plane"*/ STR(1820))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real({ DsMap(map).Value(/*"inflate"*/ STR(1821)), IntType(0) }));
			if (self->type == /*"plane"*/ STR(1820))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1820))
			{
				self->is3d = value_get_real({ DsMap(map).Value(/*"3d"*/ STR(1822)), false });
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real({ DsMap(map).Value(/*"bend"*/ STR(1789)), true });
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1823))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1823));
			}
			if (self->type == /*"block"*/ STR(507))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(869) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1824), self->type });
					return null_;
				}
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1825) });
				return null_;
			}
			...
			self->pattern_type = value_get_string({ DsMap(map).Value(/*"pattern_type"*/ STR(856)), /*""*/ STR(0) });
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(794))))
			{
				...
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(743) + curstate))
						log({ /*"model/state/"*/ STR(1826) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(746) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1827) + self->value_name.Value(v) + dev_mode_name_translation_message });
							...
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge({ self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828)), true });
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge({ self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828)), true });
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge({ self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1828)), true });
								}
							...
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1829))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1829))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1830))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge({ self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1830)), true });
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(856))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(856));
						}
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1820) && self->is3d > 0)
				{
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = (global::_app->project_bend_style == /*"blocky"*/ STR(1831)) && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_))) && sVar(bend_size) == null_;
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1832) ? 4.0 : 1.0)) : sVar(bend_size));
