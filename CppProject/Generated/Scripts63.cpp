...
namespace CppProject
{
	...
	void temp_particles_init(ScopeAny self)
	{
		...
		sVar(pc_spawn_region_type) = /*"sphere"*/ STR(523);
		...
		sVar(pc_bounding_box_type) = /*"none"*/ STR(919);
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3821), string(ds_list_size(sInt(pc_type_list))) });
	...
	{
		if (idVar(temp, model_name) != /*"armor"*/ STR(791))
			return IntType(0);
		VarType state, type;
		state = /*""*/ STR(0);
		type = /*""*/ STR(0);
	...
	void temp_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(816) + DsList(global::temp_type_name_list).Value(sVar(type)) });
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_SPECIAL_BLOCK:
				{
					if (sVar(model_file) != null_)
						sVar(display_name) = minecraft_asset_get_name(/*"model"*/ STR(525), idVar(sVar(model_file), name));
					break;
				}
				...
				{
					sVar(display_name) = minecraft_asset_get_name(/*"block"*/ STR(518), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					break;
				}
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3324), minecraft_asset_get_name(/*"modelpart"*/ STR(789), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(525), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3325) });
	...
	BoolType textbox_draw(ScopeAny self, VarType tbx, VarType xx, VarType yy, VarType w, RealType h, BoolType contextmenu, BoolType right)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3823)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1101))))
		...
		if (sVar(window_focus) == string(tbx))
		{
				...
				if (key_press.Value(vk_right) || key_press.Value(vk_left))
				{
					if (keyboard_check(vk_control))
					{
						...
						else
						{
							...
							if (char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(984) || sVar(textbox_select_mousepos) == IntType(0))
							{
								sVar(textbox_select_mousepos)--;
							...
							{
								char_ = string_char_at(idVar(tbx, text), (IntType)(sVar(textbox_select_mousepos)));
								if (char_ == /*""*/ STR(0) || char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(984) || (idArr(tbx, line).Value(sVar(textbox_select_mouseline)) != /*""*/ STR(0) && sVar(textbox_select_mousepos) == IntType(0)))
									break;
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1101)))
				sVar(window_busy) = /*""*/ STR(0);
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3823);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3824), idArr(tbx, line).Value(l));
						if (p == IntType(0))
				...
				for (l = IntType(0); l < idVar(tbx, lines); l++)
				{
					if (string_width(idArr(tbx, line).Value(l)) > w)
					{
						...
						for (p = string_length(idArr(tbx, line).Value(l)); p > IntType(1); p--)
						{
							...
							if ((a == /*" "*/ STR(17) || a == /*"-"*/ STR(1128)) && string_width(string_copy(idArr(tbx, line).Value(l), IntType(1), (IntType)(p - IntType(1)))) < w)
								break;
						}
						if (p == IntType(1))
						{
				...
				if ((gmlGlobal::mouse_x >= xx || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_x < xx + w || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_y >= yy + ly || (sVar(window_busy) == string(tbx) && ly == IntType(0))) && (gmlGlobal::mouse_y < yy + ly + hh || (sVar(window_busy) == string(tbx) && (ly + lineheight > h || l == idVar(tbx, lines) - IntType(1)))))
				{
					if (sBool(mouse_left))
					{
						if (idReal(tbx, select_on_focus) > 0 && sVar(textbox_lastfocus) != tbx)
						{
							...
							sVar(window_busy) = string(tbx) + /*"click"*/ STR(1101);
						}
						else
						{
								...
								if (sVar(textbox_select_clickline) == sVar(textbox_select_mouseline) && sVar(textbox_select_clickpos) == sVar(textbox_select_mousepos))
								{
									if (current_time() - sInt(textbox_click) < IntType(500))
									{
										...
										while (true)
										{
											...
											if (c == /*" "*/ STR(17) || c == /*", "*/ STR(1064) || c == /*"("*/ STR(1123) || c == /*")"*/ STR(1127) || c == /*"["*/ STR(1012) || c == /*"]"*/ STR(1252) || c == /*"+"*/ STR(1129))
												break;
											sVar(textbox_select_startpos)--;
											if (sVar(textbox_select_startpos) <= IntType(0))
											{
