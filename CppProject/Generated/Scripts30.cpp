...
namespace CppProject
{
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1112) + string(num++) + /*")"*/ STR(1088) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(20), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"\\"*/ STR(1113), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*":"*/ STR(760), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"*"*/ STR(1091), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"?"*/ STR(1114), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"\""*/ STR(1115), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"<"*/ STR(1116), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*">"*/ STR(1117), /*"_"*/ STR(1110));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1113) || c == /*"/"*/ STR(20))
				return fn + newext;
			if (c == /*"."*/ STR(1087))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1118) }) + /*"|"*/ STR(856) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1119) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1120) }) + /*" (*.ttf)|*.ttf"*/ STR(1121), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1122) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1123) }) + /*" (*.png; *.jpg; *.dat)|*.png;*.jpg;*.jpeg;*.dat;"*/ STR(1124), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1125) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1126) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1127), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1128) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1129) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1130), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1131) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1132) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1133), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1134) }));
	}
	
	StringType file_dialog_open_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1135) }) + /*" (*.zip)|*.zip"*/ STR(1136), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1137) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1138) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1139), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1140) }));
	}
	
	VarType file_dialog_open_project(ScopeAny self)
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1141) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1142) + text_get({ /*"filedialogopenbackup"*/ STR(1143) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1144), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1145) }));
	}
