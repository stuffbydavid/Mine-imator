...
namespace CppProject
{
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenrender"*/ STR(1185) }) + /*" (*.mirender)|*mirender"*/ STR(1186), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenrendercaption"*/ STR(1187) }));
	}
	
	StringType file_dialog_open_scenery()
	{
		return file_dialog_open(text_get({ /*"filedialogopenscenery"*/ STR(1188) }) + /*" (*.schematic; *.nbt; *.blocks)|*.schematic;*.nbt;*.blocks"*/ STR(1189), /*""*/ STR(0), schematics_directory, text_get({ /*"filedialogopenscenerycaption"*/ STR(1190) }));
	}
	
	StringType file_dialog_open_sound()
	{
		return file_dialog_open(text_get({ /*"filedialogopensound"*/ STR(1191) }) + /*" (*.mp3; *.wav; *.ogg; *.flac; *.wma; *.m4a)|*.mp3;*.wav;*.ogg;*.flac;*.wma;*.m4a;"*/ STR(1192), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopensoundcaption"*/ STR(1193) }));
	}
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveimage"*/ STR(1194) }) + /*" (*.png)|*.png"*/ STR(1195), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveimagecaption"*/ STR(1196) }));
	}
	
	StringType file_dialog_save_keyframes(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavekeyframes"*/ STR(1197) }) + /*" (*.miframes)|*.miframes"*/ STR(1198), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsavekeyframescaption"*/ STR(1199) }));
	}
	
	StringType file_dialog_save_movie_mov(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemov"*/ STR(1200) }) + /*" (*.mov)|*.mov"*/ STR(1201), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1202) }));
	}
	
	StringType file_dialog_save_movie_mp4(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviemp4"*/ STR(1203) }) + /*" (*.mp4)|*.mp4"*/ STR(1204), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1202) }));
	}
	
	StringType file_dialog_save_movie_png(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviepng"*/ STR(1205) }) + /*" (*.png)|*.png"*/ STR(1195), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1202) }));
	}
	
	StringType file_dialog_save_movie_wmv(ScopeAny self, VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsavemoviewmv"*/ STR(1206) }) + /*" (*.wmv)|*.wmv"*/ STR(1207), filename_get_valid(fn), sStr(project_folder), text_get({ /*"filedialogsavemoviecaption"*/ STR(1202) }));
	}
	
	StringType file_dialog_save_object(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1208) }) + /*" (*.miobject)|*.miobject"*/ STR(1209), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1210) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1211) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1212), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1213) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1214) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1215) }) + /*" (*.mirender)|*.mirender"*/ STR(1216), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1217) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(714) + exts + /*"|*"*/ STR(1218) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1219) }));
	}
	...
	ArrType file_find(StringType dir, StringType exts)
	{
		...
		StringType f;
		ret = ArrType();
		f = file_find_first(dir + /*"*"*/ STR(1130), IntType(0));
		while (f != /*""*/ STR(0))
		{
	...
	{
		log({ /*"working_directory"*/ STR(1220), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1221) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1222);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1222);
		log({ /*"Trying to save files"*/ STR(1223) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1224) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1225) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1226) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1227) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1228) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1229) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1230) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1231) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1232) });
		return true;
	}
	...
	{
		StringType tmpfile = file_directory + /*"tmp.ttf"*/ STR(1233);
	...
	{
		StringType str, hex;
		str = string_replace(oldstr, /*"#"*/ STR(870), /*""*/ STR(0));
		str = string_upper(str + string_repeat(/*"0"*/ STR(1085), (IntType)(IntType(6) - string_length(oldstr))));
