...
namespace CppProject
{
	...
	void render_update_text()
	{
		...
		withAll (obj_preview, noone)
			if (instance_exists(self->select) && Obj(self->select)->subAssetId != ID_obj_resource && idVar(self->select, type) == e_temp_type_TEXT)
				render_generate_text(ScopeAny(self), { /*"AaBbCc"*/ STR(2037), idVar(self->select, text_font), idVar(self->select, text_3d) });
	...
	RealType render_world_block(VarArgs argument)
	{
		...
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2455), ObjType(obj_resource, res.Value(IntType(1)))->material_format);
		texprev = tex;
		texmatprev = texmat;
