...
namespace CppProject
{
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1248);
					return false;
				}
	...
	void json_save_var_bool(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		buffer_write_string((val > 0) ? /*"true"*/ STR(786) : /*"false"*/ STR(787));
		global::json_add_comma = true;
	}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(870) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	BoolType keybind_check(VarType keybindobj, StringType checkscript)
	{
		...
		if (checkscript == /*"keyboard_check"*/ STR(716))
		{
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1264), fn });
	...
	BoolType key_valid(VarType key)
	{
		...
		keystr = /*"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"*/ STR(1266);
	...
	void langauge_new(StringType fn)
	{
		...
		withOne (obj_language, obj, noone)
		{
			...
			self->name = text_get({ /*"filelanguage"*/ STR(1267) });
			self->locale = text_exists(/*"filelocale"*/ STR(1268)) ? string(text_get({ /*"filelocale"*/ STR(1268) })) : /*""*/ STR(0);
		}
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1269), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1271));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			map = DsList(list).Value(i);
			VarType name, locale, fn;
			name = value_get_string(DsMap(map).Value(/*"name"*/ STR(830)), /*""*/ STR(0));
			locale = value_get_string(DsMap(map).Value(/*"locale"*/ STR(1272)), /*""*/ STR(0));
			fn = value_get_string(DsMap(map).Value(/*"filename"*/ STR(1273)), /*""*/ STR(0));
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1274), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(88), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1271) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(830), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1272), self->locale);
			json_save_var(/*"filename"*/ STR(1273), self->filename);
			json_save_object_done();
		}
	...
	RealType language_add(ScopeAny self)
	{
		...
		if (file_exists_lib(languages_directory + filename_name(fn)))
			if (!question(/*"This file already exists the language file directory. Do you want to replace the file?"*/ STR(1276)))
				return IntType(0);
	...
	{
		log({ /*"Loading language file"*/ STR(1277), fn });
		ds_map_clear(map);
		if (filename_ext(fn) == /*".milanguage"*/ STR(1278))
		{
			StringType convfn = file_directory + /*"conv.tmp"*/ STR(1279);
			json_file_convert_unicode(fn, convfn);
			if (!file_exists_lib(convfn))
			{
				StringType msg = /*"An error occurred while reading the language file:\nCould not convert."*/ STR(1280);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1281));
				show_message(msg);
			...
			{
				StringType msg = /*"An error occured while reading the language file:\n"*/ STR(1282) + global::json_error + /*" on line "*/ STR(1240) + string(global::json_line) + /*", column "*/ STR(1241) + string(global::json_column);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1281));
				show_message(msg);
				window_set_caption(/*""*/ STR(0));
		...
		if (!reload && map != global::language_english_map)
		{
			...
			if (ds_list_size(missingkeyslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(missingkeyslist); i++)
					msg += DsList(missingkeyslist).Value(i) + /*": "*/ STR(1063) + string_replace_all(DsMap(global::language_english_map).Value(DsList(missingkeyslist).Value(i)), /*"\n"*/ STR(984), /*"\\n"*/ STR(1259)) + /*"\n"*/ STR(984);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1281));
	...
	RealType language_load_legacy(VarType fn, IntType map)
	{
		...
		while (!file_text_eof(f))
		{
			...
			commapos = string_pos(/*":"*/ STR(798), line);
	...
	{
		log({ /*"Loading legacy file"*/ STR(1285) });
		IntType map = json_load({ legacy_file });
		if (!ds_map_valid(map))
		{
			log({ /*"Error loading legacy.midata"*/ STR(1286) });
			return false;
		}
		global::legacy_model_id_05_map = DsMap(map).Value(/*"legacy_model_id_05"*/ STR(1287));
		global::legacy_model_id_06_map = DsMap(map).Value(/*"legacy_model_id_06"*/ STR(1288));
		global::legacy_model_id_100_demo_map = DsMap(map).Value(/*"legacy_model_id_100_demo"*/ STR(1289));
		ds_map_merge(global::legacy_model_id_05_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1290)));
		ds_map_merge(global::legacy_model_id_06_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1290)));
		ds_map_merge(global::legacy_model_id_100_demo_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1290)));
		global::legacy_model_part_map = DsMap(map).Value(/*"legacy_model_part"*/ STR(1291));
		global::legacy_model_name_map = DsMap(map).Value(/*"legacy_model_name"*/ STR(1292));
		global::legacy_block_set[IntType(255)] = false;
		global::legacy_block_id = DsMap(map).Value(/*"legacy_block_id"*/ STR(1293));
		global::legacy_block_texture_name_map = DsMap(map).Value(/*"legacy_block_texture_name"*/ STR(1294));
		global::legacy_block_05_texture_list = DsMap(map).Value(/*"legacy_block_05_textures"*/ STR(1295));
		global::legacy_block_07_demo_texture_list = DsMap(map).Value(/*"legacy_block_07_demo_textures"*/ STR(1296));
		global::legacy_block_100_texture_list = DsMap(map).Value(/*"legacy_block_100_textures"*/ STR(1297));
		...
		global::legacy_item_texture_name_map = DsMap(map).Value(/*"legacy_item_texture_name"*/ STR(1298));
		global::legacy_biomes_map = DsMap(map).Value(/*"legacy_biomes"*/ STR(1299));
		global::legacy_biomes_ids_map = ds_int_map_create();
		VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1300))));
		while (!is_undefined(key))
		{
			DsMap(global::legacy_biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1300))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1300))), key);
		}
		
		global::biomes_ids_map = ds_int_map_create();
		key = ds_map_find_first((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1301))));
		while (!is_undefined(key))
		{
			DsMap(global::biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1301))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1301))), key);
		}
		
		global::legacy_model_names_map = DsMap(map).Value(/*"legacy_model_names"*/ STR(1302));
		global::legacy_model_states_map = DsMap(map).Value(/*"legacy_model_states"*/ STR(1303));
		global::legacy_model_state_values_map = DsMap(map).Value(/*"legacy_model_state_values"*/ STR(1304));
		global::legacy_block_names_map = DsMap(map).Value(/*"legacy_block_names"*/ STR(1305));
		global::legacy_particles_map = DsMap(map).Value(/*"legacy_particles"*/ STR(1306));
		return true;
	}
	...
	BoolType lib_startup()
	{
		...
		if (is_cpp())
			return true;
		log({ /*"External library init"*/ STR(1307) });
		StringType libpath, pathfile, pathmovie, pathwindow;
		libpath = /*"Data/Libraries/"*/ STR(1308);
		pathfile = libpath + /*"file.dll"*/ STR(1309);
		pathmovie = libpath + /*"movie.dll"*/ STR(1310);
		pathwindow = libpath + /*"window.dll"*/ STR(1311);
		...
		log({ /*"External library"*/ STR(1312), pathwindow });
		global::lib_window_maximize = external_define({ pathwindow, /*"window_maximize"*/ STR(1313), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_window_set_focus = external_define({ pathwindow, /*"window_set_focus"*/ STR(1314), dll_cdecl, ty_real, IntType(1), ty_string });
		log({ /*"External library"*/ STR(1312), pathfile });
		global::file_copy_temp = true;
		global::lib_open_url = external_define({ pathfile, /*"open_url"*/ STR(1315), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_execute = external_define({ pathfile, /*"execute"*/ STR(1141), dll_cdecl, ty_real, IntType(3), ty_string, ty_string, ty_real });
		global::lib_unzip = external_define({ pathfile, /*"unzip"*/ STR(1316), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_gzunzip = external_define({ pathfile, /*"gzunzip"*/ STR(1317), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_rename = external_define({ pathfile, /*"file_rename"*/ STR(1318), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_copy = external_define({ pathfile, /*"file_copy"*/ STR(1319), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_delete = external_define({ pathfile, /*"file_delete"*/ STR(1320), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_file_exists = external_define({ pathfile, /*"file_exists"*/ STR(1321), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_create = external_define({ pathfile, /*"directory_create"*/ STR(1322), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_delete = external_define({ pathfile, /*"directory_delete"*/ STR(1323), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_exists = external_define({ pathfile, /*"directory_exists"*/ STR(1324), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_json_file_convert_unicode = external_define({ pathfile, /*"json_file_convert_unicode"*/ STR(1325), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		log({ /*"External library"*/ STR(1312), pathmovie });
		global::lib_movie_init = external_define({ pathmovie, /*"movie_init"*/ STR(1326), dll_cdecl, ty_real, IntType(0) });
		global::lib_movie_set = external_define({ pathmovie, /*"movie_set"*/ STR(1327), dll_cdecl, ty_real, IntType(5), ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_start = external_define({ pathmovie, /*"movie_start"*/ STR(1328), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_decode = external_define({ pathmovie, /*"movie_audio_file_decode"*/ STR(1329), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_add = external_define({ pathmovie, /*"movie_audio_file_add"*/ STR(1330), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_audio_sound_add = external_define({ pathmovie, /*"movie_audio_sound_add"*/ STR(1331), dll_cdecl, ty_real, IntType(6), ty_real, ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_frame = external_define({ pathmovie, /*"movie_frame"*/ STR(1332), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_done = external_define({ pathmovie, /*"movie_done"*/ STR(1333), dll_cdecl, ty_real, IntType(0) });
		log({ /*"External library"*/ STR(1312), /*"movie init"*/ STR(1334) });
		lib_movie_init();
	...
	IntType list_init(ScopeAny self, VarType name)
	{
		...
		if (sVar(menu_model_current) != null_)
		{
			for (IntType i = IntType(0); i < idReal(sInt(menu_model_state), value_amount); i++)
				list_item_add(minecraft_asset_get_name(/*"modelstatevalue"*/ STR(788), idVar(sInt(menu_model_state), value_name).Value(i)), idVar(sInt(menu_model_state), value_name).Value(i));
		}
		if (sVar(menu_block_current) != null_)
		{
			for (IntType i = IntType(0); i < idReal(sInt(menu_block_state), value_amount); i++)
				list_item_add(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(807), idVar(sInt(menu_block_state), value_name).Value(i)), idVar(sInt(menu_block_state), value_name).Value(i));
		}
		...
		{
			case 774: // benchskin
			case 775: // benchskinmaterial
			case 776: // benchskinnormal
			case 778: // benchspblocktex
			case 779: // benchspblocktexmaterial
			case 780: // benchspblocktexnormal
			case 781: // benchbodypartskin
			case 782: // benchbodypartskinmaterial
			case 783: // benchbodypartskinnormal
			case 1335: // libraryskin
			case 1336: // libraryskinmaterial
			case 1337: // libraryskinnormal
			case 1338: // libraryspblocktex
			case 1339: // libraryspblocktexmaterial
			case 1340: // libraryspblocktexnormal
			case 1341: // librarybodypartskin
			case 1342: // librarybodypartskinmaterial
			case 1343: // librarybodypartskinnormal
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(762)))
					temp = sInt(bench_settings);
				else
					temp = global::temp_edit;
				
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				if (idVar(temp, model_file) != null_ && idVar(idVar(temp, model_file), player_skin) > 0 && (name == /*"benchskin"*/ STR(774) || name == /*"libraryskin"*/ STR(1335)))
					menu_add_item({ e_option_DOWNLOAD_SKIN, text_get({ /*"libraryskindownload"*/ STR(1345) }), null_, icons_DOWNLOAD });
				VarType tex;
				withOne (obj_resource, global::mc_res, self->id)
				{
					if (string_contains(name, /*"material"*/ STR(654)))
						...
						if (string_contains(name, /*"normal"*/ STR(643)))
			...
			case 820: // benchmodeltex
			case 1346: // librarymodeltex
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(762)))
					temp = sInt(bench_settings);
				...
				if (texobj != null_)
				{
					...
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }), tex });
				}
				else
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), text_get({ /*"listnone"*/ STR(797) }) }) });
			...
			case 821: // benchmodeltexmaterial
			case 1347: // librarymodeltexmaterial
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(762)))
					temp = sInt(bench_settings);
			...
			case 822: // benchmodeltexnormal
			case 1348: // librarymodeltexnormal
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(762)))
					temp = sInt(bench_settings);
			...
			case 793: // benchscenery
			case 1349: // libraryscenery
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				menu_add_item({ e_option_IMPORT_WORLD, text_get({ /*"librarysceneryimport"*/ STR(1350) }), null_, icons_SCENERY });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 794: // benchblocktex
			case 795: // benchblocktexmaterial
			case 796: // benchblocktexnormal
			case 1351: // libraryblocktex
			case 1352: // libraryblocktexmaterial
			case 1353: // libraryblocktexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
			...
			case 803: // benchitemtex
			case 804: // benchitemtexmaterial
			case 805: // benchitemtexnormal
			case 1354: // libraryitemtex
			case 1355: // libraryitemtexmaterial
			case 1356: // libraryitemtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 784: // benchbodypart
			{
				for (IntType p = IntType(0); p < ds_list_size(idInt(ObjType(obj_bench_settings, sInt(bench_settings))->model_file, file_part_list)); p++)
				{
					...
					menu_add_item({ ObjType(obj_model_part, part)->name, minecraft_asset_get_name(/*"modelpart"*/ STR(789), ObjType(obj_model_part, part)->name) });
				}
				break;
			}
			
			case 1357: // templateeditorbodypart
			{
			...
			case 809: // benchtextfont
			case 1358: // librarytextfont
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
			...
			case 812: // benchshapetype
			{
				for (IntType i = IntType(0); i < e_shape_type_amount; i++)
					menu_add_item({ i, text_get({ /*"type"*/ STR(816) + DsList(global::tl_type_name_list).Value(e_tl_type_CUBE + i) }) });
				break;
			}
			
			case 813: // benchshapetex
			case 814: // benchshapetexmaterial
			case 815: // benchshapetexnormal
			case 1359: // libraryshapetex
			case 1360: // libraryshapetexmaterial
			case 1361: // libraryshapetexnormal
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				...
				if (name == /*"benchshapetex"*/ STR(813) || name == /*"libraryshapetex"*/ STR(1359))
				{
			...
			case 1363: // particleeditorspawnregiontype
			{
				menu_add_item({ /*"sphere"*/ STR(523), text_get({ /*"particleeditorspawnregiontypesphere"*/ STR(1364) }), null_, icons_BOUNDARY_CIRCLE });
				menu_add_item({ /*"cube"*/ STR(3), text_get({ /*"particleeditorspawnregiontypecube"*/ STR(1365) }), null_, icons_BOUNDARY_CUBE });
				menu_add_item({ /*"box"*/ STR(1366), text_get({ /*"particleeditorspawnregiontypebox"*/ STR(1367) }), null_, icons_BOUNDARY_BOX });
				menu_add_item({ /*"path"*/ STR(530), text_get({ /*"particleeditorspawnregiontypepath"*/ STR(1368) }), null_, icons_PATH });
				break;
			}
			
			case 1369: // particleeditorspawnregionpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1370: // particleeditorboundingbox
			{
				menu_add_item({ /*"none"*/ STR(919), text_get({ /*"particleeditorboundingboxtypenone"*/ STR(1371) }) });
				menu_add_item({ /*"spawn"*/ STR(1372), text_get({ /*"particleeditorboundingboxtypespawn"*/ STR(1373) }) });
				menu_add_item({ /*"ground"*/ STR(136), text_get({ /*"particleeditorboundingboxtypeground"*/ STR(1374) }) });
				menu_add_item({ /*"custom"*/ STR(594), text_get({ /*"particleeditorboundingboxtypecustom"*/ STR(1375) }) });
				break;
			}
			
			case 1376: // particleeditortypetemp
			{
				menu_add_item({ particle_template, text_get({ /*"particleeditortypetemplate"*/ STR(1377) }) });
				menu_add_item({ particle_sheet, text_get({ /*"particleeditortypespritesheet"*/ STR(1378) }) });
			...
			case 1379: // particleeditortypespritetex
			{
			...
			case 1380: // particleeditortypespritetemplatepack
			{
				VarType img = idVar(global::ptype_edit, sprite_tex_image);
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
			...
			case 1381: // particleeditortypespritetemplate
			{
				for (IntType i = IntType(0); i < ds_list_size(global::particle_template_list); i++)
				{
					...
					if (ObjType(obj_particle_template, temp)->animated)
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1381) + ObjType(obj_particle_template, temp)->name }) + /*" "*/ STR(17) + text_get({ /*"particleeditortypespritetemplateframes"*/ STR(1382), ObjType(obj_particle_template, temp)->frames }) });
					else
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1381) + ObjType(obj_particle_template, temp)->name }) });
					
				}
				break;
			}
			
			case 1383: // backgroundimage
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 1384: // backgroundimagetype
			{
				menu_add_item({ /*"image"*/ STR(90), text_get({ /*"backgroundimagetypeimage"*/ STR(1385) }) });
				menu_add_item({ /*"sphere"*/ STR(523), text_get({ /*"backgroundimagetypesphere"*/ STR(1386) }) });
				menu_add_item({ /*"box"*/ STR(1366), text_get({ /*"backgroundimagetypebox"*/ STR(1387) }) });
				break;
			}
			
			case 1388: // backgroundskysuntex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
			...
			case 1389: // backgroundskymoontex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
			...
			case 1390: // backgroundskymoonphase
			{
				for (IntType p = IntType(0); p < IntType(8); p++)
					menu_add_item({ p, text_get({ /*"backgroundskymoonphase"*/ STR(1390) + string(p + IntType(1)) }), idArr(sVar(background_sky_moon_tex), moon_texture).Value(p) });
				break;
			}
			
			case 1391: // backgroundskycloudstex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
			...
			case 1392: // backgroundgroundtex
			case 1393: // backgroundgroundtexmaterial
			case 1394: // backgroundgroundtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (name == /*"backgroundgroundtexmaterial"*/ STR(1393))
					{
						if (res != global::mc_res && idInt(res, block_sheet_texture_material) != null_)
							menu_add_item({ res, idVar(res, display_name), idInt(res, block_preview_texture) });
					}
					else
						if (name == /*"backgroundgroundtexnormal"*/ STR(1394))
						{
			...
			case 1395: // resourcespackimage
			{
				menu_add_item({ /*"preview"*/ STR(1396), text_get({ /*"resourcespackpreview"*/ STR(1397) }) });
				menu_add_item({ /*"modeltextures"*/ STR(1398), text_get({ /*"resourcespackmodeltextures"*/ STR(1399) }) });
				menu_add_item({ /*"blocksheet"*/ STR(539), text_get({ /*"resourcespackblocksheet"*/ STR(1400) }) });
				menu_add_item({ /*"colormap"*/ STR(1401), text_get({ /*"resourcespackcolormap"*/ STR(1402) }) });
				menu_add_item({ /*"itemsheet"*/ STR(537), text_get({ /*"resourcespackitemsheet"*/ STR(1403) }) });
				menu_add_item({ /*"particlesheet"*/ STR(541), text_get({ /*"resourcespackparticlesheet"*/ STR(1404) }) });
				menu_add_item({ /*"suntexture"*/ STR(1405), text_get({ /*"resourcespacksuntexture"*/ STR(1406) }) });
				menu_add_item({ /*"moontexture"*/ STR(1407), text_get({ /*"resourcespackmoontexture"*/ STR(1408) }) });
				menu_add_item({ /*"cloudtexture"*/ STR(1409), text_get({ /*"resourcespackcloudtexture"*/ STR(1410) }) });
				break;
			}
			
			case 1411: // resourcespackmaterial
			{
				menu_add_item({ /*"diffuse"*/ STR(649), text_get({ /*"resourcespackmaterialdiffuse"*/ STR(1412) }) });
				menu_add_item({ /*"material"*/ STR(654), text_get({ /*"resourcespackmaterialmaterial"*/ STR(1413) }) });
				menu_add_item({ /*"normal"*/ STR(643), text_get({ /*"resourcespackmaterialnormal"*/ STR(1414) }) });
				break;
			}
			
			case 1415: // resourcespackimagemodeltexture
			{
			...
			case 1416: // resourcesscenerystructurepalette
			{
				for (IntType p = IntType(0); p < idInt(global::res_edit, scenery_palette_size); p++)
					menu_add_item({ p, text_get({ /*"resourcesscenerystructurepalettenumber"*/ STR(1417), p + IntType(1) }) });
				break;
			}
			
			case 1418: // frameeditorpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1419: // frameeditorchartex
			case 1420: // frameeditorspblocktex
			case 1421: // frameeditorbodyparttex
			case 1422: // frameeditormodeltex
			{
				...
				if (texobj != null_)
				{
					...
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), text_get({ /*"listnone"*/ STR(797) }) }), null_ });
			...
			case 1423: // frameeditorchartexmaterial
			case 1424: // frameeditorspblocktexmaterial
			case 1425: // frameeditorbodyparttexmaterial
			case 1426: // frameeditormodeltexmaterial
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1427: // frameeditorchartexnormal
			case 1428: // frameeditorspblocktexnormal
			case 1429: // frameeditorbodyparttexnormal
			case 1430: // frameeditormodeltexnormal
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1431: // frameeditorblocktex
			case 1432: // frameeditorblocktexmaterial
			case 1433: // frameeditorblocktexnormal
			{
				VarType texobj = null_;
				if (name == /*"frameeditorblocktexmaterial"*/ STR(1432))
					texobj = idVar(idVar(global::tl_edit, temp), block_tex_material);
				else
					if (name == /*"frameeditorblocktexnormal"*/ STR(1433))
						texobj = idVar(idVar(global::tl_edit, temp), block_tex_normal);
					else
						if (name == /*"frameeditorblocktex"*/ STR(1431))
				...
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }), idInt(texobj, block_preview_texture) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 1434: // frameeditoritemtex
			case 1435: // frameeditoritemtexmaterial
			case 1436: // frameeditoritemtexnormal
			{
				VarType texobj;
				if (name == /*"frameeditoritemtexmaterial"*/ STR(1435))
					texobj = idVar(idVar(global::tl_edit, temp), item_tex_material);
				else
					if (name == /*"frameeditoritemtexnormal"*/ STR(1436))
				...
				if (idVar(texobj, type) == e_res_type_TEXTURE)
					menu_add_item({ texobj, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }), idVar(texobj, texture) });
				else
					if (idInt(texobj, item_sheet_texture) != null_)
						menu_add_item({ texobj, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }), idInt(texobj, block_preview_texture) });
			...
			case 1437: // frameeditorshapetex
			case 1438: // frameeditorshapetexmaterial
			case 1439: // frameeditorshapetexnormal
			{
				VarType texobj;
				if (idVar(global::tl_edit, temp) == null_)
					texobj = null_;
				else
					if (name == /*"frameeditorshapetex"*/ STR(1437))
						texobj = idVar(idVar(global::tl_edit, temp), shape_tex);
					else
						if (name == /*"frameeditorshapetexmaterial"*/ STR(1438))
				...
				if (texobj != null_)
				{
					if (Obj(texobj)->subAssetId == ID_obj_timeline)
						menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }) });
					else
						menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), idVar(texobj, display_name) }), idVar(texobj, texture) });
					
					menu_add_item({ IntType(0), text_get({ /*"listnone"*/ STR(797) }) });
				}
				else
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), text_get({ /*"listnone"*/ STR(797) }) }) });
				...
				if (name == /*"frameeditorshapetex"*/ STR(1437) && idVar(global::tl_edit, type) != e_tl_type_PATH)
				{
					withAll (obj_timeline, self->id)
			...
			case 1440: // frameeditorcameralensdirttexture
			{
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), text_get({ /*"listnone"*/ STR(797) }) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1344) }), null_, null_, ID_action_tl_frame_cam_lens_dirt_tex_browse });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (idVar(res, texture) > 0)
			...
			case 1441: // frameeditorsoundfile
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(797) }) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
			...
			case 1442: // frameeditortextfont
			{
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(823), idVar(idVar(idVar(global::tl_edit, temp), text_font), display_name) }) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 1443: // settingsminecraftversion
			{
			...
			case 1444: // rendershadowssunbuffersize
			case 1445: // rendershadowsspotbuffersize
			case 1446: // rendershadowspointbuffersize
			{
				menu_add_item({ IntType(256), text_get({ /*"rendershadowsbuffersize256"*/ STR(1447) }) + /*" (256x256)"*/ STR(1448) });
				menu_add_item({ IntType(512), text_get({ /*"rendershadowsbuffersize512"*/ STR(1449) }) + /*" (512x512)"*/ STR(1450) });
				menu_add_item({ IntType(1024), text_get({ /*"rendershadowsbuffersize1024"*/ STR(1451) }) + /*" (1024x1024)"*/ STR(1452) });
				menu_add_item({ IntType(2048), text_get({ /*"rendershadowsbuffersize2048"*/ STR(1453) }) + /*" (2048x2048)"*/ STR(1454) });
				menu_add_item({ IntType(4096), text_get({ /*"rendershadowsbuffersize4096"*/ STR(1455) }) + /*" (4096x4096)"*/ STR(1456) });
				if (name != /*"rendershadowspointbuffersize"*/ STR(1446))
					menu_add_item({ IntType(8192), text_get({ /*"rendershadowsbuffersize8192"*/ STR(1457) }) + /*" (8192x8192)"*/ STR(1458) });
				break;
			}
			
			case 1459: // settingswatermarkpositionx
			{
				menu_add_item({ /*"left"*/ STR(191), text_get({ /*"settingswatermarkleft"*/ STR(1460) }) });
				menu_add_item({ /*"center"*/ STR(1461), text_get({ /*"settingswatermarkcenter"*/ STR(1462) }) });
				menu_add_item({ /*"right"*/ STR(192), text_get({ /*"settingswatermarkright"*/ STR(1463) }) });
				break;
			}
			
			case 1464: // settingswatermarkpositiony
			{
				menu_add_item({ /*"top"*/ STR(190), text_get({ /*"settingswatermarktop"*/ STR(1465) }) });
				menu_add_item({ /*"center"*/ STR(1461), text_get({ /*"settingswatermarkcenter"*/ STR(1462) }) });
				menu_add_item({ /*"bottom"*/ STR(189), text_get({ /*"settingswatermarkbottom"*/ STR(1466) }) });
				break;
			}
			
			case 1467: // projectvideosize
			case 1468: // exportmovievideosize
			case 1469: // exportimageimagesize
			case 1470: // frameeditorcameravideosize
			{
				if (ObjType(obj_menu, sVar(menu_current))->menu_name == /*"frameeditorcameravideosize"*/ STR(1470))
					menu_add_item({ null_, text_get({ /*"frameeditorcameravideosizeuseproject"*/ STR(1471) }) });
				for (IntType i = IntType(0); i < ds_list_size(global::videotemplate_list); i++)
					withOne (Object, DsList(global::videotemplate_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"projectvideosizetemplate"*/ STR(1472) + idVar(self->id, name) }) + /*" ("*/ STR(1151) + string(sVar(width)) + /*"x"*/ STR(861) + string(sVar(height)) + /*")"*/ STR(1127) });
				
				menu_add_item({ IntType(0), text_get({ /*"projectvideosizecustom"*/ STR(1473) }) });
				break;
			}
			
			case 1474: // exportmovieformat
			{
				menu_add_item({ /*"mp4"*/ STR(57), text_get({ /*"exportmovieformatmp4"*/ STR(1475) }) });
				menu_add_item({ /*"mov"*/ STR(58), text_get({ /*"exportmovieformatmov"*/ STR(1476) }) });
				menu_add_item({ /*"wmv"*/ STR(59), text_get({ /*"exportmovieformatwmv"*/ STR(1477) }) });
				menu_add_item({ /*"png"*/ STR(60), text_get({ /*"exportmovieformatpng"*/ STR(1478) }) });
				break;
			}
			
			case 1479: // exportmovievideoquality
			{
				for (IntType i = IntType(0); i < ds_list_size(global::videoquality_list); i++)
					withOne (Object, DsList(global::videoquality_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"exportmovievideoquality"*/ STR(1479) + idVar(self->id, name) }) });
				
				menu_add_item({ IntType(0), text_get({ /*"exportmovievideoqualitycustom"*/ STR(1480) }) });
				break;
			}
			
			case 1481: // exportmovieframerate
			{
				menu_add_item({ IntType(24), /*"24"*/ STR(1482) });
				menu_add_item({ IntType(30), /*"30"*/ STR(1483) });
				menu_add_item({ IntType(60), /*"60"*/ STR(1484) });
				menu_add_item({ IntType(0), text_get({ /*"exportmovieframeratecustom"*/ STR(1485) }) });
				break;
			}
			
			case 1486: // projectrendersettings
			{
				menu_add_item({ /*""*/ STR(0), text_get({ /*"projectrendersettingscustom"*/ STR(1487) }) });
				menu_add_item({ /*"performance"*/ STR(1488), text_get({ /*"projectrendersettingsperformance"*/ STR(1489) }), null_ });
				menu_add_item({ /*"balanced"*/ STR(1490), text_get({ /*"projectrendersettingsbalanced"*/ STR(1491) }), null_ });
				menu_add_item({ /*"extreme"*/ STR(1492), text_get({ /*"projectrendersettingsextreme"*/ STR(1493) }), null_ });
				StringType file = file_find_first(render_directory + /*"*.mirender"*/ STR(1494), IntType(0));
				while (file != /*""*/ STR(0))
				{
					if (file != /*"performance.mirender"*/ STR(1495) && file != /*"balanced.mirender"*/ STR(1496) && file != /*"extreme.mirender"*/ STR(1497))
			...
			case 1498: // timelineeditorblendmode
			{
				for (IntType i = IntType(0); i < ds_list_size(global::blend_mode_list); i++)
					menu_add_item({ DsList(global::blend_mode_list).Value(i), text_get({ /*"timelineeditorblendmode"*/ STR(1498) + DsList(global::blend_mode_list).Value(i) }) });
				break;
			}
			
			case 1499: // startupsortby
			{
				list_item_add(text_get({ /*"recentsortdatenewest"*/ STR(1500) }), e_recent_sort_date_newest, /*""*/ STR(0), null_, null_, null_, ID_action_recent_sort);
				list_item_add(text_get({ /*"recentsortdateoldest"*/ STR(1501) }), e_recent_sort_date_oldest, /*""*/ STR(0), null_, null_, null_, ID_action_recent_sort);
				list_item_add(text_get({ /*"recentsortnameaz"*/ STR(1502) }), e_recent_sort_name_az, /*""*/ STR(0), null_, null_, null_, ID_action_recent_sort);
				list_item_add(text_get({ /*"recentsortnameza"*/ STR(1503) }), e_recent_sort_name_za, /*""*/ STR(0), null_, null_, null_, ID_action_recent_sort);
				break;
			}
			
			case 1504: // timelinemarkercolor
			{
				for (IntType i = IntType(0); i <= IntType(8); i++)
				{
					list_item_add(text_get({ /*"timelinemarkercolor"*/ STR(1504) + string(i) }), i, /*""*/ STR(0), ID_spr_16, null_, null_, null_);
					ObjType(obj_list_item, global::list_item_last)->thumbnail_blend = ObjType(obj_theme, sInt(setting_theme))->accent_list.Value(i);
				}
				break;
			}
			
			case 1505: // settingslanguage
			{
			...
			case 1506: // viewmodepass
			{
				for (IntType i = IntType(0); i < e_render_pass_amount; i++)
					list_item_add(text_get({ /*"viewmodepass"*/ STR(1506) + DsList(global::render_pass_list).Value(i) }), i);
				break;
			}
			
			case 1507: // viewcameramain
			case 1508: // viewcamerasecond
			{
				list_item_add(text_get({ /*"viewcamerawork"*/ STR(1509) }), -IntType(4));
				VarType tlname = (sInt(timeline_camera) == -IntType(4) ? text_get({ /*"viewcamerawork"*/ STR(1509) }) : ObjType(obj_timeline, sInt(timeline_camera))->display_name);
				list_item_add(text_get({ /*"viewcameraactive"*/ STR(1510), tlname }), -IntType(5));
				withAll (obj_timeline, self->id)
					if (self->type == e_tl_type_CAMERA)
			...
			case 1511: // worldimportworld
			{
				world_import_world_menu_init();
				break;
			}
			
			case 1512: // worldimportdimension
			{
				world_import_dimension_menu_init();
				break;
			}
			
			case 1513: // settingsinterfacescale
			{
				menu_add_item({ IntType(1), /*"100%"*/ STR(1514) });
				if (interface_scale_default_get() >= IntType(2))
					menu_add_item({ IntType(2), /*"200%"*/ STR(1515) });
				if (interface_scale_default_get() >= IntType(3))
					menu_add_item({ IntType(3), /*"300%"*/ STR(1516) });
				break;
			}
			
			case 1517: // renderalphamode
			case 1518: // timelineeditoralphamode
			{
				if (name == /*"timelineeditoralphamode"*/ STR(1518))
					menu_add_item({ e_alpha_mode_DEFAULT, text_get({ /*"renderalphamodedefault"*/ STR(1519) }) });
				menu_add_item({ e_alpha_mode_HASHED, text_get({ /*"renderalphamodehashed"*/ STR(1520) }) });
				menu_add_item({ e_alpha_mode_BLEND, text_get({ /*"renderalphamodeblend"*/ STR(1521) }) });
				break;
			}
			
			case 1522: // rendertonemapper
			case 1523: // frameeditorcameratonemapper
			{
				menu_add_item({ e_tonemapper_NONE, text_get({ /*"rendertonemappernone"*/ STR(1524) }) });
				menu_add_item({ e_tonemapper_REINHARD, text_get({ /*"rendertonemapperreinhard"*/ STR(1525) }) });
				menu_add_item({ e_tonemapper_ACES, text_get({ /*"rendertonemapperaces"*/ STR(1526) }) });
				break;
			}
			
			case 1527: // armoreditorpatternhelmet
			case 1528: // armoreditorpatternchestplate
			case 1529: // armoreditorpatternleggings
			case 1530: // armoreditorpatternboots
			{
				menu_add_item({ /*"none"*/ STR(919), text_get({ /*"armoreditorpattern"*/ STR(1531) + /*"none"*/ STR(919) }) });
				for (IntType i = IntType(0); i < ds_list_size((IntType)(global::minecraft_armor_trim_pattern_list)); i++)
					menu_add_item({ DsList(global::minecraft_armor_trim_pattern_list).Value(i), text_get({ /*"armoreditorpattern"*/ STR(1531) + DsList(global::minecraft_armor_trim_pattern_list).Value(i) }) });
				break;
			}
			
			case 1532: // armoreditormaterialhelmet
			case 1533: // armoreditormaterialchestplate
			case 1534: // armoreditormaterialleggings
			case 1535: // armoreditormaterialboots
			{
				menu_add_item({ /*"none"*/ STR(919), text_get({ /*"armoreditormaterial"*/ STR(1536) + /*"none"*/ STR(919) }) });
				for (IntType i = IntType(0); i < ds_list_size((IntType)(global::minecraft_armor_trim_material_list)); i++)
					menu_add_item({ DsList(global::minecraft_armor_trim_material_list).Value(i), text_get({ /*"armoreditormaterial"*/ STR(1536) + DsList(global::minecraft_armor_trim_material_list).Value(i) }) });
				break;
			}
			
			case 1537: // timelineeditorglinttex
			{
