...
namespace CppProject
{
	...
	BoolType key_valid(VarType key)
	{
		...
		keystr = /*"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"*/ STR(1214);
	...
	void langauge_new(StringType fn)
	{
		...
		withOne (obj_language, obj, noone)
		{
			...
			self->name = text_get({ /*"filelanguage"*/ STR(1215) });
			self->locale = text_exists(/*"filelocale"*/ STR(1216)) ? string(text_get({ /*"filelocale"*/ STR(1216) })) : /*""*/ STR(0);
		}
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1217), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1219));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			locale = value_get_string({ DsMap(map).Value(/*"locale"*/ STR(1220)), /*""*/ STR(0) });
			fn = value_get_string({ DsMap(map).Value(/*"filename"*/ STR(1221)), /*""*/ STR(0) });
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1222), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(88), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1219) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(786), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1220), self->locale);
			json_save_var(/*"filename"*/ STR(1221), self->filename);
	...
	RealType language_add(ScopeAny self)
	{
		...
		if (file_exists_lib(languages_directory + filename_name(fn)))
			if (!question(/*"This file already exists the language file directory. Do you want to replace the file?"*/ STR(1224)))
				return IntType(0);
	...
	RealType language_load(VarArgs argument)
	{
		...
		log({ /*"Loading language file"*/ STR(1225), fn });
		ds_map_clear((IntType)(map));
		if (filename_ext(fn) == /*".milanguage"*/ STR(1226))
		{
			StringType convfn = file_directory + /*"conv.tmp"*/ STR(1227);
			json_file_convert_unicode(fn, convfn);
			if (!file_exists_lib(convfn))
			{
				StringType msg = /*"An error occurred while reading the language file:\nCould not convert."*/ STR(1228);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1229));
				show_message(msg);
			...
			{
				StringType msg = /*"An error occured while reading the language file:\n"*/ STR(1230) + global::json_error + /*" on line "*/ STR(1188) + string(global::json_line) + /*", column "*/ STR(1189) + string(global::json_column);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1229));
				show_message(msg);
				window_set_caption(/*""*/ STR(0));
		...
		if (!(reload > 0) && map != global::language_english_map)
		{
			...
			if (ds_list_size(missingkeyslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(missingkeyslist); i++)
					msg += DsList(missingkeyslist).Value(i) + /*": "*/ STR(1011) + string_replace_all(DsMap(global::language_english_map).Value(DsList(missingkeyslist).Value(i)), /*"\n"*/ STR(938), /*"\\n"*/ STR(1207)) + /*"\n"*/ STR(938);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1229));
	...
	{
		log({ /*"Loading legacy file"*/ STR(1233) });
		IntType map = json_load({ legacy_file });
		if (!ds_map_valid(map))
		{
			log({ /*"Error loading legacy.midata"*/ STR(1234) });
			return false;
		}
		global::legacy_model_id_05_map = DsMap(map).Value(/*"legacy_model_id_05"*/ STR(1235));
		global::legacy_model_id_06_map = DsMap(map).Value(/*"legacy_model_id_06"*/ STR(1236));
		global::legacy_model_id_100_demo_map = DsMap(map).Value(/*"legacy_model_id_100_demo"*/ STR(1237));
		ds_map_merge({ global::legacy_model_id_05_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1238)) });
		ds_map_merge({ global::legacy_model_id_06_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1238)) });
		ds_map_merge({ global::legacy_model_id_100_demo_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1238)) });
		global::legacy_model_part_map = DsMap(map).Value(/*"legacy_model_part"*/ STR(1239));
		global::legacy_model_name_map = DsMap(map).Value(/*"legacy_model_name"*/ STR(1240));
		global::legacy_block_set[IntType(255)] = false;
		global::legacy_block_id = DsMap(map).Value(/*"legacy_block_id"*/ STR(1241));
		global::legacy_block_texture_name_map = DsMap(map).Value(/*"legacy_block_texture_name"*/ STR(1242));
		global::legacy_block_05_texture_list = DsMap(map).Value(/*"legacy_block_05_textures"*/ STR(1243));
		global::legacy_block_07_demo_texture_list = DsMap(map).Value(/*"legacy_block_07_demo_textures"*/ STR(1244));
		global::legacy_block_100_texture_list = DsMap(map).Value(/*"legacy_block_100_textures"*/ STR(1245));
		...
		global::legacy_item_texture_name_map = DsMap(map).Value(/*"legacy_item_texture_name"*/ STR(1246));
		global::legacy_biomes_map = DsMap(map).Value(/*"legacy_biomes"*/ STR(1247));
		global::legacy_biomes_ids_map = ds_int_map_create();
		VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1248))));
		while (!is_undefined(key))
		{
			DsMap(global::legacy_biomes_ids_map)[string_get_real({ key })] = ds_map_find_value((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1248))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1248))), key);
		}
		
		global::biomes_ids_map = ds_int_map_create();
		key = ds_map_find_first((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1249))));
		while (!is_undefined(key))
		{
			DsMap(global::biomes_ids_map)[string_get_real({ key })] = ds_map_find_value((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1249))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1249))), key);
		}
		
		global::legacy_model_names_map = DsMap(map).Value(/*"legacy_model_names"*/ STR(1250));
		global::legacy_model_states_map = DsMap(map).Value(/*"legacy_model_states"*/ STR(1251));
		global::legacy_model_state_values_map = DsMap(map).Value(/*"legacy_model_state_values"*/ STR(1252));
		global::legacy_block_names_map = DsMap(map).Value(/*"legacy_block_names"*/ STR(1253));
		global::legacy_particles_map = DsMap(map).Value(/*"legacy_particles"*/ STR(1254));
		return true;
	}
	...
	BoolType lib_startup()
	{
		...
		if (is_cpp())
			return true;
		log({ /*"External library init"*/ STR(1255) });
		StringType libpath, pathfile, pathmovie, pathwindow;
		libpath = /*"Data/Libraries/"*/ STR(1256);
		pathfile = libpath + /*"file.dll"*/ STR(1257);
		pathmovie = libpath + /*"movie.dll"*/ STR(1258);
		pathwindow = libpath + /*"window.dll"*/ STR(1259);
		...
		log({ /*"External library"*/ STR(1260), pathwindow });
		global::lib_window_maximize = external_define({ pathwindow, /*"window_maximize"*/ STR(1261), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_window_set_focus = external_define({ pathwindow, /*"window_set_focus"*/ STR(1262), dll_cdecl, ty_real, IntType(1), ty_string });
		log({ /*"External library"*/ STR(1260), pathfile });
		global::file_copy_temp = true;
		global::lib_open_url = external_define({ pathfile, /*"open_url"*/ STR(1263), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_execute = external_define({ pathfile, /*"execute"*/ STR(1089), dll_cdecl, ty_real, IntType(3), ty_string, ty_string, ty_real });
		global::lib_unzip = external_define({ pathfile, /*"unzip"*/ STR(1264), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_gzunzip = external_define({ pathfile, /*"gzunzip"*/ STR(1265), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_rename = external_define({ pathfile, /*"file_rename"*/ STR(1266), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_copy = external_define({ pathfile, /*"file_copy"*/ STR(1267), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_delete = external_define({ pathfile, /*"file_delete"*/ STR(1268), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_file_exists = external_define({ pathfile, /*"file_exists"*/ STR(1269), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_create = external_define({ pathfile, /*"directory_create"*/ STR(1270), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_delete = external_define({ pathfile, /*"directory_delete"*/ STR(1271), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_exists = external_define({ pathfile, /*"directory_exists"*/ STR(1272), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_json_file_convert_unicode = external_define({ pathfile, /*"json_file_convert_unicode"*/ STR(1273), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		log({ /*"External library"*/ STR(1260), pathmovie });
		global::lib_movie_init = external_define({ pathmovie, /*"movie_init"*/ STR(1274), dll_cdecl, ty_real, IntType(0) });
		global::lib_movie_set = external_define({ pathmovie, /*"movie_set"*/ STR(1275), dll_cdecl, ty_real, IntType(5), ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_start = external_define({ pathmovie, /*"movie_start"*/ STR(1276), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_decode = external_define({ pathmovie, /*"movie_audio_file_decode"*/ STR(1277), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_add = external_define({ pathmovie, /*"movie_audio_file_add"*/ STR(1278), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_audio_sound_add = external_define({ pathmovie, /*"movie_audio_sound_add"*/ STR(1279), dll_cdecl, ty_real, IntType(5), ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_frame = external_define({ pathmovie, /*"movie_frame"*/ STR(1280), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_done = external_define({ pathmovie, /*"movie_done"*/ STR(1281), dll_cdecl, ty_real, IntType(0) });
		log({ /*"External library"*/ STR(1260), /*"movie init"*/ STR(1282) });
		lib_movie_init();
	...
	IntType list_init(ScopeAny self, VarType name)
	{
		...
		switch (StringType(name).id)
		{
			...
			case 1283: // libraryskin
			case 1284: // libraryskinmaterial
			case 1285: // libraryskinnormal
			case 1286: // libraryspblocktex
			case 1287: // libraryspblocktexmaterial
			case 1288: // libraryspblocktexnormal
			case 1289: // librarybodypartskin
			case 1290: // librarybodypartskinmaterial
			case 1291: // librarybodypartskinnormal
			{
				VarType temp;
				...
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				if (idVar(temp, model_file) != null_ && idVar(idVar(temp, model_file), player_skin) > 0 && (name == /*"benchskin"*/ STR(732) || name == /*"libraryskin"*/ STR(1283)))
					menu_add_item({ e_option_DOWNLOAD_SKIN, text_get({ /*"libraryskindownload"*/ STR(1293) }), null_, icons_DOWNLOAD });
				VarType tex;
			...
			case 1295: // librarymodeltexmaterial
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(720)))
					temp = sInt(bench_settings);
			...
			case 1296: // librarymodeltexnormal
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(720)))
					temp = sInt(bench_settings);
			...
			case 1297: // libraryscenery
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				menu_add_item({ e_option_IMPORT_WORLD, text_get({ /*"librarysceneryimport"*/ STR(1298) }), null_, icons_SCENERY });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 1299: // libraryblocktex
			case 1300: // libraryblocktexmaterial
			case 1301: // libraryblocktexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
			...
			case 1302: // libraryitemtex
			case 1303: // libraryitemtexmaterial
			case 1304: // libraryitemtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 1305: // templateeditorbodypart
			{
			...
			case 1307: // libraryshapetex
			case 1308: // libraryshapetexmaterial
			case 1309: // libraryshapetexnormal
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				...
				if (name == /*"benchshapetex"*/ STR(769) || name == /*"libraryshapetex"*/ STR(1307))
				{
			...
			case 1311: // particleeditorspawnregiontype
			{
				menu_add_item({ /*"sphere"*/ STR(512), text_get({ /*"particleeditorspawnregiontypesphere"*/ STR(1312) }), null_, icons_BOUNDARY_CIRCLE });
				menu_add_item({ /*"cube"*/ STR(3), text_get({ /*"particleeditorspawnregiontypecube"*/ STR(1313) }), null_, icons_BOUNDARY_CUBE });
				menu_add_item({ /*"box"*/ STR(1314), text_get({ /*"particleeditorspawnregiontypebox"*/ STR(1315) }), null_, icons_BOUNDARY_BOX });
				menu_add_item({ /*"path"*/ STR(519), text_get({ /*"particleeditorspawnregiontypepath"*/ STR(1316) }), null_, icons_PATH });
				break;
			}
			
			case 1317: // particleeditorspawnregionpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1318: // particleeditorboundingbox
			{
				menu_add_item({ /*"none"*/ STR(873), text_get({ /*"particleeditorboundingboxtypenone"*/ STR(1319) }) });
				menu_add_item({ /*"spawn"*/ STR(1320), text_get({ /*"particleeditorboundingboxtypespawn"*/ STR(1321) }) });
				menu_add_item({ /*"ground"*/ STR(135), text_get({ /*"particleeditorboundingboxtypeground"*/ STR(1322) }) });
				menu_add_item({ /*"custom"*/ STR(583), text_get({ /*"particleeditorboundingboxtypecustom"*/ STR(1323) }) });
				break;
			}
			
			case 1324: // particleeditortypetemp
			{
				menu_add_item({ particle_template, text_get({ /*"particleeditortypetemplate"*/ STR(1325) }) });
				menu_add_item({ particle_sheet, text_get({ /*"particleeditortypespritesheet"*/ STR(1326) }) });
			...
			case 1327: // particleeditortypespritetex
			{
			...
			case 1328: // particleeditortypespritetemplatepack
			{
				VarType img = idVar(global::ptype_edit, sprite_tex_image);
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
			...
			case 1329: // particleeditortypespritetemplate
			{
				for (IntType i = IntType(0); i < ds_list_size(global::particle_template_list); i++)
				{
					...
					if (ObjType(obj_particle_template, temp)->animated)
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1329) + ObjType(obj_particle_template, temp)->name }) + /*" "*/ STR(17) + text_get({ /*"particleeditortypespritetemplateframes"*/ STR(1330), ObjType(obj_particle_template, temp)->frames }) });
					else
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1329) + ObjType(obj_particle_template, temp)->name }) });
					
				}
				break;
			}
			
			case 1331: // backgroundimage
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 1332: // backgroundimagetype
			{
				menu_add_item({ /*"image"*/ STR(90), text_get({ /*"backgroundimagetypeimage"*/ STR(1333) }) });
				menu_add_item({ /*"sphere"*/ STR(512), text_get({ /*"backgroundimagetypesphere"*/ STR(1334) }) });
				menu_add_item({ /*"box"*/ STR(1314), text_get({ /*"backgroundimagetypebox"*/ STR(1335) }) });
				break;
			}
			
			case 1336: // backgroundskysuntex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
			...
			case 1337: // backgroundskymoontex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
			...
			case 1338: // backgroundskymoonphase
			{
				for (IntType p = IntType(0); p < IntType(8); p++)
					menu_add_item({ p, text_get({ /*"backgroundskymoonphase"*/ STR(1338) + string(p + IntType(1)) }), idArr(sVar(background_sky_moon_tex), moon_texture).Value(p) });
				break;
			}
			
			case 1339: // backgroundskycloudstex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
			...
			case 1340: // backgroundgroundtex
			case 1341: // backgroundgroundtexmaterial
			case 1342: // backgroundgroundtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (name == /*"backgroundgroundtexmaterial"*/ STR(1341))
					{
						if (res != global::mc_res && idVar(res, block_sheet_texture_material) != null_)
							menu_add_item({ res, idVar(res, display_name), idInt(res, block_preview_texture) });
					}
					else
						if (name == /*"backgroundgroundtexnormal"*/ STR(1342))
						{
			...
			case 1343: // resourcespackimage
			{
				menu_add_item({ /*"preview"*/ STR(1344), text_get({ /*"resourcespackpreview"*/ STR(1345) }) });
				menu_add_item({ /*"modeltextures"*/ STR(1346), text_get({ /*"resourcespackmodeltextures"*/ STR(1347) }) });
				menu_add_item({ /*"blocksheet"*/ STR(528), text_get({ /*"resourcespackblocksheet"*/ STR(1348) }) });
				menu_add_item({ /*"colormap"*/ STR(1349), text_get({ /*"resourcespackcolormap"*/ STR(1350) }) });
				menu_add_item({ /*"itemsheet"*/ STR(526), text_get({ /*"resourcespackitemsheet"*/ STR(1351) }) });
				menu_add_item({ /*"particlesheet"*/ STR(530), text_get({ /*"resourcespackparticlesheet"*/ STR(1352) }) });
				menu_add_item({ /*"suntexture"*/ STR(1353), text_get({ /*"resourcespacksuntexture"*/ STR(1354) }) });
				menu_add_item({ /*"moontexture"*/ STR(1355), text_get({ /*"resourcespackmoontexture"*/ STR(1356) }) });
				menu_add_item({ /*"cloudtexture"*/ STR(1357), text_get({ /*"resourcespackcloudtexture"*/ STR(1358) }) });
				break;
			}
			
			case 1359: // resourcespackmaterial
			{
				menu_add_item({ /*"diffuse"*/ STR(606), text_get({ /*"resourcespackmaterialdiffuse"*/ STR(1360) }) });
				menu_add_item({ /*"material"*/ STR(611), text_get({ /*"resourcespackmaterialmaterial"*/ STR(1361) }) });
				menu_add_item({ /*"normal"*/ STR(600), text_get({ /*"resourcespackmaterialnormal"*/ STR(1362) }) });
				break;
			}
			
			case 1363: // resourcespackimagemodeltexture
			{
			...
			case 1364: // resourcesscenerystructurepalette
			{
				for (IntType p = IntType(0); p < idInt(global::res_edit, scenery_palette_size); p++)
					menu_add_item({ p, text_get({ /*"resourcesscenerystructurepalettenumber"*/ STR(1365), p + IntType(1) }) });
				break;
			}
			
			case 1366: // frameeditorpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1367: // frameeditorchartex
			case 1368: // frameeditorspblocktex
			case 1369: // frameeditorbodyparttex
			case 1370: // frameeditormodeltex
			{
			...
			case 1371: // frameeditorchartexmaterial
			case 1372: // frameeditorspblocktexmaterial
			case 1373: // frameeditorbodyparttexmaterial
			case 1374: // frameeditormodeltexmaterial
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1375: // frameeditorchartexnormal
			case 1376: // frameeditorspblocktexnormal
			case 1377: // frameeditorbodyparttexnormal
			case 1378: // frameeditormodeltexnormal
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1379: // frameeditorblocktex
			case 1380: // frameeditorblocktexmaterial
			case 1381: // frameeditorblocktexnormal
			{
				VarType texobj = null_;
				if (name == /*"frameeditorblocktexmaterial"*/ STR(1380))
					texobj = idVar(idVar(global::tl_edit, temp), block_tex_material);
				else
					if (name == /*"frameeditorblocktexnormal"*/ STR(1381))
						texobj = idVar(idVar(global::tl_edit, temp), block_tex_normal);
					else
						if (name == /*"frameeditorblocktex"*/ STR(1379))
			...
			case 1382: // frameeditoritemtex
			case 1383: // frameeditoritemtexmaterial
			case 1384: // frameeditoritemtexnormal
			{
				VarType texobj;
				if (name == /*"frameeditoritemtexmaterial"*/ STR(1383))
					texobj = idVar(idVar(global::tl_edit, temp), item_tex_material);
				else
					if (name == /*"frameeditoritemtexnormal"*/ STR(1384))
			...
			case 1385: // frameeditorshapetex
			case 1386: // frameeditorshapetexmaterial
			case 1387: // frameeditorshapetexnormal
			{
				VarType texobj;
				if (idVar(global::tl_edit, temp) == null_)
					texobj = null_;
				else
					if (name == /*"frameeditorshapetex"*/ STR(1385))
						texobj = idVar(idVar(global::tl_edit, temp), shape_tex);
					else
						if (name == /*"frameeditorshapetexmaterial"*/ STR(1386))
				...
				if (name == /*"frameeditorshapetex"*/ STR(1385) && idVar(global::tl_edit, type) != e_tl_type_PATH)
				{
					withAll (obj_timeline, self->id)
			...
			case 1388: // frameeditorcameralensdirttexture
			{
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(779), text_get({ /*"listnone"*/ STR(753) }) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1292) }), null_, null_, ID_action_tl_frame_cam_lens_dirt_tex_browse });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (idVar(res, texture) > 0)
			...
			case 1389: // frameeditorsoundfile
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(753) }) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
			...
			case 1391: // settingsminecraftversion
			{
			...
			case 1392: // rendershadowssunbuffersize
			case 1393: // rendershadowsspotbuffersize
			case 1394: // rendershadowspointbuffersize
			{
				menu_add_item({ IntType(256), text_get({ /*"rendershadowsbuffersize256"*/ STR(1395) }) + /*" (256x256)"*/ STR(1396) });
				menu_add_item({ IntType(512), text_get({ /*"rendershadowsbuffersize512"*/ STR(1397) }) + /*" (512x512)"*/ STR(1398) });
				menu_add_item({ IntType(1024), text_get({ /*"rendershadowsbuffersize1024"*/ STR(1399) }) + /*" (1024x1024)"*/ STR(1400) });
				menu_add_item({ IntType(2048), text_get({ /*"rendershadowsbuffersize2048"*/ STR(1401) }) + /*" (2048x2048)"*/ STR(1402) });
				menu_add_item({ IntType(4096), text_get({ /*"rendershadowsbuffersize4096"*/ STR(1403) }) + /*" (4096x4096)"*/ STR(1404) });
				if (name != /*"rendershadowspointbuffersize"*/ STR(1394))
					menu_add_item({ IntType(8192), text_get({ /*"rendershadowsbuffersize8192"*/ STR(1405) }) + /*" (8192x8192)"*/ STR(1406) });
				break;
			}
			
			case 1407: // settingswatermarkpositionx
			{
				menu_add_item({ /*"left"*/ STR(188), text_get({ /*"settingswatermarkleft"*/ STR(1408) }) });
				menu_add_item({ /*"center"*/ STR(1409), text_get({ /*"settingswatermarkcenter"*/ STR(1410) }) });
				menu_add_item({ /*"right"*/ STR(189), text_get({ /*"settingswatermarkright"*/ STR(1411) }) });
				break;
			}
			
			case 1412: // settingswatermarkpositiony
			{
				menu_add_item({ /*"top"*/ STR(187), text_get({ /*"settingswatermarktop"*/ STR(1413) }) });
				menu_add_item({ /*"center"*/ STR(1409), text_get({ /*"settingswatermarkcenter"*/ STR(1410) }) });
				menu_add_item({ /*"bottom"*/ STR(186), text_get({ /*"settingswatermarkbottom"*/ STR(1414) }) });
				break;
			}
			
			case 1415: // projectvideosize
			case 1416: // exportmovievideosize
			case 1417: // exportimageimagesize
			case 1418: // frameeditorcameravideosize
			{
				if (ObjType(obj_menu, sVar(menu_current))->menu_name == /*"frameeditorcameravideosize"*/ STR(1418))
					menu_add_item({ null_, text_get({ /*"frameeditorcameravideosizeuseproject"*/ STR(1419) }) });
				for (IntType i = IntType(0); i < ds_list_size(global::videotemplate_list); i++)
					withOne (Object, DsList(global::videotemplate_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"projectvideosizetemplate"*/ STR(1420) + idVar(self->id, name) }) + /*" ("*/ STR(1099) + string(sVar(width)) + /*"x"*/ STR(817) + string(sVar(height)) + /*")"*/ STR(1075) });
				
				menu_add_item({ IntType(0), text_get({ /*"projectvideosizecustom"*/ STR(1421) }) });
				break;
			}
			
			case 1422: // exportmovieformat
			{
				menu_add_item({ /*"mp4"*/ STR(57), text_get({ /*"exportmovieformatmp4"*/ STR(1423) }) });
				menu_add_item({ /*"mov"*/ STR(58), text_get({ /*"exportmovieformatmov"*/ STR(1424) }) });
				menu_add_item({ /*"wmv"*/ STR(59), text_get({ /*"exportmovieformatwmv"*/ STR(1425) }) });
				menu_add_item({ /*"png"*/ STR(60), text_get({ /*"exportmovieformatpng"*/ STR(1426) }) });
				break;
			}
			
			case 1427: // exportmovievideoquality
			{
				for (IntType i = IntType(0); i < ds_list_size(global::videoquality_list); i++)
					withOne (Object, DsList(global::videoquality_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"exportmovievideoquality"*/ STR(1427) + idVar(self->id, name) }) });
				
				menu_add_item({ IntType(0), text_get({ /*"exportmovievideoqualitycustom"*/ STR(1428) }) });
				break;
			}
			
			case 1429: // exportmovieframerate
			{
				menu_add_item({ IntType(24), /*"24"*/ STR(1430) });
				menu_add_item({ IntType(30), /*"30"*/ STR(1431) });
				menu_add_item({ IntType(60), /*"60"*/ STR(1432) });
				break;
			}
