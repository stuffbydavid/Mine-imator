...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		{
			if (draw_button_icon(self, { /*"listfilter"*/ STR(2973) + string(slist), xx, yy, IntType(24), IntType(24), !ds_list_empty(idInt(slist, filter_list)), icons_FILTER, null_, false, /*"tooltipfilterlist"*/ STR(2974) }))
			{
				menu_settings_set(self, xx, yy, /*"listfilter"*/ STR(2973) + string(slist), IntType(24));
				sInt(settings_menu_script) = ID_sortlist_filters_draw;
				sVar(settings_menu_sortlist) = slist;
				sVar(settings_menu_h_max) = IntType(256);
			}
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(2973) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(720))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(2975) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(2975) }), /*"none"*/ STR(878)))
		{
		...
		if (sVar(window_busy) == /*"sortlist_resize"*/ STR(2976) && sInt(sortlist_resize) == slist)
		{
		...
		for (IntType c = IntType(0); c < idInt(slist, columns); c++)
		{
			...
			if (c > IntType(0) && app_mouse_box(self, xx + dx - IntType(5), yy, IntType(10), colsh) && sBool(content_mouseon))
			{
				...
				if (sBool(mouse_left_pressed))
				{
					...
					sVar(window_busy) = /*"sortlist_resize"*/ STR(2976);
				}
			}
			icon = null_;
			if (idInt(slist, column_sort) == c)
				icon = ((idReal(slist, sort_asc) > 0) ? icons_SORT_UP : icons_SORT_DOWN);
			if (sortlist_draw_button(self, /*"column"*/ STR(2977) + idArr(slist, column_name).Value(c), xx + dx, yy + IntType(4), idArr(slist, column_w).Value(c), colsh, idInt(slist, column_sort) == c, icon, (c == IntType(0)), (c == idInt(slist, columns) - IntType(1)), colmouseon))
			{
				if (idInt(slist, column_sort) == c)
				{
		...
		if (sVar(window_focus) == string(idInt(slist, scroll)))
		{
			...
			if (!app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && sBool(mouse_left) && sVar(window_busy) != /*"scrollbar"*/ STR(2638))
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1087), /*""*/ STR(0)), /*"."*/ STR(1085), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2978)))
			str = string_replace(str, /*"-."*/ STR(2978), /*"-0."*/ STR(2979));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	VarType string_get_state_vars(VarType argument0)
	{
		...
		{
			ArrType nameval = string_split(vars.Value(i), /*"="*/ STR(2980));
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2981)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(943))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2981)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2981);
					c = linestart;
					break;
				}
