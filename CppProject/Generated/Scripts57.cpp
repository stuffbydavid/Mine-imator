...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		if (filter && name == /*""*/ STR(0))
		{
			...
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(2975) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(2977) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(2977) }), /*"none"*/ STR(880)))
		{
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1110), /*" "*/ STR(21));
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			{
				if (char_ == /*" "*/ STR(21) || char_ == /*", "*/ STR(1029) || char_ == /*"."*/ STR(1087) || char_ == /*"/"*/ STR(20) || char_ == /*"-"*/ STR(1089))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1113) + sep, str);
	...
	StringType string_time(RealType hours, RealType mnts, RealType secs)
	{
		...
		return string_repeat(/*"0"*/ STR(1050), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1050), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1050), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(21), /*""*/ STR(0));
	}
