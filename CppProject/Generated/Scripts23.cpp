...
namespace CppProject
{
	...
	{
		if (block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(913)) == /*"true"*/ STR(744))
			return IntType(0);
		...
		if (!sBool(build_edge_zp))
		{
			...
			if (otherblock != null_ && idVar(otherblock, type) == /*"snow"*/ STR(914))
				snowy = /*"true"*/ STR(744);
		}
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(913), snowy);
		return IntType(0);
	}
	...
	RealType block_set_stairs(ScopeAny self)
	{
		...
		StringType shape;
		VarType half, facing;
		shape = /*"straight"*/ STR(915);
		half = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"half"*/ STR(886));
		facing = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"facing"*/ STR(871));
		if (!sBool(build_edge_xp) && (facing == /*"east"*/ STR(864) || facing == /*"west"*/ STR(865)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(916))
			{
				IntType stateid = builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(886)) == half)
				{
					VarType otherfacing = block_get_state_id_value(block, stateid, /*"facing"*/ STR(871));
					if (facing == /*"east"*/ STR(864))
					{
						if (otherfacing == /*"south"*/ STR(866))
							shape = /*"outer_right"*/ STR(917);
						else
							if (otherfacing == /*"north"*/ STR(867))
								shape = /*"outer_left"*/ STR(918);
						
					}
					else
					{
						if (otherfacing == /*"south"*/ STR(866))
							shape = /*"inner_left"*/ STR(919);
						else
							if (otherfacing == /*"north"*/ STR(867))
								shape = /*"inner_right"*/ STR(920);
		...
		if (!sBool(build_edge_xn) && (facing == /*"east"*/ STR(864) || facing == /*"west"*/ STR(865)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) - IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(916))
			{
		...
		if (!sBool(build_edge_yp) && (facing == /*"south"*/ STR(866) || facing == /*"north"*/ STR(867)))
		{
			...
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(916))
			{
				...
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(886)) == half)
				{
					...
					{
						if (otherfacing == /*"east"*/ STR(864))
							shape = /*"outer_left"*/ STR(918);
						else
							if (otherfacing == /*"west"*/ STR(865))
								shape = /*"outer_right"*/ STR(917);
		...
		if (!sBool(build_edge_yn) && (facing == /*"south"*/ STR(866) || facing == /*"north"*/ STR(867)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y) - IntType(1), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(916))
			{
	...
	RealType block_set_tripwire(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && (idVar(block, name) == /*"tripwire"*/ STR(921) || idVar(block, name) == /*"tripwire_hook"*/ STR(922)))
				east = /*"true"*/ STR(744);
		}
	...
	RealType block_set_twisting_vines(ScopeAny self)
	{
		if (!sBool(build_edge_zn))
		{
			...
			if (block == null_ || idVar(block, type) != /*"twisting_vines"*/ STR(923))
				global::vertex_wave_zmin = sInt(block_pos_z);
		}
		else
			global::vertex_wave_zmin = sInt(block_pos_z);
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(782)) == /*"twisting_vines_plant"*/ STR(924))
			return IntType(0);
		if (sInt(build_pos_z) != (sInt(build_size_z) - IntType(1)))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(782), /*"twisting_vines_plant"*/ STR(924) }));
		return IntType(0);
	}
	...
	RealType block_set_vine(ScopeAny self)
	{
		...
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"up"*/ STR(883), up);
		return IntType(0);
	}
	...
	RealType block_set_wall(ScopeAny self)
	{
		...
		states = ArrType::From({ /*"none"*/ STR(873), /*"low"*/ STR(581), /*"tall"*/ STR(925) });
		tall = false;
		i = IntType(0);
		if (!sBool(build_edge_zp))
		{
			...
			if (!sBool(build_edge_xp) && (i == IntType(0) || (i == IntType(1) && east > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_xp) == e_block_depth_DEPTH0 && sBool(block_face_full_xp)))
						...
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z) + i), /*"facing"*/ STR(871));
			...
			if (!sBool(build_edge_yp) && (i == IntType(0) || (i == IntType(1) && south > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_yp) == e_block_depth_DEPTH0 && sBool(block_face_full_yp)))
						...
						if (idVar(block, type) == /*"fence_gate"*/ STR(901))
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x), sInt(build_pos_y) + IntType(1), sInt(build_pos_z) + i), /*"facing"*/ STR(871));
		...
		sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(782), variant, /*"east"*/ STR(864), states.Value(east), /*"west"*/ STR(865), states.Value(west), /*"south"*/ STR(866), states.Value(south), /*"north"*/ STR(867), states.Value(north), /*"up"*/ STR(883), up }));
		return IntType(0);
	}
	...
	RealType block_set_weeping_vines(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z) + IntType(1));
			if (block == null_ || idVar(block, type) != /*"weeping_vines"*/ STR(926))
				global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		}
		else
			global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(782)) == /*"weeping_vines_plant"*/ STR(927))
			return IntType(0);
		if (sInt(build_pos_z) == IntType(0))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(782), /*"weeping_vines"*/ STR(926) }));
		else
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(782), /*"weeping_vines_plant"*/ STR(927) }));
	...
	void block_tile_entity_banner(Scope<obj_builder_thread> self, IntType map)
	{
		...
		patterns = DsMap(map).Value(/*"Patterns"*/ STR(932));
		if (ds_map_exists(map, /*"Base"*/ STR(933)))
		{
			base = DsMap(map).Value(/*"Base"*/ STR(933));
			legacy = true;
		}
		else
		{
		...
		if (ds_list_valid(patterns))
		{
			for (IntType i = IntType(0); i < ds_list_size((IntType)(patterns)); i++)
			{
				...
				pattern = value_get_string({ DsMap(patternmap).Value(/*"Pattern"*/ STR(934)), DsList(global::minecraft_pattern_short_list).Value(IntType(1)) });
				color = value_get_real({ DsMap(patternmap).Value(/*"Color"*/ STR(935)), IntType(0) });
		...
		{
			VarType color = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(885));
	...
	{
		VarType colorid = DsMap(map).Value(/*"color"*/ STR(885));
		if (!is_real(colorid))
			return IntType(0);
		if (is_undefined(colorid) || colorid >= ds_list_size(global::minecraft_color_list))
			return IntType(0);
		RealType newstate = block_set_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(885), DsList(global::minecraft_color_name_list).Value(colorid));
	...
	RealType block_tile_entity_sign(Scope<obj_builder_thread> self, IntType map)
	{
		...
		IntType color;
		text = /*""*/ STR(0);
		colorname = value_get_string({ DsMap(map).Value(/*"Color"*/ STR(935)), /*"black"*/ STR(599) });
		...
		{
			VarType line = DsMap(map).Value(/*"Text"*/ STR(936) + string(i + IntType(1)));
			if (!is_string(line))
				return IntType(0);
			...
			if (ds_map_valid(textmap))
			{
				...
				if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(937))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(937)))) > IntType(0))
				{
					IntType extramap = ds_list_find_value(DsMap(textmap).Value(/*"extra"*/ STR(937)), IntType(0));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(939);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(940);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(941)));
		map = DsMap(map).Value(/*"Properties"*/ STR(942));
		if (!ds_map_valid(map))
			return IntType(0);
		...
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(943)));
		...
		map = DsMap(map).Value(/*"SKIN"*/ STR(944));
		texurl = DsMap(map).Value(/*"url"*/ STR(945)).Str();
