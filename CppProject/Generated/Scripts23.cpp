...
namespace CppProject
{
	VarType block_load_state_file(StringType fname, IntType block, ArrType state)
	{
		...
		{
			log({ /*"Could not find state file"*/ STR(842), filename_name(fname) });
			return null_;
		}
		...
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse state file"*/ STR(843), filename_name(fname) });
			ds_map_destroy(jsontypemap);
			return null_;
		}
		VarType variantsmap, multipartlist;
		variantsmap = DsMap(map).Value(/*"variants"*/ STR(844));
		multipartlist = DsMap(map).Value(/*"multipart"*/ STR(845));
		if (!ds_map_valid(variantsmap) && !ds_list_valid(multipartlist))
		{
			log({ /*"No models in the states file"*/ STR(846), fname });
			ds_map_destroy(map);
			return null_;
		}
		withOne (obj_block_load_state_file, (new obj_block_load_state_file)->id, noone)
		{
			...
			if (ds_map_valid(DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_preview_map).Value(sVar(name))))
			{
				...
				if (DsMap(blockmap).Value(/*"colorZ"*/ STR(847)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorZ"*/ STR(847))))
						self->model_preview_color_yp = hex_to_color(DsMap(blockmap).Value(/*"colorZ"*/ STR(847)));
					else
					{
				...
				if (self->model_preview_alpha_yp != null_ && DsMap(blockmap).Value(/*"alphaZ"*/ STR(848)) != VarType())
					self->model_preview_alpha_yp = DsMap(blockmap).Value(/*"alphaZ"*/ STR(848));
				if (DsMap(blockmap).Value(/*"colorY"*/ STR(849)) != VarType())
				{
					if (is_string(DsMap(blockmap).Value(/*"colorY"*/ STR(849))))
						self->model_preview_color_zp = hex_to_color(DsMap(blockmap).Value(/*"colorY"*/ STR(849)));
					else
					{
				...
				if (self->model_preview_alpha_zp != null_ && DsMap(blockmap).Value(/*"alphaY"*/ STR(850)) != VarType())
					self->model_preview_alpha_zp = DsMap(blockmap).Value(/*"alphaY"*/ STR(850));
			}
			...
			if (ds_map_valid(variantsmap))
			{
				...
				if (ds_list_valid(multipartlist))
				{
					for (IntType c = IntType(0); c < ds_list_size((IntType)(multipartlist)); c++)
					{
						...
						{
							VarType whenmap = DsMap(mcase).Value(/*"when"*/ STR(851));
							if (ds_map_valid(whenmap) && ds_map_size((IntType)(whenmap)) > IntType(0))
							{
								VarType orlist = DsMap(whenmap).Value(/*"OR"*/ STR(852));
								VarType andlist = DsMap(whenmap).Value(/*"AND"*/ STR(853));
									...
									for (IntType oc = IntType(0); oc < ds_list_size((IntType)(orlist)); oc++)
									{
										...
										while (!is_undefined(cond))
										{
											...
											if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(curcondmap)), cond) == e_json_type_BOOL)
												val = ((val > 0) ? /*"true"*/ STR(746) : /*"false"*/ STR(747));
											if (string_contains(val, /*"|"*/ STR(854)))
												state_vars_set_value(VarType::CreateRef(condvars), cond, string_split(val, /*"|"*/ STR(854)));
											else
							...
							self->model_amount = IntType(0);
							self->total_weight = IntType(0);
							if (ds_map_find_value((IntType)(DsMap(jsontypemap).Value(mcase)), /*"apply"*/ STR(855)) == e_json_type_ARRAY)
							{
								VarType modellist = DsMap(mcase).Value(/*"apply"*/ STR(855));
								for (IntType i = IntType(0); i < ds_list_size((IntType)(modellist)); i++)
									if (!block_load_variant_model(ScopeAny(self), DsList(modellist).Value(i), ObjType(obj_block, block)->type))
										return false;
							}
							else
								if (!block_load_variant_model(ScopeAny(self), DsMap(mcase).Value(/*"apply"*/ STR(855)), ObjType(obj_block, block)->type))
									return null_;
	...
	void block_load_timeline(Scope<obj_block> self, VarType map, VarType typemap)
	{
		...
		if (is_string(DsMap(map).Value(/*"model"*/ STR(8))))
		{
			...
			if (DsMap(typemap).Value(/*"model_state"*/ STR(856)) == e_json_type_STRING)
			{
				self->tl_model_state = string_get_state_vars(DsMap(map).Value(/*"model_state"*/ STR(856)));
				self->tl_has_model_state = true;
			}
			else
				if (DsMap(typemap).Value(/*"model_state"*/ STR(856)) == e_json_type_OBJECT)
				{
					VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"model_state"*/ STR(856))));
					while (!is_undefined(key))
					{
						withOne (obj_block_tl_state, (new obj_block_tl_state)->id, self->id)
						{
							self->vars = string_get_state_vars(key);
							self->value = string_get_state_vars(ds_map_find_value((IntType)(DsMap(map).Value(/*"model_state"*/ STR(856))), key));
						...
						key = ds_map_find_next((IntType)(DsMap(map).Value(/*"model_state"*/ STR(856))), key);
					}
		...
		if (is_bool(DsMap(map).Value(/*"model_double"*/ STR(857))))
			self->model_double = DsMap(map).Value(/*"model_double"*/ STR(857));
		...
		if (ds_map_valid(DsMap(map).Value(/*"text"*/ STR(103))))
		{
			VarType textmap = DsMap(map).Value(/*"text"*/ STR(103));
			if (ds_map_valid(DsMap(textmap).Value(/*"front"*/ STR(858))))
			{
				VarType front = DsMap(textmap).Value(/*"front"*/ STR(858));
				self->tl_text_front_position = value_get_point3D(DsMap(front).Value(/*"position"*/ STR(44)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_front_rotation = value_get_point3D(DsMap(front).Value(/*"rotation"*/ STR(816)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_front_scale = value_get_real(DsMap(front).Value(/*"scale"*/ STR(859)), IntType(1));
			}
			if (ds_map_valid(DsMap(textmap).Value(/*"back"*/ STR(860))))
			{
				VarType back = DsMap(textmap).Value(/*"back"*/ STR(860));
				self->tl_text_back_position = value_get_point3D(DsMap(back).Value(/*"position"*/ STR(44)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_back_rotation = value_get_point3D(DsMap(back).Value(/*"rotation"*/ STR(816)), point3D(IntType(0), IntType(0), IntType(0)));
				self->tl_text_back_scale = value_get_real(DsMap(back).Value(/*"scale"*/ STR(859)), IntType(1));
			}
			self->tl_has_text = true;
		}
		self->tl_pattern_type = /*""*/ STR(0);
		if (is_string(DsMap(map).Value(/*"pattern_type"*/ STR(861))))
			self->tl_pattern_type = DsMap(map).Value(/*"pattern_type"*/ STR(861));
		self->tl_rot_point = point3D(IntType(0), IntType(0), IntType(0));
		self->tl_rot_point_state_amount = IntType(0);
		if (DsMap(typemap).Value(/*"rotation_point"*/ STR(862)) == e_json_type_ARRAY)
			self->tl_rot_point = value_get_point3D(DsMap(map).Value(/*"rotation_point"*/ STR(862)), point3D(IntType(0), IntType(0), IntType(0)));
		else
			if (DsMap(typemap).Value(/*"rotation_point"*/ STR(862)) == e_json_type_OBJECT)
			{
				VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"rotation_point"*/ STR(862))));
				while (!is_undefined(key))
				{
					withOne (obj_block_tl_state, (new obj_block_tl_state)->id, self->id)
					{
						self->vars = string_get_state_vars(key);
						self->value = value_get_point3D(ds_map_find_value((IntType)(DsMap(map).Value(/*"rotation_point"*/ STR(862))), key), point3D(IntType(0), IntType(0), IntType(0)));
						self->state_id = ArrType();
						for (IntType i = IntType(0); i < ObjType(obj_block, self.otherId)->state_id_amount; i++)
							if (state_vars_match_state_id(self->vars, self.otherId, i))
								self->state_id = array_add(VarType::CreateRef(self->state_id), i);
					...
					key = ds_map_find_next((IntType)(DsMap(map).Value(/*"rotation_point"*/ STR(862))), key);
				}
		...
		if (DsMap(typemap).Value(/*"rotation"*/ STR(816)) == e_json_type_ARRAY)
			self->tl_rotation = value_get_point3D(DsMap(map).Value(/*"rotation"*/ STR(816)), point3D(IntType(0), IntType(0), IntType(0)));
		else
			if (DsMap(typemap).Value(/*"rotation"*/ STR(816)) == e_json_type_OBJECT)
			{
				VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"rotation"*/ STR(816))));
				while (!is_undefined(key))
				{
					withOne (obj_block_tl_state, (new obj_block_tl_state)->id, self->id)
					{
						self->vars = string_get_state_vars(key);
						self->value = value_get_point3D(ds_map_find_value((IntType)(DsMap(map).Value(/*"rotation"*/ STR(816))), key), point3D(IntType(0), IntType(0), IntType(0)));
						self->state_id = ArrType();
						for (IntType i = IntType(0); i < ObjType(obj_block, self.otherId)->state_id_amount; i++)
							if (state_vars_match_state_id(self->vars, self.otherId, i))
								self->state_id = array_add(VarType::CreateRef(self->state_id), i);
					...
					key = ds_map_find_next((IntType)(DsMap(map).Value(/*"rotation"*/ STR(816))), key);
				}
	...
	BoolType block_load_variant_model(ScopeAny self, VarType map, VarType type)
	{
		...
		{
			log({ /*"Missing parameter \"model\""*/ STR(863) });
			return false;
		}
		StringType blockname;
		VarType blockmodel;
		blockname = string_replace(DsMap(map).Value(/*"model"*/ STR(8)), /*"minecraft:"*/ STR(808), /*""*/ STR(0));
		...
		if (!is_undefined(DsMap(map).Value(/*"x"*/ STR(820))))
			rot.Real(X_) = clamp(snap(DsMap(map).Value(/*"x"*/ STR(820)), IntType(90)), IntType(0), IntType(270));
		if (!is_undefined(DsMap(map).Value(/*"y"*/ STR(803))))
			rot.Real(Z_) = clamp(snap(DsMap(map).Value(/*"y"*/ STR(803)), IntType(90)), IntType(0), IntType(270));
		VarType uvlock = false;
		if (is_bool(DsMap(map).Value(/*"uvlock"*/ STR(864))))
			uvlock = DsMap(map).Value(/*"uvlock"*/ STR(864));
		else
			if (is_string(DsMap(map).Value(/*"uvlock"*/ STR(864))))
				uvlock = (DsMap(map).Value(/*"uvlock"*/ STR(864)) == /*"true"*/ STR(746));
		
		VarType weight = IntType(1);
		if (!is_undefined(DsMap(map).Value(/*"weight"*/ STR(865))))
			weight = DsMap(map).Value(/*"weight"*/ STR(865));
		...
		switch ((IntType)culldir)
		{
			case e_dir_EAST:
			{
				if (!idBool(el, face_edge_xp) || self->block_face_min_depth_xp == null_ || self->block_face_min_depth_xp > ObjType(obj_block_render_model, model)->face_min_depth_xp)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_xp > e_block_depth_DEPTH0)
					return false;
			...
			case e_dir_WEST:
			{
				if (!idBool(el, face_edge_xn) || self->block_face_min_depth_xn == null_ || self->block_face_min_depth_xn > ObjType(obj_block_render_model, model)->face_min_depth_xn)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_xn > e_block_depth_DEPTH0)
					return false;
			...
			case e_dir_SOUTH:
			{
				if (!idBool(el, face_edge_yp) || self->block_face_min_depth_yp == null_ || self->block_face_min_depth_yp > ObjType(obj_block_render_model, model)->face_min_depth_yp)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_yp > e_block_depth_DEPTH0)
					return false;
			...
			case e_dir_NORTH:
			{
				if (!idBool(el, face_edge_yn) || self->block_face_min_depth_yn == null_ || self->block_face_min_depth_yn > ObjType(obj_block_render_model, model)->face_min_depth_yn)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_yn > e_block_depth_DEPTH0)
					return false;
			...
			case e_dir_UP:
			{
				if (!idBool(el, face_edge_zp) || self->block_face_min_depth_zp == null_ || self->block_face_min_depth_zp > ObjType(obj_block_render_model, model)->face_min_depth_zp)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_zp > e_block_depth_DEPTH0)
					return false;
			...
			case e_dir_DOWN:
			{
				if (!idBool(el, face_edge_zn) || self->block_face_min_depth_zn == null_ || self->block_face_min_depth_zn > ObjType(obj_block_render_model, model)->face_min_depth_zn)
					return false;
				if (self->block_current != null_ && ObjType(obj_block, self->block_current)->type == /*"leaves"*/ STR(866) && self->block_face_min_depth_zn > e_block_depth_DEPTH0)
					return false;
	...
	RealType block_set_bars(ScopeAny self)
	{
		...
		if (!sBool(build_edge_xp))
		{
			...
			if ((block != null_ && (idVar(block, type) == /*"bars"*/ STR(867) || idVar(block, type) == /*"colored_bars"*/ STR(868))) || (sInt(block_face_min_depth_xp) != e_block_depth_DEPTH1 && sBool(block_face_full_xp)))
				east = /*"true"*/ STR(746);
		}
		if (!sBool(build_edge_xn))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) - IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if ((block != null_ && (idVar(block, type) == /*"bars"*/ STR(867) || idVar(block, type) == /*"colored_bars"*/ STR(868))) || (sInt(block_face_min_depth_xn) != e_block_depth_DEPTH1 && sBool(block_face_full_xn)))
				west = /*"true"*/ STR(746);
		}
		if (!sBool(build_edge_yp))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y) + IntType(1), sInt(build_pos_z));
			if ((block != null_ && (idVar(block, type) == /*"bars"*/ STR(867) || idVar(block, type) == /*"colored_bars"*/ STR(868))) || (sInt(block_face_min_depth_yp) != e_block_depth_DEPTH1 && sBool(block_face_full_yp)))
				south = /*"true"*/ STR(746);
		}
		if (!sBool(build_edge_yn))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y) - IntType(1), sInt(build_pos_z));
			if ((block != null_ && (idVar(block, type) == /*"bars"*/ STR(867) || idVar(block, type) == /*"colored_bars"*/ STR(868))) || (sInt(block_face_min_depth_yn) != e_block_depth_DEPTH1 && sBool(block_face_full_yn)))
				north = /*"true"*/ STR(746);
		}
		sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"east"*/ STR(869), east, /*"west"*/ STR(870), west, /*"south"*/ STR(871), south, /*"north"*/ STR(872), north }));
		return IntType(0);
	}
	
	VarType block_set_bed(Scope<obj_builder_thread> self)
	{
		if (block_get_state_id_value(self->block_current, self->block_state_id_current, /*"part"*/ STR(873)) == /*"head"*/ STR(874))
			return null_;
		return IntType(0);
	}
	...
	{
		if ((self->builder_scenery && !self->builder_scenery_legacy) || block_get_state_id_value(self->block_current, self->block_state_id_current, /*"type"*/ STR(775)) == /*"big_dripleaf_stem"*/ STR(875))
			return IntType(0);
		if (self->build_pos_z != (self->build_size_z - IntType(1)))
		{
			VarType facing = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"facing"*/ STR(876));
			self->block_state_id_current = block_get_state_id(self->block_current, ArrType::From({ /*"type"*/ STR(775), /*"big_dripleaf_stem"*/ STR(875), /*"facing"*/ STR(876), facing, /*"tilt"*/ STR(877), /*"none"*/ STR(878) }));
		}
		return IntType(0);
	}
	...
	RealType block_set_cave_vines(ScopeAny self)
	{
		if (!sBool(build_edge_zp))
		{
			...
			if (block == null_ || idVar(block, type) != /*"cave_vines"*/ STR(879))
				global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		}
		else
			global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"type"*/ STR(775)) == /*"cave_vines_plant"*/ STR(880))
			return IntType(0);
		VarType berries = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"berries"*/ STR(881));
		if (sInt(build_pos_z) == IntType(0))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"type"*/ STR(775), /*"cave_vines"*/ STR(879), /*"berries"*/ STR(881), berries }));
		else
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"type"*/ STR(775), /*"cave_vines_plant"*/ STR(880), /*"berries"*/ STR(881), berries }));
	...
	VarType block_set_chest(ScopeAny self)
	{
		VarType type;
		StringType double_;
		type = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"type"*/ STR(775));
		double_ = /*"false"*/ STR(747);
		switch (StringType(type).id)
		{
			...
			case 883: // legacy
			{
				if (idVar(sInt(block_current), name) == /*"ender_chest"*/ STR(884))
					break;
				IntType connectblock, discardblock;
				connectblock = null_;
				discardblock = null_;
				VarType facing = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"facing"*/ STR(876));
		...
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"double"*/ STR(885), double_);
		return IntType(0);
	}
	...
	RealType block_set_chorus_plant(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && (block == sInt(block_current) || idVar(block, type) == /*"chorus_plant_connect"*/ STR(886)))
				east = /*"true"*/ STR(746);
		}
		if (!sBool(build_edge_xn))
		{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z) - IntType(1));
			if (block != null_ && (block == sInt(block_current) || idVar(block, type) == /*"chorus_plant_connect"*/ STR(886) || idVar(block, name) == /*"end_stone"*/ STR(887)))
				down = /*"true"*/ STR(746);
		}
		sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"east"*/ STR(869), east, /*"west"*/ STR(870), west, /*"south"*/ STR(871), south, /*"north"*/ STR(872), north, /*"up"*/ STR(888), up, /*"down"*/ STR(889), down }));
		return IntType(0);
	}
	...
	RealType block_set_colored_bars(ScopeAny self)
	{
		...
		if (!sBool(builder_scenery) && sInt(build_size_x) == IntType(1) && sInt(build_size_y) == IntType(1))
			return IntType(0);
		VarType color = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"color"*/ STR(890));
		block_set_bars(self);
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"color"*/ STR(890), color);
		return IntType(0);
	}
	
	VarType block_set_door(ScopeAny self)
	{
		if (block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"half"*/ STR(891)) == /*"upper"*/ STR(892))
			return null_;
		VarType hinge = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"hinge"*/ STR(893));
		if (!sBool(build_edge_zp))
		{
			...
			if (aboveblock == sInt(block_current))
			{
				...
				hinge = block_get_state_id_value(sInt(block_current), abovestateid, /*"hinge"*/ STR(893));
				sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"hinge"*/ STR(893), hinge);
			}
		}
		VarType facing, open, dir;
		StringType location;
		facing = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"facing"*/ STR(876));
		open = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"open"*/ STR(894));
		if (facing == /*"east"*/ STR(869))
		{
			location = (hinge == /*"right"*/ STR(196) ? /*"south_west"*/ STR(895) : /*"north_west"*/ STR(896));
			if (open == /*"false"*/ STR(747))
				dir = (hinge == /*"right"*/ STR(196) ? /*"north"*/ STR(872) : /*"south"*/ STR(871));
		}
		else
			if (facing == /*"west"*/ STR(870))
			{
				location = (hinge == /*"right"*/ STR(196) ? /*"north_east"*/ STR(897) : /*"south_east"*/ STR(898));
				if (open == /*"false"*/ STR(747))
					dir = (hinge == /*"right"*/ STR(196) ? /*"south"*/ STR(871) : /*"north"*/ STR(872));
			}
			else
				if (facing == /*"south"*/ STR(871))
				{
					location = (hinge == /*"right"*/ STR(196) ? /*"north_west"*/ STR(896) : /*"north_east"*/ STR(897));
					if (open == /*"false"*/ STR(747))
						dir = (hinge == /*"right"*/ STR(196) ? /*"east"*/ STR(869) : /*"west"*/ STR(870));
				}
				else
					if (facing == /*"north"*/ STR(872))
					{
						location = (hinge == /*"right"*/ STR(196) ? /*"south_east"*/ STR(898) : /*"south_west"*/ STR(895));
						if (open == /*"false"*/ STR(747))
							dir = (hinge == /*"right"*/ STR(196) ? /*"west"*/ STR(870) : /*"east"*/ STR(869));
					}
			
		
		
		if (open == /*"true"*/ STR(746))
			dir = facing;
		VarType bottommodel = idVar(sInt(block_current), state_id_model_obj).Value(sReal(block_state_id_current));
		VarType topmodel = idVar(sInt(block_current), state_id_model_obj).Value(block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"half"*/ STR(891), /*"upper"*/ STR(892)));
		...
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"location"*/ STR(899), location);
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"direction"*/ STR(900), dir);
	...
	RealType block_set_dripstone(Scope<obj_builder_thread> self)
	{
		...
		thicknessprev = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"thickness"*/ STR(901));
		if (thicknessprev == /*"base"*/ STR(902))
			return IntType(0);
		dir = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"vertical_direction"*/ STR(903));
		size = (dir == /*"up"*/ STR(888) ? (self->build_size_z - IntType(1)) - self->build_pos_z : self->build_pos_z);
		sizeoffset = size;
		if (thicknessprev == /*"frustum"*/ STR(904))
			sizeoffset += IntType(1);
		else
			if (thicknessprev == /*"middle"*/ STR(905))
		...
		if (sizeoffset == IntType(0))
			...
			if (sizeoffset == IntType(1))
				thickness = /*"frustum"*/ STR(904);
			else
				if (size >= (self->build_size_z - IntType(1)))
					thickness = /*"base"*/ STR(902);
				else
					thickness = /*"middle"*/ STR(905);
			
		
		
		self->block_state_id_current = block_get_state_id(self->block_current, ArrType::From({ /*"vertical_direction"*/ STR(903), dir, /*"thickness"*/ STR(901), thickness }));
		return IntType(0);
	}
