...
namespace CppProject
{
	...
	void block_tile_entity_banner(Scope<obj_builder_thread> self, IntType map)
	{
		...
		patterns = DsMap(map).Value(/*"Patterns"*/ STR(934));
		if (ds_map_exists(map, /*"Base"*/ STR(935)))
		{
			base = DsMap(map).Value(/*"Base"*/ STR(935));
			legacy = true;
		}
		else
		{
		...
		if (ds_list_valid(patterns))
		{
			for (IntType i = IntType(0); i < ds_list_size((IntType)(patterns)); i++)
			{
				...
				pattern = value_get_string({ DsMap(patternmap).Value(/*"Pattern"*/ STR(936)), DsList(global::minecraft_pattern_short_list).Value(IntType(1)) });
				color = value_get_real({ DsMap(patternmap).Value(/*"Color"*/ STR(937)), IntType(0) });
	...
	RealType block_tile_entity_sign(Scope<obj_builder_thread> self, IntType map)
	{
		...
		IntType color;
		text = /*""*/ STR(0);
		colorname = value_get_string({ DsMap(map).Value(/*"Color"*/ STR(937)), /*"black"*/ STR(600) });
		...
		{
			VarType line = DsMap(map).Value(/*"Text"*/ STR(938) + string(i + IntType(1)));
			if (!is_string(line))
				return IntType(0);
			...
			if (ds_map_valid(textmap))
			{
				...
				if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(939))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(939)))) > IntType(0))
				{
					IntType extramap = ds_list_find_value(DsMap(textmap).Value(/*"extra"*/ STR(939)), IntType(0));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(941);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(942);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(943)));
		map = DsMap(map).Value(/*"Properties"*/ STR(944));
		if (!ds_map_valid(map))
			return IntType(0);
		...
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(945)));
		...
		map = DsMap(map).Value(/*"SKIN"*/ STR(946));
		texurl = DsMap(map).Value(/*"url"*/ STR(947)).Str();
