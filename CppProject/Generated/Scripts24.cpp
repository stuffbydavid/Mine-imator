...
namespace CppProject
{
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(947), /*"Not a schematic file"*/ STR(948) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(949)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(950));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(951));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(952));
		log({ /*"Size"*/ STR(55), string(self->build_size_x) + /*" x "*/ STR(953) + string(self->build_size_y) + /*" x "*/ STR(953) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(947), /*"Size not fully defined"*/ STR(954) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(947), /*"Size cannot be 0"*/ STR(955) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(956));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(947), /*"Version not available"*/ STR(957) });
				return false;
			}
			log({ /*"Version"*/ STR(956), DsMap(map).Value(/*"Version"*/ STR(956)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(947), /*"Unsupported format, version too high"*/ STR(958) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(949));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(947), /*"Palette not found"*/ STR(959) });
				return false;
			}
			...
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(960)))
				{
					VarType index;
					IntType bracketindex;
					index = DsMap(palettemap).Value(key);
					bracketindex = string_pos(/*"["*/ STR(961), key);
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(802)) != /*"false"*/ STR(745))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(802)) == /*"true"*/ STR(744))
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(962));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(947), /*"BlockData array not found"*/ STR(963) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(964)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(965), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(966));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(967));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(968));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(947), /*"Blocks array not found"*/ STR(969) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(970));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(947), /*"Data array not found"*/ STR(971) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(967));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(972));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(973))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(973));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(974), /*"Unsupported format, version too low"*/ STR(975) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(976));
		...
		{
			log({ /*"Structure error"*/ STR(974), /*"Size cannot be 0"*/ STR(955) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(977));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(978));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(974), /*"Palette not found"*/ STR(959) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(979));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(943));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(980));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(974), /*"Block list not found"*/ STR(981) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(982));
				state = DsMap(blockmap).Value(/*"state"*/ STR(983));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(984));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(985));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(986) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(805), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(987));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(942));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(989));
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(817));
						ey = DsMap(entity).Value(/*"z"*/ STR(818));
						ez = DsMap(entity).Value(/*"y"*/ STR(800));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(986) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(805), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(990));
			}
	...
	void colorpicker_draw(ScopeAny self)
	{
		...
		if (app_mouse_box(self, bx, by, bw, bh))
		{
			...
			if (sBool(mouse_left))
				sVar(window_busy) = /*"colorpickersatbripick"*/ STR(992);
		}
		if (sVar(window_busy) == /*"colorpickersatbripick"*/ STR(992))
		{
			if (!sBool(mouse_left))
			{
		...
		if (app_mouse_box(self, bx - IntType(8), by, bw + IntType(16), bh))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (sBool(mouse_left))
				sVar(window_busy) = /*"colorpickerhuepick"*/ STR(993);
		}
		if (sVar(window_busy) == /*"colorpickerhuepick"*/ STR(993))
		{
			if (!sBool(mouse_left))
			{
				sVar(window_busy) = /*""*/ STR(0);
		...
		togglebutton_add(self, { /*"colorpickerrgb"*/ STR(994), null_, /*"rgb"*/ STR(995), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"rgb"*/ STR(995), ID_colorpicker_set_mode });
		togglebutton_add(self, { /*"colorpickerhsv"*/ STR(996), null_, /*"hsv"*/ STR(997), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hsv"*/ STR(997), ID_colorpicker_set_mode });
		togglebutton_add(self, { /*"colorpickerhex"*/ STR(998), null_, /*"hex"*/ STR(999), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hex"*/ STR(999), ID_colorpicker_set_mode });
		draw_togglebutton(self, { /*"colorpickermode"*/ STR(1000), sVar(dx), sVar(dy), true, false });
		tab_next(self);
		if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"rgb"*/ STR(995))
		{
			textfield_group_add(self, { /*"colorpickerr"*/ STR(1001), color_get_red((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_red(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_red, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_red });
			textfield_group_add(self, { /*"colorpickerg"*/ STR(1002), color_get_green((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_green(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_green, Y_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_green });
			textfield_group_add(self, { /*"colorpickerb"*/ STR(1003), color_get_blue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_blue(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_blue, Z_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_blue });
			if (draw_textfield_group(self, { /*"colorpickerrgb"*/ STR(994), sVar(dx), sVar(dy), IntType(176), IntType(1), IntType(0), IntType(255), IntType(1), false, false, IntType(3), true, false }))
			{
			...
			if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hsv"*/ STR(997))
			{
				textfield_group_add(self, { /*"colorpickerh"*/ STR(1004), floor(color_get_hue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_hue(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_hue, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hue });
				textfield_group_add(self, { /*"colorpickers"*/ STR(1005), floor(color_get_saturation((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_saturation(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_saturation, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_saturation });
				textfield_group_add(self, { /*"colorpickerv"*/ STR(1006), floor(color_get_value((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_value(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_brightness, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_brightness });
				VarType update = draw_textfield_group(self, { /*"colorpickerhsv"*/ STR(996), sVar(dx), sVar(dy), IntType(176), IntType(1), IntType(0), IntType(255), IntType(1), false, false, IntType(0), true, false });
				...
				if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hex"*/ STR(999))
				{
					if (draw_inputbox(self, { /*"colorpickerhex"*/ STR(998), sVar(dx), sVar(dy), IntType(176), IntType(24), color_to_hex(ObjType(obj_colorpicker, sInt(colorpicker))->def), ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal, null_ }))
					...
					if (ObjType(obj_textbox, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal)->text == /*""*/ STR(0) && sVar(window_focus) == /*""*/ STR(0))
						ObjType(obj_textbox, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal)->text = /*"000000"*/ STR(657);
				}
	...
	{
		if (sStr(settings_menu_name) == /*"colorpicker"*/ STR(677) && ObjType(obj_colorpicker, sInt(colorpicker))->value_script == script)
			popup_close(self);
		else
		{
			sVar(settings_menu_busy_prev) = sVar(window_busy);
			sVar(window_busy) = /*"settingsmenu"*/ STR(1007);
			sVar(window_focus) = /*""*/ STR(0);
			app_mouse_clear(self);
			sInt(settings_menu_window) = window_get_current();
			sStr(settings_menu_name) = /*"colorpicker"*/ STR(677);
			...
			ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"rgb"*/ STR(995);
	...
	void colorpicker_startup()
	{
		...
		withOne (obj_colorpicker, global::_app->colorpicker, global::_app->id)
		{
			...
			self->tbx_red = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_green = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_blue = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_hue = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_saturation = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_brightness = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1008));
			self->tbx_hexadecimal = new_textbox_hex();
			self->mode = /*"rgb"*/ STR(995);
		}
