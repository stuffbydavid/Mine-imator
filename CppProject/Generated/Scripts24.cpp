...
namespace CppProject
{
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(949), /*"Not a schematic file"*/ STR(950) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(951)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(952));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(953));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(954));
		log({ /*"Size"*/ STR(55), string(self->build_size_x) + /*" x "*/ STR(955) + string(self->build_size_y) + /*" x "*/ STR(955) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(949), /*"Size not fully defined"*/ STR(956) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(949), /*"Size cannot be 0"*/ STR(957) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(958));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(949), /*"Version not available"*/ STR(959) });
				return false;
			}
			log({ /*"Version"*/ STR(958), DsMap(map).Value(/*"Version"*/ STR(958)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(949), /*"Unsupported format, version too high"*/ STR(960) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(951));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(949), /*"Palette not found"*/ STR(961) });
				return false;
			}
			...
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(962)))
				{
					VarType index;
					IntType bracketindex;
					index = DsMap(palettemap).Value(key);
					bracketindex = string_pos(/*"["*/ STR(963), key);
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(964));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(949), /*"BlockData array not found"*/ STR(965) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(966)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(967), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(968));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(969));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(970));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(949), /*"Blocks array not found"*/ STR(971) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(972));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(949), /*"Data array not found"*/ STR(973) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(969));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(974));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(975))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(975));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(976), /*"Unsupported format, version too low"*/ STR(977) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(978));
		...
		{
			log({ /*"Structure error"*/ STR(976), /*"Size cannot be 0"*/ STR(957) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(979));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(980));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(976), /*"Palette not found"*/ STR(961) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(981));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(945));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(982));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(976), /*"Block list not found"*/ STR(983) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(984));
				state = DsMap(blockmap).Value(/*"state"*/ STR(985));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(986));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(987));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(988) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(806), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(989));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(944));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(991));
					...
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(988) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(806), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(992));
			}
	...
	void colorpicker_draw(ScopeAny self)
	{
		...
		if (app_mouse_box(self, bx, by, bw, bh))
		{
			...
			if (sBool(mouse_left))
				sVar(window_busy) = /*"colorpickersatbripick"*/ STR(994);
		}
		if (sVar(window_busy) == /*"colorpickersatbripick"*/ STR(994))
		{
			if (!sBool(mouse_left))
			{
		...
		if (app_mouse_box(self, bx - IntType(8), by, bw + IntType(16), bh))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (sBool(mouse_left))
				sVar(window_busy) = /*"colorpickerhuepick"*/ STR(995);
		}
		if (sVar(window_busy) == /*"colorpickerhuepick"*/ STR(995))
		{
			if (!sBool(mouse_left))
			{
				sVar(window_busy) = /*""*/ STR(0);
		...
		togglebutton_add(self, { /*"colorpickerrgb"*/ STR(996), null_, /*"rgb"*/ STR(997), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"rgb"*/ STR(997), ID_colorpicker_set_mode });
		togglebutton_add(self, { /*"colorpickerhsv"*/ STR(998), null_, /*"hsv"*/ STR(999), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hsv"*/ STR(999), ID_colorpicker_set_mode });
		togglebutton_add(self, { /*"colorpickerhex"*/ STR(1000), null_, /*"hex"*/ STR(1001), ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hex"*/ STR(1001), ID_colorpicker_set_mode });
		draw_togglebutton(self, /*"colorpickermode"*/ STR(1002), sVar(dx), sVar(dy), true, false);
		tab_next(self);
		if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"rgb"*/ STR(997))
		{
			textfield_group_add(self, { /*"colorpickerr"*/ STR(1003), color_get_red((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_red(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_red, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_red });
			textfield_group_add(self, { /*"colorpickerg"*/ STR(1004), color_get_green((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_green(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_green, Y_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_green });
			textfield_group_add(self, { /*"colorpickerb"*/ STR(1005), color_get_blue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color)), color_get_blue(ObjType(obj_colorpicker, sInt(colorpicker))->def), ID_colorpicker_blue, Z_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_blue });
			if (draw_textfield_group(self, /*"colorpickerrgb"*/ STR(996), sVar(dx), sVar(dy), IntType(176), IntType(1), IntType(0), IntType(255), IntType(1), false, false, IntType(3), true, false))
			{
			...
			if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hsv"*/ STR(999))
			{
				textfield_group_add(self, { /*"colorpickerh"*/ STR(1006), floor(color_get_hue((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_hue(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_hue, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hue });
				textfield_group_add(self, { /*"colorpickers"*/ STR(1007), floor(color_get_saturation((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_saturation(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_saturation, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_saturation });
				textfield_group_add(self, { /*"colorpickerv"*/ STR(1008), floor(color_get_value((IntType)(ObjType(obj_colorpicker, sInt(colorpicker))->color))), floor(color_get_value(ObjType(obj_colorpicker, sInt(colorpicker))->def)), ID_colorpicker_brightness, X_, ObjType(obj_colorpicker, sInt(colorpicker))->tbx_brightness });
				VarType update = draw_textfield_group(self, /*"colorpickerhsv"*/ STR(998), sVar(dx), sVar(dy), IntType(176), IntType(1), IntType(0), IntType(255), IntType(1), false, false, IntType(0), true, false);
				...
				if (ObjType(obj_colorpicker, sInt(colorpicker))->mode == /*"hex"*/ STR(1001))
				{
					if (draw_inputbox(self, { /*"colorpickerhex"*/ STR(1000), sVar(dx), sVar(dy), IntType(176), IntType(24), color_to_hex(ObjType(obj_colorpicker, sInt(colorpicker))->def), ObjType(obj_colorpicker, sInt(colorpicker))->tbx_hexadecimal, null_ }))
	...
	void colorpicker_show(ScopeAny self, VarType name, VarType color, IntType def, IntType script, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		else
		{
			...
			sVar(window_busy) = /*"settingsmenu"*/ STR(1009);
			sVar(window_focus) = /*""*/ STR(0);
			app_mouse_clear(self);
			...
			ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"rgb"*/ STR(997);
	...
	void colorpicker_startup()
	{
		...
		withOne (obj_colorpicker, global::_app->colorpicker, global::_app->id)
		{
			...
			self->tbx_red = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_green = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_blue = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_hue = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_saturation = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_brightness = new_textbox(IntType(1), IntType(3), /*"0123456789"*/ STR(1010));
			self->tbx_hexadecimal = new_textbox_hex();
			self->mode = /*"rgb"*/ STR(997);
		}
