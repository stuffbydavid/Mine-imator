...
namespace CppProject
{
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(989);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(990);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(991)));
		map = DsMap(map).Value(/*"Properties"*/ STR(992));
		if (!ds_map_valid(map))
			return IntType(0);
		texlist = DsMap(map).Value(/*"textures"*/ STR(851));
		if (!ds_list_valid(texlist))
			return IntType(0);
		map = DsList(texlist).Value(IntType(0));
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(993)));
		map = json_decode(texvalue);
		if (!ds_map_valid(map))
			return IntType(0);
		if (!ds_map_valid(DsMap(map).Value(/*"textures"*/ STR(851))))
			return IntType(0);
		map = DsMap(map).Value(/*"textures"*/ STR(851));
		map = DsMap(map).Value(/*"SKIN"*/ STR(994));
		texurl = DsMap(map).Value(/*"url"*/ STR(995)).Str();
	...
	RealType builder_generate(ScopeAny self)
	{
		...
		if (builder_get_waterlogged(Scope<obj_builder_thread>(self), sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z)))
		{
			...
			sInt(block_pos_y) = sInt(build_pos_y) * block_size_;
			sInt(block_pos_z) = sInt(build_pos_z) * block_size_;
			sInt(block_current) = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(/*"water"*/ STR(826));
	...
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(996), /*"Not a schematic file"*/ STR(997) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(998)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(999));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(1000));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(1001));
		log({ /*"Size"*/ STR(55), string(self->build_size_x) + /*" x "*/ STR(1002) + string(self->build_size_y) + /*" x "*/ STR(1002) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(996), /*"Size not fully defined"*/ STR(1003) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(996), /*"Size cannot be 0"*/ STR(1004) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(1005));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(996), /*"Version not available"*/ STR(1006) });
				return false;
			}
			log({ /*"Version"*/ STR(1005), DsMap(map).Value(/*"Version"*/ STR(1005)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(996), /*"Unsupported format, version too high"*/ STR(1007) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(998));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(996), /*"Palette not found"*/ STR(1008) });
				return false;
			}
			...
			while (!is_undefined(key))
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(1009)))
				{
					VarType index;
					IntType bracketindex;
					index = DsMap(palettemap).Value(key);
					bracketindex = string_pos(/*"["*/ STR(1010), key);
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(844)) != /*"false"*/ STR(785))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(844)) == /*"true"*/ STR(784))
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(1011));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(996), /*"BlockData array not found"*/ STR(1012) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(1013)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(1014), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(1015));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(1016));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(1017));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(996), /*"Blocks array not found"*/ STR(1018) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(1019));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(996), /*"Data array not found"*/ STR(1020) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(1016));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(1021));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(1022))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(1022));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(1023), /*"Unsupported format, version too low"*/ STR(1024) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(1025));
		...
		{
			log({ /*"Structure error"*/ STR(1023), /*"Size cannot be 0"*/ STR(1004) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(1026));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(1027));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(1023), /*"Palette not found"*/ STR(1008) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(1028));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(992));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(1029));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(1023), /*"Block list not found"*/ STR(1030) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(1031));
				state = DsMap(blockmap).Value(/*"state"*/ STR(1032));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(1033));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(1034));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1035) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(847), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(1036));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(991));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(1038));
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(859));
						ey = DsMap(entity).Value(/*"z"*/ STR(860));
						ez = DsMap(entity).Value(/*"y"*/ STR(842));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1035) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(847), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(1039));
			}
