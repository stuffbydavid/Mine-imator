...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					case IntType(0):
					{
						return /*"linear"*/ STR(545);
					}
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(548);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(549);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(550);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(546);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(919), /*"ground"*/ STR(136), /*"spawn"*/ STR(1372), /*"custom"*/ STR(594) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(517))
				typename_ = /*"itemsheet"*/ STR(537);
			if (typename_ == /*"block"*/ STR(518) || typename_ == /*"blocksheet"*/ STR(539))
				typename_ = /*"legacyblocksheet"*/ STR(538);
			if (typename_ == /*"particles"*/ STR(520))
				typename_ = /*"particlesheet"*/ STR(541);
			if (typename_ == /*"schematic"*/ STR(2216))
				typename_ = /*"scenery"*/ STR(516);
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2219), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2218));
				buffer_delete(global::buffer_current);
				return false;
			}
	...
	void project_load_legacy_template()
	{
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				{
					self->model_name = DsMap(modelmap).Value(/*"name"*/ STR(830));
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(1034))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(1034)));
					else
						self->model_state = ArrType();
					
				}
				else
					log({ /*"Could not convert model "*/ STR(2220), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2221), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2205))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2222), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2205))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2205))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			withOne (obj_marker, (new obj_marker)->id, noone)
			{
				...
				self->name = value_get_string(DsMap(markermap).Value(/*"name"*/ STR(830)), /*""*/ STR(0));
				self->color = value_get_real(DsMap(markermap).Value(/*"color"*/ STR(931)), IntType(0));
			}
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2214));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2215));
