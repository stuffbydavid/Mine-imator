...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					case IntType(0):
					{
						return /*"linear"*/ STR(543);
					}
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(546);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(547);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(548);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(544);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(917), /*"ground"*/ STR(135), /*"spawn"*/ STR(1370), /*"custom"*/ STR(592) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(515))
				typename_ = /*"itemsheet"*/ STR(535);
			if (typename_ == /*"block"*/ STR(516) || typename_ == /*"blocksheet"*/ STR(537))
				typename_ = /*"legacyblocksheet"*/ STR(536);
			if (typename_ == /*"particles"*/ STR(518))
				typename_ = /*"particlesheet"*/ STR(539);
			if (typename_ == /*"schematic"*/ STR(2210))
				typename_ = /*"scenery"*/ STR(514);
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2213), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2212));
				buffer_delete(global::buffer_current);
				return false;
			}
	...
	void project_load_legacy_template()
	{
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				{
					self->model_name = DsMap(modelmap).Value(/*"name"*/ STR(828));
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(1032))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(1032)));
					else
						self->model_state = ArrType();
					
				}
				else
					log({ /*"Could not convert model "*/ STR(2214), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2215), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2199))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2216), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2199))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2199))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			withOne (obj_marker, (new obj_marker)->id, noone)
			{
				...
				self->name = value_get_string(DsMap(markermap).Value(/*"name"*/ STR(828)), /*""*/ STR(0));
				self->color = value_get_real(DsMap(markermap).Value(/*"color"*/ STR(929)), IntType(0));
			}
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2208));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2209));
		...
		VarType reslist = DsMap(map).Value(/*"resources"*/ STR(214));
