...
namespace CppProject
{
	IntType list_init_context_menu(ScopeAny self, VarType name)
	{
		...
		switch (StringType(name).id)
		{
			...
			case 1476: // contextmenucategory
			{
				...
				if (name == /*"contextmenuvalue"*/ STR(1040))
				{
			...
			case 1491: // timelinelist
			{
				...
				if (sReal(setting_advanced_mode) > 0)
				{
					...
					ObjType(obj_list_item, global::list_item_last)->context_menu_name = /*"color"*/ STR(885);
					ObjType(obj_list_item, global::list_item_last)->disabled = (sVar(context_menu_value) == null_ && global::tl_edit == null_);
				}
	...
	void marker_editor_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, { /*"timelinemarkerlabel"*/ STR(1647), sVar(dx), sVar(dy), sVar(settings_menu_w) - IntType(24), IntType(24), idInt(sVar(timeline), tbx_marker_name), null_ }))
	...
	{
		log({ /*"External library"*/ STR(1260), path });
		global::lib_math_simplex1d = external_define({ path, /*"simplex1D"*/ STR(1648), dll_cdecl, ty_real, IntType(1), ty_real });
		global::lib_math_simplex2d = external_define({ path, /*"simplex2D"*/ STR(1649), dll_cdecl, ty_real, IntType(2), ty_real, ty_real });
		global::lib_math_simplex3d = external_define({ path, /*"simplex3D"*/ STR(1650), dll_cdecl, ty_real, IntType(3), ty_real, ty_real, ty_real });
		global::lib_math_simplex4d = external_define({ path, /*"simplex4D"*/ STR(1651), dll_cdecl, ty_real, IntType(4), ty_real, ty_real, ty_real, ty_real });
	}
	...
	RealType menu_add_timeline(VarType tl, RealType level, VarType menu)
	{
		if (tl == global::tl_edit && !ObjType(obj_menu, menu)->menu_include_tl_edit)
			return IntType(0);
		if (tl == null_)
			list_item_add({ text_get({ /*"timelinenone"*/ STR(1652) }), tl });
		else
		{
			list_item_add({ string_remove_newline(idVar(tl, display_name)), tl });
			if (ds_list_size(idInt(tl, tree_list)))
				list_item_add_action({ global::list_item_last, string(tl) + /*"extend"*/ STR(1653), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(188), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1654) : /*"tooltiptlexpand"*/ STR(1655), ID_spr_chevron_ani });
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			{
				list_item_add_action({ global::list_item_last, string(biome) + /*"extend"*/ STR(1653), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(188), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1654) : /*"tooltiptlexpand"*/ STR(1655), ID_spr_chevron_ani });
	...
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1041) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(718) && menu_active)
			...
			if (!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(718))
			{
				...
				if (close)
				{
					...
					sVar(window_busy) = (ds_list_size(sInt(menu_list)) > IntType(1) ? /*"menu"*/ STR(1041) : /*""*/ STR(0));
				}
			}
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(1.0);
			...
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(718) && menu_active)
				sVar(window_busy) = /*"menu"*/ STR(1041);
		}
