...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var(/*"type"*/ STR(816), DsList(global::tl_type_name_list).Value(self->type));
		json_save_var(/*"name"*/ STR(830), json_string_encode(self->name));
		json_save_var_save_id(/*"temp"*/ STR(2248), self->temp);
		json_save_var_nullable(/*"color_tag"*/ STR(2486), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(760), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2487), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2488), self->ghost);
		json_save_var(/*"depth"*/ STR(653), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2489), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			if (self->type == e_temp_type_SPECIAL_BLOCK)
			{
				...
				json_save_var(/*"name"*/ STR(830), self->model_name);
				json_save_var_state_vars(/*"state"*/ STR(1034), self->model_state);
				...
				if (self->type == e_temp_type_BLOCK)
				{
					...
					json_save_var(/*"name"*/ STR(830), self->block_name);
					json_save_var_state_vars(/*"state"*/ STR(1034), self->block_state);
					json_save_object_done();
				}
			
			json_save_var_save_id(/*"part_of"*/ STR(2490), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2491), self->part_root);
			if (self->pattern_type != /*""*/ STR(0) && self->pattern_base_color != null_)
			{
				json_save_var(/*"pattern_type"*/ STR(902), self->pattern_type);
		...
		project_save_values(/*"default_values"*/ STR(2493), self->value_default, global::_app->value_default);
		...
		if (self->parent != global::_app->id && idBool(self->parent, save))
			json_save_var_save_id(/*"parent"*/ STR(852), self->parent);
		else
			json_save_var_save_id(/*"parent"*/ STR(852), global::_app->id);
		
		json_save_var(/*"parent_tree_index"*/ STR(2494), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1865), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2495), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2496) });
			json_save_var_bool(/*"position"*/ STR(40), self->inherit_position);
			json_save_var_bool(/*"rotation"*/ STR(857), self->inherit_rotation);
			json_save_var_bool(/*"scale"*/ STR(900), self->inherit_scale);
			json_save_var_bool(/*"alpha"*/ STR(2331), self->inherit_alpha);
			json_save_var_bool(/*"color"*/ STR(931), self->inherit_color);
			json_save_var_bool(/*"texture"*/ STR(542), self->inherit_texture);
			json_save_var_bool(/*"surface"*/ STR(524), self->inherit_surface);
			json_save_var_bool(/*"subsurface"*/ STR(124), self->inherit_subsurface);
			json_save_var_bool(/*"visibility"*/ STR(2498), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1867), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2497), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2499), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2500), self->inherit_select);
			json_save_var_bool(/*"pose"*/ STR(2501), self->inherit_pose);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2502), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2503), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2497), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(1866), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2504), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2420), self->texture_filtering);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2505), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2506), self->only_render_glow);
			json_save_var(/*"glint_mode"*/ STR(2507), self->glint_mode);
			json_save_var(/*"glint_scale"*/ STR(2508), self->glint_scale);
			json_save_var_save_id(/*"glint_tex"*/ STR(2509), self->glint_tex);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2510), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2511), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2512), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2513), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2514), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(530) });
			json_save_var_bool(/*"smooth"*/ STR(2515), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2484), self->path_closed);
			json_save_var(/*"detail"*/ STR(2485), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2516), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2517), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2518), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2519), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2520), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2521), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2522), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2523), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2539) });
		...
		self->sprite_template = /*"generic"*/ STR(2540);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2541);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		VarType recentlist = DsMap(recentmap).Value(/*"list"*/ STR(1117));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->name = value_get_string(DsMap(projectmap).Value(/*"name"*/ STR(830)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->author = value_get_string(DsMap(projectmap).Value(/*"author"*/ STR(2355)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->description = value_get_string(DsMap(projectmap).Value(/*"description"*/ STR(1849)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->filename = value_get_string(DsMap(projectmap).Value(/*"filename"*/ STR(1273)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->last_opened = value_get_real(DsMap(projectmap).Value(/*"last_opened"*/ STR(2542)), -IntType(1));
				ObjType(obj_recent, recentobj)->pinned = value_get_real(DsMap(projectmap).Value(/*"pinned"*/ STR(2543)), false);
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2544);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2545) });
		json_save_start(recent_file);
		json_save_object_start();
		json_save_array_start({ /*"list"*/ STR(1117) });
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			VarType recent = DsList(global::_app->recent_list).Value(i);
			json_save_object_start();
			json_save_var(/*"name"*/ STR(830), json_string_encode(idVar(recent, name)));
			json_save_var(/*"author"*/ STR(2355), json_string_encode(idVar(recent, author)));
			json_save_var(/*"description"*/ STR(1849), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2546), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1273), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2542), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2543), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2547) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2548) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2549) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2550) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2551) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2552) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2553), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2554), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2555), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2556), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
