...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var_nullable(/*"color_tag"*/ STR(2390), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(719), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2391), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2392), self->ghost);
		json_save_var(/*"depth"*/ STR(611), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2393), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			if (self->type == e_temp_type_SPECIAL_BLOCK)
			{
				...
				json_save_var_state_vars(/*"state"*/ STR(985), self->model_state);
				...
				if (self->type == e_temp_type_BLOCK)
				{
					...
					json_save_var_state_vars(/*"state"*/ STR(985), self->block_state);
					json_save_object_done();
				}
			
			json_save_var_save_id(/*"part_of"*/ STR(2394), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2395), self->part_root);
		...
		{
			json_save_array_start({ /*"parts"*/ STR(1772) });
			for (IntType p = IntType(0); p < ds_list_size(self->part_list); p++)
				json_save_array_value(save_id_get(DsList(self->part_list).Value(p)));
			json_save_array_done();
		}
		project_save_values(/*"default_values"*/ STR(2397), self->value_default, global::_app->value_default);
		...
		json_save_var(/*"parent_tree_index"*/ STR(2398), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1792), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2399), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2400) });
			json_save_var_bool(/*"position"*/ STR(40), self->inherit_position);
			json_save_var_bool(/*"rotation"*/ STR(814), self->inherit_rotation);
			json_save_var_bool(/*"scale"*/ STR(1779), self->inherit_scale);
			json_save_var_bool(/*"alpha"*/ STR(2249), self->inherit_alpha);
			...
			json_save_var_bool(/*"visibility"*/ STR(2402), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1794), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2401), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2403), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2404), self->inherit_select);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2405), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2406), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2401), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(1793), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2407), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2336), self->texture_filtering);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2408), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2409), self->only_render_glow);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2410), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2411), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2412), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2413), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2414), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(520) });
			json_save_var_bool(/*"smooth"*/ STR(2415), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2388), self->path_closed);
			json_save_var(/*"detail"*/ STR(2389), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2416), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2417), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2418), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2419), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2420), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2421), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2422), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2423), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2439) });
		...
		self->sprite_template = /*"generic"*/ STR(2440);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2441);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		VarType recentlist = DsMap(recentmap).Value(/*"list"*/ STR(1068));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->author = value_get_string(DsMap(projectmap).Value(/*"author"*/ STR(2273)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->description = value_get_string(DsMap(projectmap).Value(/*"description"*/ STR(1775)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->filename = value_get_string(DsMap(projectmap).Value(/*"filename"*/ STR(1224)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->last_opened = value_get_real(DsMap(projectmap).Value(/*"last_opened"*/ STR(2442)), -IntType(1));
				ObjType(obj_recent, recentobj)->pinned = value_get_real(DsMap(projectmap).Value(/*"pinned"*/ STR(2443)), false);
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2444);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2445) });
		json_save_start(recent_file);
		json_save_object_start();
		json_save_array_start({ /*"list"*/ STR(1068) });
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"author"*/ STR(2273), json_string_encode(idVar(recent, author)));
			json_save_var(/*"description"*/ STR(1775), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2446), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1224), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2442), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2443), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2447) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2448) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2449) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2450) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2451) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2452) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2453), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2454), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2455), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2456), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
