...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var_nullable(/*"color_tag"*/ STR(2386), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(719), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2387), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2388), self->ghost);
		json_save_var(/*"depth"*/ STR(611), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2389), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			if (self->type == e_temp_type_SPECIAL_BLOCK)
			{
				...
				json_save_var_state_vars(/*"state"*/ STR(984), self->model_state);
				...
				if (self->type == e_temp_type_BLOCK)
				{
					...
					json_save_var_state_vars(/*"state"*/ STR(984), self->block_state);
					json_save_object_done();
				}
			
			json_save_var_save_id(/*"part_of"*/ STR(2390), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2391), self->part_root);
		...
		{
			json_save_array_start({ /*"parts"*/ STR(1771) });
			for (IntType p = IntType(0); p < ds_list_size(self->part_list); p++)
				json_save_array_value(save_id_get(DsList(self->part_list).Value(p)));
			json_save_array_done();
		}
		project_save_values(/*"default_values"*/ STR(2393), self->value_default, global::_app->value_default);
		...
		json_save_var(/*"parent_tree_index"*/ STR(2394), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1791), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2395), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2396) });
			json_save_var_bool(/*"position"*/ STR(40), self->inherit_position);
			json_save_var_bool(/*"rotation"*/ STR(814), self->inherit_rotation);
			json_save_var_bool(/*"scale"*/ STR(1778), self->inherit_scale);
			json_save_var_bool(/*"alpha"*/ STR(2245), self->inherit_alpha);
			...
			json_save_var_bool(/*"visibility"*/ STR(2398), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1792), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2397), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2399), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2400), self->inherit_select);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2401), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2402), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2397), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(2403), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2404), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2332), self->texture_filtering);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2405), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2406), self->only_render_glow);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2407), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2408), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2409), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2410), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2411), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(520) });
			json_save_var_bool(/*"smooth"*/ STR(2412), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2384), self->path_closed);
			json_save_var(/*"detail"*/ STR(2385), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2413), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2414), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2415), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2416), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2417), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2418), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2419), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2420), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	RealType recent_load(ScopeAny self)
	{
		...
		VarType recentlist = DsMap(recentmap).Value(/*"list"*/ STR(1067));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->author = value_get_string({ DsMap(projectmap).Value(/*"author"*/ STR(2269)), /*""*/ STR(0) });
				ObjType(obj_recent, recentobj)->description = value_get_string({ DsMap(projectmap).Value(/*"description"*/ STR(1774)), /*""*/ STR(0) });
				ObjType(obj_recent, recentobj)->filename = value_get_string({ DsMap(projectmap).Value(/*"filename"*/ STR(1223)), /*""*/ STR(0) });
	...
	void recent_save()
	{
		...
		json_save_object_start();
		json_save_array_start({ /*"list"*/ STR(1067) });
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"author"*/ STR(2269), json_string_encode(idVar(recent, author)));
			json_save_var(/*"description"*/ STR(1774), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2444), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1223), json_string_encode(idVar(recent, filename)));
