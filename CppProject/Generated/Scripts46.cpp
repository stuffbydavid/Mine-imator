...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var(/*"type"*/ STR(814), DsList(global::tl_type_name_list).Value(self->type));
		json_save_var(/*"name"*/ STR(828), json_string_encode(self->name));
		json_save_var_save_id(/*"temp"*/ STR(2242), self->temp);
		json_save_var_nullable(/*"color_tag"*/ STR(2478), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(758), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2479), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2480), self->ghost);
		json_save_var(/*"depth"*/ STR(651), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2481), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			if (self->type == e_temp_type_SPECIAL_BLOCK)
			{
				...
				json_save_var(/*"name"*/ STR(828), self->model_name);
				json_save_var_state_vars(/*"state"*/ STR(1032), self->model_state);
				...
				if (self->type == e_temp_type_BLOCK)
				{
					...
					json_save_var(/*"name"*/ STR(828), self->block_name);
					json_save_var_state_vars(/*"state"*/ STR(1032), self->block_state);
					json_save_object_done();
				}
			
			json_save_var_save_id(/*"part_of"*/ STR(2482), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2483), self->part_root);
			if (self->pattern_type != /*""*/ STR(0) && self->pattern_base_color != null_)
			{
				json_save_var(/*"pattern_type"*/ STR(900), self->pattern_type);
		...
		{
			json_save_array_start({ /*"parts"*/ STR(1841) });
			for (IntType p = IntType(0); p < ds_list_size(self->part_list); p++)
				json_save_array_value(save_id_get(DsList(self->part_list).Value(p)));
			json_save_array_done();
		}
		project_save_values(/*"default_values"*/ STR(2485), self->value_default, global::_app->value_default);
		...
		if (self->parent != global::_app->id && idBool(self->parent, save))
			json_save_var_save_id(/*"parent"*/ STR(850), self->parent);
		else
			json_save_var_save_id(/*"parent"*/ STR(850), global::_app->id);
		
		json_save_var(/*"parent_tree_index"*/ STR(2486), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1860), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2487), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2488) });
			json_save_var_bool(/*"position"*/ STR(40), self->inherit_position);
			json_save_var_bool(/*"rotation"*/ STR(855), self->inherit_rotation);
			json_save_var_bool(/*"scale"*/ STR(898), self->inherit_scale);
			json_save_var_bool(/*"alpha"*/ STR(2325), self->inherit_alpha);
			json_save_var_bool(/*"color"*/ STR(929), self->inherit_color);
			json_save_var_bool(/*"texture"*/ STR(540), self->inherit_texture);
			json_save_var_bool(/*"surface"*/ STR(522), self->inherit_surface);
			json_save_var_bool(/*"subsurface"*/ STR(124), self->inherit_subsurface);
			json_save_var_bool(/*"visibility"*/ STR(2490), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1862), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2489), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2491), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2492), self->inherit_select);
			json_save_var_bool(/*"pose"*/ STR(2493), self->inherit_pose);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2494), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2495), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2489), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(1861), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2496), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2412), self->texture_filtering);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2497), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2498), self->only_render_glow);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2499), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2500), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2501), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2502), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2503), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(528) });
			json_save_var_bool(/*"smooth"*/ STR(2504), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2476), self->path_closed);
			json_save_var(/*"detail"*/ STR(2477), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2505), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2506), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2507), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2508), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2509), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2510), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2511), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2512), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2528) });
		...
		self->sprite_template = /*"generic"*/ STR(2529);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2530);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		VarType recentlist = DsMap(recentmap).Value(/*"list"*/ STR(1115));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->name = value_get_string(DsMap(projectmap).Value(/*"name"*/ STR(828)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->author = value_get_string(DsMap(projectmap).Value(/*"author"*/ STR(2349)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->description = value_get_string(DsMap(projectmap).Value(/*"description"*/ STR(1844)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->filename = value_get_string(DsMap(projectmap).Value(/*"filename"*/ STR(1271)), /*""*/ STR(0));
				ObjType(obj_recent, recentobj)->last_opened = value_get_real(DsMap(projectmap).Value(/*"last_opened"*/ STR(2531)), -IntType(1));
				ObjType(obj_recent, recentobj)->pinned = value_get_real(DsMap(projectmap).Value(/*"pinned"*/ STR(2532)), false);
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2533);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2534) });
		json_save_start(recent_file);
		json_save_object_start();
		json_save_array_start({ /*"list"*/ STR(1115) });
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			VarType recent = DsList(global::_app->recent_list).Value(i);
			json_save_object_start();
			json_save_var(/*"name"*/ STR(828), json_string_encode(idVar(recent, name)));
			json_save_var(/*"author"*/ STR(2349), json_string_encode(idVar(recent, author)));
			json_save_var(/*"description"*/ STR(1844), json_string_encode(idVar(recent, description)));
			json_save_var(/*"thumbnail"*/ STR(2535), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1271), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2531), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2532), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2536) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2537) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2538) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2539) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2540) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2541) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2542), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2543), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2544), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2545), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
