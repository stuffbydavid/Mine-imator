...
namespace CppProject
{
	...
	RealType render_world_ground(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2555), c_white, IntType(1));
		VarType blend = block_texture_get_blend(sVar(background_ground_name), sVar(background_ground_tex));
		BoolType iswater = (sVar(background_ground_name) == /*"block/water_flow"*/ STR(1769) || sVar(background_ground_name) == /*"block/water_still"*/ STR(1767));
		render_set_uniform_int(/*"uIsGround"*/ STR(2556), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1944), blend, IntType(1));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2557), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2558), IntType(0));
		render_set_uniform_int(/*"uFogShow"*/ STR(1953), global::_app->background_fog_show);
		render_set_uniform_int(/*"uIsWater"*/ STR(2554), iswater);
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), idVar(sVar(background_ground_tex_material), material_format));
		if (sVar(background_ground_tex_material) == global::mc_res)
		{
			render_set_uniform(/*"uMetallic"*/ STR(2551), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2552), (iswater && global::_app->project_render_water_reflections > 0 ? .07 : 1.0));
			render_set_uniform(/*"uEmissive"*/ STR(2553), IntType(0));
		}
		else
		{
			render_set_uniform(/*"uMetallic"*/ STR(2551), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2552), IntType(0));
			render_set_uniform(/*"uEmissive"*/ STR(2553), IntType(0));
		}
		...
		if (sBool(background_ground_material_ani))
			render_set_texture(sArr(background_ground_ani_texture_material).Value(block_texture_get_frame()), /*"Material"*/ STR(2549));
		else
			render_set_texture(sInt(background_ground_texture_material), /*"Material"*/ STR(2549));
		
		if (sBool(background_ground_normal_ani))
			render_set_texture(sArr(background_ground_ani_texture_normal).Value(block_texture_get_frame()), /*"Normal"*/ STR(2550));
		else
			render_set_texture(sInt(background_ground_texture_normal), /*"Normal"*/ STR(2550));
	...
	void render_world_item(VarType vbuffer, VarType is3d, VarType facecamera, VarType bounce, VarType rotate, ArrType res)
	{
		...
		if (bounce > 0)
		{
			...
			if (t < d)
				offz = ease(/*"easeinoutquad"*/ STR(548), t / d) * IntType(2) - IntType(1);
			else
				offz = IntType(1) - ease(/*"easeinoutquad"*/ STR(548), (t - d) / d) * IntType(2);
		...
		if (res.Value(IntType(1)) != null_ && res.Value(IntType(1)) != global::mc_res)
		{
			if (global::shader_uniform_metallic != IntType(0))
			{
				...
				render_set_uniform(/*"uMetallic"*/ STR(2551), global::shader_uniform_metallic);
			}
			if (global::shader_uniform_roughness != IntType(0))
			{
				global::shader_uniform_roughness = IntType(0);
				render_set_uniform(/*"uRoughness"*/ STR(2552), global::shader_uniform_roughness);
			}
			if (global::shader_uniform_emissive != IntType(0))
			{
				global::shader_uniform_emissive = IntType(0);
				render_set_uniform(/*"uEmissive"*/ STR(2553), global::shader_uniform_emissive);
			}
			if (idInt(res.Value(IntType(1)), item_sheet_texture_material) != null_)
				render_set_texture(idInt(res.Value(IntType(1)), item_sheet_texture_material), /*"Material"*/ STR(2549));
			else
				render_set_texture(idVar(res.Value(IntType(1)), texture), /*"Material"*/ STR(2549));
			
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), idVar(res.Value(IntType(1)), material_format));
		}
		else
		{
			render_set_texture(ID_spr_default_material, /*"Material"*/ STR(2549));
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), e_material_FORMAT_NONE);
		}
		
		if (res.Value(IntType(2)) != null_ && res.Value(IntType(2)) != global::mc_res)
		{
			if (idInt(res.Value(IntType(2)), item_sheet_tex_normal) != null_)
				render_set_texture(idInt(res.Value(IntType(2)), item_sheet_tex_normal), /*"Normal"*/ STR(2550));
			else
				render_set_texture(idVar(res.Value(IntType(2)), texture), /*"Normal"*/ STR(2550));
			
		}
		else
			render_set_texture(ID_spr_default_normal, /*"Normal"*/ STR(2550));
	...
	void render_world_model_file_parts(ScopeAny self, VarType modelfile, VarType res, IntType texnamemap, IntType hidelist, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, IntType matrixmap)
	{
		...
		for (IntType p = IntType(0); p < ds_list_size(partlist); p++)
		{
			...
			if (idBool(part, part_mixing_shapes))
				render_set_uniform_int(/*"uColorsExt"*/ STR(2108), idBool(part, part_mixing_shapes));
	...
	RealType render_world_model_part(ScopeAny self, VarType part, VarType res, IntType texnamemap, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, VarType tlobject)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			if (tlobject > 0)
			{
				...
				render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), sArr(model_part_shape_material_res).Value(s));
				...
				{
					render_set_texture(sArr(model_part_shape_tex_material).Value(s), /*"Material"*/ STR(2549));
				...
				if (sArr(model_part_shape_tex_normal).Value(s) == null_)
					...
					render_set_texture(sArr(model_part_shape_tex_normal).Value(s), /*"Normal"*/ STR(2550));
			...
			if (self->subAssetId == ID_obj_preview && idVar(sVar(select), model_name) == /*"armor"*/ STR(789))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2024))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"chestplate"*/ STR(2025))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"leggings"*/ STR(2026))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"boots"*/ STR(2027))
				{
			...
			if (tlobject != null_ && idVar(idVar(tlobject, temp), model_name) == /*"armor"*/ STR(789))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2024))
				{
			...
			{
				if (tlobject != null_)
					render_set_uniform_color(/*"uMixColor"*/ STR(2114), merge_color((IntType)(ObjType(obj_model_shape, shape)->color_mix), (IntType)(sArr(value_inherit).Value(e_value_MIX_COLOR)), sArr(value_inherit).Value(e_value_MIX_PERCENT)), lerp(ObjType(obj_model_shape, shape)->color_mix_percent, sArr(value_inherit).Value(e_value_MIX_PERCENT), sArr(value_inherit).Value(e_value_MIX_PERCENT)));
				else
					render_set_uniform_color(/*"uMixColor"*/ STR(2114), ObjType(obj_model_shape, shape)->color_mix, ObjType(obj_model_shape, shape)->color_mix_percent);
	...
	void render_world_shape(VarType type, VarType vbuf, VarType facecamera, ArrType tex)
	{
		...
		render_set_texture(tex.Value(IntType(1)), /*"Material"*/ STR(2549));
		render_set_texture(tex.Value(IntType(2)), /*"Normal"*/ STR(2550));
		vbuffer_render({ vbuf });
	}
	...
	RealType render_world_sky()
	{
		...
		{
			VarType vbuf;
			if (global::_app->background_image_type == /*"sphere"*/ STR(521))
			{
	...
	RealType render_world_sky_clouds(ScopeAny self)
	{
		if (!(sVar(background_sky_clouds_show) > 0) || !(global::render_background > 0) || global::render_mode == e_render_mode_DEPTH_NO_SKY)
			return IntType(0);
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2555), c_black, IntType(1));
		VarType res = sVar(background_sky_clouds_tex);
		if (!res_is_ready(res))
			res = global::mc_res;
		render_set_uniform_int(/*"uIsSky"*/ STR(2559), IntType(1));
		...
		render_set_uniform(/*"uRoughness"*/ STR(2552), IntType(1));
		render_set_uniform(/*"uEmissive"*/ STR(2553), IntType(0));
		render_set_uniform(/*"uLightSpecular"*/ STR(2560), IntType(0));
	...
	RealType render_world_tl(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_CLICK)
		{
			if (sReal(selected) > 0 || sVar(lock) > 0 || !tl_update_list_filter(self->id))
				return IntType(0);
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2555), self->id, IntType(1));
		}
		...
		if (global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2555), (sVar(ssao) > 0) ? c_white : c_black, IntType(1));
		if (sBool(colors_ext) != global::shader_uniform_color_ext || sArr(value_inherit).Value(e_value_RGB_ADD) != global::shader_uniform_rgb_add || sArr(value_inherit).Value(e_value_HSB_ADD) != global::shader_uniform_hsb_add || sArr(value_inherit).Value(e_value_RGB_SUB) != global::shader_uniform_rgb_sub || sArr(value_inherit).Value(e_value_HSB_SUB) != global::shader_uniform_hsb_sub || sArr(value_inherit).Value(e_value_HSB_MUL) != global::shader_uniform_hsb_mul || sArr(value_inherit).Value(e_value_MIX_COLOR) != global::shader_uniform_mix_color || sArr(value_inherit).Value(e_value_MIX_PERCENT) != global::shader_uniform_mix_percent)
		{
			...
			render_set_uniform_int(/*"uColorsExt"*/ STR(2108), global::shader_uniform_color_ext);
			render_set_uniform_color(/*"uRGBAdd"*/ STR(2109), global::shader_uniform_rgb_add, IntType(1));
			render_set_uniform_color(/*"uHSBAdd"*/ STR(2111), global::shader_uniform_hsb_add, IntType(1));
			render_set_uniform_color(/*"uRGBSub"*/ STR(2110), global::shader_uniform_rgb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBSub"*/ STR(2112), global::shader_uniform_hsb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBMul"*/ STR(2113), global::shader_uniform_hsb_mul, IntType(1));
			render_set_uniform_color(/*"uMixColor"*/ STR(2114), global::shader_uniform_mix_color, global::shader_uniform_mix_percent);
		}
		...
		if (sArr(value_inherit).Value(e_value_SUBSURFACE) != global::shader_uniform_sss || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_RED) != global::shader_uniform_sss_red || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_GREEN) != global::shader_uniform_sss_green || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_BLUE) != global::shader_uniform_sss_blue || sArr(value_inherit).Value(e_value_SUBSURFACE_COLOR) != global::shader_uniform_sss_color)
		{
			...
			render_set_uniform(/*"uSSS"*/ STR(2561), global::shader_uniform_sss);
			render_set_uniform_vec3(/*"uSSSRadius"*/ STR(2562), global::shader_uniform_sss_red, global::shader_uniform_sss_green, global::shader_uniform_sss_blue);
			render_set_uniform_color(/*"uSSSColor"*/ STR(2563), global::shader_uniform_sss_color, 1.0);
		}
		...
		if (sVar(blend_mode) != /*"normal"*/ STR(641) && (global::render_mode == e_render_mode_COLOR || global::render_mode == e_render_mode_COLOR_FOG || global::render_mode == e_render_mode_COLOR_FOG_LIGHTS || global::render_mode == e_render_mode_ALPHA_FIX))
		{
		...
		if (sVar(glow) != global::shader_uniform_glow || sVar(glow_texture) != global::shader_uniform_glow_texture || sArr(value_inherit).Value(e_value_GLOW_COLOR) != global::shader_uniform_glow_color)
		{
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2564), IntType(1));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2558), sVar(glow_texture));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2557), global::shader_uniform_glow_color, IntType(1));
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2564), IntType(0));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2558), IntType(0));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2557), c_black, IntType(0));
			}
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_tl_type_PATH:
				{
					if (sVar(path_vbuffer) != null_)
					{
						...
						else
						{
							...
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), idVar(sArr(value_inherit).Value(e_value_TEXTURE_MATERIAL_OBJ), material_format));
						}
						...
						render_set_texture(tex);
						render_set_texture(texmat, /*"Material"*/ STR(2549));
						render_set_texture(texnorm, /*"Normal"*/ STR(2550));
				...
				default:
				{
					...
					withOne (Object, sVar(temp), self->id)
					{
						...
						if (matres != null_)
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), idVar(matres, material_format));
						else
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2548), e_material_FORMAT_NONE);
