...
namespace CppProject
{
	...
	RealType render_world_ground(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2566), c_white, IntType(1));
		...
		render_set_uniform_int(/*"uIsGround"*/ STR(2567), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1950), blend, IntType(1));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2568), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2569), IntType(0));
		render_set_uniform_int(/*"uFogShow"*/ STR(1959), global::_app->background_fog_show);
		render_set_uniform_int(/*"uIsWater"*/ STR(2565), iswater);
		render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), idVar(sVar(background_ground_tex_material), material_format));
		if (sVar(background_ground_tex_material) == global::mc_res)
		{
			render_set_uniform(/*"uMetallic"*/ STR(2562), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2563), (iswater && global::_app->project_render_water_reflections > 0 ? .07 : 1.0));
			render_set_uniform(/*"uEmissive"*/ STR(2564), IntType(0));
		}
		else
		{
			render_set_uniform(/*"uMetallic"*/ STR(2562), IntType(0));
			render_set_uniform(/*"uRoughness"*/ STR(2563), IntType(0));
			render_set_uniform(/*"uEmissive"*/ STR(2564), IntType(0));
		}
		...
		if (sBool(background_ground_material_ani))
			render_set_texture(sArr(background_ground_ani_texture_material).Value(block_texture_get_frame()), /*"Material"*/ STR(2560));
		else
			render_set_texture(sInt(background_ground_texture_material), /*"Material"*/ STR(2560));
		
		if (sBool(background_ground_normal_ani))
			render_set_texture(sArr(background_ground_ani_texture_normal).Value(block_texture_get_frame()), /*"Normal"*/ STR(2561));
		else
			render_set_texture(sInt(background_ground_texture_normal), /*"Normal"*/ STR(2561));
	...
	void render_world_item(VarType vbuffer, VarType is3d, VarType facecamera, VarType bounce, VarType rotate, ArrType res)
	{
		...
		if (bounce > 0)
		{
			...
			if (t < d)
				offz = ease(/*"easeinoutquad"*/ STR(550), t / d) * IntType(2) - IntType(1);
			else
				offz = IntType(1) - ease(/*"easeinoutquad"*/ STR(550), (t - d) / d) * IntType(2);
		...
		if (res.Value(IntType(1)) != null_ && res.Value(IntType(1)) != global::mc_res)
		{
			if (global::shader_uniform_metallic != IntType(0))
			{
				...
				render_set_uniform(/*"uMetallic"*/ STR(2562), global::shader_uniform_metallic);
			}
			if (global::shader_uniform_roughness != IntType(0))
			{
				global::shader_uniform_roughness = IntType(0);
				render_set_uniform(/*"uRoughness"*/ STR(2563), global::shader_uniform_roughness);
			}
			if (global::shader_uniform_emissive != IntType(0))
			{
				global::shader_uniform_emissive = IntType(0);
				render_set_uniform(/*"uEmissive"*/ STR(2564), global::shader_uniform_emissive);
			}
			if (idInt(res.Value(IntType(1)), item_sheet_texture_material) != null_)
				render_set_texture(idInt(res.Value(IntType(1)), item_sheet_texture_material), /*"Material"*/ STR(2560));
			else
				render_set_texture(idVar(res.Value(IntType(1)), texture), /*"Material"*/ STR(2560));
			
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), idVar(res.Value(IntType(1)), material_format));
		}
		else
		{
			render_set_texture(ID_spr_default_material, /*"Material"*/ STR(2560));
			render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), e_material_FORMAT_NONE);
		}
		
		if (res.Value(IntType(2)) != null_ && res.Value(IntType(2)) != global::mc_res)
		{
			if (idInt(res.Value(IntType(2)), item_sheet_tex_normal) != null_)
				render_set_texture(idInt(res.Value(IntType(2)), item_sheet_tex_normal), /*"Normal"*/ STR(2561));
			else
				render_set_texture(idVar(res.Value(IntType(2)), texture), /*"Normal"*/ STR(2561));
			
		}
		else
			render_set_texture(ID_spr_default_normal, /*"Normal"*/ STR(2561));
	...
	void render_world_model_file_parts(ScopeAny self, VarType modelfile, VarType res, IntType texnamemap, IntType hidelist, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, IntType matrixmap)
	{
		...
		for (IntType p = IntType(0); p < ds_list_size(partlist); p++)
		{
			...
			if (idBool(part, part_mixing_shapes))
				render_set_uniform_int(/*"uColorsExt"*/ STR(2114), idBool(part, part_mixing_shapes));
	...
	RealType render_world_model_part(ScopeAny self, VarType part, VarType res, IntType texnamemap, IntType shapevbuffermap, IntType colornamemap, IntType shapehidelist, IntType shapetexnamemap, VarType tlobject)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			if (tlobject > 0)
			{
				...
				render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), sArr(model_part_shape_material_res).Value(s));
				...
				{
					render_set_texture(sArr(model_part_shape_tex_material).Value(s), /*"Material"*/ STR(2560));
				...
				if (sArr(model_part_shape_tex_normal).Value(s) == null_)
					...
					render_set_texture(sArr(model_part_shape_tex_normal).Value(s), /*"Normal"*/ STR(2561));
			...
			if (self->subAssetId == ID_obj_preview && idVar(sVar(select), model_name) == /*"armor"*/ STR(791))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2030))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"chestplate"*/ STR(2031))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"leggings"*/ STR(2032))
				{
				...
				if (ObjType(obj_model_shape, shape)->description == /*"boots"*/ STR(2033))
				{
			...
			if (tlobject != null_ && idVar(idVar(tlobject, temp), model_name) == /*"armor"*/ STR(791))
			{
				if (ObjType(obj_model_shape, shape)->description == /*"helmet"*/ STR(2030))
				{
			...
			{
				render_set_uniform_color(/*"uBlendColor"*/ STR(1950), blendcolor, alpha);
			...
			{
				if (tlobject != null_)
					render_set_uniform_color(/*"uMixColor"*/ STR(2120), merge_color((IntType)(ObjType(obj_model_shape, shape)->color_mix), (IntType)(sArr(value_inherit).Value(e_value_MIX_COLOR)), sArr(value_inherit).Value(e_value_MIX_PERCENT)), lerp(ObjType(obj_model_shape, shape)->color_mix_percent, sArr(value_inherit).Value(e_value_MIX_PERCENT), sArr(value_inherit).Value(e_value_MIX_PERCENT)));
				else
					render_set_uniform_color(/*"uMixColor"*/ STR(2120), ObjType(obj_model_shape, shape)->color_mix, ObjType(obj_model_shape, shape)->color_mix_percent);
	...
	RealType render_world_particle(ScopeAny self)
	{
		...
		render_set_uniform_color(/*"uBlendColor"*/ STR(1950), global::shader_blend_color, global::shader_blend_alpha);
	...
	void render_world_shape(VarType type, VarType vbuf, VarType facecamera, ArrType tex)
	{
		...
		render_set_texture(tex.Value(IntType(1)), /*"Material"*/ STR(2560));
		render_set_texture(tex.Value(IntType(2)), /*"Normal"*/ STR(2561));
		vbuffer_render({ vbuf });
	}
	...
	RealType render_world_sky()
	{
		...
		{
			VarType vbuf;
			if (global::_app->background_image_type == /*"sphere"*/ STR(523))
			{
				...
				if (global::_app->background_image_type == /*"box"*/ STR(1366))
				{
		...
		if (global::_app->background_fog_show > 0 && global::_app->background_fog_sky > 0)
		{
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1950), global::_app->background_fog_color_final, IntType(1));
		...
		if (!(global::_app->background_image_show > 0))
		{
			...
			if (global::_app->background_night_alpha > IntType(0))
			{
				...
				render_set_uniform_color(/*"uBlendColor"*/ STR(1950), c_white, 0.4 * global::_app->background_night_alpha);
			...
			render_set_uniform_color(/*"uBlendColor"*/ STR(1950), c_white, vis);
			...
			vbuffer_render_matrix(global::_app->background_sky_sun_moon_vbuffer, matrix_multiply(matrix_build(0.0, 0.0, min({ dis * 0.7, (dis * 0.7) / global::_app->background_sky_sun_scale }), 90.0, 0.0, 0.0 + global::_app->background_sky_sun_angle, sca * global::_app->background_sky_sun_scale, sca * global::_app->background_sky_sun_scale, sca), skymat));
			vis = percent(vec3_dot(global::_app->background_sun_direction, vec3(IntType(0), IntType(0), -IntType(1))), -0.15, IntType(0));
			render_set_uniform_color(/*"uBlendColor"*/ STR(1950), c_white, vis);
			...
			vbuffer_render_matrix(global::_app->background_sky_sun_moon_vbuffer, matrix_multiply(matrix_build(0.0, 0.0, max({ -dis * 0.7, (-dis * 0.7) / global::_app->background_sky_moon_scale }), -90.0, 0.0, IntType(0) - global::_app->background_sky_moon_angle, sca * global::_app->background_sky_moon_scale, sca * global::_app->background_sky_moon_scale, sca), skymat));
	...
	RealType render_world_sky_clouds(ScopeAny self)
	{
		if (!(sVar(background_sky_clouds_show) > 0) || !(global::render_background > 0) || global::render_mode == e_render_mode_DEPTH_NO_SKY)
			return IntType(0);
		if (global::render_mode == e_render_mode_SCENE_TEST || global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2566), c_black, IntType(1));
		VarType res = sVar(background_sky_clouds_tex);
		if (!res_is_ready(res))
			res = global::mc_res;
		render_set_uniform_int(/*"uIsSky"*/ STR(2570), IntType(1));
		render_set_uniform_color(/*"uBlendColor"*/ STR(1950), sInt(background_sky_clouds_final), sReal(background_clouds_alpha));
		render_set_uniform_color(/*"uGlowColor"*/ STR(2568), c_black, IntType(1));
		render_set_uniform_int(/*"uGlowTexture"*/ STR(2569), IntType(0));
		render_set_uniform(/*"uMetallic"*/ STR(2562), IntType(0));
		render_set_uniform(/*"uRoughness"*/ STR(2563), IntType(1));
		render_set_uniform(/*"uEmissive"*/ STR(2564), IntType(0));
		render_set_uniform(/*"uLightSpecular"*/ STR(2571), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1959), IntType(0));
		...
		if (!(sVar(background_fog_show) > 0) || !(sVar(background_fog_sky) > 0))
			render_set_uniform(/*"uFogShow"*/ STR(1959), global::_app->background_fog_show);
		return 0.0;
	}
	...
	void render_world_text(VarType vbuffer, VarType tex, VarType facecamera, VarType res, VarType outline)
	{
		...
		if (outline != null_)
		{
			render_set_uniform_color(/*"uBlendColor"*/ STR(1950), outline, global::shader_blend_alpha);
	...
	RealType render_world_tl(ScopeAny self)
	{
		...
		if (global::render_mode == e_render_mode_CLICK)
		{
			if (sReal(selected) > 0 || sVar(lock) > 0 || !tl_update_list_filter(self->id))
				return IntType(0);
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2566), self->id, IntType(1));
		}
		...
		if (global::render_mode == e_render_mode_AO_MASK)
			render_set_uniform_color(/*"uReplaceColor"*/ STR(2566), (sVar(ssao) > 0) ? c_white : c_black, IntType(1));
		if (sBool(colors_ext) != global::shader_uniform_color_ext || sArr(value_inherit).Value(e_value_RGB_ADD) != global::shader_uniform_rgb_add || sArr(value_inherit).Value(e_value_HSB_ADD) != global::shader_uniform_hsb_add || sArr(value_inherit).Value(e_value_RGB_SUB) != global::shader_uniform_rgb_sub || sArr(value_inherit).Value(e_value_HSB_SUB) != global::shader_uniform_hsb_sub || sArr(value_inherit).Value(e_value_HSB_MUL) != global::shader_uniform_hsb_mul || sArr(value_inherit).Value(e_value_MIX_COLOR) != global::shader_uniform_mix_color || sArr(value_inherit).Value(e_value_MIX_PERCENT) != global::shader_uniform_mix_percent)
		{
			...
			render_set_uniform_int(/*"uColorsExt"*/ STR(2114), global::shader_uniform_color_ext);
			render_set_uniform_color(/*"uRGBAdd"*/ STR(2115), global::shader_uniform_rgb_add, IntType(1));
			render_set_uniform_color(/*"uHSBAdd"*/ STR(2117), global::shader_uniform_hsb_add, IntType(1));
			render_set_uniform_color(/*"uRGBSub"*/ STR(2116), global::shader_uniform_rgb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBSub"*/ STR(2118), global::shader_uniform_hsb_sub, IntType(1));
			render_set_uniform_color(/*"uHSBMul"*/ STR(2119), global::shader_uniform_hsb_mul, IntType(1));
			render_set_uniform_color(/*"uMixColor"*/ STR(2120), global::shader_uniform_mix_color, global::shader_uniform_mix_percent);
		}
		...
		if (sVar(wind) != global::shader_uniform_wind)
		{
			...
			render_set_uniform(/*"uWindEnable"*/ STR(1952), global::shader_uniform_wind);
		}
		if (sVar(wind_terrain) != global::shader_uniform_wind_terrain)
		{
			global::shader_uniform_wind_terrain = sVar(wind_terrain);
			render_set_uniform(/*"uWindTerrain"*/ STR(1953), global::shader_uniform_wind_terrain);
		}
		if ((global::_app->background_fog_show > 0 && sVar(fog) > 0) != global::shader_uniform_fog)
		{
			...
			render_set_uniform_int(/*"uFogShow"*/ STR(1959), global::shader_uniform_fog);
		}
		if (sArr(value_inherit).Value(e_value_SUBSURFACE) != global::shader_uniform_sss || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_RED) != global::shader_uniform_sss_red || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_GREEN) != global::shader_uniform_sss_green || sArr(value_inherit).Value(e_value_SUBSURFACE_RADIUS_BLUE) != global::shader_uniform_sss_blue || sArr(value_inherit).Value(e_value_SUBSURFACE_COLOR) != global::shader_uniform_sss_color)
		{
			...
			render_set_uniform(/*"uSSS"*/ STR(2572), global::shader_uniform_sss);
			render_set_uniform_vec3(/*"uSSSRadius"*/ STR(2573), global::shader_uniform_sss_red, global::shader_uniform_sss_green, global::shader_uniform_sss_blue);
			render_set_uniform_color(/*"uSSSColor"*/ STR(2574), global::shader_uniform_sss_color, 1.0);
		}
		if (sArr(value_inherit).Value(e_value_WIND_INFLUENCE) != global::shader_uniform_wind_strength)
		{
			...
			render_set_uniform(/*"uWindStrength"*/ STR(1955), global::shader_uniform_wind_strength);
			render_set_uniform(/*"uWindDirectionalStrength"*/ STR(1958), global::shader_uniform_wind_strength * global::_app->background_wind_directional_strength);
		}
		IntType prevblend = null_;
		if (sVar(blend_mode) != /*"normal"*/ STR(643) && (global::render_mode == e_render_mode_COLOR || global::render_mode == e_render_mode_COLOR_FOG || global::render_mode == e_render_mode_COLOR_FOG_LIGHTS || global::render_mode == e_render_mode_ALPHA_FIX))
		{
		...
		if (sVar(glow) != global::shader_uniform_glow || sVar(glow_texture) != global::shader_uniform_glow_texture || sArr(value_inherit).Value(e_value_GLOW_COLOR) != global::shader_uniform_glow_color)
		{
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2575), IntType(1));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2569), sVar(glow_texture));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2568), global::shader_uniform_glow_color, IntType(1));
			...
			{
				render_set_uniform_int(/*"uGlow"*/ STR(2575), IntType(0));
				render_set_uniform_int(/*"uGlowTexture"*/ STR(2569), IntType(0));
				render_set_uniform_color(/*"uGlowColor"*/ STR(2568), c_black, IntType(0));
			}
			
		}
		if (idVar(sVar(glint_tex), texture) > 0)
			sVar(tex) = idVar(sVar(glint_tex), texture);
		else
			sVar(tex) = (sVar(glint_mode) == e_glint_ITEM ? idInt(sVar(glint_tex), glint_item_texture) : idInt(sVar(glint_tex), glint_entity_texture));
		
		if (DsMap(idInt(global::render_shader_obj, uniform_map)).Value(/*"uGlintEnabled"*/ STR(2576)) > -IntType(1))
			texture_set_stage(DsMap(idInt(global::render_shader_obj, sampler_map)).Value(/*"uGlintTexture"*/ STR(2577)), sprite_get_texture((IntType)(sVar(tex)), IntType(0)));
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2576), sVar(glint_mode) == e_glint_NONE ? IntType(0) : IntType(1));
		render_set_uniform_vec2(/*"uGlintSize"*/ STR(2578), sprite_get_width((IntType)(sVar(tex))) * IntType(2) * sVar(glint_scale), sprite_get_height((IntType)(sVar(tex))) * IntType(2) * sVar(glint_scale));
		if (sVar(type) != e_tl_type_PARTICLE_SPAWNER)
		{
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_tl_type_PATH:
				{
					if (sVar(path_vbuffer) != null_)
					{
						...
						else
						{
							...
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), idVar(sArr(value_inherit).Value(e_value_TEXTURE_MATERIAL_OBJ), material_format));
						}
						...
						render_set_texture(tex);
						render_set_texture(texmat, /*"Material"*/ STR(2560));
						render_set_texture(texnorm, /*"Normal"*/ STR(2561));
				...
				default:
				{
					...
					withOne (Object, sVar(temp), self->id)
					{
						...
						if (matres != null_)
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), idVar(matres, material_format));
						else
							render_set_uniform_int(/*"uMaterialFormat"*/ STR(2559), e_material_FORMAT_NONE);
