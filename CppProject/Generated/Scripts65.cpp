...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3834) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2970) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3835) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2970) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3836) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2970) + text) : /*""*/ STR(0));
		return text;
	}
	...
	BoolType textbox_draw(ScopeAny self, VarType tbx, VarType xx, VarType yy, VarType w, VarType h, BoolType contextmenu, BoolType right)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3840)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1062))))
		...
		if (sVar(window_focus) == string(tbx))
		{
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3840);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3841), idArr(tbx, line).Value(l));
						if (p == IntType(0))
