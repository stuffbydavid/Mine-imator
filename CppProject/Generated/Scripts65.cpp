		...
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					...
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3743) });
			...
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3744), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3745), idVar(sVar(temp), display_name) });
