...
namespace CppProject
{
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1941), /*"Invalid tag"*/ STR(1940) });
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1722), name);
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1945) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1946), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1273), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1947));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1948));
			new_shader_uniform(self, /*"uTextureSize"*/ STR(1949));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1950));
			new_shader_uniform(self, /*"uTime"*/ STR(1951));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1952));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1953));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1954));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1955));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1956));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1957));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1958));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1959));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1960));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1961));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1962));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1963));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1964));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1965));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1966));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1967));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1968), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1969), /*"_secondary"*/ STR(1970));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1971) + /*"+-/*^()"*/ STR(1972));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1973));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1059) + /*"+-/*^()"*/ STR(1972));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1974) + /*"+-/*^()"*/ STR(1972));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1975) + /*"+-/*^()"*/ STR(1972));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(547))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(725));
		log({ /*"Saving object"*/ STR(1976), fn });
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1977) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1978), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(567), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(567), self->glow_ani));
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1979))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1980))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1981) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1980))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1980);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1979);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1982);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1982))
		{
