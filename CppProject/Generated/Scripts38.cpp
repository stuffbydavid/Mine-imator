...
namespace CppProject
{
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1863), /*"Invalid tag"*/ STR(1862) });
	...
	{
		return font_add_sprite_ext(ID_spr_font, /*" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ"*/ STR(1864), true, IntType(1));
	}
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1865) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1866), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1221), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1867));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1868));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1869));
			new_shader_uniform(self, /*"uTime"*/ STR(1870));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1871));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1872));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1873));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1874));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1875));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1876));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1877));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1878));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1879));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1880));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1881));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1882));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1883));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1884));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1885));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1886));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1887), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1888), /*"_secondary"*/ STR(1889));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1890) + /*"+-/*^()"*/ STR(1891));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1892));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1007) + /*"+-/*^()"*/ STR(1891));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1893) + /*"+-/*^()"*/ STR(1891));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1894) + /*"+-/*^()"*/ STR(1891));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(536))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(683));
		log({ /*"Saving object"*/ STR(1895), fn });
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1896) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1897), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(556), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(556), self->glow_ani));
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1898))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1899))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1900) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1899))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1899);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1898);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1901);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1901))
		{
