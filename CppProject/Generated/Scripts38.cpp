...
namespace CppProject
{
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1870), /*"Invalid tag"*/ STR(1869) });
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1660), name);
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1874) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1875), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1224), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1876));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1877));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1878));
			new_shader_uniform(self, /*"uTime"*/ STR(1879));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1880));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1881));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1882));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1883));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1884));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1885));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1886));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1887));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1888));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1889));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1890));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1891));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1892));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1893));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1894));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1895));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1896), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1897), /*"_secondary"*/ STR(1898));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1899) + /*"+-/*^()"*/ STR(1900));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1901));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1010) + /*"+-/*^()"*/ STR(1900));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1902) + /*"+-/*^()"*/ STR(1900));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1903) + /*"+-/*^()"*/ STR(1900));
	}
	...
	RealType object_save(ScopeAny self)
	{
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1905) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1906), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1907))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1908))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1909) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1908))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1908);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1907);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1910);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1910))
		{
