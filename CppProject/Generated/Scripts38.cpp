...
namespace CppProject
{
	...
	void nbt_debug_tag_compound(VarType name, VarType map)
	{
		...
		debug({ /*"[TAG_Compound] "*/ STR(1869) + name });
		debug({ /*"{"*/ STR(1209) });
		global::debug_indent++;
		while (!is_undefined(key))
		{
			...
			{
				switch ((IntType)DsMap(map).Value(key + /*"_NBT_type"*/ STR(1870)))
				{
					case e_nbt_TAG_BYTE:
					{
						debug({ /*"[TAG_Byte] "*/ STR(1871) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_SHORT:
					{
						debug({ /*"[TAG_Short] "*/ STR(1872) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_INT:
					{
						debug({ /*"[TAG_Int] "*/ STR(1873) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LONG:
					{
						debug({ /*"[TAG_Long] "*/ STR(1874) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_FLOAT:
					{
						debug({ /*"[TAG_Float] "*/ STR(1875) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_DOUBLE:
					{
						debug({ /*"[TAG_Double] "*/ STR(1876) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_BYTE_ARRAY:
					{
						debug({ /*"[TAG_Byte_Array] "*/ STR(1877) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(965), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1878)) });
						global::debug_indent--;
						break;
					}
					case e_nbt_TAG_STRING:
					{
						debug({ /*"[TAG_String] "*/ STR(1879) + key, DsMap(map).Value(key) });
						break;
					}
					case e_nbt_TAG_LIST:
					{
						nbt_debug_tag_list(key, DsMap(map).Value(key), DsMap(map).Value(key + /*"_NBT_listtype"*/ STR(1880)));
						break;
					}
					...
					{
						debug({ /*"[TAG_Int_Array] "*/ STR(1881) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(965), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1878)) });
						global::debug_indent--;
						break;
					...
					{
						debug({ /*"[TAG_Long_Array] "*/ STR(1882) + key });
						global::debug_indent++;
						debug({ /*"Length"*/ STR(965), DsMap(map).Value(key + /*"_NBT_length"*/ STR(1878)) });
						global::debug_indent--;
						break;
					...
					default:
						debug({ /*"[TAG_Empty] "*/ STR(1883) + key, DsMap(map).Value(key) });
						break;
				}
	...
	void nbt_debug_tag_list(VarType name, VarType list, VarType listtype)
	{
		...
		switch ((IntType)listtype)
		{
			case e_nbt_TAG_BYTE:
			{
				listtypestr = /*"TAG_Byte"*/ STR(1884);
				break;
			}
			case e_nbt_TAG_SHORT:
			{
				listtypestr = /*"TAG_Short"*/ STR(1885);
				break;
			}
			case e_nbt_TAG_INT:
			{
				listtypestr = /*"TAG_Int"*/ STR(1886);
				break;
			}
			case e_nbt_TAG_LONG:
			{
				listtypestr = /*"TAG_Long"*/ STR(1887);
				break;
			}
			case e_nbt_TAG_FLOAT:
			{
				listtypestr = /*"TAG_Float"*/ STR(1888);
				break;
			}
			case e_nbt_TAG_DOUBLE:
			{
				listtypestr = /*"TAG_Double"*/ STR(1889);
				break;
			}
			case e_nbt_TAG_BYTE_ARRAY:
			{
				listtypestr = /*"TAG_Byte_Array"*/ STR(1890);
				break;
			}
			case e_nbt_TAG_STRING:
			{
				listtypestr = /*"TAG_String"*/ STR(1891);
				break;
			}
			case e_nbt_TAG_LIST:
			{
				listtypestr = /*"TAG_List"*/ STR(1892);
				break;
			}
			case e_nbt_TAG_COMPOUND:
			{
				listtypestr = /*"TAG_Compound"*/ STR(1893);
				break;
			}
			case e_nbt_TAG_INT_ARRAY:
			{
				listtypestr = /*"TAG_Int_Array"*/ STR(1894);
				break;
			}
			case e_nbt_TAG_LONG_ARRAY:
			{
				listtypestr = /*"TAG_Long_Array"*/ STR(1895);
				break;
			}
			default:
				listtypestr = /*"TAG_Empty"*/ STR(1896);
				break;
		}
		
		debug({ /*"[TAG_List of "*/ STR(1897) + listtypestr + /*"] "*/ STR(1898) + name });
		debug({ /*"{"*/ STR(1209) });
		global::debug_indent++;
	...
	IntType nbt_read_tag_compound()
	{
		...
		while (!buffer_is_eof())
		{
			...
			{
				log({ /*"TAG_Compound error"*/ STR(1899), /*"Invalid tag"*/ STR(1900) });
				ds_map_destroy(map);
				return null_;
			}
			StringType name = buffer_read_string_short_be();
			DsMap(map)[name + /*"_NBT_type"*/ STR(1870)] = type;
			switch ((IntType)type)
			{
				case e_nbt_TAG_BYTE:
				{
				...
				case e_nbt_TAG_BYTE_ARRAY:
				{
					...
					DsMap(map)[name + /*"_NBT_length"*/ STR(1878)] = len;
					buffer_skip(len);
					break;
				}
				case e_nbt_TAG_STRING:
				...
				case e_nbt_TAG_LIST:
				{
					...
					DsMap(map)[name + /*"_NBT_listtype"*/ STR(1880)] = listtype;
					break;
				}
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1901), /*"Invalid tag"*/ STR(1900) });
