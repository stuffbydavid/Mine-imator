...
namespace CppProject
{
		...
		for (IntType _it = 0, _it_max = listlen; _it < _it_max; _it++)
		{
			switch ((IntType)listtype)
			{
				...
				case e_nbt_TAG_LIST:
				{
					...
					{
						log({ /*"TAG_List error"*/ STR(1936), /*"Invalid tag"*/ STR(1935) });
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1940) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1941), DsList(global::res_type_name_list).Value(type) });
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1942));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1943));
	...
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1057) + /*"+-/*^()"*/ STR(1966));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(545))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(723));
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(565), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(565), self->glow_ani));
