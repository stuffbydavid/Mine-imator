...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"viewmove"*/ STR(3770) && view == sInt(view_second))
		{
		...
		if (view == sInt(view_main) && ObjType(obj_view, sInt(view_second))->show > 0 && !window_exists(e_window_VIEW_SECOND) && sVar(window_busy) != /*"viewmove"*/ STR(3770))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3769))
				captionx += ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3767))
				dx -= ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3769) || ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3767))
		...
		if (view == sInt(view_second))
		{
			if (window_get_current() != e_window_VIEW_SECOND)
			{
				if (draw_button_icon(self, { /*"viewclose"*/ STR(3771), dx, dy, sVar(dw), sVar(dh), false, icons_CLOSE, null_, false, /*"viewclose"*/ STR(3771) }))
				{
					ObjType(obj_view, view)->show = false;
					sBool(view_render) = false;
				}
				dx -= sVar(dw) + padding;
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3772), dx, dy, sVar(dw), sVar(dh), false, icons_EXTERNAL, null_, false, /*"viewpopout"*/ STR(3772) }))
					window_create(e_window_VIEW_SECOND, boxx, boxy, boxw, boxh);
			}
			else
			{
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3772), dx, dy, sVar(dw), sVar(dh), false, icons_INTERNAL, null_, false, /*"viewpopin"*/ STR(3773) }))
				{
		...
		else
		{
			...
			if (draw_button_icon(self, { /*"viewsecond"*/ STR(3774), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, sInt(view_second))->show, icons_VIEWPORT_SECONDARY, null_, false, (ObjType(obj_view, sInt(view_second))->show > 0) ? /*"viewseconddisable"*/ STR(3775) : /*"viewsecondenable"*/ STR(3776) }))
		...
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewqualitysettings"*/ STR(3777), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3777)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewqualitysettings"*/ STR(3777)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_quality_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3777)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
		...
		if (draw_button_icon(self, { /*"viewmoderender"*/ STR(3778), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_RENDER, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_MATERIAL__DARK : icons_SPHERE_MATERIAL, null_, false, /*"viewmoderender"*/ STR(3778) }))
		{
		...
		dx -= sVar(dw) + padding;
		tip_set_keybind(self, e_keybind_RENDER_MODE);
		if (draw_button_icon(self, { /*"viewmodeshaded"*/ STR(3779), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_SHADED, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_SHADING__DARK : icons_SPHERE_SHADING, null_, false, /*"viewmodeshaded"*/ STR(3779) }))
			ObjType(obj_view, view)->quality = e_view_mode_SHADED;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewmodeflat"*/ STR(3780), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_FLAT, icons_SPHERE_FLAT, null_, false, /*"viewmodeflat"*/ STR(3780) }))
			ObjType(obj_view, view)->quality = e_view_mode_FLAT;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewparticles"*/ STR(3781), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->particles, icons_FIREWORKS, null_, false, (ObjType(obj_view, view)->particles > 0) ? /*"viewparticlesdisable"*/ STR(3782) : /*"viewparticlesenable"*/ STR(3783) }))
			ObjType(obj_view, view)->particles = !(ObjType(obj_view, view)->particles > 0);
		if (cam != null_)
		{
			dx -= sVar(dw) + padding;
			if (draw_button_icon(self, { /*"vieweffects"*/ STR(3784), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->effects, icons_WAND, null_, false, (ObjType(obj_view, view)->effects > 0) ? /*"vieweffectsdisable"*/ STR(3785) : /*"vieweffectsenable"*/ STR(3786) }))
			{
		...
		draw_divide_vertical(dx, dy, sVar(dh));
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewgridsettings"*/ STR(3787), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3787)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewgridsettings"*/ STR(3787)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_grid_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3787)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		if (draw_button_icon(self, { /*"viewgrid"*/ STR(3788), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->grid, icons_GRID, null_, false, (ObjType(obj_view, view)->grid > 0) ? /*"viewgriddisable"*/ STR(3789) : /*"viewgridenable"*/ STR(3790) }))
			ObjType(obj_view, view)->grid = !(ObjType(obj_view, view)->grid > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewaspectratio"*/ STR(3791), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->aspect_ratio, icons_LETTERBOX, null_, false, (ObjType(obj_view, view)->aspect_ratio > 0) ? /*"viewaspectratiodisable"*/ STR(3792) : /*"viewaspectratioenable"*/ STR(3793) }))
			ObjType(obj_view, view)->aspect_ratio = !(ObjType(obj_view, view)->aspect_ratio > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewoverlays"*/ STR(3794), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->gizmos, icons_OVERLAYS, null_, false, (ObjType(obj_view, view)->gizmos > 0) ? /*"viewoverlaysdisable"*/ STR(3795) : /*"viewoverlaysenable"*/ STR(3796) }))
		...
		if (view == sInt(view_main))
		{
			...
			draw_divide_vertical(dx, dy, sVar(dh));
			dx -= 16.0 + padding;
			if (draw_button_icon(self, { /*"viewsnapsettings"*/ STR(3797), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3797)), icons_CHEVRON_DOWN_TINY }))
			{
				menu_settings_set(self, dx, dy, (string(view) + /*"viewsnapsettings"*/ STR(3797)), IntType(24));
				sInt(settings_menu_view) = view;
				sInt(settings_menu_script) = ID_menu_snap_settings;
			}
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3797)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
			tip_set_keybind(self, e_keybind_SNAP);
			if (draw_button_icon(self, { /*"viewsnap"*/ STR(3798), dx, dy, sVar(dw), sVar(dh), sReal(setting_snap), icons_MAGNET, null_, false, (sReal(setting_snap) > 0) ? /*"viewsnapdisable"*/ STR(3799) : /*"viewsnapenable"*/ STR(3800) }))
		...
		if (ObjType(obj_view, view)->camera == -IntType(4) && cam == -IntType(4))
			camname = text_get({ /*"viewworkcamera"*/ STR(3801) });
		else
			if (ObjType(obj_view, view)->camera == -IntType(5))
				camname = text_get({ /*"viewactivecamera"*/ STR(3802), (cam == -IntType(4) ? text_get({ /*"viewworkcamera"*/ STR(3801) }) : string_remove_newline(idVar(cam, display_name))) });
			else
		...
		if (location != /*"full"*/ STR(2669) && location != /*"top"*/ STR(187) && location != /*"bottom"*/ STR(186))
		{
			...
			if (!string_contains(location, /*"right"*/ STR(189)))
				sVar(content_width) -= 3.0;
			if (location == /*"right_top"*/ STR(3767) || location == /*"left_top"*/ STR(3769))
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(118) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3803));
		...
		if (view == sInt(view_main))
		{
			...
			if (sBool(bench_open))
			{
				sStr(bench_show_ani_type) = /*"show"*/ STR(719);
			...
			if (ObjType(obj_view, sInt(view_main))->mouseon && app_mouse_box(self, benchx, benchy, IntType(86), IntType(86)) && !sBool(popup_mouseon) && !sBool(toast_mouseon) && !sBool(context_menu_mouseon) && !(ObjType(obj_view, sInt(view_second))->show > 0 && ObjType(obj_view, sInt(view_second))->mouseon))
			{
				...
				tip_set(self, { text_get({ /*"viewworkbenchtip"*/ STR(3804) }), benchx, benchy, IntType(86), IntType(86), false });
			...
			if (sVar(window_busy) == /*"bench"*/ STR(720) || sVar(bench_show_ani) == IntType(1))
			{
				sReal(bench_hover_ani_goal) = .5;
				sReal(bench_click_ani_goal) = .9;
			}
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			if (ObjType(obj_view, view)->mouseon && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
			{
				...
				{
					if (ObjType(obj_view, view)->location == /*"right_top"*/ STR(3767) || ObjType(obj_view, view)->location == /*"left_bottom"*/ STR(3768))
					...
					if (sBool(mouse_left_pressed))
					{
						sVar(window_busy) = /*"viewresizeboth"*/ STR(3763);
					...
					if (mouseonresizehor)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							sVar(window_busy) = /*"viewresizehor"*/ STR(3762);
							sVar(view_resize_width) = ObjType(obj_view, view)->width;
						}
					}
					else
						if (mouseonresizever)
						{
							...
							if (sBool(mouse_left_pressed))
							{
								sVar(window_busy) = /*"viewresizever"*/ STR(3764);
								sVar(view_resize_height) = ObjType(obj_view, view)->height;
							}
						}
						else
							if (app_mouse_box(self, boxx, boxy, boxw, captionh) && !sBool(popup_mouseon) && sBool(mouse_left_pressed))
								sVar(window_busy) = /*"viewclickcaption"*/ STR(3805);
					
				
				
			}
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3805))
			{
				if (sReal(mouse_move) > IntType(10))
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2669);
					sVar(window_busy) = /*"viewmove"*/ STR(3770);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3770))
			{
				...
				if (gmlGlobal::mouse_y < sReal(view_area_y) + sReal(view_area_height) * 0.3)
					mouselocation += /*"_top"*/ STR(1887);
				else
					if (gmlGlobal::mouse_y >= sReal(view_area_y) + sReal(view_area_height) * 0.7)
						mouselocation += /*"_bottom"*/ STR(1888);
				
				if (mouselocation == /*"_top"*/ STR(1887))
					mouselocation = /*"top"*/ STR(187);
				if (mouselocation == /*"_bottom"*/ STR(1888))
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3770))
			{
				...
				RealType ani = ceil((sVar(view_glow_ani) - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(556), sVar(view_glow_ani))));
		...
		if (mouseonresizesplit && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
		{
			...
			{
				sInt(mouse_cursor) = cr_size_we;
				mouselocation = /*"hor"*/ STR(3806);
			}
			else
			{
				sInt(mouse_cursor) = cr_size_ns;
				mouselocation = /*"ver"*/ STR(3807);
			}
			
			if (sBool(mouse_left_pressed))
				sVar(window_busy) = /*"viewresizesplit"*/ STR(3808) + mouselocation;
		}
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(187) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3766)))
				linetop = true;
			if (location == /*"bottom"*/ STR(186) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3766)))
				linebottom = true;
			if (location == /*"left"*/ STR(188) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3765)))
				lineright = true;
			if (location == /*"right"*/ STR(189) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3765)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3763) || sVar(window_busy) == /*"viewresizever"*/ STR(3764) || sVar(window_busy) == /*"viewresizehor"*/ STR(3762)))
			{
				if (location == /*"left_top"*/ STR(3769))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3762) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3764) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
				}
				if (location == /*"right_top"*/ STR(3767))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3762) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3764) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
				}
				if (location == /*"left_bottom"*/ STR(3768))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3762) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3764) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
				}
				if (location == /*"right_bottom"*/ STR(2670))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3762) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3764) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3763));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			if (sBool(view_render_real_time))
				infotext = text_get({ /*"viewrenderfps"*/ STR(3809), string(gmlGlobal::fps), max({ IntType(1), global::render_samples }), sVar(project_render_samples) });
			else
				infotext = /*""*/ STR(0);
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3810), text_get({ /*"viewmodepass"*/ STR(1453) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2669);
		self->location_last = /*"full"*/ STR(2669);
		self->surface = null_;
