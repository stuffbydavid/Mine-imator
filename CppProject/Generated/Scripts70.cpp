...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"viewmove"*/ STR(3773) && view == sInt(view_second))
		{
		...
		if (view == sInt(view_main) && ObjType(obj_view, sInt(view_second))->show > 0 && !window_exists(e_window_VIEW_SECOND) && sVar(window_busy) != /*"viewmove"*/ STR(3773))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3772))
				captionx += ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3770))
				dx -= ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3772) || ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3770))
		...
		if (view == sInt(view_second))
		{
			if (window_get_current() != e_window_VIEW_SECOND)
			{
				if (draw_button_icon(self, { /*"viewclose"*/ STR(3774), dx, dy, sVar(dw), sVar(dh), false, icons_CLOSE, null_, false, /*"viewclose"*/ STR(3774) }))
				{
					ObjType(obj_view, view)->show = false;
					sBool(view_render) = false;
				}
				dx -= sVar(dw) + padding;
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3775), dx, dy, sVar(dw), sVar(dh), false, icons_EXTERNAL, null_, false, /*"viewpopout"*/ STR(3775) }))
					window_create(e_window_VIEW_SECOND, boxx, boxy, boxw, boxh);
			}
			else
			{
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3775), dx, dy, sVar(dw), sVar(dh), false, icons_INTERNAL, null_, false, /*"viewpopin"*/ STR(3776) }))
				{
		...
		else
		{
			...
			if (draw_button_icon(self, { /*"viewsecond"*/ STR(3777), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, sInt(view_second))->show, icons_VIEWPORT_SECONDARY, null_, false, (ObjType(obj_view, sInt(view_second))->show > 0) ? /*"viewseconddisable"*/ STR(3778) : /*"viewsecondenable"*/ STR(3779) }))
		...
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewqualitysettings"*/ STR(3780), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3780)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewqualitysettings"*/ STR(3780)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_quality_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3780)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
		...
		if (draw_button_icon(self, { /*"viewmoderender"*/ STR(3781), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_RENDER, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_MATERIAL__DARK : icons_SPHERE_MATERIAL, null_, false, /*"viewmoderender"*/ STR(3781) }))
		{
		...
		dx -= sVar(dw) + padding;
		tip_set_keybind(self, e_keybind_RENDER_MODE);
		if (draw_button_icon(self, { /*"viewmodeshaded"*/ STR(3782), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_SHADED, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_SHADING__DARK : icons_SPHERE_SHADING, null_, false, /*"viewmodeshaded"*/ STR(3782) }))
			ObjType(obj_view, view)->quality = e_view_mode_SHADED;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewmodeflat"*/ STR(3783), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_FLAT, icons_SPHERE_FLAT, null_, false, /*"viewmodeflat"*/ STR(3783) }))
			ObjType(obj_view, view)->quality = e_view_mode_FLAT;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewparticles"*/ STR(3784), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->particles, icons_FIREWORKS, null_, false, (ObjType(obj_view, view)->particles > 0) ? /*"viewparticlesdisable"*/ STR(3785) : /*"viewparticlesenable"*/ STR(3786) }))
			ObjType(obj_view, view)->particles = !(ObjType(obj_view, view)->particles > 0);
		if (cam != null_)
		{
			dx -= sVar(dw) + padding;
			if (draw_button_icon(self, { /*"vieweffects"*/ STR(3787), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->effects, icons_WAND, null_, false, (ObjType(obj_view, view)->effects > 0) ? /*"vieweffectsdisable"*/ STR(3788) : /*"vieweffectsenable"*/ STR(3789) }))
			{
		...
		draw_divide_vertical(dx, dy, sVar(dh));
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewgridsettings"*/ STR(3790), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3790)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewgridsettings"*/ STR(3790)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_grid_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3790)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		if (draw_button_icon(self, { /*"viewgrid"*/ STR(3791), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->grid, icons_GRID, null_, false, (ObjType(obj_view, view)->grid > 0) ? /*"viewgriddisable"*/ STR(3792) : /*"viewgridenable"*/ STR(3793) }))
			ObjType(obj_view, view)->grid = !(ObjType(obj_view, view)->grid > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewaspectratio"*/ STR(3794), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->aspect_ratio, icons_LETTERBOX, null_, false, (ObjType(obj_view, view)->aspect_ratio > 0) ? /*"viewaspectratiodisable"*/ STR(3795) : /*"viewaspectratioenable"*/ STR(3796) }))
			ObjType(obj_view, view)->aspect_ratio = !(ObjType(obj_view, view)->aspect_ratio > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewoverlays"*/ STR(3797), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->gizmos, icons_OVERLAYS, null_, false, (ObjType(obj_view, view)->gizmos > 0) ? /*"viewoverlaysdisable"*/ STR(3798) : /*"viewoverlaysenable"*/ STR(3799) }))
		...
		if (view == sInt(view_main))
		{
			...
			draw_divide_vertical(dx, dy, sVar(dh));
			dx -= 16.0 + padding;
			if (draw_button_icon(self, { /*"viewsnapsettings"*/ STR(3800), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3800)), icons_CHEVRON_DOWN_TINY }))
			{
				menu_settings_set(self, dx, dy, (string(view) + /*"viewsnapsettings"*/ STR(3800)), IntType(24));
				sInt(settings_menu_view) = view;
				sInt(settings_menu_script) = ID_menu_snap_settings;
			}
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3800)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(718))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
			tip_set_keybind(self, e_keybind_SNAP);
			if (draw_button_icon(self, { /*"viewsnap"*/ STR(3801), dx, dy, sVar(dw), sVar(dh), sReal(setting_snap), icons_MAGNET, null_, false, (sReal(setting_snap) > 0) ? /*"viewsnapdisable"*/ STR(3802) : /*"viewsnapenable"*/ STR(3803) }))
				sReal(setting_snap) = !(sReal(setting_snap) > 0);
		}
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1455) : /*"viewcamerasecond"*/ STR(1456));
		menuactive = false;
		...
		if (ObjType(obj_view, view)->camera == -IntType(4) && cam == -IntType(4))
			camname = text_get({ /*"viewworkcamera"*/ STR(3804) });
		else
			if (ObjType(obj_view, view)->camera == -IntType(5))
				camname = text_get({ /*"viewactivecamera"*/ STR(3805), (cam == -IntType(4) ? text_get({ /*"viewworkcamera"*/ STR(3804) }) : string_remove_newline(idVar(cam, display_name))) });
			else
		...
		if (location != /*"full"*/ STR(2672) && location != /*"top"*/ STR(187) && location != /*"bottom"*/ STR(186))
		{
			...
			if (!string_contains(location, /*"right"*/ STR(189)))
				sVar(content_width) -= 3.0;
			if (location == /*"right_top"*/ STR(3770) || location == /*"left_top"*/ STR(3772))
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(118) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3806));
		...
		if (view == sInt(view_main))
		{
			...
			if (ObjType(obj_view, sInt(view_main))->mouseon && app_mouse_box(self, benchx, benchy, IntType(86), IntType(86)) && !sBool(popup_mouseon) && !sBool(toast_mouseon) && !sBool(context_menu_mouseon) && !(ObjType(obj_view, sInt(view_second))->show > 0 && ObjType(obj_view, sInt(view_second))->mouseon))
			{
				...
				tip_set(self, { text_get({ /*"viewworkbenchtip"*/ STR(3807) }), benchx, benchy, IntType(86), IntType(86), false });
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			if (ObjType(obj_view, view)->mouseon && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
			{
				...
				{
					if (ObjType(obj_view, view)->location == /*"right_top"*/ STR(3770) || ObjType(obj_view, view)->location == /*"left_bottom"*/ STR(3771))
					...
					if (sBool(mouse_left_pressed))
					{
						sVar(window_busy) = /*"viewresizeboth"*/ STR(3766);
					...
					if (mouseonresizehor)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							sVar(window_busy) = /*"viewresizehor"*/ STR(3765);
							sVar(view_resize_width) = ObjType(obj_view, view)->width;
						}
					}
					else
						if (mouseonresizever)
						{
							...
							if (sBool(mouse_left_pressed))
							{
								sVar(window_busy) = /*"viewresizever"*/ STR(3767);
								sVar(view_resize_height) = ObjType(obj_view, view)->height;
							}
						}
						else
							if (app_mouse_box(self, boxx, boxy, boxw, captionh) && !sBool(popup_mouseon) && sBool(mouse_left_pressed))
								sVar(window_busy) = /*"viewclickcaption"*/ STR(3808);
					
				
				
			}
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3808))
			{
				if (sReal(mouse_move) > IntType(10))
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2672);
					sVar(window_busy) = /*"viewmove"*/ STR(3773);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3773))
			{
		...
		if (mouseonresizesplit && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
		{
			...
			{
				sInt(mouse_cursor) = cr_size_we;
				mouselocation = /*"hor"*/ STR(3809);
			}
			else
			{
				sInt(mouse_cursor) = cr_size_ns;
				mouselocation = /*"ver"*/ STR(3810);
			}
			
			if (sBool(mouse_left_pressed))
				sVar(window_busy) = /*"viewresizesplit"*/ STR(3811) + mouselocation;
		}
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(187) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3769)))
				linetop = true;
			if (location == /*"bottom"*/ STR(186) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3769)))
				linebottom = true;
			if (location == /*"left"*/ STR(188) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3768)))
				lineright = true;
			if (location == /*"right"*/ STR(189) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3768)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3766) || sVar(window_busy) == /*"viewresizever"*/ STR(3767) || sVar(window_busy) == /*"viewresizehor"*/ STR(3765)))
			{
				if (location == /*"left_top"*/ STR(3772))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3765) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3767) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
				}
				if (location == /*"right_top"*/ STR(3770))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3765) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3767) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
				}
				if (location == /*"left_bottom"*/ STR(3771))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3765) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3767) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
				}
				if (location == /*"right_bottom"*/ STR(2673))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3765) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3767) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3766));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			if (sBool(view_render_real_time))
				infotext = text_get({ /*"viewrenderfps"*/ STR(3812), string(gmlGlobal::fps), max({ IntType(1), global::render_samples }), sVar(project_render_samples) });
			else
				infotext = /*""*/ STR(0);
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3813), text_get({ /*"viewmodepass"*/ STR(1454) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2672);
		self->location_last = /*"full"*/ STR(2672);
		self->surface = null_;
