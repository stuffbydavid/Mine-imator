...
namespace CppProject
{
	...
	IntType toast_new(ScopeAny self, IntType variant, VarType text)
	{
		...
		log({ /*"New toast"*/ STR(3856), ObjType(obj_toast, toast)->text, type });
		return toast;
	}
	...
	void toolbar_draw(ScopeAny self)
	{
		...
		sVar(dx) += capwid + padding;
		sVar(dx) += IntType(8);
		draw_label(self, { text_get({ /*"toolbarbackup"*/ STR(3857) }), sVar(dx), sVar(dy) + IntType(22), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary * clamp(sReal(backup_text_ani), IntType(0), IntType(1)), sInt(font_value) });
		if (!(sReal(setting_advanced_mode) > 0))
		{
			if (draw_button_label(self, /*"toolbarsimplemode"*/ STR(3858), sVar(content_x) + sVar(content_width) - IntType(10), sVar(dy), null_, null_, e_button_TOOLBAR, null_, fa_right))
			{
	...
	void trial_startup()
	{
		...
		{
			log({ /*"Found key_file"*/ STR(3859), key_file });
	...
	BoolType trial_upgrade(ScopeAny self, VarType argument0)
	{
		...
		settings_save(self);
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertupgraded"*/ STR(3860) }));
		return true;
	}
	...
	VarType unzip(VarType fn, StringType dir)
	{
		...
		log({ /*"Unzipping"*/ STR(3861), fn, dir });
		VarType num = lib_unzip(fn, dir);
		log({ string(num) + /*" files were extracted"*/ STR(3862) });
		return num;
	}
	...
	StringType unzip_asset(VarType fn)
	{
		...
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"miproject;.mproj;.mani;"*/ STR(3863));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"miproject;.mproj;.mani;"*/ STR(3863));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory, /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3864));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3864));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	StringType unzip_model(StringType fn)
	{
		...
		name = filename_new_ext(filename_name(fn), /*""*/ STR(0));
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"json"*/ STR(3865));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"json"*/ STR(3865));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/item/"*/ STR(3866), /*".json;"*/ STR(3867));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/block/"*/ STR(3868), /*".json;"*/ STR(3867));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	RealType window_drop_enter(ScopeAny self, ArrType files)
	{
		...
		if (array_length(VarType::CreateRef(files)) > IntType(1))
			return false;
		return string_contains(asset_exts, /*"*"*/ STR(1089) + filename_ext(files.Value(IntType(0))) + /*";"*/ STR(3871));
	}
