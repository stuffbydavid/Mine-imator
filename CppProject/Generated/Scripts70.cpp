...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1456) : /*"viewcamerasecond"*/ STR(1457));
		menuactive = false;
		...
		if (location != /*"full"*/ STR(2676) && location != /*"top"*/ STR(187) && location != /*"bottom"*/ STR(186))
		{
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			...
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3813))
			{
				...
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2676);
		...
		if (view == sInt(view_second))
		{
			...
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3771) || sVar(window_busy) == /*"viewresizever"*/ STR(3772) || sVar(window_busy) == /*"viewresizehor"*/ STR(3770)))
			{
				...
				if (location == /*"right_bottom"*/ STR(2677))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3770) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3771));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3772) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3771));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3818), text_get({ /*"viewmodepass"*/ STR(1455) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2676);
		self->location_last = /*"full"*/ STR(2676);
		self->surface = null_;
