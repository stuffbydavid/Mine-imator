...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"viewmove"*/ STR(3837) && view == sInt(view_second))
		{
		...
		if (view == sInt(view_main) && ObjType(obj_view, sInt(view_second))->show > 0 && !window_exists(e_window_VIEW_SECOND) && sVar(window_busy) != /*"viewmove"*/ STR(3837))
		{
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3836))
				captionx += ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3834))
				dx -= ObjType(obj_view, sInt(view_second))->width;
			if (ObjType(obj_view, sInt(view_second))->location == /*"left_top"*/ STR(3836) || ObjType(obj_view, sInt(view_second))->location == /*"right_top"*/ STR(3834))
		...
		if (view == sInt(view_second))
		{
			if (window_get_current() != e_window_VIEW_SECOND)
			{
				if (draw_button_icon(self, { /*"viewclose"*/ STR(3838), dx, dy, sVar(dw), sVar(dh), false, icons_CLOSE, null_, false, /*"viewclose"*/ STR(3838) }))
				{
					ObjType(obj_view, view)->show = false;
					sBool(view_render) = false;
				}
				dx -= sVar(dw) + padding;
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3839), dx, dy, sVar(dw), sVar(dh), false, icons_EXTERNAL, null_, false, /*"viewpopout"*/ STR(3839) }))
					window_create(e_window_VIEW_SECOND, boxx, boxy, boxw, boxh);
			}
			else
			{
				if (draw_button_icon(self, { /*"viewpopout"*/ STR(3839), dx, dy, sVar(dw), sVar(dh), false, icons_INTERNAL, null_, false, /*"viewpopin"*/ STR(3840) }))
				{
		...
		else
		{
			...
			if (draw_button_icon(self, { /*"viewsecond"*/ STR(3841), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, sInt(view_second))->show, icons_VIEWPORT_SECONDARY, null_, false, (ObjType(obj_view, sInt(view_second))->show > 0) ? /*"viewseconddisable"*/ STR(3842) : /*"viewsecondenable"*/ STR(3843) }))
		...
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewqualitysettings"*/ STR(3844), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3844)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewqualitysettings"*/ STR(3844)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_quality_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3844)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(757))
		...
		if (draw_button_icon(self, { /*"viewmoderender"*/ STR(3845), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_RENDER, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_MATERIAL__DARK : icons_SPHERE_MATERIAL, null_, false, /*"viewmoderender"*/ STR(3845) }))
		{
		...
		dx -= sVar(dw) + padding;
		tip_set_keybind(self, e_keybind_RENDER_MODE);
		if (draw_button_icon(self, { /*"viewmodeshaded"*/ STR(3846), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_SHADED, ObjType(obj_theme, sInt(setting_theme))->dark ? icons_SPHERE_SHADING__DARK : icons_SPHERE_SHADING, null_, false, /*"viewmodeshaded"*/ STR(3846) }))
			ObjType(obj_view, view)->quality = e_view_mode_SHADED;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewmodeflat"*/ STR(3847), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->quality == e_view_mode_FLAT, icons_SPHERE_FLAT, null_, false, /*"viewmodeflat"*/ STR(3847) }))
			ObjType(obj_view, view)->quality = e_view_mode_FLAT;
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewparticles"*/ STR(3848), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->particles, icons_FIREWORKS, null_, false, (ObjType(obj_view, view)->particles > 0) ? /*"viewparticlesdisable"*/ STR(3849) : /*"viewparticlesenable"*/ STR(3850) }))
			ObjType(obj_view, view)->particles = !(ObjType(obj_view, view)->particles > 0);
		if (cam != null_)
		{
			dx -= sVar(dw) + padding;
			if (draw_button_icon(self, { /*"vieweffects"*/ STR(3851), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->effects, icons_WAND, null_, false, (ObjType(obj_view, view)->effects > 0) ? /*"vieweffectsdisable"*/ STR(3852) : /*"vieweffectsenable"*/ STR(3853) }))
			{
		...
		draw_divide_vertical(dx, dy, sVar(dh));
		dx -= 16.0 + padding;
		if (draw_button_icon(self, { /*"viewgridsettings"*/ STR(3854), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3854)), icons_CHEVRON_DOWN_TINY }))
		{
			menu_settings_set(self, dx, dy, (string(view) + /*"viewgridsettings"*/ STR(3854)), IntType(24));
			sInt(settings_menu_view) = view;
			sInt(settings_menu_script) = ID_menu_grid_settings;
		}
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3854)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(757))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		if (draw_button_icon(self, { /*"viewgrid"*/ STR(3855), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->grid, icons_GRID, null_, false, (ObjType(obj_view, view)->grid > 0) ? /*"viewgriddisable"*/ STR(3856) : /*"viewgridenable"*/ STR(3857) }))
			ObjType(obj_view, view)->grid = !(ObjType(obj_view, view)->grid > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewaspectratio"*/ STR(3858), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->aspect_ratio, icons_LETTERBOX, null_, false, (ObjType(obj_view, view)->aspect_ratio > 0) ? /*"viewaspectratiodisable"*/ STR(3859) : /*"viewaspectratioenable"*/ STR(3860) }))
			ObjType(obj_view, view)->aspect_ratio = !(ObjType(obj_view, view)->aspect_ratio > 0);
		dx -= sVar(dw) + padding;
		if (draw_button_icon(self, { /*"viewoverlays"*/ STR(3861), dx, dy, sVar(dw), sVar(dh), ObjType(obj_view, view)->gizmos, icons_OVERLAYS, null_, false, (ObjType(obj_view, view)->gizmos > 0) ? /*"viewoverlaysdisable"*/ STR(3862) : /*"viewoverlaysenable"*/ STR(3863) }))
		...
		if (view == sInt(view_main))
		{
			...
			draw_divide_vertical(dx, dy, sVar(dh));
			dx -= 16.0 + padding;
			if (draw_button_icon(self, { /*"viewsnapsettings"*/ STR(3864), dx, dy, IntType(16), IntType(24), sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3864)), icons_CHEVRON_DOWN_TINY }))
			{
				menu_settings_set(self, dx, dy, (string(view) + /*"viewsnapsettings"*/ STR(3864)), IntType(24));
				sInt(settings_menu_view) = view;
				sInt(settings_menu_script) = ID_menu_snap_settings;
			}
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3864)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(757))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
			tip_set_keybind(self, e_keybind_SNAP);
			if (draw_button_icon(self, { /*"viewsnap"*/ STR(3865), dx, dy, sVar(dw), sVar(dh), sReal(setting_snap), icons_MAGNET, null_, false, (sReal(setting_snap) > 0) ? /*"viewsnapdisable"*/ STR(3866) : /*"viewsnapenable"*/ STR(3867) }))
				sReal(setting_snap) = !(sReal(setting_snap) > 0);
		}
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1502) : /*"viewcamerasecond"*/ STR(1503));
		menuactive = false;
		...
		if (ObjType(obj_view, view)->camera == -IntType(4) && cam == -IntType(4))
			camname = text_get({ /*"viewworkcamera"*/ STR(3868) });
		else
			if (ObjType(obj_view, view)->camera == -IntType(5))
				camname = text_get({ /*"viewactivecamera"*/ STR(3869), (cam == -IntType(4) ? text_get({ /*"viewworkcamera"*/ STR(3868) }) : string_remove_newline(idVar(cam, display_name))) });
			else
		...
		if (location != /*"full"*/ STR(2726) && location != /*"top"*/ STR(188) && location != /*"bottom"*/ STR(187))
		{
			if (!string_contains(location, /*"left"*/ STR(189)))
			{
				sVar(content_x) += IntType(3);
				sVar(content_width) -= 3.0;
			}
			if (!string_contains(location, /*"right"*/ STR(190)))
				sVar(content_width) -= 3.0;
			if (location == /*"right_top"*/ STR(3834) || location == /*"left_top"*/ STR(3836))
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(118) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3870));
		...
		if (view == sInt(view_main))
		{
			...
			if (sBool(bench_open))
			{
				sStr(bench_show_ani_type) = /*"show"*/ STR(758);
			...
			if (ObjType(obj_view, sInt(view_main))->mouseon && app_mouse_box(self, benchx, benchy, IntType(86), IntType(86)) && !sBool(popup_mouseon) && !sBool(toast_mouseon) && !sBool(context_menu_mouseon) && !(ObjType(obj_view, sInt(view_second))->show > 0 && ObjType(obj_view, sInt(view_second))->mouseon))
			{
				...
				tip_set(self, text_get({ /*"viewworkbenchtip"*/ STR(3871) }), benchx, benchy, IntType(86), IntType(86), false);
			...
			if (sVar(window_busy) == /*"bench"*/ STR(759) || sVar(bench_show_ani) == IntType(1))
			{
				sReal(bench_hover_ani_goal) = .5;
				sReal(bench_click_ani_goal) = .9;
			}
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			if (ObjType(obj_view, view)->mouseon && (sInt(mouse_cursor) == cr_default || sBool(content_mouseon)))
			{
				...
				{
					if (ObjType(obj_view, view)->location == /*"right_top"*/ STR(3834) || ObjType(obj_view, view)->location == /*"left_bottom"*/ STR(3835))
					...
					if (sBool(mouse_left_pressed))
					{
						sVar(window_busy) = /*"viewresizeboth"*/ STR(3830);
					...
					if (mouseonresizehor)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							sVar(window_busy) = /*"viewresizehor"*/ STR(3829);
							sVar(view_resize_width) = ObjType(obj_view, view)->width;
						}
					}
					else
						if (mouseonresizever)
						{
							...
							if (sBool(mouse_left_pressed))
							{
								sVar(window_busy) = /*"viewresizever"*/ STR(3831);
								sVar(view_resize_height) = ObjType(obj_view, view)->height;
							}
						}
						else
							if (app_mouse_box(self, boxx, boxy, boxw, captionh) && !sBool(popup_mouseon) && sBool(mouse_left_pressed))
								sVar(window_busy) = /*"viewclickcaption"*/ STR(3872);
					
				
				
			}
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3872))
			{
				if (sReal(mouse_move) > IntType(10))
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2726);
					sVar(window_busy) = /*"viewmove"*/ STR(3837);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3837))
			{
				...
				if (!sBool(mouse_left))
				{
					...
					switch (StringType(ObjType(obj_view, view)->location).id)
					{
						case 188: // top
						{
							ObjType(obj_view, sInt(view_main))->location = /*"bottom"*/ STR(187);
							break;
						}
						case 187: // bottom
						{
							ObjType(obj_view, sInt(view_main))->location = /*"top"*/ STR(188);
							break;
						}
						case 190: // right
						{
							ObjType(obj_view, sInt(view_main))->location = /*"left"*/ STR(189);
							break;
						}
						case 189: // left
						{
							ObjType(obj_view, sInt(view_main))->location = /*"right"*/ STR(190);
							break;
						}
						default:
							ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2726);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3837))
			{
				...
				RealType ani = ceil((sVar(view_glow_ani) - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(564), sVar(view_glow_ani))));
		...
		{
			StringType mouselocation = /*""*/ STR(0);
			if (ObjType(obj_view, view)->location == /*"right"*/ STR(190) || ObjType(obj_view, view)->location == /*"left"*/ STR(189))
			{
				sInt(mouse_cursor) = cr_size_we;
				mouselocation = /*"hor"*/ STR(3873);
			}
			else
			{
				sInt(mouse_cursor) = cr_size_ns;
				mouselocation = /*"ver"*/ STR(3874);
			}
			
			if (sBool(mouse_left_pressed))
				sVar(window_busy) = /*"viewresizesplit"*/ STR(3875) + mouselocation;
		}
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(188) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3833)))
				linetop = true;
			if (location == /*"bottom"*/ STR(187) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3833)))
				linebottom = true;
			if (location == /*"left"*/ STR(189) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3832)))
				lineright = true;
			if (location == /*"right"*/ STR(190) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3832)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3830) || sVar(window_busy) == /*"viewresizever"*/ STR(3831) || sVar(window_busy) == /*"viewresizehor"*/ STR(3829)))
			{
				if (location == /*"left_top"*/ STR(3836))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3829) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3831) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
				}
				if (location == /*"right_top"*/ STR(3834))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3829) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
					linetop = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3831) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
				}
				if (location == /*"left_bottom"*/ STR(3835))
				{
					lineright = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3829) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3831) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
				}
				if (location == /*"right_bottom"*/ STR(2727))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3829) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3831) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3830));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			if (sBool(view_render_real_time))
				infotext = text_get({ /*"viewrenderfps"*/ STR(3876), string(gmlGlobal::fps), max({ IntType(1), global::render_samples }), sVar(project_render_samples) });
			else
				infotext = /*""*/ STR(0);
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3877), text_get({ /*"viewmodepass"*/ STR(1501) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2726);
		self->location_last = /*"full"*/ STR(2726);
		self->surface = null_;
