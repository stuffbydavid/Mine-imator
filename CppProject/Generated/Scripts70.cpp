...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		if (sStr(settings_menu_name) == (string(view) + /*"viewqualitysettings"*/ STR(3889)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(758))
		...
		if (sStr(settings_menu_name) == (string(view) + /*"viewgridsettings"*/ STR(3899)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(758))
			ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
		dx -= sVar(dw);
		...
		if (view == sInt(view_main))
		{
			...
			if (sStr(settings_menu_name) == (string(view) + /*"viewsnapsettings"*/ STR(3909)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(758))
				ObjType(value_animation, ObjType(micro_animation, global::current_microani)->active)->value = true;
			dx -= sVar(dw);
		...
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1505) : /*"viewcamerasecond"*/ STR(1506));
		menuactive = false;
		...
		if (location != /*"full"*/ STR(2767) && location != /*"top"*/ STR(188) && location != /*"bottom"*/ STR(187))
		{
		...
		if (sVar(content_width) > IntType(0) && sVar(content_height) > IntType(0))
		{
			...
			if (sBool(content_mouseon))
				sStr(shortcut_bar_state) = /*"viewport"*/ STR(118) + (cam == null_ ? /*""*/ STR(0) : /*"cam"*/ STR(3915));
		...
		if (view == sInt(view_main))
		{
			...
			if (sBool(bench_open))
			{
				sStr(bench_show_ani_type) = /*"show"*/ STR(759);
			...
			if (sVar(window_busy) == /*"bench"*/ STR(760) || sVar(bench_show_ani) == IntType(1))
			{
				sReal(bench_hover_ani_goal) = .5;
				sReal(bench_click_ani_goal) = .9;
			}
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			...
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3917))
			{
				...
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2767);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3882))
			{
				...
				RealType ani = ceil((sVar(view_glow_ani) - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(565), sVar(view_glow_ani))));
		...
		if (view == sInt(view_second))
		{
			...
			lineleft = false;
			lineright = false;
			if (location == /*"top"*/ STR(188) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3878)))
				linetop = true;
			if (location == /*"bottom"*/ STR(187) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplitver"*/ STR(3878)))
				linebottom = true;
			if (location == /*"left"*/ STR(189) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3877)))
				lineright = true;
			if (location == /*"right"*/ STR(190) && (mouseonresizesplit || sVar(window_busy) == /*"viewresizesplithor"*/ STR(3877)))
				lineleft = true;
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3875) || sVar(window_busy) == /*"viewresizever"*/ STR(3876) || sVar(window_busy) == /*"viewresizehor"*/ STR(3874)))
			{
				...
				if (location == /*"right_bottom"*/ STR(2768))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3874) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3875));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3876) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3875));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3922), text_get({ /*"viewmodepass"*/ STR(1504) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2767);
		self->location_last = /*"full"*/ STR(2767);
		self->surface = null_;
