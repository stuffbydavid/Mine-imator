...
void main()
{
	...
	vec3 originNormal = unpackNormal(texture2D(uNormalBuffer, vTexCoord));//getNormal(vTexCoord);
	vec3 hitNormal    = unpackNormal(texture2D(uNormalBuffer, rayUV));//getNormal(rayUV);
