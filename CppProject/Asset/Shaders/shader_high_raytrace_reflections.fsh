...
void main()
{
	...
	if (rtCoord.r < 1.0)
	{
		...
		vis *= 1.0 - percent(originMat.r, .85, .95);
		
		// Clamp
		vis = clamp(vis, 0.0, 1.0);
		
		float metallic = rayMat.g * vis;
		
		if (metallic > 0.0)
			refColor *= mix(vec3(1.0), pow(texture2D(uDiffuseBuffer, rayUV).rgb, vec3(gamma)), metallic);
		
		// Mix in fallback via fresnel if ray hit a reflective surface
		vec3 surfCol = mix(pow(texture2D(uSceneBuffer, rayUV).rgb, vec3(gamma)), refColor, rayMat.g);
