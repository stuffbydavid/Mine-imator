...
if (!background_sky_clouds_show || !render_background || render_mode = e_render_mode.HIGH_SSAO_DEPTH_NORMAL || render_mode = e_render_mode.SCENE_TEST || render_mode = e_render_mode.DEPTH_NO_SKY)
	return 0
...
render_set_uniform_color("uBlendColor", merge_color(background_sky_clouds_color, make_color_rgb(120, 120, 255), background_night_alpha), (background_sky_clouds_story_mode ? 1 - min(background_night_alpha, 0.95) : .8 - min(background_night_alpha, 0.75)) * alphay)
...
yo = (cam_from[Y] div size) * size - ((background_sky_clouds_speed * (background_time * 0.25 + background_sky_time * 100) + background_sky_clouds_offset) mod size)
