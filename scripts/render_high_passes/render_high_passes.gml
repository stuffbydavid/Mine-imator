...
function render_high_passes()
{
	...
	render_surface_normal = surface_require(render_surface_normal, render_width, render_height)
	render_surface_normal_ext = surface_require(render_surface_normal_ext, render_width, render_height)
	render_surface_emissive = surface_require(render_surface_emissive, render_width, render_height)
	render_surface_material = surface_require(render_surface_material, render_width, render_height) // Use material as temp for depth, need to correct background
	...
	surface_set_target_ext(2, render_surface_normal_ext)
	{
	...
	// Correct depth background
	surface_set_target(render_surface_depth)
	{
		draw_clear(c_white)
		draw_surface_exists(render_surface_material, 0, 0)
	}
	surface_reset_target()
	...
	if (render_pass = e_render_pass.NORMAL)
		render_pass_surf = surface_duplicate(render_surface_normal_ext)
}