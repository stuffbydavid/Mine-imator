...
function tab_timeline_editor_path()
{
	...
	if (collapse_map[?"path_shape"] && tl_edit.path_shape_generate)
	{
		...
		if (collapse_map[?"path_shape_tube"] && tl_edit.path_shape_tube)
		{
			...
			tab_control_dragger()
			draw_dragger("timelineeditorpathshapedetail", dx, dy, dragger_width, tl_edit.path_shape_detail, 1 / 4, 4, no_limit, 6, 1, tab.path.tbx_shape_detail, action_tl_path_shape_detail)
			tab_next()
